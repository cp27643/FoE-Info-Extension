...
FoEproxy.addHandler('ResourceShopService', 'buyOffer', (data) => {
  if (
    data['responseData']['gains'] === undefined ||
    data['responseData']['gains']['resources'] === undefined ||
    data['responseData']['gains']['resources']['strategy_points'] === undefined
  ) {
    return;
  }
...
let StrategyPoints = {
  ...
  ShowFPBar: () => {
    ...
    if ($('.fp-bar-main').length === 0) {
      $('#fp-bar')
        ...
        .append(
          `<div class="fp-bar-main"><div class="number"></div><div class="bars"></div></div>`,
        );
    }
    // necessary to wait for gift in gg + diplomatic gift
    setTimeout(() => {
      const availableFPs =
        ResourceStock['strategy_points'] !== undefined ?
          ResourceStock['strategy_points']
        : 0;
  ...
  RefreshBuyableForgePoints: (formula) => {
    ...
    if ($('div.buyable-fp').length === 0) {
      $('#fp-bar').append(
        `<div class="buyable-fp" title="${HTML.i18nTooltip(i18n('StrategyPoints.BuyableFP'))}">${HTML.Format(amount)}</div>`,
      );
    } else {
  ...
  RefreshBar: () => {
    ...
    StrategyPoints.InventoryFP = Object.values(MainParser.Inventory)
      .filter((x) => x.item.__class__ == 'ForgePointPackagePayload')
      .reduce(
        (total, x) =>
          total + (x.inStock || 0) * (x.item?.resource_package?.gain || 0),
        0,
      );
