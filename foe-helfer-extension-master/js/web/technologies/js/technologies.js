...
FoEproxy.addHandler('ResearchService', 'spendForgePoints', (data, postData) => {
  ...
  for (let i in Technologies.UnlockedTechnologies.inProgressTechnologies) {
    if (
      !Technologies.UnlockedTechnologies.inProgressTechnologies.hasOwnProperty(
        i,
      )
    )
      continue;
    ...
    if (
      Technologies.UnlockedTechnologies.inProgressTechnologies[i]['tech_id'] ===
      ID
    ) {
      TechFound = true;
      Technologies.UnlockedTechnologies.inProgressTechnologies[i]['currentSP'] =
        CurrentTech['progress']['currentSP'];
  ...
  if (!TechFound) {
    let TechCount =
      Technologies.UnlockedTechnologies.inProgressTechnologies.length;
    Technologies.UnlockedTechnologies.inProgressTechnologies[TechCount] =
      CurrentTech['progress'];
  }
...
let Technologies = {
  ...
  getMaxEra: () => {
    ...
    if (!Technologies.maxEra)
      Technologies.maxEra = Math.max(
        ...Object.values(MainParser.CityEntities)
          .filter((x) => x.type == 'greatbuilding')
          .map((x) => Technologies.Eras[x.requirements.min_era]),
      );
  ...
  getEraName: (entityId, level) => {
    ...
    if (eraName == 'MultiAge') return Technologies.InnoEraNames[level];
    return eraName;
  },
  ...
  getNextEraIdByCurrentEraName: (eraName) => {
    ...
    let era =
      Technologies.InnoEras[eraName] === Technologies.getMaxEra() - 1 ?
        parseInt(Technologies.InnoEras[eraName])
      : parseInt(Technologies.InnoEras[eraName] + 1);
    return era;
  },
  ...
  Show: () => {
    ...
    $('#technologies').on('click', '.ignoreprevera', function () {
      ...
      localStorage.setItem(
        'TechnologiesIgnorePrevEra',
        Technologies.IgnorePrevEra,
      );
    ...
    $('#technologies').on('click', '.ignorecurrenteraoptional', function () {
      ...
      localStorage.setItem(
        'TechnologiesIgnoreCurrentEraOptional',
        Technologies.IgnoreCurrentEraOptional,
      );
  ...
  BuildBox: () => {
    Technologies.IgnorePrevEra =
      localStorage.getItem('TechnologiesIgnorePrevEra') !== 'false' ?
        'true'
      : 'false';
    Technologies.IgnoreCurrentEraOptional =
      localStorage.getItem('TechnologiesIgnoreCurrentEraOptional') !== 'false' ?
        'true'
      : 'false';
  ...
  CalcBody: () => {
    ...
    for (
      let i = 0;
      i < Technologies.UnlockedTechnologies['unlockedTechnologies'].length;
      i++
    ) {
      let TechName =
        Technologies.UnlockedTechnologies['unlockedTechnologies'][i];
      let Index = TechDict[TechName];
      Technologies.AllTechnologies[Index]['isResearched'] = true;
      Technologies.AllTechnologies[Index]['currentSP'] =
        Technologies.AllTechnologies[Index]['maxSP'];
    }

    // Teilweise erforscht
    for (
      let i = 0;
      i < Technologies.UnlockedTechnologies['inProgressTechnologies'].length;
      i++
    ) {
      let InProgTech =
        Technologies.UnlockedTechnologies['inProgressTechnologies'][i];
      let Index = TechDict[InProgTech['tech_id']];
      Technologies.AllTechnologies[Index]['currentSP'] =
        InProgTech['currentSP'];
    }
    ...
    for (let i = 1; i < Technologies.AllTechnologies.length; i++) {
      ...
      if (!Tech['isResearched'] && !Tech['isTeaser']) {
        ...
        if (
          EraID >= CurrentEraID &&
          Tech['childTechnologies'].length === 0 &&
          Technologies.IgnoreCurrentEraOptional
        )
          continue; // Aktuelles/zukünfiges ZA und optionale Technologie ausblenden
        ...
        if (EraID >= CurrentEraID && EraID <= Technologies.SelectedEraID) {
          ...
          RequiredResources['strategy_points'] +=
            Tech['maxSP'] - Tech['currentSP'];
          ...
          for (let ResourceName in Tech['requirements']['resources']) {
            ...
            RequiredResources[ResourceName] +=
              Tech['requirements']['resources'][ResourceName];
          }
    ...
    let PreviousEraID = Math.max(Technologies.SelectedEraID - 1, CurrentEraID),
      NextEraID = Math.min(
        Technologies.SelectedEraID + 1,
        Technologies.getMaxEra(),
      );

    h.push('<div class="dark-bg" style="margin-bottom: 3px">');
    h.push('<div class="techno-head">');
    h.push(
      '<button class="btn btn-switchage" data-value="' +
        PreviousEraID +
        '">' +
        i18n('Eras.' + PreviousEraID) +
        '</button>',
    );
    h.push(
      '<div class="text-center"><strong>' +
        i18n('Eras.' + Technologies.SelectedEraID) +
        '</strong></div>',
    );
    h.push(
      '<button class="btn btn-switchage" data-value="' +
    ...
    h.push(
      '<input id="IgnorePrevEra" class="ignoreprevera game-cursor" ' +
        (Technologies.IgnorePrevEra ? 'checked' : '') +
        ' type="checkbox">' +
        i18n('Boxes.Technologies.IgnorePrevEra') +
        '<br>',
    );
    h.push(
      '<input id="IgnoreCurrentEraOptional" class="ignorecurrenteraoptional game-cursor" ' +
        (Technologies.IgnoreCurrentEraOptional ? 'checked' : '') +
        ' type="checkbox">' +
        i18n('Boxes.Technologies.IgnoreCurrentEraOptional') +
        '<br>',
    );
    h.push('</div>');
    h.push('</div>');
    ...
    h.push(
      '<thead class="sticky">' +
        '<tr>' +
        '<th colspan="2" data-export2="resource">' +
        i18n('Boxes.Technologies.Resource') +
        '</th>' +
        '<th data-export="required">' +
        i18n('Boxes.Technologies.DescRequired') +
        '</th>' +
        '<th data-export="instock">' +
        i18n('Boxes.Technologies.DescInStock') +
        '</th>' +
        '<th data-export="remaining" class="text-right">' +
        i18n('Boxes.Technologies.DescStillMissing') +
        '</th>' +
        '</tr>' +
        '</thead>',
    );
    ...
    if (TechCount > 0) {
      ...
      for (let i = 0; i < OutputList.length; i++) {
        ...
        if (RequiredResources[ResourceName] !== undefined) {
          ...
          let Stock =
            ResourceName === 'strategy_points' ?
              StrategyPoints.AvailableFP
            : ResourceStock[ResourceName];
          if (Stock === undefined) Stock = 0;
          let Diff = Stock - Required;

          h.push('<tr>');
          h.push(
            '<td class="goods-image" style="width:25px"><span class="goods-sprite sprite-35 ' +
              GoodsData[ResourceName]['id'] +
              '"></span></td>',
          );
      ...
      h.push(
        '<td colspan="5" class="text-center">' +
          i18n('Boxes.Technologies.NoTechs') +
          '</td>',
      );
      h.push('</tr>');
    }
  ...
  ShowSettingsButton: () => {
    let h = [];
    h.push(
      `<p class="text-center"><button class="btn" onclick="HTML.ExportTable($('#technologiesBody').find('.foe-table.exportable'), 'csv', 'technologies')">${i18n('Boxes.General.ExportCSV')}</button></p>`,
    );
    h.push(
      `<p class="text-center"><button class="btn" onclick="HTML.ExportTable($('#technologiesBody').find('.foe-table.exportable'), 'json', 'technologies')">${i18n('Boxes.General.ExportJSON')}</button></p>`,
    );
