...
FoEproxy.addHandler('ItemShopService', 'purchaseItem', (data, postData) => {
  ...
  if (d.__class__ && d.__class__ === 'ItemShopSlot') {
    ...
    Castle.curShopItems.date = moment(MainParser.getCurrentDateTime())
      .startOf('day')
      .unix();

    localStorage.setItem(
      'CastleCurShopItems',
      JSON.stringify(Castle.curShopItems),
    );
...
FoEproxy.addHandler('ItemAuctionService', 'getAuction', (data, postData) => {
  if (!data || !data.responseData || !data.responseData['state']) {
    return;
  }

  if (data.responseData['state'] === 'collectable') {
    let d = JSON.parse(localStorage.getItem('CastleCurAuctionWinning'));
    const startOfDay = moment(MainParser.getCurrentDateTime())
      .startOf('day')
      .unix();
    ...
    d.rewards.push({
      time: moment(MainParser.getCurrentDateTime()).unix(),
      reward: Castle.AuctionWinningReward,
    });
...
FoEproxy.addHandler('ItemShopService', 'getShop', (data, postData) => {
  ...
  if (d['context'] && d['context'] === 'antiques_dealer' && d['slots']) {
    ...
    d.slots.forEach((item) => {
      if (
        item['cost'] === undefined ||
        item['cost']['resources'] === undefined
      ) {
        return;
      }
      const r = item['cost']['resources'];
      const gem =
        r['gemstones'] ? r['gemstones'] / Castle.ShopGemstonesDivisor : 0;
      const coins =
        r['trade_coins'] ? r['trade_coins'] / Castle.ShopTradeCoinsDivisor : 0;
...
FoEproxy.addHandler('GuildExpeditionService', 'getState', (data, postData) => {
  if (!data || !data.responseData || !data.responseData[0]) {
    return;
  }
  ...
  if (r['currentEntityId'] === undefined) {
    return;
  }
...
FoEproxy.addHandler('ChallengeService', 'getOptions', (data, postData) => {
  ...
  if (r.options.length === 2) {
    ...
    localStorage.setItem(
      'CastleCurDailyChallenge',
      JSON.stringify(Castle.curDailyChallenge),
    );
  }
...
FoEproxy.addHandler('CastleSystemService', 'all', (data, postData) => {
  if (!data || !data.responseData || !data.requestMethod) {
    return;
  }
  ...
  if (data.requestMethod === 'getOverview') {
    Castle.dailyPointsCollectionAvailableAt =
      data.responseData.dailyPointsCollectionAvailableAt;
    localStorage.setItem(
      'CastleDailyPointsCollectionAvailableAt',
      Castle.dailyPointsCollectionAvailableAt,
    );
...
FoEproxy.addHandler(
  ...
  (data, postData) => {
    ...
    if (!d['flags'] || !d['flags'][0] || d['flags'][0] !== 'static_counter') {
      return;
    }
    ...
    if (
      d['currentProgress'] &&
      d['maxProgress'] &&
      d['maxProgress'] === Castle.SevenDayChallenge
    ) {
...
FoEproxy.addHandler('ChallengeService', 'collectReward', (data, postData) => {
  if (!data || !data.responseData || !data.responseData.__class__) {
    return;
  }

  if (data.responseData.__class__ === 'Success') {
    const curDailyChallenge = JSON.parse(
      localStorage.getItem('CastleCurDailyChallenge'),
    );
      ...
      if (d[c].type === 'daily_challenge_counter') {
        if (d[c].tasks && d[c].tasks[0] && d[c].tasks[0].currentProgress) {
          Castle.curSevenDayChallenge = {
            id: d[c].id,
            state: d[c].state,
            currentProgress: d[c].tasks[0].currentProgress,
          };
        }
      }
    }
...
let Castle = {
  ...
  BuildBox: () => {
    ...
    if (!Castle.CastlePointLog) {
      Castle.CastlePointLog = JSON.parse(
        localStorage.getItem('CastlePointLog') || '[]',
      );
    }
  ...
  UpdateCastlePoints: (rid) => {
    if (Castle.curCastlePoints === undefined) {
      Castle.curCastlePoints =
        localStorage.getItem('CastleCurCastlePoints') || 0;
    }
    ...
    if (ResourceStock.castle_points) {
      ...
      if (Castle.curCastlePoints > 0 && increased) {
        Castle.curCastlePointsDiff = {
          rid: rid,
          diff: ResourceStock.castle_points - Castle.curCastlePoints,
        };
  ...
  UpdateCastleData: (d) => {
    ...
    if (d.points && d.nextCollectionPoints) {
      ...
      localStorage.setItem(
        'CastleCurDailyCastlePoints',
        JSON.stringify(Castle.curDailyCastlePoints),
      );
    }
    ...
    if (d.nextCastlePoints) {
      ...
      if (
        Castle.NextWinningBattlesPoints &&
      ...
      Castle.NextWinningBattlesPoints =
        n.castlePointsWinBattle !== undefined ?
          n.castlePointsWinBattle
        : Castle.NextWinningBattlesPoints;
      Castle.NextNegotiationPoints =
        n.castlePointsWinNegotation !== undefined ?
          n.castlePointsWinNegotation
        : Castle.NextNegotiationPoints;
      Castle.curWinningBattles =
        Castle.NextWinningBattlesPoints < Castle.DailyWinningBattlesReward ?
          Castle.NextWinningBattlesPoints / 2
        : Castle.DailyWinningBattles;
      Castle.curNegotiations =
        Castle.NextNegotiationPoints < Castle.DailyNegotiationsReward ?
          Castle.NextNegotiationPoints / 2
        : Castle.DailyNegotiations;
      Castle.ShopGemstonesDivisor =
        n.castlePointsItemShopGemstonesDivisor ?
          n.castlePointsItemShopGemstonesDivisor
        : Castle.ShopGemstonesDivisor;
      Castle.ShopTradeCoinsDivisor =
        n.castlePointsItemShopTradeCoinsDivisor ?
          n.castlePointsItemShopTradeCoinsDivisor
        : Castle.ShopTradeCoinsDivisor;
      Castle.AuctionWinningReward =
        n.castlePointsItemAuctionWinBidding ?
          n.castlePointsItemAuctionWinBidding
        : 30;
    ...
    if (rid && rtype) {
      Castle.UpdateCastlePointsLog(rid, rtype);
    }
  ...
  UpdateCastlePointsLog: (rid, rtype) => {
    const idx =
      Castle.CastlePointLog ?
        Castle.CastlePointLog.findIndex(
          (obj) => obj.rid && obj.rid === rid && obj.date === Castle.startOfDay,
        )
      : -1;
    ...
    if (idx >= 0) {
      ...
      localStorage.setItem(
        'CastlePointLog',
        JSON.stringify(Castle.CastlePointLog),
      );
  ...
  CreateRewardList: () => {
    ...
    let battlePointReward = 0,
      ...
      dcp = 0,
      dcr = 0, //Daily Challenge
      scp = 0,
      scr = 0, //Seven Day Challenge
      glsp = 0,
      glsr = 0, //Gex Last Sections
      sir = 0,
      sip = 0,
      sipsum = 0,
      sirsum = 0,
      siwarn = false, //Bought Shop Items
      aup = 0,
      aur = 0, //Auction
      cp,
      cpwarn = false; //Castle Points
    const startOfDay = moment(MainParser.getCurrentDateTime())
      .startOf('day')
      .unix();
    ...
    if (Castle.startOfDay !== startOfDay) {
      ...
      Castle.startOfDay = moment(MainParser.getCurrentDateTime())
        .startOf('day')
        .unix();
    }
    ...
    for (
      let i = Castle.DailyWinningBattlesReward;
      i > Castle.NextWinningBattlesPoints;
      i -= 2
    ) {
      battlePointReward += i;
    }
    ...
    d.push({
      ...
      warning: false,
      success:
        Castle.DailyWinningBattles - Castle.curWinningBattles ===
        Castle.DailyWinningBattles,
      date: startOfDay,
    });
    ...
    for (
      let i = Castle.DailyNegotiationsReward;
      i > Castle.NextNegotiationPoints;
      i -= 2
    ) {
    ...
    d.push({
      ...
      warning: false,
      success:
        Castle.DailyNegotiations - Castle.curNegotiations ===
        Castle.DailyNegotiations,
      date: startOfDay,
    });
    ...
    if (Castle.curDailyCastlePoints === undefined) {
      Castle.curDailyCastlePoints = JSON.parse(
        localStorage.getItem('CastleCurDailyCastlePoints'),
      );
    }

    const colAvailableAt =
      localStorage.getItem('CastleDailyPointsCollectionAvailableAt') || 0;
    ...
    if (Castle.curDailyCastlePoints) {
      ...
      if (
        Castle.dailyPointsCollectionAvailableAt &&
        Castle.dailyPointsCollectionAvailableAt > startOfDay
      ) {
        cp.success = true;
    ...
    if (MainParser.UnlockedFeatures.includes('daily_challenges')) {
      if (
        Castle.curDailyChallenge === undefined ||
        Castle.curDailyChallenge.state === 'success'
      ) {
        dcp = 1;
    ...
    if (MainParser.UnlockedFeatures.includes('daily_challenges')) {
      scp =
        Castle.curSevenDayChallenge ?
          Castle.curSevenDayChallenge.currentProgress
        : 0;
    ...
    if (MainParser.UnlockedFeatures.includes('guild_expedition')) {
      ...
      if (Castle.curGexLastOfSection === undefined) {
        Castle.curGexLastOfSection = localStorage.getItem(
          'CastleCurGexLastOfSection',
        );
      }
    ...
    if (MainParser.UnlockedFeatures.includes('antiques_dealer')) {
      if (Castle.curShopItems === undefined) {
        Castle.curShopItems = JSON.parse(
          localStorage.getItem('CastleCurShopItems'),
        );
      }
      ...
      if (Castle.curShopItems && Castle.curShopItems.date === startOfDay) {
        ...
        sirsum =
          Castle.curShopItems.purchased.reward +
          Castle.curShopItems.available.reward;
        sip = Castle.curShopItems.purchased.count;
        sipsum =
          Castle.curShopItems.purchased.count +
          Castle.curShopItems.available.count;
      } else {
        sipsum = 6;
        siwarn = true;
      }
      ...
      d.push({
        ...
        maxreward: sirsum,
        warning: siwarn,
        warnnotice: HTML.i18nTooltip(
          i18n('Boxes.Castle.VisitAntiqueDealerWarning'),
        ),
        success: sip === sipsum,
        date: startOfDay,
      });
    ...
    if (MainParser.UnlockedFeatures.includes('antiques_dealer')) {
      if (Castle.curAuctionWinning === undefined) {
        Castle.curAuctionWinning = JSON.parse(
          localStorage.getItem('CastleCurAuctionWinning'),
        );
      }

      if (
        Castle.curAuctionWinning &&
        Castle.curAuctionWinning['date'] &&
        Castle.curAuctionWinning['rewards'] &&
        Castle.curAuctionWinning.date === startOfDay
      ) {
    ...
    $('#CastleBody')
      ...
      .done(function () {
        $('#Castle').on('click', '#casSwitchView', function () {
          Castle.CurrentView =
            Castle.CurrentView === 'log' ? 'overview' : 'log';
  ...
  ShowCastlePoints: (diff = null) => {
    if ($('#Castle #casPointsWrapper').length === 1) {
      let CastleLimit =
        MainParser.CastleSystemLevels[Castle.curLevel].requiredPoints;

      $('#Castle #casPointsWrapper')
        .html(
          `
  ...
  ShowProgressTable: () => {
    if ($('#Castle').length === 0 || Castle.CurrentView !== 'overview') {
      return;
    }
    ...
    if (!Castle.Settings.showGroupNames) {
      h.push(
        `<thead class="sticky"><tr class="caption"><th>${i18n('Boxes.Castle.Type')}</th><th class="text-right"><span>${HTML.i18nTooltip(i18n('Boxes.Castle.Progress'))}</span></th><th class="text-right"><span>${HTML.i18nTooltip(i18n('Boxes.Castle.CastlePoints'))}</span></th></tr></thead>`,
      );
    }
      ...
      list
        .map((i) => {
          ...
          if (Castle.Settings.showGroupNames && i.sort === 1) {
            let GroupName = Object.keys(Castle.RewardGroups).find(
              (k) => Castle.RewardGroups[k] === i.group,
            );
            r =
              r +
              `<tr class="caption"><td>${i18n('Boxes.Castle.' + GroupName)}</td><td class="text-right"><span>${firstCaption ? i18n('Boxes.Castle.Progress') : ''}</span></td><td class="text-right"><span>${firstCaption ? i18n('Boxes.Castle.CastlePoints') : ''}</span></td></tr>`;
            firstCaption = false;
          }

          let progressStr =
            i.progress === i.maxprogress ?
              i.maxprogress
            : i.progress + ' / ' + i.maxprogress;
          let rewardStr =
            i.reward === i.maxreward ?
              i.maxreward
            : i.reward + ' / ' + i.maxreward;

          return (
            r +
            `<tr ${i.warning && i.warnnotice ? ' title="' + i.warnnotice + '" ' : ''}class="${i.warning ? 'warning ' : ''}${i.success ? 'bg-green' : 'pending'}">
                <td>${i.name}</td>
  ...
  ShowLog: () => {
    ...
    h.push(
      Castle.CastlePointLog.sort((a, b) => b.time - a.time)
        .map((e) => {
          let r = '';
          if (!c.includes(e.date)) {
            r =
              r +
              `<tr class="caption open" data-group="${e.date}"><td>${moment.unix(e.date).format(i18n('Date'))} <span></span></td>
          ...
          return (
            r +
            `<tr data-group="${e.date}"><td>${moment.unix(e.time).format(i18n('DateTime'))}</td>
                <td>${
                  e.name && e.name.length ?
                    e.name
                      .map((n) => {
                        return typeNames[n] ? typeNames[n] : '';
                      })
                      .join(' + ')
                  : i18n('Boxes.Castle.Unknown')
                }</td>
    ...
    h.push(
      `<div id="casLogFooter">${i18n('Boxes.Castle.LogDuration') + ': ' + Castle.Settings.logDays + ' ' + i18n('Boxes.Castle.Days')}</div>`,
    );
    h.push(`</div>`);
    ...
    $('#casLogTable tbody tr.caption').on('click', function () {
      ...
      $('#casLogTable tr[data-group="' + group + '"]')
        .not('.caption')
        .toggleClass('hide');
    });
    ...
    $('#casLogTable thead tr.caption').on('click', function () {
      ...
      $(this).toggleClass('closed open');
      $('#casLogTable tbody tr[data-group].caption')
        .removeClass('closed')
        .removeClass('open')
        .addClass(isopen ? 'closed' : 'open');
      $('#casLogTable tr[data-group]')
        .not('.caption')
        .removeClass('hide')
        .removeClass('show')
        .addClass(tc);
  ...
  CastleSettings: () => {
    ...
    c.push(
      `<p class="text-left"><span class="settingtitle">${i18n('Boxes.Castle.Overview')}</span><input id="casShowGroupNames" name="showgroupnames" value="1" type="checkbox" ${Settings.showGroupNames === true ? ' checked="checked"' : ''} /> <label for="casShowGroupNames">${i18n('Boxes.Castle.ShowGroupNames')}</label></p>`,
    );
    c.push(
      `<p class="text-left"><span class="settingtitle">${i18n('Boxes.Castle.Log')}</span><label for="casLogDuration">${i18n('Boxes.Castle.LogDuration')}</label> <select id="casLogDuration" name="logduration">`,
    );
    logdurations.forEach((l) => {
      c.push(
        `<option value="${l}"${Castle.Settings.logDays === l ? ' selected="selected"' : ''}>${l + ' ' + i18n('Boxes.Castle.Days')}</option>`,
      );
    });
    c.push(`</select></p>`);

    c.push(
      `<hr><p><button id="save-Castle-settings" class="btn" style="width:100%" onclick="Castle.SettingsSaveValues()">${i18n('Boxes.General.Save')}</button></p>`,
    );
  ...
  SettingsSaveValues: () => {
    ...
    Castle.Settings.logDays =
      parseInt($('#casLogDuration').val()) || Castle.Settings.logDays;
