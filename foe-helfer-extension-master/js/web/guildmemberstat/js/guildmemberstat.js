...
FoEproxy.addHandler('ClanService', 'getOwnClanData', (data, postData) => {
  ...
  if (requestMethod === 'getOwnClanData') {
    ...
    if (
      GuildMemberStat.Data !== undefined &&
      !GuildMemberStat.hasUpdateProgress
    ) {
...
FoEproxy.addHandler('ClanService', 'getTreasuryBag', (data, postData) => {
  if (
    data.responseData?.type?.value &&
    data.responseData?.type?.value !== 'ClanMain'
  )
    return; // for now ignore all other source types
  let requestMethod = postData[0]['requestMethod'];
  if (requestMethod === 'getTreasuryBag') {
    let Goods = data.responseData.resources.resources;

    if (Goods !== undefined) GuildMemberStat.setEraGoods(Goods);
  }
});
...
FoEproxy.addHandler(
  ...
  (data, postData) => {
    ...
    if (ConversationData !== undefined) {
      ...
      if (
        ConversationData.type !== undefined &&
        (ConversationData.type === 3 || ConversationData.type === 7)
      ) {
...
FoEproxy.addHandler(
  ...
  (data, postData) => {
    ...
    if (Data !== undefined) {
      ...
      else GuildMemberStat.GEXId = Data.nextStateTime * 1 - 86400;
    }
  },
);
...
FoEproxy.addHandler(
  ...
  (data, postData) => {
    let Data = data.responseData;
    if (Data !== undefined) GuildMemberStat.GBGId = Data.endsAt;
  },
);
  ...
  (data, postData) => {
    if (data.responseData['stateId'] !== 'participating') {
      ...
      if (Data !== undefined) {
        ...
        GuildMemberStat.UpdateData(
          'gbg',
          data.responseData['playerLeaderboardEntries'],
        );
      }
    }
  },
...
let GuildMemberStat = {
  ...
  ReadGuildMemberBuildings: async (Buildings, Member) => {
    ...
    for (let i in entity) {
      ...
      if (entity.hasOwnProperty(i)) {
        ...
        let EntityEraId =
          entity[i].level !== undefined ? entity[i].level + 1 : null;
        let EntityEra =
          EntityEraId !== null ? Technologies.EraNames[EntityEraId] : null;
        ...
        if (entity[i]['type'] === 'greatbuilding') {
          let lgd = {
            ...
            forge_points_for_level_up:
              entity[i]['state']['forge_points_for_level_up'],
          };
        ...
        if (
          CityEntity['type'] &&
          CityEntity['type'] === 'clan_power_production'
        ) {
          let EntityLevel = EntityEra ? EntityEra : Member.era;
          let value = CityEntity['entity_levels'].find(
            (data) => data.era === EntityLevel,
          );
          let clan_power =
            typeof value.clan_power !== 'undefined' ? value.clan_power : 0;

          GuildPowerBuildings.push({
            gbid: GBTempID,
            entity_id: EntityID,
            name: CityEntity['name'],
            power: { value: clan_power, motivateable: null },
            level: EntityEraId,
            era: Member.era,
          });
        }
        ...
        if (CityEntity['abilities']) {
          for (let AbilityIndex in CityEntity['abilities']) {
            if (
              !CityEntity['abilities'].hasOwnProperty(AbilityIndex) ||
              CityEntity['abilities'][AbilityIndex]['__class__'] ===
                undefined ||
              CityEntity['abilities'][AbilityIndex]['__class__'] !==
                'AddResourcesToGuildTreasuryAbility'
            )
              continue;
            ...
            if (
              EntityEra !== null &&
              Ability['additionalResources'][EntityEra] !== undefined &&
              Ability['additionalResources'][EntityEra]['resources'] !==
                undefined &&
              Ability['additionalResources'][EntityEra]['resources'][
                'clan_power'
              ] !== undefined
            ...
            if (EntityEra !== null) {
              goods = Object.values(GoodsData)
                .filter(function (Good) {
                  return (
                    Good.era === EntityEra &&
                    Good.abilities.goodsProduceable !== undefined
                  );
                })
                .map(function (row) {
                ...
                .sort(function (a, b) {
                  return a.good_id.localeCompare(b.good_id);
                });
            }

            GuildGoodsBuildings.push({
              gbid: GBTempID,
              entity_id: EntityID,
              name: CityEntity['name'],
              resources: { totalgoods: goodSum, goods: goods },
              level: EntityEraId,
              era: EntityEra,
            });
          }
        ...
        if (
          EntityEra &&
          CityEntity['components'] &&
          CityEntity['components'][EntityEra] &&
          CityEntity['components'][EntityEra]['production'] &&
          CityEntity['components'][EntityEra]['production']['options']
        ) {
          let opt =
            CityEntity['components'][EntityEra]['production']['options'];
          let goods = null;
            ...
            for (let p in products) {
              ...
              GuildGoodsBuildings.push({
                ...
                resources: {
                  totalgoods: goodSum,
                  goods: goods,
                  onlywhenmotivated: onlywhenmotivated,
                },
                level: EntityEraId,
                era: EntityEra,
              });
          ...
          EntityEra &&
          CityEntity?.components?.AllAge?.production?.options
        ) {
        ...
        if (entity[i].state && entity[i]['state']['current_product']) {
          ...
          let totalgoods = entity[i]['state']['current_product']['goods']
            .map((good) => good.value)
            .reduce((sum, good) => good + sum);
          ...
          GuildGoodsBuildings.push({
            ...
            resources: {
              ...
              goods: entity[i]['state']['current_product']['goods'].sort(
                function (a, b) {
                  return a.good_id.localeCompare(b.good_id);
                },
              ),
    ...
    let PlayerGuildBuildings = {
      player_id: Member.player_id,
      era: Member.era,
      guildbuildings: GuildPowerBuildings.concat(GuildGoodsBuildings),
      greatbuildings: LgBuildings,
    };
  ...
  UpdateData: async (source, data) => {
    ...
    GuildMemberStat.hasGuildMemberRights =
      (ExtGuildPermission & GuildMemberStat.GuildPermission_Leader) > 0 ||
      (ExtGuildPermission & GuildMemberStat.GuildPermission_Founder) > 0;

    switch (source) {
      case 'clandata':
        let currentClanId =
          GuildMemberStat.Data.id !== undefined ?
            GuildMemberStat.Data.id
          : undefined;
        ...
        let ActiveMembers = [];
        let localClanId = JSON.parse(
          localStorage.getItem('GuildMemberStatClanId'),
        );
        ...
        let deleteOlderThan =
          localClanId !== currentClanId ? -1 : (
            GuildMemberStat.Settings.deleteOlderThan
          );
        ...
        for (let i in memberdata) {
          if (memberdata.hasOwnProperty(i)) {
            memberdata[i]['activity'] =
              GuildMemberStat.hasGuildMemberRights ?
                memberdata[i]['activity']
              : null;
            memberdata[i]['won_battles'] =
              memberdata[i]['won_battles'] ? memberdata[i]['won_battles'] : 0;
        ...
        if (
          MainParser.CityMapData &&
          Object.keys(MainParser.CityMapData).length > 0
        ) {
          let self = {
            player_id: ExtPlayerID,
            era: CurrentEra,
          };
          await GuildMemberStat.ReadGuildMemberBuildings(
            { city_map: { entities: Object.values(MainParser.CityMapData) } },
            self,
          );
        }
        ...
        localStorage.setItem(
          ...
          JSON.stringify(GuildMemberStat.Settings),
        );
        ...
        if (
          GuildMemberStat.hasGuildMemberRights &&
          GuildMemberStat.Settings.autoStartOnUpdate
        ) {
          GuildMemberStat.CurrentStatGroup = 'Member';
      ...
      case 'forum':
        ...
        for (let i in messagedata) {
          if (messagedata.hasOwnProperty(i)) {
            if (
              typeof messagedata[i].conversationId === 'undefined' &&
              !GuildMemberStat.ConversationIds.includes(
                messagedata[i].conversationId,
              )
            ) {
              continue;
            }
            ...
            if (
              typeof messagedata[i].sender != 'undefined' &&
              typeof messagedata[i].sender.player_id != 'undefined'
            ) {
              let m = {
      ...
      case 'guildbuildings':
        ...
        if (
          ...
          data['guildbuildings'] !== undefined &&
          data['greatbuildings'] !== undefined
        ) {
  ...
  SetActivityWarning: async (Warning, force) => {
    ...
    if (CurrentMember === undefined) {
      ...
      if (
        moment(MainParser.getCurrentDate()).format('DD.MM.YYYY') !==
          moment(CurrentMember.lastwarn).format('DD.MM.YYYY') ||
        CurrentMember.lastactivity !== Warning.lastactivity
      ) {
        await GuildMemberStat.db.activity
          .where('player_id')
          .equals(playerID)
          .modify((x) => x.warnings.push(Warning.warnings[0]));
  ...
  RefreshPlayerGBGDB: async (Member) => {
    ...
    let battlesWon =
      typeof GBGPlayer['battlesWon'] != 'undefined' ?
        GBGPlayer['battlesWon']
      : 0;
    let negotiationsWon =
      typeof GBGPlayer['negotiationsWon'] != 'undefined' ?
        GBGPlayer['negotiationsWon']
      : 0;
  ...
  RefreshPlayerGexDB: async (Member) => {
    ...
    let solvedEncounters =
      typeof GexPlayer['solvedEncounters'] != 'undefined' ?
        GexPlayer['solvedEncounters']
      : 0;
    let expeditionPoints =
      typeof GexPlayer['expeditionPoints'] != 'undefined' ?
        GexPlayer['expeditionPoints']
      : 0;
  ...
  RefreshGuildMemberDB: async (Member) => {
    ...
    if (CurrentMember === undefined) {
      ...
      let prevRank =
        typeof CurrentMember.rank !== 'undefined' ?
  ...
  MarkMemberAsDeleted: async (arr) => {
    let unknownMembers = await GuildMemberStat.db.player
      .where('player_id')
      .noneOf(arr)
      .toArray();
  ...
  DeleteExMembersOlderThan: async (days) => {
    let currentExMembers = await GuildMemberStat.db.player
      .where('deleted')
      .notEqual(0)
      .toArray();
    ...
    currentExMembers.forEach((member) => {
      ...
      if (Math.floor((+MainParser.getCurrentDate() - time) / 86400000) > days) {
        ...
        db.transaction(
          ...
          db.forum,
          async () => {
            await GuildMemberStat.db.player
              .where('id')
              .equals(member.id)
              .delete();
            await GuildMemberStat.db.gex
              .where('player_id')
              .equals(member.player_id)
              .delete();
            await GuildMemberStat.db.gbg
              .where('player_id')
              .equals(member.player_id)
              .delete();
            await GuildMemberStat.db.activity
              ...
              .equals(member.player_id)
              .delete();
            await GuildMemberStat.db.warning
              .where('player_id')
              .equals(member.player_id)
              .delete();
            await GuildMemberStat.db.forum
              .where('player_id')
              .equals(member.player_id)
              .delete();
          },
  ...
  Show: async () => {
    ...
    h.push(
      `<li${GuildMemberStat.CurrentStatGroup === 'Member' ? ' class="active"' : ''}><a class="toggle-statistic" data-value="Member"><span>${i18n('Boxes.GuildMemberStat.GuildMembers')}</span></a></li>`,
    );
    h.push(
      `<li${GuildMemberStat.CurrentStatGroup === 'Eras' ? ' class="active"' : ''}><a class="toggle-statistic" data-value="Eras"><span>${i18n('Boxes.GuildMemberStat.Eras')}</span></a></li>`,
    );
    h.push(
      `<li${GuildMemberStat.CurrentStatGroup === 'GreatBuildings' ? ' class="active"' : ''}><a class="toggle-statistic" data-value="GreatBuildings"><span>${i18n('Boxes.GuildMemberStat.GreatBuildings')}</span></a></li>`,
    );
    h.push(
      `<li${GuildMemberStat.CurrentStatGroup === 'GuildBuildings' ? ' class="active"' : ''}><a class="toggle-statistic" data-value="GuildBuildings"><span>${i18n('Boxes.GuildMemberStat.GuildBuildings')}</span></a></li>`,
    );
    h.push(
      `<li${GuildMemberStat.CurrentStatGroup === 'GuildGoods' ? ' class="active"' : ''}><a class="toggle-statistic" data-value="GuildGoods"><span>${i18n('Boxes.GuildMemberStat.GuildGoods')}</span></a></li>`,
    );

    if (GuildMemberStat.Settings.showSearchbar) {
      h.push(
        `<li style="float:right"><input type="text" name="filter" id="gms-filter-input" placeholder="${i18n('Boxes.GuildMemberStat.Search')}" /></li>`,
      );
    }
    ...
    if (GuildMemberStat.Settings.showBattlesWon) {
      h.push(
        `<th class="is-number" data-type="gms-group">${i18n('Boxes.GuildMemberStat.Battles')}</th>`,
      );
    }
    ...
    if (GuildMemberStat.hasGuildMemberRights) {
      h.push(
        `<th class="is-number gms-tooltip" data-type="gms-group" title="${HTML.i18nTooltip(i18n('Boxes.GuildMemberStat.MemberActiviy'))}"><span class="activity"></span></th>`,
      );
    }
    ...
    let CurrentGexActivity = await GuildMemberStat.db.gex
      .where('player_id')
      .above(0)
      .and(function (item) {
    ...
    let CurrentGbgActivity = await GuildMemberStat.db.gbg
      .where('player_id')
      .above(0)
      .and(function (item) {
        if (GuildMemberStat.Settings.showZeroValues === 1) {
    ...
    for (let x = 0; x < data.length; x++) {
      ...
      member['guildgoods'] = 0;
      if (
        member['guildbuildings'] !== undefined &&
        member['guildbuildings']['buildings'] !== undefined
      ) {
      ...
      let deletedMember =
        typeof member['deleted'] !== 'undefined' && member['deleted'] !== 0;
      ...
      member['rank'] =
        typeof member['rank'] !== 'undefined' ? member['rank'] : [rank, rank];
      let rankDiffClass =
        member['rank'] && member['rank'][1] > member['rank'][0] ? ' decreased'
        : member['rank'][0] > member['rank'][1] ? ' increased'
        : '';

      scoreDiff = scoreDiff > 0 ? '+' + scoreDiff : scoreDiff;

      // build an dictionary for detail views
      if (GuildMemberStat.MemberDict[MemberID] === undefined)
        GuildMemberStat.MemberDict[MemberID] = {};
      if (ActWarnCount > 0)
        GuildMemberStat.MemberDict[MemberID]['activity'] = activityWarnings;
      if (gexActivityCount > 0)
        GuildMemberStat.MemberDict[MemberID]['gex'] = gexActivity;
      if (gbgActivityCount > 0)
        GuildMemberStat.MemberDict[MemberID]['gbg'] = gbgActivity;
      if (guildBuildingsCount > 0)
        GuildMemberStat.MemberDict[MemberID]['guildbuildings'] =
          member['guildbuildings'];
      if (guildBuildingsCount > 0)
        GuildMemberStat.MemberDict[MemberID]['guildgoods'] =
          member['guildgoods'];
      ...
      if (GuildMemberStat.Settings.showBattlesWon) {
        ...
        h.push(
          `<td class="is-number" data-number="${member['won_battles']}">${HTML.Format(member['won_battles'] ? member['won_battles'] : 0)}${battleDiff > 0 ? '<span class="prev_score green">+' + HTML.Format(battleDiff) + '</span>' : ''}</td>`,
        );
      }

      h.push(
        `<td class="is-number" data-number="${Technologies.Eras[member['era']]}">${i18n('Eras.' + Technologies.Eras[member['era']] + '.short')}</td>`,
      );
      h.push(
      ...
      if (GuildMemberStat.hasGuildMemberRights)
        h.push(
          `<td class="is-number" data-number="${member['activity']}"><span class="activity activity_${member['activity']}"></span> ${ActWarnCount > 0 ? '<span class="warn">(' + ActWarnCount + ')</span>' : ''}</td>`,
        );

      h.push(
        `<td class="is-number text-center" data-number="${forumActivityCount}">${forumActivityCount}</td>`,
      );
      h.push(
      ...
      exportData.push([
        ...
        member['name'],
        member['score'],
        Technologies.Eras[member['era']],
        i18n('Eras.' + Technologies.Eras[member['era']] + '.short'),
        member['guildgoods'],
        ActWarnCount,
        forumActivityCount,
        gexActivityCount,
    ...
    if (GuildMemberStat.Settings.lastupdate !== 0) {
      ...
      h.push(
        `<div class="last-update-message"><span class="icon ${uptodateClass}"></span> <span class="${uptodateClass}">${moment(GuildMemberStat.Settings.lastupdate).format(i18n('DateTime'))}</span></div>`,
      );
    }
    ...
    $('#GuildMemberStatBody')
      ...
      .done(function () {
        ...
        $('#GuildMemberTable thead .expand-all')
          .off('click')
          .on('click', async function (e) {
            ...
            if (expand === true) {
              helper.preloader.show('#GuildMemberStat');
              setTimeout(() => {
                GuildMemberStat.ShowMemberDetail(tr, expand), 300;
              });
            } else {
        ...
        $('#GuildMemberTable > thead th:not(:last)').on('click', function () {
          ...
          $('#GuildMemberTable th.expand-all')
            .removeClass('open')
            .addClass('closed');
        });
  ...
  ShowMemberDetail: (arr, expand) => {
    ...
    $.each(arr, function () {
      ...
      if (Member['activity'] !== undefined) {
        d.push(
          `<div class="detail-item warnings"><div class="scrollable"><table><thead class="sticky"><tr><th>${i18n('Boxes.GuildMemberStat.Inactivity')}</th><th>${i18n('Boxes.GuildMemberStat.Date')}</th><th class="text-right"><span class="edit"></span></th></tr></thead><tbody class="copyable">`,
        );
          ...
          if (warnings.hasOwnProperty(i)) {
            ...
            if (warnlist.length >= 1) {
              for (let k in warnlist) {
                ...
                d.push(
                  `<tr><td><img class="small" src="${extUrl}js/web/guildmemberstat/images/act_${warnlist[k].activity}.png" /> #${warnlist.length - parseInt(k)}<span class="hidden-text">&nbsp;-&nbsp;${activityWarnState[warnlist[k].activity]}</span></td>` +
      ...
      if (Member['gex'] !== undefined) {
        d.push(
          `<div class="detail-item gex"><div class="scrollable"><table><thead class="sticky"><tr><th><span class="gex"></span> ${i18n('Boxes.GuildMemberStat.GEXWeek')}</th><th>${i18n('Boxes.GuildMemberStat.Rank')}</th>` +
        ...
        for (let i in gex) {
          ...
          let activeGexClass =
            gex[i].gexweek >= currentTime ? ' activeCircle' : '';
          let tooltip =
            gexstartdate.format(i18n('Date')) +
            ' - ' +
            gexenddate.format(i18n('Date'));
          let strDate =
            GuildMemberStat.Settings.gexgbgDateFormat === 'date' ? gexdate
            : GuildMemberStat.Settings.gexgbgDateFormat === 'enddate' ?
              gexenddate.format(i18n('Date'))
            : gexweek;

          d.push(
            `<tr><td><span class="gms-tooltip" title="${HTML.i18nTooltip(tooltip)}">${strDate}</span><span class="${activeGexClass}"></span></td><td>${gex[i].rank}</td>` +
      ...
      if (Member['gbg'] !== undefined) {
        ...
        for (let i in gbg) {
          ...
          let activeGbgClass =
            gbg[i].gbgid >= currentTime ? ' activeCircle' : '';
          let gbgenddate = moment.unix(gbg[i].gbgid);
          let gbgstartdate = moment.unix(gbg[i].gbgid).subtract(11, 'd');
          let tooltip =
            gbgstartdate.format(i18n('Date')) +
            ' - ' +
            gbgenddate.format(i18n('Date'));
          let week = moment.unix(gbg[i].gbgid).week();
          let lastweek = week - 1;

          week = week.toString().length === 1 ? '0' + week : week;
          lastweek =
            lastweek.toString().length === 1 ? '0' + lastweek : lastweek;

          let strDate =
            GuildMemberStat.Settings.gexgbgDateFormat === 'date' ?
              gbgstartdate.format(i18n('Date'))
            : GuildMemberStat.Settings.gexgbgDateFormat === 'enddate' ?
              gbgenddate.format(i18n('Date'))
            : `${moment.unix(gbg[i].gbgid).year()} - ${lastweek}/${week}`;

          d.push(
            `<tr><td><span class="gms-tooltip" title="${HTML.i18nTooltip(tooltip)}">${strDate}</span><span class="${activeGbgClass}"></span></td>` +
              `<td>${gbg[i].rank}</td>` +
      ...
      if (Member['guildbuildings'] !== undefined) {
        ...
        d.push(
          `<div class="detail-item buildings"><table><thead class="sticky ${expand === true ? 'open ' : ''}hasdetail"><tr><th><span class="guildbuild"></span> ${i18n('Boxes.GuildMemberStat.GuildSupportBuildings')} (${i18n('Boxes.GuildMemberStat.LastUpdate') + ' ' + moment(guildbuildings.date).fromNow()})</th><th></th></tr></thead><tbody class="${expand !== true ? 'closed ' : ''}copyable">`,
        );

        // Group GuildGoods buildings by name and their era
        let guildGoodsBuildings = guildbuildings['buildings']
          .filter(function (data) {
            return data.resources !== undefined;
          })
          .reduce(
            function (res, obj) {
        ...
        let guildPowerBuildings = guildbuildings['buildings']
          .filter(function (data) {
        ...
        d.push(
          `<tr class="nohover"><td colspan="2"><div class="detail-wrapper">`,
        );

        if (guildGoodsBuildings.length) {
          d.push(
            `<div class="detail-item guildgoods"><table class="copyable"><thead class="sticky"><tr><th colspan="3"><i>${i18n('Boxes.GuildMemberStat.GuildGoods')}</i></th></tr></thead><tbody>`,
          );

          guildGoodsBuildings.forEach((plbuilding) => {
            let goodslist = '';
            let countBuilding =
              typeof plbuilding.count != 'undefined' ? plbuilding.count : 1;
            let goodCount =
              plbuilding.resources && plbuilding.resources.totalgoods ?
                plbuilding.resources.totalgoods
              : 0;
            totalGoods += goodCount;
            if (
              plbuilding.resources.goods !== undefined &&
              plbuilding.resources.goods !== null
            ) {
              goodslist = plbuilding.resources.goods
                .map((good) => {
            ...
            if (plbuilding.level === null) {
              ...
              LevelString =
                '(' + i18n('Eras.' + plbuilding.level + '.short') + ')';
            }

            d.push(
              `<tr><td>${countBuilding} x ${plbuilding.name.replace(/\#[0-9]+\#/, '')} ${LevelString}</td><td class="text-right">${goodslist !== '' ? `<span class="goods-count">${goodCount / 5}x</span>${goodslist}` : ''}</td><td class="text-right">${HTML.Format(goodCount)}</td></tr>`,
            );
          });

          d.push(
            `<tr><td class="text-bright">${i18n('Boxes.GuildMemberStat.TotalGuildGoods')}</td><td></td><td class="text-right text-bright">${HTML.Format(totalGoods)}</td></tr>`,
          );
        ...
        if (guildPowerBuildings.length) {
          d.push(
            `<div class="detail-item guildgoods"><table class="copyable"><thead class="sticky"><tr><th colspan="2"><i>${i18n('Boxes.GuildMemberStat.GuildPower')}</i></th></tr></thead><tbody>`,
          );
          ...
          guildPowerBuildings.forEach((plbuilding) => {
            let countBuilding =
              typeof plbuilding.count != 'undefined' ? plbuilding.count : 1;
            let powerCount =
              plbuilding.power && plbuilding.power.value ?
                plbuilding.power.value
              : 0;
            totalPower += powerCount;
            d.push(
              `<tr><td>${countBuilding} x  ${plbuilding.name}</td><td class="text-right">${HTML.Format(powerCount)}</td></tr>`,
            );
          });

          d.push(
            `<tr><td class="text-bright">${i18n('Boxes.GuildMemberStat.TotalGuildPower')}</td><td class="text-right text-bright">${HTML.Format(totalPower)}</td></tr>`,
          );
          d.push(`</tbody></table></div>`);
        }
      ...
      .on('click', function (e) {
        ...
        if (!GuildMemberStat.acceptedDeleteWarning) {
          $('<div/>', {
            ...
            html:
              '<div class="warningoverlay-content">' +
              i18n('Boxes.GuildMemberStat.DeleteDataWarning') +
              '<br /><br /><button id="gms-accept-modal" class="btn">' +
              i18n('Boxes.GuildMemberStat.GotIt') +
              '</button><div>',
          }).appendTo('#GuildMemberStatBody');

          $('#GuildMemberStatBody').on(
            'click',
            '#gms-accept-modal',
            function () {
              $('#gms-modal-warning').fadeOut(400, function () {
                $('#gms-modal-warning').remove();
                GuildMemberStat.acceptedDeleteWarning = true;
              });
            },
          );
        }
    ...
    $('#GuildMemberTable .deleteInactivity')
      .off('click')
      .on('click', function () {
        let button = $(this);
        ...
        let delObj = {
          player_id: player_id,
          data: GuildMemberStat.MemberDict[player_id]['activity'][0][
            'warnings'
          ][index],
          content: 'activity',
        };
    ...
    // Delete an gex entry
    $('#GuildMemberTable .deleteGexWeek')
      .off('click')
      .on('click', function () {
        let button = $(this);
        let delObj = {
    ...
    $('#GuildMemberTable thead.hasdetail')
      .off('click')
      .on('click', function () {
        let thead = $(this);
  ...
  ShowGuildEras: async () => {
    ...
    let GuildMembers = await GuildMemberStat.db.player
      .where({ deleted: 0 })
      .reverse()
      .sortBy('score');
    let d = [];
    ...
    let hasTreasuryTotals =
      TreasuryGoodsData !== undefined &&
      TreasuryGoodsData !== null &&
      TreasuryGoodsData.hasOwnProperty('totals');
    let ExportContent = (GuildMemberStat.ExportData = []);

    // group members to era
    let EraGroup = (GuildMemberStat.EraGroup = GuildMembers.reduce(
      (res, obj) => {
    ...
    if (EraGroup) {
      ...
      d.push(
        `<th class="is-number ${hasTreasuryTotals ? 'text-right' : 'text-center'}" data-type="gms-era">${i18n('Boxes.GuildMemberStat.TreasuryGoods')}</th>`,
      );
      d.push(
        `<th class="is-number text-right" data-type="gms-era">${i18n('Boxes.GuildMemberStat.Points')}</th>` +
      ...
      for (let era in EraGroup) {
        if (!EraGroup.hasOwnProperty(era)) break;

        let countEra =
          typeof EraGroup[era].members.length != 'undefined' ?
            EraGroup[era].members.length
          : 1;
        let eraTotals = 0;

        d.push(
          `<tr id="era${EraGroup[era].eraId}" data-id="${EraGroup[era].eraId}" class="hasdetail">` +
        ...
        if (hasTreasuryTotals) {
          eraTotals =
            TreasuryGoodsData['totals'].hasOwnProperty([EraGroup[era].era]) ?
              TreasuryGoodsData.totals[EraGroup[era].era]
            : 0;
          d.push(
            `<td title="${HTML.i18nTooltip(i18n('Boxes.GuildMemberStat.LastUpdate')) + ' ' + moment(TreasuryGoodsData.updated).fromNow()}" class="is-number text-right gms-tooltip" data-number="${eraTotals}">${HTML.Format(eraTotals)}</td>`,
          );
        } else {
          d.push(
            `<td title="${HTML.i18nTooltip(i18n('Boxes.GuildMemberStat.GuildTreasuryNotification'))}" class="gms-tooltip is-number text-center" data-number="${EraGroup[era].eraId}">-</td>`,
          );
        }
    ...
    $('#gmsContentWrapper')
      ...
      .done(function () {
        ...
        $('#GuildErasTable thead .expand-all')
          .off('click')
          .on('click', async function (e) {
            e.preventDefault();
            ...
            if (expand === true) {
              helper.preloader.show('#GuildMemberStat');
              setTimeout(() => {
                GuildMemberStat.ShowGuildEraDetail(tr, expand), 300;
              });
            } else {
  ...
  ShowGuildEraDetail: (arr, expand) => {
    ...
    $.each(arr, function () {
      ...
      if (e.next('tr.detailview').length) {
        ...
        if (!e.hasClass('hasdetail')) return;
        ...
        h.push(
          `<tr class="detailview dark-bg"><td colspan="${e.find('td').length}"><div class="detail-wrapper"><div class="detail-item">` +
        ...
        if (EraGroup[eraId].members !== undefined) {
          let EraMembers = EraGroup[eraId].members.sort(function (a, b) {
            return a.score > b.score;
          });

          EraMembers.forEach((member) => {
            h.push(
              `<tr><td>${member.rank[1]}</td><td>${MainParser.GetPlayerLink(member.player_id, member.name)}</td><td>${i18n('Eras.' + eraId)}</td><td class="text-right">${HTML.Format(member.score)}</td></tr>`,
            );
          });
        }
        ...
        if (
          TreasuryGoodsData !== null &&
          typeof TreasuryGoodsData[EraGroup[eraId].era] !== 'undefined'
        ) {
          let EraTreasuryGoods = TreasuryGoodsData[EraGroup[eraId].era];

          h.push(
            `<div class="detail-item"><table><thead class="sticky"><tr><th colspan="3">${i18n('Boxes.GuildMemberStat.EraTreasuryGoods')}</th></tr></thead><tbody>`,
          );
          EraTreasuryGoods.forEach((good) => {
            h.push(
              `<tr><td class="goods-image"><span class="goods-sprite sprite-35 ${good.good}"></span></td><td>${good.name}</td><td class="text-right">${HTML.Format(good.value)}</td></tr>`,
            );
          });

          h.push(
            `<tr><td colspan="3" class="text-right"><i>${i18n('Boxes.GuildMemberStat.LastUpdate') + ' ' + moment(TreasuryGoodsData.updated).fromNow()}</i></td></tr>`,
          );
          h.push(`</tbody></table></div>`);
        }
        h.push('</div></td></tr>');
  ...
  ShowGuildBuildings: async () => {
    ...
    let ExportContent = (GuildMemberStat.ExportData = []);
    let gmsBuildingDict =
      await GuildMemberStat.GetGuildMemberBuildings('guildbuildings');
    ...
    d.push(
      `<div class="view_notification">${i18n('Boxes.GuildMemberStat.GuildBuildingNotification')} <button style="float:right" id="toggleBuildingView" class="btn">${i18n('Boxes.GuildMemberStat.ChangeView')}</button></div>`,
    );
    ...
    let allGuildBuildings = fullGBList
      .reduce(
        function (res, obj) {
    ...
    let guildGoodsBuildings = gmsBuildingDict
      .reduce(
        function (res, obj) {
    ...
    let guildPowerBuildings = gmsBuildingDict
      .reduce(
        function (res, obj) {
    ...
    if (guildGoodsBuildings.length) {
      d.push(
        `<div class="detail-item guildgoods"><table class="foe-table copyable sticky"><thead><tr><th colspan="3"><i>${i18n('Boxes.GuildMemberStat.GuildGoods')}</i></th></tr></thead><tbody>`,
      );
      ...
      guildGoodsBuildings.forEach((plbuilding) => {
        ...
        totalGoods += goodCount;
        d.push(
          `<tr><td class="text-right">${countBuilding} x</td><td>${plbuilding.name}</td><td class="text-right">${HTML.Format(goodCount)}</td></tr>`,
        );
      });

      d.push(
        `<tr><td></td><td class="text-bright">${i18n('Boxes.GuildMemberStat.TotalGuildGoods')}</td><td class="text-right text-bright">${HTML.Format(totalGoods)}</td></tr>`,
      );
      d.push(`</tbody></table></div>`);
    }
    ...
    if (guildPowerBuildings.length) {
      d.push(
        `<div class="detail-item guildgoods"><table class="foe-table copyable sticky"><thead><tr><th colspan="3"><i>${i18n('Boxes.GuildMemberStat.GuildPower')}</i></th></tr></thead><tbody>`,
      );
      ...
      guildPowerBuildings.forEach((plbuilding) => {
        ...
        totalPower += powerCount;
        d.push(
          `<tr><td class="text-right">${countBuilding} x</td><td>${plbuilding.name}</td><td class="text-right">${HTML.Format(powerCount)}</td></tr>`,
        );
      });

      d.push(
        `<tr><td></td><td class="text-bright">${i18n('Boxes.GuildMemberStat.TotalGuildPower')}</td><td class="text-right text-bright">${HTML.Format(totalPower)}</td></tr>`,
      );
      d.push(`</tbody></table></div>`);
    }
    ...
    d.push(
      `<div class="detail-item guildbuildings"><table id="guildbuildingslist" class="foe-table"><thead class="sticky"><tr class="sorter-header">` +
    ...
    allGuildBuildings.forEach((plbuilding) => {
      let goodCount =
        ...
        : 0;
      let powerCount =
        plbuilding.power && plbuilding.power.value ? plbuilding.power.value : 0;
      let Entity = MainParser.CityEntities[plbuilding.entity_id];
      let level =
        (
          Entity &&
          Entity['type'] === 'greatbuilding' &&
          plbuilding.level !== null &&
          plbuilding.level !== undefined
        ) ?
          plbuilding.level
        : 0;
      let goodslist = '';
      ...
      if (
        plbuilding.resources &&
        plbuilding.resources.goods &&
        plbuilding.resources.goods !== null
      ) {
        goodslist = plbuilding.resources.goods
          .map((good) => {
      ...
      ExportContent.push([
        plbuilding.name,
        plbuilding.level !== null ? plbuilding.level : '',
        Technologies.Eras[plbuilding.era],
        i18n('Eras.' + Technologies.Eras[plbuilding.era]),
        plbuilding.member,
        plbuilding.power !== undefined ? plbuilding.power.value : 0,
        plbuilding.resources !== undefined ?
          plbuilding.resources.totalgoods
        : 0,
      ]);

      d.push(
        `<tr${plbuilding.gbid === undefined ? ` class="outdated" title="${HTML.i18nTooltip(i18n('Boxes.GuildMemberStat.GuildBuildingNotification'))}"` : ''}">` +
  ...
  ShowGuildGoods: async () => {
    ...
    ErasGuildGoods = ErasGuildGoods.reduce(function (res, obj) {
      if (
        ...
        obj.resources.goods !== undefined &&
        obj.resources.goods !== null
      ) {
        let eraId = Technologies.Eras[obj['era']];
        ...
        if (!(eraId in res)) {
          ...
          obj.resources.goods.forEach((good) => {
            res[eraId][good.good_id] = {
              good: good.good_id,
              name: GoodsData[good.good_id].name,
              value: good.value,
            };
          });
        } else {
    ...
    d.push(
      `<div class="view_notification">1. ${i18n('Boxes.GuildMemberStat.GuildBuildingNotification')}<br />2. ${i18n('Boxes.GuildMemberStat.GuildTreasuryNotification')}</div>`,
    );
    ...
    d.push(
      `<table id="TreasuryGoodsTable" class="foe-table"><thead class="sticky"><tr><th>${i18n('Boxes.GuildMemberStat.Eras')}</th><th> ${i18n('Boxes.GuildMemberStat.ProducedTreasuryGoods')}</th><th> ${i18n('Boxes.GuildMemberStat.TreasuryGoods')}</th></thead><tbody>`,
    );
    ...
    for (
      let eraId = Technologies.Eras.IronAge;
      eraId < Technologies.Eras.NextEra;
      eraId++
    ) {
      ...
      let countEra =
        typeof EraGroup[eraId]?.members?.length != 'undefined' ?
          EraGroup[eraId]?.members?.length
        : 0;

      d.push(
        `<tr><td>${i18n('Eras.' + eraId)}<br>(${countEra} ${i18n('Boxes.GuildMemberStat.GuildMembers')})</td>`,
      );
      ...
      if (ErasGuildGoods[eraId] !== undefined) {
        ...
        for (let i in DailyGuildGoods) {
          if (DailyGuildGoods.hasOwnProperty(i)) {
            if (exportGood[DailyGuildGoods[i].good] === undefined)
              exportGood[DailyGuildGoods[i].good] = {
                eraId: eraId,
                era: currentEra,
                good: DailyGuildGoods[i].name,
                produceable: 0,
                instock: 0,
              };

            d.push(
              `<tr><td class="goods-image"><span class="goods-sprite sprite-35 ${i}"></span></td><td>${DailyGuildGoods[i].name}</td><td class="text-right">${HTML.Format(DailyGuildGoods[i].value)}</td></tr>`,
            );
            exportGood[DailyGuildGoods[i].good].produceable =
              DailyGuildGoods[i].value;
          }
        }
        d.push(`</tbody></table></div>`);
        d.push(`</td>`);
        ...
        d.push(
          `<td class="detail text-center dark gms-tooltip" title="${HTML.i18nTooltip(i18n('Boxes.GuildMemberStat.GuildBuildingNotification'))}">-</td>`,
        );
      }
      ...
      if (
        TreasuryGoodsData !== undefined &&
        TreasuryGoodsData !== null &&
        typeof TreasuryGoodsData[Technologies.EraNames[eraId]] !== 'undefined'
      ) {
        let EraTreasuryGoods = TreasuryGoodsData[
          Technologies.EraNames[eraId]
        ].sort(function (a, b) {
        ...
        d.push(
          `<td class="detail text-center gms-tooltip" ${TreasuryGoodsData === null ? `title="${HTML.i18nTooltip(i18n('Boxes.GuildMemberStat.GuildTreasuryNotification'))}"` : ''}>-</td>`,
        );
      }
  ...
  ShowGreatBuildings: async () => {
    ...
    let GreatBuildings =
      await GuildMemberStat.GetGuildMemberBuildings('greatbuildings');
    let allMemberIds = Object.keys(GuildMemberStat.MemberDict);

    d.push(
      `<div class="view_notification">${i18n('Boxes.GuildMemberStat.GuildBuildingNotification')} <button style="float:right" id="toggleGreatBuildingView" class="btn">${i18n('Boxes.GuildMemberStat.ChangeView')}</button></div>`,
    );
    ...
    let GBOverview = GreatBuildings.reduce(
      function (res, obj) {
        obj.invested_forge_points =
          obj.invested_forge_points ? obj.invested_forge_points : 0;
        ...
        if (!(obj.cityentity_id in res)) {
          ...
          res[obj.cityentity_id].player = [
            {
              ...
              member: obj.member,
              fplevelup:
                obj.max_level > obj.level ? obj.forge_points_for_level_up : 0,
              investedfp: obj.invested_forge_points,
              level: obj.level,
              max_level: obj.max_level,
            },
          ];
          res[obj.cityentity_id].NoGbMember = allMemberIds.filter(
            function (value) {
          ...
          res[obj.cityentity_id].count += 1;
          res[obj.cityentity_id].max_level =
            res[obj.cityentity_id].max_level < obj.level ?
              obj.level
            : res[obj.cityentity_id].max_level;
          res[obj.cityentity_id].min_level =
            res[obj.cityentity_id].min_level > obj.level ?
              obj.level
            : res[obj.cityentity_id].min_level;
          ...
          res[obj.cityentity_id].NoGbMember = res[
            obj.cityentity_id
          ].NoGbMember.filter(function (value) {
            return value != obj.player_id;
          });
        }
    ...
    d.push(
      `<table id="gblist" class="foe-table"><thead class="sticky"><tr class="sorter-header">` +
    ...
    for (let x = 0; x < GBOverview.length; x++) {
      ...
      d.push(
        `<tr class="hasdetail" id="gms_${x}" data-id="${x}">` +
    ...
    d.push(
      `<table id="gblist_detail" class="foe-table"><thead class="sticky"><tr class="sorter-header">` +
    ...
    for (let x = 0; x < GreatBuildingsDetail.length; x++) {
      ...
      let invested_forge_points =
        dbuilding.invested_forge_points ? dbuilding.invested_forge_points : 0;
      let forge_points_for_level_up =
        dbuilding.max_level > dbuilding.level ?
          dbuilding.forge_points_for_level_up
        : 0;
      let investable = dbuilding.max_level > dbuilding.level ? true : false;

      d.push(
        `<tr id="gms_detail_${x}" data-id="${x}">` +
    ...
    $('#gmsContentWrapper')
      ...
      .done(function () {
        ...
        $('#gmsContentWrapper #toggleGreatBuildingView').on(
          'click',
          function () {
            $('#gmsContentWrapper .greatbuildinglist')
              .toggleClass('hide show')
              .promise()
              .done(function () {
                if (
                  $('#gms-filter-input').is(':visible') &&
                  $('#gms-filter-input').val() !== ''
                ) {
                  if ($('.greatbuildinglist.grouped').hasClass('show'))
                    GuildMemberStat.filterTable('gms-filter-input', 'gblist');
                  else if ($('.greatbuildinglist.detail').hasClass('show'))
                    GuildMemberStat.filterTable(
                      'gms-filter-input',
                      'gblist_detail',
                    );
                }
        ...
        $('#gblist > tbody tr.hasdetail')
          ...
          .on('click', function () {
            if ($(this).next('tr.detailview').length) {
              ...
              let d = [];
              if (
                !$(this).hasClass('hasdetail') ||
                !GBOverview ||
                !GBOverview[id] ||
                !GBOverview[id]['player']
              )
                return;

              d.push(
                '<tr class="detailview dark-bg"><td colspan="' +
                  $(this).find('td').length +
                  '"><div class="detail-wrapper">',
              );
              $(this).addClass('open');

              const player = GBOverview[id]['player'].sort(function (a, b) {
                return a.level - b.level;
              });
              let NoGbMember = GBOverview[id]['NoGbMember'];

              d.push(
                `<div class="detail-item gb_player"><div>` +
              ...
              for (let i in player) {
                ...
                d.push(
                  `<tr><td data-text="${helper.str.cleanup(player[i].member)}">${MainParser.GetPlayerLink(player[i].player_id, player[i].member)}</td>` +
              ...
              if (NoGbMember !== undefined && NoGbMember.length) {
                NoGbMember = NoGbMember.filter((member) => {
                  return !GuildMemberStat.MemberDict[member].deleted;
                });
                NoGbMember = NoGbMember.sort(function (a, b) {
                  return GuildMemberStat.MemberDict[a].name.localeCompare(
                    GuildMemberStat.MemberDict[b].name,
                  );
                });

                d.push(
                  `<div class="no_gb_member copyable"><span class="text-bright"><i>${HTML.i18nReplacer(i18n('Boxes.GuildMemberStat.MemberWithoutGB'), { greatbuilding: GBOverview[id]['name'] })}</i>: </span>`,
                );
                for (let i = 0; i < NoGbMember.length; i++) {
                  d.push(
                    MainParser.GetPlayerLink(
                      NoGbMember[i],
                      GuildMemberStat.MemberDict[NoGbMember[i]].name,
                    ),
                  );
  ...
  GetGuildMemberBuildings: async (type) => {
    ...
    for (let x = 0; x < GuildMembers.length; x++) {
      ...
      switch (type) {
        case 'guildbuildings':
          b =
            typeof member['guildbuildings'] !== 'undefined' ?
              member['guildbuildings']['buildings']
            : undefined;
          break;
        case 'greatbuildings':
          b =
            typeof member['greatbuildings'] !== 'undefined' ?
              member['greatbuildings']
            : undefined;
          break;
      }
  ...
  DeletePlayerDetail: async (delObj) => {
    if (delObj === undefined || delObj === null) return;
    ...
    switch (content) {
        ...
        await GuildMemberStat.db.activity
          ...
          .modify((x) => {
            x.warnings = x.warnings.filter((el) => {
              return !(
                el.activity === data.activity &&
                +moment(el.date) === +moment(data.date)
              );
            });
          });
      ...
      case 'gex':
        await GuildMemberStat.db.gex
          .where({ player_id: player_id, gexweek: data.gexweek })
          .delete();
        break;
      case 'gbg':
        await GuildMemberStat.db.gbg
          .where({ player_id: player_id, gbgid: data.gbgid })
          .delete();
        break;
    }
  },
  ...
  InitSettings: () => {
    ...
    let TreasuryGoods = JSON.parse(
      localStorage.getItem('GuildMemberStatTreasuryGoods'),
    );

    if (!Settings) return;

    GuildMemberStat.Settings.lastupdate =
      Settings.lastupdate !== undefined ? Settings.lastupdate : 0;
    GuildMemberStat.Settings.showDeletedMembers =
      Settings.showDeletedMembers !== undefined ?
        Settings.showDeletedMembers
      : GuildMemberStat.Settings.showDeletedMembers;
    GuildMemberStat.Settings.deleteOlderThan =
      Settings.deleteOlderThan !== undefined ?
        Settings.deleteOlderThan
      : GuildMemberStat.Settings.deleteOlderThan;
    GuildMemberStat.Settings.autoStartOnUpdate =
      Settings.autoStartOnUpdate !== undefined ?
        Settings.autoStartOnUpdate
      : GuildMemberStat.Settings.autoStartOnUpdate;
    GuildMemberStat.Settings.showSearchbar =
      Settings.showSearchbar !== undefined ?
        Settings.showSearchbar
      : GuildMemberStat.Settings.showSearchbar;
    GuildMemberStat.Settings.showBattlesWon =
      Settings.showBattlesWon !== undefined ?
        Settings.showBattlesWon
      : GuildMemberStat.Settings.showBattlesWon;
    GuildMemberStat.Settings.gexgbgDateFormat =
      Settings.gexgbgDateFormat !== undefined ?
        Settings.gexgbgDateFormat
      : GuildMemberStat.Settings.gexgbgDateFormat;
    GuildMemberStat.Settings.showZeroValues =
      Settings.showZeroValues !== undefined ?
        Settings.showZeroValues
      : GuildMemberStat.Settings.showZeroValues;
    GuildMemberStat.TreasuryGoodsData =
      TreasuryGoods !== undefined ? TreasuryGoods : {};
  },
  ...
  GuildMemberStatSettings: () => {
    ...
    c.push(
      `<p class="text-left"><input id="gmsAutoStartOnUpdate" name="autostartonupdate" value="1" type="checkbox" ${Settings.autoStartOnUpdate === 1 ? ' checked="checked"' : ''} /> <label for="gmsAutoStartOnUpdate">${i18n('Boxes.GuildMemberStat.AutoStartOnUpdate')}</label></p>`,
    );
    c.push(
      `<p class="text-left"><input id="gmsShowSearchbar" name="showsearchbar" value="1" type="checkbox" ${Settings.showSearchbar === 1 ? ' checked="checked"' : ''} /> <label for="gmsShowSearchbar">${i18n('Boxes.GuildMemberStat.ShowSearchbar')}</label></p>`,
    );
    c.push(
      `<hr><p class="text-left"><span class="settingtitle">${i18n('Boxes.GuildMemberStat.GuildMembers')}</span><input id="gmsShowDeletedMembers" name="showdeletedmembers" value="1" type="checkbox" ${Settings.showDeletedMembers === 1 ? ' checked="checked"' : ''} /> <label for="gmsShowDeletedMembers">${i18n('Boxes.GuildMemberStat.ShowDeletedMembers')}</label></p>`,
    );
    c.push(
      `<p class="text-left"><input id="gmsShowBattlesWon" name="showbattleswon" value="1" type="checkbox" ${Settings.showBattlesWon === 1 ? ' checked="checked"' : ''} /> <label for="gmsShowBattlesWon">${i18n('Boxes.GuildMemberStat.ShowBattlesWon')}</label></p>`,
    );
    c.push(
      `<p class="text-left"><input id="gmsShowZeroValues" name="showzerovalues" value="1" type="checkbox" ${Settings.showZeroValues === 1 ? ' checked="checked"' : ''} /> <label for="gmsShowZeroValues">${i18n('Boxes.GuildMemberStat.ShowZeroValues')}</label></p>`,
    );
    c.push(
      `<p class="text-left">${i18n('Boxes.GuildMemberStat.GexGbgDateFormat')} <select id="gmsGexGbgDateFormat" name="gexgbgdateformat">` +
    ...
    c.push(
      `<p class="text-left">${i18n('Boxes.GuildMemberStat.DeleteExMembersAfter')} <select id="gmsDeleteOlderThan" name="deleteolderthan">`,
    );
    ...
    deleteAfterDays.forEach((days) => {
      ...
      if (days === -1) {
        option = i18n('Boxes.GuildMemberStat.Instantly');
      } else if (days === 0) {
        option = i18n('Boxes.GuildMemberStat.Never');
      } else {
        option = days + ' ' + i18n('Boxes.GuildMemberStat.Days');
      }

      c.push(
        `<option value="${days}" ${Settings.deleteOlderThan === days ? ' selected="selected"' : ''}>${option}</option>`,
      );
    });

    c.push(`</select>`);
    c.push(
      `<p class="text-left">${i18n('Boxes.GuildMemberStat.ResetMessageCounter')} ` +
        `<select id="gmsResetMessageCounter" name="resetmessagecounter">`,
    );
    c.push(
      `<option value="0">${i18n('Boxes.GuildMemberStat.ConfirmNo')}</option>`,
    );
    c.push(
      `<option value="reset">${i18n('Boxes.GuildMemberStat.ConfirmYes')}</option>`,
    );
    c.push(`</select></p>`);
    c.push(
      `<hr><p><button id="save-GuildMemberStat-settings" class="btn" style="width:100%" onclick="GuildMemberStat.SettingsSaveValues()">${i18n('Boxes.Investment.Overview.SettingsSave')}</button></p>`,
    );
    c.push(
      `<hr><p class="text-left">${i18n('Boxes.General.Export')}: <button class="btn" onclick="GuildMemberStat.ExportContent('${GuildMemberStat.CurrentStatGroup}','csv')" title="${HTML.i18nTooltip(i18n('Boxes.General.ExportCSV'))}">CSV</button>`,
    );
    c.push(
      `<button class="btn" onclick="GuildMemberStat.ExportContent('${GuildMemberStat.CurrentStatGroup}','json')" title="${HTML.i18nTooltip(i18n('Boxes.General.ExportJSON'))}">JSON</button></p>`,
    );
  ...
  SettingsSaveValues: async () => {
    ...
    GuildMemberStat.Settings.showDeletedMembers =
      $('#gmsShowDeletedMembers').is(':checked') ? 1 : 0;
    GuildMemberStat.Settings.autoStartOnUpdate =
      $('#gmsAutoStartOnUpdate').is(':checked') ? 1 : 0;
    GuildMemberStat.Settings.showSearchbar =
      $('#gmsShowSearchbar').is(':checked') ? 1 : 0;
    GuildMemberStat.Settings.showBattlesWon =
      $('#gmsShowBattlesWon').is(':checked') ? 1 : 0;
    GuildMemberStat.Settings.showZeroValues =
      $('#gmsShowZeroValues').is(':checked') ? 1 : 0;
    GuildMemberStat.Settings.gexgbgDateFormat =
      $('#gmsGexGbgDateFormat').val() || 'week';

    if (
      GuildMemberStat.Settings.deleteOlderThan !== tmpDeleteOlder &&
      (tmpDeleteOlder > 0 || tmpDeleteOlder === -1)
    ) {
      helper.preloader.show('#GuildMemberStat');
    ...
    $(`#GuildMemberStatSettingsBox`).fadeToggle('fast', function () {
      ...
      if (GuildMemberStat.CurrentStatGroup === 'Member') GuildMemberStat.Show();
    });
  },
  ...
  setEraGoods: (d) => {
    ...
    for (let i in d) {
      if (d.hasOwnProperty(i)) {
        ...
        GuildMemberStat.TreasuryGoodsData[era].push({
          good: i,
          name: name,
          value: d[i],
        });
    ...
    localStorage.setItem(
      'GuildMemberStatTreasuryGoods',
      JSON.stringify(GuildMemberStat.TreasuryGoodsData),
    );
  ...
  ExportContent: (filename, type) => {
    ...
    let Blob1 = new Blob([BOM + FileContent], {
      type: 'application/octet-binary;charset=ANSI',
    });
    MainParser.ExportFile(
      Blob1,
      filename + '-' + moment().format('YYYY-MM-DD') + '.' + type,
    );
  ...
  filterTable: (i, t) => {
    ...
    if (input.value.length > 0) input.classList.add('highlight');
