...
FoEproxy.addHandler('CampaignService', 'start', (data, postData) => {
  ...
  maxShip = Math.floor(
    Object.values(data.responseData.provinces)
      .map((x) => x.id || 0)
      .pop() / 100,
  );
  for (let province of data.responseData.provinces) {
    if (province.provinceType == 'ship')
      province.parentIds = province.parentIds.concat(
        [...Array(maxShip - province.id / 100).keys()].map(
          (x) => (x + province.id / 100 + 1) * 100,
        ),
      );
  ...
  scoutingTimes.scoutTarget =
    data.responseData.scout?.path[data.responseData.scout?.path?.length - 1] |
    0;
  ...
  (data, postData) => {
    ...
    for (resp of postData) {
      if (resp.requestMethod === 'moveScoutToProvince') {
        scoutingTimes.scoutTarget =
          resp.requestData[0][resp.requestData[0].length - 1];
...
let scoutingTimes = {
  ...
  ShowDialog: () => {
    ...
    let castlebonus = 1;
    if ((Castle.curLevel | 0) > 0)
      castlebonus = scoutingTimes.castleBonuses[Castle.curLevel];
        ...
        for (let element of province.children) {
          ...
          if (child.isScouted) {
            ...
            if (!scoutingTimes.Provinces[child.id].fromCurrent) {
              ...
              scoutingTimes.Provinces[child.id].travelTime =
                (element.travelTime +
                  Math.max(
                    scoutingTimes.distance(
                      scoutingTimes.scoutPosition,
                      child.id,
                    ) - 1,
                    0,
                  ) *
  ...
  GetDistances: (StartId, limit) => {
    ...
    for (let Province of temp) {
      ...
      if (!scoutingTimes.Provinces[Province[0]]?.parentIds || !isShorter)
        continue;
  ...
  ShowSettings: () => {
    ...
    let h = [];
    h.push(
      `<p><label><input id="autoStartScout" type="checkbox" ${autoOpen === true ? ' checked="checked"' : ''} />${i18n('Boxes.Settings.Autostart')}</label></p>`,
    );
    h.push(
      `<p><button onclick="scoutingTimes.SaveSettings()" id="save-bghelper-settings" class="btn" style="width:100%">${i18n('Boxes.Settings.Save')}</button></p>`,
    );
  ...
  SaveSettings: () => {
    let value = false;
    if ($('#autoStartScout').is(':checked')) value = true;
