...
FoEproxy.addHandler(
  ...
  (data, postData) => {
    if (data['responseData']['category']['type'] === 'social') {
      CompareFriendsThreads.ParseThreads(
        data['responseData']['category']['teasers'],
      );
    }
  },
);
...
FoEproxy.addHandler(
  ...
  (data, postData) => {
    if (
      postData[0]['requestClass'] === 'ConversationSettingsService' &&
      postData[0]['requestMethod'] === 'getSettings'
    ) {
      if (data['responseData']['participants']) {
        CompareFriendsThreads.CurrentThread = postData[0]['requestData'][0];
        CompareFriendsThreads.ParseThreadParticipants(
          data['responseData']['participants'],
        );
...
let CompareFriendsThreads = {
  ...
  ParseThreads: (data) => {
    for (let i in data) {
      ...
      let index = CompareFriendsThreads.Threads.findIndex(
        (t) => t.id === data[i].id,
      );
  ...
  ParseThreadParticipants: (data) => {
    let key = CompareFriendsThreads.Threads.findIndex(
      (t) => t.id === CompareFriendsThreads.CurrentThread,
    );
  ...
  BuildBody: (rebuild = false) => {
    ...
    let PlayerList = Object.values(PlayerDict).filter(
    ...
    for (let p in PlayerList) {
      ...
      t.push(
        `<td>#${parseInt(p) + 1} <img style="max-width: 22px" src="${srcLinks.GetPortrait(Player['Avatar'])}" alt="${Player['PlayerName']}"> ` +
          `${
      ...
      for (let x in CompareFriendsThreads.Threads) {
        ...
        if (
          CompareFriendsThreads.Threads[x]['participants'].findIndex(
            (p) => p.playerId === Player.PlayerID,
          ) === -1
        ) {
          t.push(
            `<td class="text-center text-danger"><svg viewBox="0 0 48 48"><use href="#cross-sign" xlink:href="#cross-sign" /></svg></td>`,
          );
        } else {
          t.push(
            `<td class="text-center"><svg viewBox="0 0 48 48"><use href="#tick-sign" xlink:href="#tick-sign" /></svg></td>`,
          );
        }
      }
    ...
    t.push('</table>');
    t.push(
      '<svg xmlns="http://www.w3.org/2000/svg" style="display:none" viewBox="0 0 48 48" width="48px" height="48px"><symbol id="tick-sign" viewBox="0 0 48 48"><path fill="#43A047" d="M40.6 12.1L17 35.7 7.4 26.1 4.6 29 17 41.3 43.4 14.9z"></path></symbol><symbol id="cross-sign" viewBox="0 0 48 48"><path fill="#F44336" d="M21.5 4.5H26.501V43.5H21.5z" transform="rotate(45.001 24 24)"></path><path fill="#F44336" d="M21.5 4.5H26.5V43.501H21.5z" transform="rotate(135.008 24 24)"></path></symbol></svg>',
    );
