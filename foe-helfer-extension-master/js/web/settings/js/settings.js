...
let Settings = {
   ...
   * Tab groups
   */
  BoxGroups: ['About', 'Extension', 'Auto', 'Boxes'],
   ...
   */
  LoadConfig: (callback) => {
    fetch(`${extUrl}js/web/settings/config/config.json`).then((response) => {
  ...
  BuildBody: () => {
    ...
    for (let i = 0; i < Settings.BoxGroups.length; i++) {
      ...
      parentLis.push(
        `<li><a href="#tab-${i}"><span>${i18n('Settings.Tab.' + g)}</span></a></li>`,
      );
      ...
      for (let x in grps) {
        ...
        if (status !== undefined) {
          cs.append(
            $('<span />')
              .addClass('check ' + (status ? '' : 'unchecked'))
              .append(
                $('<span />')
                  .addClass('toogle-word')
                  .text(
                    status ?
                      i18n('Boxes.Settings.Active')
                    : i18n('Boxes.Settings.Inactive'),
                  ),
              )
              .append(
                $(
                  '<input class="setting-check game-cursor" type="checkbox" data-id="' +
                    d['name'] +
                    '" ' +
                    (status ? 'checked' : '') +
                    '/>',
        ...
        childLis.push(
          `<li><a href="#subtab-${cnt}" title="${i18n('Settings.Entry.' + d['name'])}">${i18n('Settings.Entry.' + d['name'])}</a></li>`,
        );

        let h = c.append(cr.append(ct, cd, cs));
        childDivs.push(
          '<div id="subtab-' + cnt + '" class="sub-tab">' + h.html() + '</div>',
        );
      ...
      div.push(
        `<div id='tab-${i}' class="settings-wrapper">${subcontent}</div>`,
      );
    }
  ...
  GetSetting: (name, is_string = false) => {
    ...
    if (s !== null) {
      ...
      if (Settings.Preferences === null) {
        console.error(
          'Error getting default value of setting "' +
            name +
  ...
  VersionInfo: () => {
    let v =
      extVersion.includes('beta') ? `` : (
        `<p>${i18n('Settings.Version.Link').replace('__version__', '')}</p>`
      );
    v += `<dl class="info-box">
      ...
      if (element[Object.keys(element)[0]]) {
        dp.push(
          '<option value="' +
            element +
            '"' +
            (MainParser.SelectedMenu === element ? ' selected' : '') +
            '>' +
            i18n('Menu.' + element) +
            '</option>',
        );
  ...
  About: () => {
    return (
      '<hr>' +
      '<h2>' +
      i18n('Settings.About.TranslateTitle') +
      '</h2>' +
      '<p>' +
      i18n('Settings.About.TranslateDesc') +
      ' <a href="http://i18n.foe-helper.com/projects/foe-helper/extension/" target="_blank">Weblate</a></p>' +
      '<hr>' +
      '<h2>' +
      i18n('Settings.About.RatingTitle') +
      '</h2>' +
      '<p>' +
      i18n('Settings.About.RatingDesc') +
      '</p>'
    );
  },
  ...
  Help: () => {
    return (
      '<ul class="helplist">' +
      '<li><a href="https://foe-helper.com" target="_blank"><span class="website">&nbsp;</span>' +
      i18n('Settings.Help.Website') +
      '</a></li>' +
      '<li><a href="https://docs.foe-helper.com" target="_blank"><span class="website">&nbsp;</span>' +
      i18n('Settings.Help.Documentation') +
      '</a></li>' +
      '<li><a href="https://discord.gg/uQY7rqDJ7z" target="_blank"><span class="discord">&nbsp;</span>' +
      i18n('Settings.Help.Discord') +
      '</a></li>' +
      '<li><a href="https://github.com/mainIine/foe-helfer-extension/issues" target="_blank"><span class="github">&nbsp;</span>' +
      i18n('Settings.Help.Github') +
      '</a></li>' +
      '</ul>'
    );
  },
  ...
  ShowEventHelpers: () => {
    let eventHelperSettings = {
      EventHelperMerge: true,
      EventHelperPresent: true,
      EventHelperIdle: true,
      EventHelperPop: true,
    };
    let dp = [];
    ...
    for (let [setting, value] of Object.entries(eventHelperSettings)) {
      ...
      dp.push(
        '<span class="check ' +
          ...
          '" data-id="' +
          setting +
          '" class="setting-check game-cursor" type="checkbox" ' +
          (value ? 'checked' : '') +
          ' />' +
          '</span>',
      );
  ...
  SelectWebsite: () => {
    ...
    dp.push(
      '<label for="scoredb"><input type="radio" value="siteScoredb" id="scoredb" name="website" ' +
        (currentSite === 'siteScoredb' ? 'checked' : '') +
        ' /> foe.scoredb.io</label><br />',
    );
    dp.push(
      '<label for="forgedb"><input type="radio" value="siteForgedb" id="forgedb" name="website" ' +
        (currentSite === 'siteForgedb' ? 'checked' : '') +
        ' /> foestats.com</label></p>',
    );
    ...
    for (let iso in Languages.PossibleLanguages) {
      ...
      dp.push(
        '<option value="' +
          iso +
          '"' +
          (MainParser.Language === iso ? ' selected' : '') +
          '>' +
          Languages.PossibleLanguages[iso] +
          '</option>',
      );
    }
  ...
  MenuContent: () => {
    ...
    for (let i in menuItems) {
      ...
      if (_menu[name + '_Btn']) {
        ...
        let btn = $(
          `<span onclick="_menu.ToggleItemVisibility('${name}')"></span>`,
        );
  ...
  NotificationView: () => {
    ...
    elements.push(
      '<select class="setting-dropdown" id="notification-position">',
    );
    ...
    for (let pos in positions) {
      ...
      elements.push(
        `<option value="${positions[pos]}"${settingPos === positions[pos] ? ' selected' : ''}>${i18n('Menu.Notification.Position.' + positions[pos])}</option>`,
      );
    }
      ...
      if (value !== '') {
        if (value.length !== 36) {
          HTML.ShowToastMsg({
            ...
            text: [i18n('Boxes.Settings.ApiTokenLengthWrongBody')],
            type: 'error',
            hideAfter: 10000,
          });
        } else {
