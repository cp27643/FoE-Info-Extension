...
FoEproxy.addHandler('PopGameService', 'getOverview', (data, postData) => {
  ...
  if (
    ...
    !(Settings.GetSetting('EventHelperPop') === undefined ?
      true
    : Settings.GetSetting('EventHelperPop'))
  )
    return;
...
mouseActions.addAction(
  [[284, 297, 'Center'], [-312, -337, 'Center'], false],
  () => {
    Popgame.clearReward();
  },
);
...
$(window).mousemove(function (e) {
  ...
  if (
    ...
    let rf = Math.floor(r);
    let rr = r - rf;
    if (cr > 0.15 && cr < 0.85 && rr > 0.15 && rr < 0.85)
      Popgame.CoordsCheck(cf, rf);
  }
});
...
let Popgame = {
  ...
  Show: () => {
    ...
    if ($('#Popgame').length === 0) {
      ...
      Popgame.minimized = JSON.parse(
        localStorage.getItem('PopgameMinimized') || 'true',
      );
      ...
      $('#PopgameButtons > span.window-minimize').on('click', function () {
        ...
        localStorage.setItem(
          'PopgameMinimized',
          JSON.stringify(Popgame.minimized),
        );
      });
    }
  },
  ...
  CoordsCheck: (x, y) => {
    ...
    if (x > Popgame.width - 1 || y > Popgame.height - 1 || x < 0 || y < 0)
      return;
    ...
    if (Popgame.tool === 'destroyer') {
      ...
      if (Popgame.tool === 'hammer' || Popgame.tool === 'fork') {
        ...
        if (Popgame.hide.length > 4) {
          $(`#PGcellX${x}Y${y}`).removeClass(
            `PG_${Popgame.event}_${Popgame.grid[x][y]}`,
          );
  ...
  hideDrops: () => {
    ...
    drops.fadeOut('fast');
    if (drops.length > 0) setTimeout(Popgame.hideDrops, 250);
  },
  ...
  resetTempChest: () => {
    if (Popgame.tempC !== null) {
      ...
      $(`#PGcellX${Popgame.lastX}Y${Popgame.lastY}`).addClass(
        `PG_${Popgame.event}_${Popgame.grid[Popgame.lastX][Popgame.lastY]}`,
      );
      Popgame.tempC = null;
    }
    Popgame.lastX = null;
  ...
  updateTiles: (tiles) => {
    ...
    for (let tile of tiles) {
      ...
      Popgame.grid[x][y] =
        tile.type +
        (tile.popType === 'default' || tile.type === 'grandPrize' ?
          ''
        : '_reward');
    }
  },
  ...
  trackingReset: () => {
    Popgame.tracking = {
      start: { total: 0, grandPrize: 0 },
      afterPop: { total: 0, grandPrize: 0 },
