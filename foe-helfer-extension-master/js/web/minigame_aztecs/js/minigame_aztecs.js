        ...
        if (AztecsHelper.ResourcesLeft == 0) {
          if (!$('#minigame_aztecs-Btn').hasClass('hud-btn-red')) {
            ...
            _menu.toolTipp(
              $('#minigame_aztecs-Btn'),
              'Aztec Helper',
              '<em id="minigame_aztecs-Btn-closed" class="tooltip-error">Opens automatically when starting a aztec mini game<br></em>Aztec Minigame Helper -BETA-',
            );
          }
...
FoEproxy.addHandler('ResourceShopService', 'buyResources', (data, postData) => {
  if (
    postData[0].requestData.filter(
      (x) =>
        x?.mainType === 'cultural_outpost' &&
        x?.subType === 'collecting_minigame_buy_turns',
    )
  ) {
    if (
      postData[0].requestData.filter((x) => x?.['resources'] !== undefined)?.[0]
        ?.resources?.aztecs_collecting_minigame_turns > 0
    ) {
...
let AztecsHelper = {
  ...
  start: () => {
    ...
    if (
      r.reward.resources === undefined ||
      Object.values(r.reward.resources) <= 0
    )
      return;
    ...
    if (AztecsHelper.MovesLeft > 0) {
      AztecsHelper.grid.forEach((rowData, x) => {
        ...
        rowData.forEach((cellData, y) => {
          ...
          if (
            typeof cellData.content !== 'number' &&
            cellData.content !== AztecsHelper.emptyCell
          )
            cell.appendChild(document.createTextNode(cellData.content));
          if (cellData.prob == 0) {
            cell.className =
              cellData.content == AztecsHelper.unknownCell ?
                'aztec colorU'
              : 'aztec color0';
      ...
      span.appendChild(
        document.createTextNode(i18n('Boxes.AztecMiniGame.Description')),
      );
  ...
  CalcAdjacentCells: () => {
    ...
    while (Object.keys(tmp).length > 0 && run < 20) {
      ...
      for (let c in tmp) {
        ...
        cell.surrUnCells = AztecsHelper.GetSurroundingCell(
          tmp[c].x,
          tmp[c].y,
          uC,
        );
        cell.surrResCells = AztecsHelper.GetSurroundingCell(
          tmp[c].x,
          tmp[c].y,
          rC,
        );
        ...
        if (cell.surrUnCells.length > 0) {
          ...
          if (unrevRes == 0 || unrevRes == cell.surrUnCells.length) {
            //Anzahl übriger Güter ist 0 oder entspricht der Anzahl unbekannter Felder
            let prob = unrevRes == 0 ? 0 : 1;
            let content = unrevRes == 0 ? nrC : rC;
            for (let cx of cell.surrUnCells) {
              //surrUnCells sind alle bekannt - entweder Ressource oder keine Ressource
              let uC = map[cx.y][cx.x];
              uC.content = content;
              uC.prob = prob;
              for (let snC of uC.surrNumCells) {
                //Zahlennachbarn der vormals unbekannten Nachbarzelle prüfen und diese der erneuten Prüfung unterziehen
                tmp2[`y${snC.y}x${snC.x}`] = snC;
              }
              if (prob == 1) leftRes -= 1;
              let uIndex = `y${cx.y}x${cx.x}`; //Zelle aus der Liste unbekannter Zellen entfernen
              if (unknownCells.hasOwnProperty(uIndex))
                delete unknownCells[uIndex];
            }
            cell.surrUnCells = []; //keine unbekannten Nachbarzellen mehr
          } else {
            for (let other in numberCells) {
              //andere Zahlenzellen durchgehen
              if (other == c) continue;
              let otherCell = map[numberCells[other].y][numberCells[other].x];
              otherCell.surrUnCells = AztecsHelper.GetSurroundingCell(
                numberCells[other].x,
                numberCells[other].y,
                uC,
              );
              otherCell.surrResCells = AztecsHelper.GetSurroundingCell(
                numberCells[other].x,
                numberCells[other].y,
                rC,
              );
              ...
              let Overlap = [],
                Diff = [];
              [Overlap, Diff] = AztecsHelper.Compare(
                cell.surrUnCells,
                otherCell.surrUnCells,
              ); //Überlapp und Unterschied der auf unbekannten Nachbarn zwischen der Zelle und der anderen Zelle bestimmen

              if (Overlap.length != 0) {
                //wenn es einen Überlapp gibt
                let testP = Math.max(
                  0,
                  otherCell.content +
                    Overlap.length -
                    otherCell.surrUnCells.length -
                    otherCell.surrResCells.length,
                );
                ...
                let min =
                  unrevRes -
                  (otherCell.content - otherCell.surrResCells.length); //min Anzahl der Diff-Zellen die eine Ressource haben
                if (min < 0) min = 0;
                let max = unrevRes - testP; //max Anzahl der Diff Zellen die eine Ressource haben können

                if (Diff.length == min || max == 0) {
                  // Anzahl Diffzellen entspricht dem minimum oder das max ist 0
                  let prob = max == 0 ? 0 : 1;
                  let content = max == 0 ? nrC : rC;
                  for (let cx of Diff) {
                    //Diff Zellen sind alle bekannt - entweder Ressource oder keine Ressource
                    let uC = map[cx.y][cx.x];
                    uC.content = content;
                    uC.prob = prob;
                    for (let snC of uC.surrNumCells) {
                    ...
                    if (unknownCells.hasOwnProperty(uIndex))
                      delete unknownCells[uIndex]; //entferne Zelle aus Liste unbekannter Zellen
                  }
            ...
            if (cell.surrUnCells.length > 0) {
              //} && leftRes > 0) { //es sind noch unbekannte Nachbarzellen vorhanden
              for (let dC of cell.surrUnCells) {
                //unbekannte Nachbarzellen durchgehen
                ...
                if (!map[dC.y][dC.x].hasOwnProperty('probList'))
                  map[dC.y][dC.x].probList = []; //probList anlegen, falls noch nicht vorhanden
                let oldP = map[dC.y][dC.x].probList[c] || 0; //bisherige Wahrscheinlichkeit auslesen
                if (oldP != newP) {
                  //wenn sich Wahrscheinlichkeit geändert hat
                  map[dC.y][dC.x].probList[c] = newP; //neue Wahrscheinlichkeit zuweisen
                  for (snC of map[dC.y][dC.x].surrNumCells) {
                    // und benachbarte Zahlenzellen erneuter Prüfung unterziehen
                    tmp2[`y${snC.y}x${snC.x}`] = snC;
                  }
                }
              }
    ...
    for (let c in unknownCells) {
      //alle noch unbekannten Zellen durchgehen
      ...
      map[cell.y][cell.x].prob = Math.max(
        ...AztecsHelper.remIndex(map[cell.y][cell.x].probList),
      ); //maximum der probList als Wahrscheilichkeit notieren
      delete map[cell.y][cell.x].probList; //probList entfernen
    }

    AztecsHelper.CalcBody();
    if (AztecsHelper.MovesLeft <= 0)
      return $('#aztecsHelper').length > 0 && HTML.CloseOpenBox('aztecsHelper');
  },

  Compare: (BaseArray, CompareArray) => {
    const Overlap = BaseArray.filter(
      (value) =>
        CompareArray.filter(
          (value2) => value.y == value2.y && value.x == value2.x,
        ).length > 0,
    );
    const Diff = BaseArray.filter(
      (value) =>
        CompareArray.filter(
          (value2) => value.y == value2.y && value.x == value2.x,
        ).length == 0,
  ...
  GetSurroundingCell: (width, height, cellContent) => {
    ...
    if (cellContent !== 'number') {
      if (width > 0)
        if (map[height][width - 1].content === cellContent)
          arr.push({ x: width - 1, y: height }); // height0/width-1
      if (height > 0)
        if (map[height - 1][width].content === cellContent)
          arr.push({ x: width, y: height - 1 }); // height-1/width0
      if (width < AztecsHelper.mapWidth - 1)
        if (map[height][width + 1].content === cellContent)
          arr.push({ x: width + 1, y: height }); // height0/width+1
      if (height < AztecsHelper.mapHeight - 1)
        if (map[height + 1][width].content === cellContent)
          arr.push({ x: width, y: height + 1 }); // height+1/width0
      if (height < AztecsHelper.mapHeight - 1 && width > 0)
        if (map[height + 1][width - 1].content === cellContent)
          arr.push({ x: width - 1, y: height + 1 }); // height+1/width-1
      if (width < AztecsHelper.mapWidth - 1 && height > 0)
        if (map[height - 1][width + 1].content === cellContent)
          arr.push({ x: width + 1, y: height - 1 }); // height-1/width+1
      if (
        height < AztecsHelper.mapHeight - 1 &&
        width < AztecsHelper.mapWidth - 1
      )
        if (map[height + 1][width + 1].content === cellContent)
          arr.push({ x: width + 1, y: height + 1 }); // +1/+1
      if (width > 0 && height > 0)
        if (map[height - 1][width - 1].content === cellContent)
          arr.push({ x: width - 1, y: height - 1 }); // -1/-1
    } else {
      if (width > 0)
        if (typeof map[height][width - 1].content === cellContent)
          arr.push({ x: width - 1, y: height }); // height0/width-1
      if (height > 0)
        if (typeof map[height - 1][width].content === cellContent)
          arr.push({ x: width, y: height - 1 }); // height-1/width0
      if (width < AztecsHelper.mapWidth - 1)
        if (typeof map[height][width + 1].content === cellContent)
          arr.push({ x: width + 1, y: height }); // height0/width+1
      if (height < AztecsHelper.mapHeight - 1)
        if (typeof map[height + 1][width].content === cellContent)
          arr.push({ x: width, y: height + 1 }); // height+1/width0
      if (height < AztecsHelper.mapHeight - 1 && width > 0)
        if (typeof map[height + 1][width - 1].content === cellContent)
          arr.push({ x: width - 1, y: height + 1 }); // height+1/width-1
      if (width < AztecsHelper.mapWidth - 1 && height > 0)
        if (typeof map[height - 1][width + 1].content === cellContent)
          arr.push({ x: width + 1, y: height - 1 }); // height-1/width+1
    ...
    for (let j = 0; j < tries; j++) {
      ...
      while (i > 0) {
        ...
        if (map[y][x] != 'G') {
          ...
          if (y < h - 1 && x > 0)
            if (map[y + 1][x - 1] !== 'G') map[y + 1][x - 1] = 'N'; // y+1/x-1
          if (x < w - 1 && y > 0)
            if (map[y - 1][x + 1] !== 'G') map[y - 1][x + 1] = 'N'; // y-1/x+1
          if (y < h - 1 && x < w - 1)
            if (map[y + 1][x + 1] !== 'G') map[y + 1][x + 1] = 'N'; // +1/+1
          if (x > 0 && y > 0)
            if (map[y - 1][x - 1] !== 'G') map[y - 1][x - 1] = 'N'; // -1/-1
          i--;
        }
  ...
  test: () => {
    ...
    AztecsHelper.grid = JSON.parse(
      '[[{"content":1},{"content":1},{"content":" "},{"content":" "},{"content":" "},{"content":1},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"}],[{"content":"?"},{"content":1},{"content":" "},{"content":" "},{"content":" "},{"content":2},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"}],[{"content":1},{"content":1},{"content":" "},{"content":" "},{"content":" "},{"content":1},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"}],[{"content":" "},{"content":" "},{"content":" "},{"content":" "},{"content":" "},{"content":2},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"}],[{"content":1},{"content":1},{"content":1},{"content":" "},{"content":" "},{"content":1},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"}],[{"content":"?"},{"content":"?"},{"content":3},{"content":2},{"content":1},{"content":2},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"}],[{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"},{"content":"?"}]]',
    );
    AztecsHelper.CalcAdjacentCells();
  },
  processResources: (data, postData) => {
    ...
    if (
      ...
      if (
        postData[0].requestData.filter(
          (x) => x?.['resources'] !== undefined,
        )?.[0]?.resources?.aztecs_collecting_minigame_turns > 0
      ) {
        AztecsHelper.boughtSomething = true;
      } else {
        AztecsHelper.boughtSomething = false;
