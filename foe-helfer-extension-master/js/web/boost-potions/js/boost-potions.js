...
let BoostPotions = {
  ...
  Show: () => {
    ...
    if ($('#BoostPotions').length < 1) {
      let div = $('<div />')
        ...
        .append(
          `<div class="shortest"></div><div class="list"><div class="active"></div><div class="table"></div></div>`,
        );
    ...
    for (let b of Object.keys(BoostPotions.active)) {
      ...
      let duration = moment.duration(
        moment.unix(BoostPotions.active[b].expire).diff(moment(new Date())),
      );
      if (duration < 0) {
      ...
      let all =
        BoostPotions.active[b].target == undefined ||
        BoostPotions.active[b].target == 'all';
      ...
      if (
        !BoostPotions.activeMapRelevance[
          all ? 'all' : BoostPotions.active[b].target
        ].includes(ActiveMap)
      )
        continue;
      d = e;
      $('#BoostPotions .shortest')
        .html(
          shortest +
            `<span><img src="${srcLinks.get(`/shared/icons/${b}.png`, true)}">${h == 0 ? '' : h + ':'}${m < 10 ? '0' + m : m}:${s < 10 ? '0' + s : s}</span>`,
        )
        .addClass('boosted');
    }
  ...
  updateList: () => {
    ...
    for (let i of Object.values(MainParser.Inventory)) {
      ...
      if (!BoostPotions.list[b.boostType][b.target || 'all'])
        BoostPotions.list[b.boostType][b.target || 'all'] = [];
      BoostPotions.list[b.boostType][b.target || 'all'].push({
        duration: b.duration,
        amount: i.inStock,
        value: b.value,
      });
    ...
    for (let target of [
      ...
      if (!BoostPotions.activeMapRelevance[target].includes(ActiveMap))
        continue;
