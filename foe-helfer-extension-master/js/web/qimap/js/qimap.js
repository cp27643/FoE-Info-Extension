const QIMap = {
  ...
  countProgress: (nodeId) => {
    if (nodeId !== null) console.log(nodeId);
  },
  ...
  showBody: () => {
    ...
    QIMap.CurrentMapData.nodes.forEach((node) => {
      ...
      QIMap.MaxY =
        (node.position.y || 0) > QIMap.MaxY ? node.position.y || 0 : QIMap.MaxY;
      QIMap.MinX = x < QIMap.MinX ? x : QIMap.MinX;
      QIMap.MinY =
        (node.position.y || 0) < QIMap.MinY ? node.position.y || 0 : QIMap.MinY;

      node.connectedNodes.forEach((connection) => {
        let findDuplicates = QIMap.NodeConnections.find(
          (x) => x.id == node.id && x.connectedNode == connection.targetNodeId,
        );
        if (!findDuplicates) {
          let newNode = {
            id: node.id,
    ...
    QIMap.CurrentMapData.nodes.forEach((node) => {
      let x =
        (node.position.x - QIMap.MinX) * QIMap.XMultiplier + QIMap.XOffset ||
        QIMap.XOffset;
      let y =
        (node.position.y - QIMap.MinY) * QIMap.YMultiplier + QIMap.YOffset ||
        QIMap.YOffset;
      let type =
        node.type.type !== undefined ? node.type.type : node.type.fightType;
      let currentProgress =
        node.state.state === 'open' ? (node.state.currentProgress || 0) + '/'
        : node.state.state === 'finished' ? node.type.requiredProgress + '/'
        : '';
      if (node.type.__class__ !== 'GuildRaidsMapNodeStart') {
        out +=
          ...
          type +
          ' ' +
          (node.type.armyType ? node.type.armyType : '') +
          (node.state.hasTarget ? ' target' : '') +
          '">';
        ...
        out +=
          ...
          'px;top:' +
          y +
          'px" class="start"><span class="img"></span></span>';
    });
    out += '</div></div>';
    ...
    $('#QIMap')
      ...
      .done(function () {
        ...
        $('#nodeMap, #nodeConnections').css({
          width:
            QIMap.MaxY * QIMap.XMultiplier +
            QIMap.XOffset +
            QIMap.XOffset +
            'px',
          height: QIMap.MaxY * QIMap.YMultiplier + QIMap.YOffset + 120 + 'px',
        });
        QIMap.mapDrag();
  ...
  showNodeConnections: () => {
    ...
    QIMap.NodeConnections.forEach((connection) => {
      ...
      for (const [i, path] of connection.connectedNodePosition.entries()) {
        ...
        if (prevX == '') {
          x =
            ((connection.nodePosition.x || 0) - QIMap.MinX) *
              QIMap.XMultiplier +
              QIMap.XOffset +
              ((connection.nodePosition.x || 0) - QIMap.MinX) * 50 ||
            QIMap.XOffset;
          y =
            (connection.nodePosition.y - QIMap.MinY) * QIMap.YMultiplier +
              QIMap.YOffset +
              (connection.nodePosition.y - QIMap.MinY) * 50 ||
            QIMap.YOffset + 50;
        } else {
          x = prevX;
          y = prevY;
        ...
        targetX =
          ((path.x || 0) - QIMap.MinX) * QIMap.XMultiplier +
            QIMap.XOffset +
            ((path.x || 0) - QIMap.MinX) * 50 || QIMap.XOffset;
        targetY =
          (path.y - QIMap.MinY) * QIMap.YMultiplier +
            QIMap.YOffset +
            (path.y - QIMap.MinY) * 50 || QIMap.YOffset + 50;
