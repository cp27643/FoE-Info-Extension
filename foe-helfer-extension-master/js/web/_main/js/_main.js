  ...
  return new Promise((resolve, reject) => {
    let timer = () => {
      ...
      for (let x of varlist) {
        if (x.substr(0, 2) == '$(' && eval(x).length === 0) {
          // jQuery object
          doResolve = false;
...
let ApiURL = 'https://api.foe-rechner.de/',
  ...
  StartUpDone = new Promise((resolve) =>
    window.addEventListener('foe-helper#StartUpDone', resolve, {
      once: true,
      passive: true,
    }),
  ),
  ...
  PlayerLinkFormat2 =
    'https://foestats.com/__server__/__world__/players/__playerid__',
  GuildLinkFormat = 'https://foe.scoredb.io/__world__/Guild/__guildid__',
  GuildLinkFormat2 =
    'https://foestats.com/__server__/__world__/guilds/__guildid__',
...
const i18n_loadPromise = (async () => {
  ...
  const vendorsLoadedPromise = new Promise((resolve) =>
    window.addEventListener('foe-helper#vendors-loaded', resolve, {
      passive: true,
      once: true,
    }),
  );
...
(function () {
  ...
  FoEproxy.addMetaHandler('city_entities', (xhr, postData) => {
    ...
    MainParser.CityEntities = Object.assign(
      {},
      ...EntityArray.map((x) => ({ [x.id]: x })),
    );
  ...
  FoEproxy.addMetaHandler('building_entity_lookup', (xhr, postData) => {
    ...
    let buildingUrls = Object.assign(
      {},
      ...buildingUrlsRaw.map((x) => ({
        [x.identifier.replace('building_entity_', '')]: {
          url: x.url,
          hash: x.url.replace(/.*?([^-]+$)/gm, '$1'),
        },
      })),
    );
    setTimeout(() => {
      MainParser.CityEntityBuilder(buildingUrls);
    }, 500);
  });
  ...
  FoEproxy.addMetaHandler('building_upgrades', (xhr, postData) => {
    ...
    MainParser.BuildingUpgrades = Object.assign(
      {},
      ...BuildingUpgradesArray.map((x) => ({ [x.upgradeItem.id]: x })),
    );
  ...
  FoEproxy.addMetaHandler('building_sets', (xhr, postData) => {
    ...
    MainParser.BuildingSets = Object.assign(
      {},
      ...BuildingSetsArray.map((x) => ({ [x.id]: x })),
    );
  });
  ...
  FoEproxy.addMetaHandler('building_chains', (xhr, postData) => {
    ...
    MainParser.BuildingChains = Object.assign(
      {},
      ...BuildingChainsArray.map((x) => ({ [x.id]: x })),
    );
  });
  ...
  FoEproxy.addMetaHandler('selection_kits', (xhr, postData) => {
    ...
    MainParser.SelectionKits = Object.assign(
      {},
      ...SelectKitsArray.map((x) => ({ [x.selectionKitId]: x })),
    );
  ...
  FoEproxy.addHandler('StartupService', 'getData', (data, postData) => {
    ...
    for (let i in data.responseData.socialbar_list) {
      vals.getNeighborList +=
        data.responseData.socialbar_list[i].is_neighbor ? 1 : 0;
      vals.getFriendsList +=
        data.responseData.socialbar_list[i].is_friend ? 1 : 0;
      vals.getClanMemberList +=
        data.responseData.socialbar_list[i].is_guild_member ? 1 : 0;
    }
    ...
    MainParser.CityMapData = Object.assign(
      {},
    ...
    // EventCountdown
    let eventCountDownFeature =
      data.responseData.feature_flags?.features.filter((v) => {
        return v.feature === 'event_start_countdown';
      });
    EventCountdown =
      eventCountDownFeature?.length > 0 ?
        eventCountDownFeature[0]['time_string']
      : false;

    // Unlocked features
    if (data.responseData.unlocked_features) {
      MainParser.UnlockedFeatures = data.responseData.unlocked_features?.map(
        function (obj) {
          return obj.feature;
        },
      );
      ...
      $('script').each((i, s) => {
        ...
        try {
          let ulf = JSON.parse(
            [
              ...s.innerHTML.matchAll(/(unlockedFeatures:\ )(.*?)(,\n)/gm),
            ][0][2],
          );
          if (Array.isArray(ulf))
    ...
    //A/B Tests
    MainParser.ABTests = Object.assign(
      {},
      ...data.responseData.active_ab_tests.map((x) => ({ [x.test_name]: x })),
    );
  ...
  FoEproxy.addHandler('StaticDataService', 'getMetadata', (data, postData) => {
    MainParser.MetaUrls = Object.assign(
      {},
      ...data.responseData.map((x) => ({ [x.identifier]: x.url })),
    );
  });
  ...
  FoEproxy.addHandler('CityMapService', 'getEntities', (data, postData) => {
    ...
    MainParser.CityMapData = Object.assign(
      {},
      ...data.responseData.map((x) => ({ [x.id]: x })),
    );
  ...
  FoEproxy.addHandler('OtherPlayerService', 'visitPlayer', (data, postData) => {
    ...
    CityMap.OtherPlayer.unlockedAreas =
      data.responseData.city_map.unlocked_areas;
    CityMap.OtherPlayer.mapData = Object.assign(
      {},
      ...data.responseData.city_map.entities.map((x) => ({ [x.id]: x })),
    );
  });
  ...
  FoEproxy.addHandler('CityMapService', (data, postData) => {
    if (
      data.requestMethod === 'moveEntity' ||
      data.requestMethod === 'moveEntities' ||
      data.requestMethod === 'updateEntity'
    ) {
  ...
  FoEproxy.addHandler('CityProductionService', (data, postData) => {
    if (
      data.requestMethod === 'pickupProduction' ||
      data.requestMethod === 'pickupAll' ||
      data.requestMethod === 'startProduction' ||
      data.requestMethod === 'cancelProduction'
    ) {
  ...
  FoEproxy.addHandler(
    ...
    (data, postData) => {
      ...
      if (
        data.responseData['armyId'] === 1 ||
        data.responseData['state']['round'] === 1 ||
        data.responseData['battleType']['totalWaves'] === 1
      ) {
        ...
        for (let i = 0; i < units.length; i++) {
          ...
          if (
            ...
            OwnUnits.push({
              ...
              startHitpoints: unit.startHitpoints,
        ...
        Fights.push({
          enemy: EnemyUnits,
          own: OwnUnits,
          won: data.responseData['state']['winnerBit'] === 1,
        });
        EnemyUnits = [];
        OwnUnits = [];
      } else if (
        data.responseData['battleType']['totalWaves'] === 2 &&
        data.responseData['battleType']['currentWaveId'] == null
      ) {
        let units = data.responseData.state.unitsOrder;
        for (let i = 0; i < units.length; i++) {
          const unit = units[i];
  ...
  FoEproxy.addHandler('OtherPlayerService', 'all', (data, postData) => {
    if (
      data.requestMethod === 'getNeighborList' ||
      data.requestMethod === 'getFriendsList' ||
      data.requestMethod === 'getClanMemberList' ||
      data.requestMethod === 'getAwaitingFriendRequestCount'
    ) {
      MainParser.UpdatePlayerDict(
        data.responseData,
        'PlayerList',
    ...
    if (data.requestMethod === 'getSocialList') {
      if (data.responseData.neighbours)
        MainParser.UpdatePlayerDict(
          data.responseData.neighbours,
          'PlayerList',
          'getNeighborList',
        );
      if (data.responseData.guildMembers)
        MainParser.UpdatePlayerDict(
          data.responseData.guildMembers,
          'PlayerList',
          'getClanMemberList',
        );
      if (data.responseData.friends)
        MainParser.UpdatePlayerDict(
          data.responseData.friends,
          'PlayerList',
          'getFriendsList',
        );
    }
  ...
  FoEproxy.addHandler('GreatBuildingsService', 'all', (data, postData) => {
    let getConstruction =
      data.requestMethod === 'getConstruction' ? data : null;
    let getConstructionRanking =
      data.requestMethod === 'getConstructionRanking' ? data : null;
    let contributeForgePoints =
      data.requestMethod === 'contributeForgePoints' ? data : null;
    let Rankings,
      Bonus = {},
      Era;
  ...
  FoEproxy.addHandler(
    ...
    (data, postData) => {
      ...
      if (formattedData.responseData[0]?.player_id === ExtPlayerID) {
        if ($('#OwnPartBox').length > 0) {
          Parts.CityMapEntity.max_level =
            formattedData.responseData[0]?.max_level;
          Parts.CalcBody();
        }
      }
    },
  ...
  FoEproxy.addRequestHandler('InventoryService', 'useItem', (postData) => {
    if (postData?.requestData?.[0]?.__class__ == 'UseItemOnBuildingPayload') {
      if (
        MainParser.Inventory[postData?.requestData?.[0]?.itemId]
          .itemAssetName == 'store_building'
      ) {
  ...
  function lgUpdate() {
    ...
    // Own GB
    if (
      CityMapEntity.responseData[0].player_id === ExtPlayerID ||
      Settings.GetSetting('ShowOwnPartOnAllGBs')
    ) {
  ...
  FoEproxy.addHandler(
    ...
    (data, postData) => {
      if (
        ...
        data.responseData?.type?.value != 'PlayerMain'
      )
  ...
  FoEproxy.addRawWsHandler((data) => {
    ...
    if (
      requestMethod === 'newEvent' &&
      responseData.type === 'trade_accepted'
    ) {
  ...
  FoEproxy.addHandler('QuestService', 'getUpdates', (data, PostData) => {
    if (
      PostData[0]?.requestClass === 'QuestService' &&
      PostData[0]?.requestMethod === 'advanceQuest'
    ) {
  ...
  FoEproxy.addHandler(
    'UnlockableFeatureService',
    'getUnlockedFeatures',
    (data, postData) => {
      MainParser.UnlockedFeatures = data.responseData.map(function (obj) {
        return obj.feature;
      });
    },
  );
...
let HelperBeta = {
  ...
  menu: ['unitsGex', 'marketOffers'],
  active:
    JSON.parse(localStorage.getItem('HelperBetaActive')) ||
    devMode === 'true' ||
    loadBeta,
};

let MainParser = {
  foeHelperBgApiHandler:
    /** @type {null|((request: {type: string}&object) => Promise<{ok:true, data: any}|{ok:false, error:string}>)}*/ (
      null
    ),
  ...
  VersionSpecificStartupCode: () => {
    ...
    if (!LastStartedVersion) {
      ...
      HTML.ShowToastMsg({
        ...
        text:
          i18n('Menu.NewVersion.Desc') +
          ' <a href="https://foe-helper.com/extension/update?lang=en" target="_blank">ChangeLog</a>',
        type: 'success',
  ...
  CityEntityBuilder: async (buildingUrls) => {
    ...
    buildingsOld = Object.assign(
      {},
      ...buildingsOld.map((x) => ({ [x.id]: x })),
    );
    let Metadata = {};
    let updated = [];
        ...
        xhr.onreadystatechange = function () {
          if (xhr.readyState === XMLHttpRequest.DONE) {
            ...
            if (xhr.status === 200) {
              ...
              setTimeout(
                () => fetchMeta(id, meta, retries - 1).then(resolve),
                1000,
              );
            } else {
  ...
  correctBuildingType: () => {
    for (let i in MainParser.CityEntities) {
      ...
      if (!CityEntity.type)
        CityEntity.type = CityEntity?.components?.AllAge?.tags?.tags?.find(
          (value) => value.hasOwnProperty('buildingType'),
        ).buildingType;
    }
  },
  ...
  sendExtMessage: async (data) => {
    ...
    if (typeof chrome !== 'undefined') {
      // @ts-ignore
      _responsePromise = new Promise((resolve) =>
    ...
    const response = await new Promise((resolve, reject) => {
      ...
      setTimeout(
        () =>
          resolve({
            ok: false,
            error: 'response timeout for: ' + JSON.stringify(data),
          }),
        1000,
      );
    });
    ...
    if (response.ok === true) {
      ...
      if (
        response.error.indexOf('"type":"alerts"') === -1 &&
        response.error.indexOf('"action":"getAll"') === -1
      )
  ...
  GetPlayerLink: (PlayerID, PlayerName) => {
    if (Settings.GetSetting('ShowLinks')) {
      let PlayerLink = HTML.i18nReplacer(PlayerLinkFormat, {
        world: ExtWorld.toUpperCase(),
        playerid: PlayerID,
      });
      if (localStorage.getItem('linkSite') === 'siteForgedb')
        PlayerLink = HTML.i18nReplacer(PlayerLinkFormat2, {
          server: ExtWorld.toLowerCase().replace(/[0-9]/g, ''),
          world: ExtWorld.toLowerCase(),
          playerid: PlayerID,
        });
  ...
  GetGuildLink: (GuildID, GuildName, WorldId) => {
    ...
    if (Settings.GetSetting('ShowLinks')) {
      let GuildLink = HTML.i18nReplacer(GuildLinkFormat, {
        world: WorldId.toUpperCase(),
        guildid: GuildID,
      });
      if (localStorage.getItem('linkSite') === 'siteForgedb')
        GuildLink = HTML.i18nReplacer(GuildLinkFormat2, {
          server: ExtWorld.toLowerCase().replace(/[0-9]/g, ''),
          world: ExtWorld.toLowerCase(),
          guildid: GuildID,
        });
  ...
  GetBuildingLink: (BuildingID, BuildingName) => {
    if (Settings.GetSetting('ShowLinks')) {
      let BuildingLink = HTML.i18nReplacer(BuildingsLinkFormat, {
  ...
  StartUp: async (d) => {
    ...
    await ExistenceConfirmed(
      'MainParser.CityEntities||srcLinks.FileList||Infoboard||EventHandler',
    );
  ...
  SendLGData: (d) => {
    const dataEntity = d['CityMapEntity']['responseData'][0],
      realData = {
        image: srcLinks.get(
          `/city/buildings/${dataEntity['cityentity_id'].replace('X_', 'X_SS_')}.png`,
          true,
        ),
        entity: dataEntity,
        ranking: d['Rankings'],
  ...
  Allies: {
    ...
    getAllies: (allies) => {
      MainParser.Allies.allyList = Object.assign(
        {},
        ...allies.map((a) => ({ [a.id]: a })),
      );
      ...
      for (let ally of allies) {
        ...
        if (list[ally.mapEntityId]) list[ally.mapEntityId][ally.id] = ally.id;
        else list[ally.mapEntityId] = { [ally.id]: ally.id };
      }
      MainParser.Allies.updateAllyList();
    },
    ...
    updateAlly: (ally) => {
      if (ally.mapEntityId) {
        ...
        if (mapID) {
          ...
          if (Object.keys(MainParser.Allies.buildingList[mapID]).length == 0)
            delete MainParser.Allies.buildingList[mapID];
        }
      }
    ...
    getProd: (CityMapId) => {
      ...
      Object.values(M.buildingList[CityMapId]).forEach((id) => {
        let a = M.allyList[id];
        if (a.currentLevel?.boosts || a.boosts)
          prod.boosts = (prod.boosts || []).concat(
            a.currentLevel?.boosts || a.boosts,
          );
      });
      return prod;
    },
    ...
    setRarities: (raw) => {
      MainParser.Allies.rarities = Object.assign(
        {},
        ...raw.map((r) => ({ [r.id.value]: r })),
      );
    },

    setTypes: (raw) => {
      MainParser.Allies.types = Object.assign(
        {},
        ...raw.map((t) => ({ [t.id]: t })),
      );
    },
    ...
    updateAllyList: () => {
        ...
        ...Object.values(MainParser.CityMapData)
          .map((x) => ({
            ...
            rooms: structuredClone(
              MainParser.CityEntities[x.cityentity_id]?.components?.AllAge?.ally
                ?.rooms,
            ),
          }))
      ...
      Object.values(buildings).forEach((b) => {
        for (let [i, r] of Object.entries(b.rooms)) {
          rooms[b.id + '#' + i] = {
            ...
            roomRarity:
              r.rarity?.value ||
              Object.keys(MainParser.Allies.rarities).join('#'),
      ...
      sortedRooms = Object.entries(rooms).sort((a, b) => {
        f = (r) => {
          return (
            Object.keys(MainParser.Allies.rarities).indexOf(r.allyRarity) +
            (r.buildingName ? 10 : 0) +
            (r.fragmentsAmount ? 100 : 0)
          );
        };
        return f(a[1]) - f(b[1]);
      });
      ...
      for (let [roomId, r] of sortedRooms) {
        ...
        if (buildingId != 0 && r.allyBoosts !== null) {
          for (let boost of r.allyBoosts) {
            let bBoost = MainParser.Allies.buildingBoostSums.find(
              (x) =>
                x.type === boost.type &&
                x.targetedFeature === boost.targetedFeature,
            );
            if (bBoost) bBoost.value += boost.value;
            else
    ...
    rarityStars: (r) => {
      ...
      if (i == -1)
        return `<img style="filter: drop-shadow(0px 2px 2px black)"  src="${srcLinks.get(`/shared/icons/when_motivated.png`, true)}">`;
      if (i == 0)
        return `<span style="font-size: large; color: transparent; text-shadow: 0px 0px 4px black;" >☆</span>`;
      let ret = '';
    ...
    ShowSettings: () => {
      ...
      let h = [];
      h.push(
        `<p><label><input id="allyListAutoOpen" type="checkbox" ${autoOpen === true ? ' checked="checked"' : ''} />${i18n('Boxes.Settings.Autostart')}</label></p>`,
      );
      h.push(
        `<p><button onclick="MainParser.Allies.SaveSettings()" id="save-bghelper-settings" class="btn" style="width:100%">${i18n('Boxes.Settings.Save')}</button></p>`,
      );
    ...
    SaveSettings: () => {
      let value = false;
      if ($('#allyListAutoOpen').is(':checked')) value = true;
  ...
  SetArkBonus: (LimitedBonuses) => {
    ...
    for (let i in LimitedBonuses) {
      if (!LimitedBonuses.hasOwnProperty(i)) {
        break;
      }
  ...
  SetArkBonus2: () => {
    ...
    for (let i of Object.values(MainParser.CityMapData).filter(
      (x) => x?.bonus?.type === 'contribution_boost',
    )) {
  ...
  UpdatePlayerDict: (d, Source, ListType = undefined) => {
    if (Source === 'Conversation') {
      for (let i in d['messages']) {
        if (!d['messages'].hasOwnProperty(i)) continue;
  ...
  UpdatePlayerDictCore: (Player) => {
    ...
    if (PlayerID !== undefined) {
      if (PlayerDict[PlayerID] === undefined)
        PlayerDict[PlayerID] = {
          Activity:
            (
              Player['is_friend'] ||
              (Player['is_guild_member'] && HasGuildPermission)
            ) ?
              0
            : undefined,
        };
      ...
      if (Player['name'] !== undefined)
        PlayerDict[PlayerID]['PlayerName'] = Player['name'];
      if (Player['clan'] !== undefined)
        PlayerDict[PlayerID]['ClanName'] = Player['clan']['name'];
      if (Player['clan_id'] !== undefined)
        PlayerDict[PlayerID]['ClanId'] = Player['clan_id'];
      if (Player['avatar'] !== undefined)
        PlayerDict[PlayerID]['Avatar'] = Player['avatar'];
      if (Player['is_neighbor'] !== undefined)
        PlayerDict[PlayerID]['IsNeighbor'] = Player['is_neighbor'];
      if (Player['is_guild_member'] !== undefined)
        PlayerDict[PlayerID]['IsGuildMember'] = Player['is_guild_member'];
      if (Player['is_friend'] !== undefined)
        PlayerDict[PlayerID]['IsFriend'] = Player['is_friend'];
      if (Player['is_self'] !== undefined)
        PlayerDict[PlayerID]['IsSelf'] = Player['is_self'];
      if (Player['score'] !== undefined)
        PlayerDict[PlayerID]['Score'] = Player['score'];
      if (Player['won_battles'] !== undefined)
        PlayerDict[PlayerID]['WonBattles'] = Player['won_battles'];
      if (Player['activity'] !== undefined)
        PlayerDict[PlayerID]['Activity'] = Player['activity'];
      if (Player['era'] !== undefined)
        PlayerDict[PlayerID]['Era'] = Player['era'];
    }
  },
  ...
  setConversations: (d, refresh = false) => {
    ...
    let LCUindex = MainParser.Conversations.findIndex(
      (obj) => obj.id === '__lastCleanup',
    );
    let LCU = day;
    if (LCUindex === -1) {
    ...
    if (d['teasers']) {
      for (let k in d['teasers']) {
        ...
        let key = MainParser.Conversations.findIndex(
          (obj) => obj.id === d['teasers'][k]['id'],
        );
        ...
        if (key !== -1) {
          ...
          MainParser.Conversations[key]['favorite'] =
            d['teasers'][k]['isFavorite'];
          MainParser.Conversations[key]['important'] =
            d['teasers'][k]['isImportant'];
    ...
    if (MainParser.Conversations.length > 0) {
      ...
      if (LCU != day) {
        ...
        MainParser.Conversations = MainParser.Conversations.filter(
          (obj) => obj.lastSeen + 30 > day,
        );
      }
      // Dopplungen entfernen und Daten lokal abspeichern
      MainParser.Conversations = [
        ...new Set(MainParser.Conversations.map((s) => JSON.stringify(s))),
      ].map((s) => JSON.parse(s));
      localStorage.setItem(
        'ConversationsHeaders',
        JSON.stringify(MainParser.Conversations),
      );
    }
  },
  ...
  Inactives: {
    list: [],
    ignore: JSON.parse(
      localStorage.getItem('LimitedBuildingsIgnoreList') || '[]',
    ),

    check: () => {
      //get list of buildings for which an alert is already set
      let LB = JSON.parse(
        localStorage.getItem('LimitedBuildingsAlertSet') || '{}',
      );
      //get list of expired limited buildings in city
      let list = Object.values(MainParser.CityMapData)
        .filter((value) => !!value.decayedFromCityEntityId)
        .map((value) => value.id);
      //remove buildings that were already tracked and that should have just triggered an alert
      for (let i = list.length - 1; i >= 0; i--) {
        if (
          LB[list[i]] ||
          MainParser.Inactives.ignore.includes(
            MainParser.CityMapData[list[i]].cityentity_id,
          )
        ) {
          list.splice(i, 1);
        }
      }
      MainParser.Inactives.list = [
        ...new Set(list.map((x) => MainParser.CityMapData[x].cityentity_id)),
      ];
      ...
      if (list.length > 0) {
        const data = {
          ...
          body: list
            .map(
              (x) =>
                MainParser.CityEntities[MainParser.CityMapData[x].cityentity_id]
                  .name,
            )
            .join('\n'),
      ...
      for (let building of buildings) {
        ...
        if (
          !LB[building.id] &&
          MainParser.CityEntities[building.cityentity_id]?.components?.AllAge
            ?.limited?.config?.expireTime
        ) {
          const data = {
            title: i18n('InactiveBuildingsAlert.title'),
            body: MainParser.CityEntities[
              MainParser.CityEntities[building.cityentity_id]?.components
                ?.AllAge?.limited?.config?.targetCityEntityId
            ].name,
            expires:
              (MainParser.CityEntities[building.cityentity_id]?.components
                ?.AllAge?.limited?.config?.expireTime +
                building.state.constructionFinishedAt -
                GameTime.Offset) *
              1000,
            repeat: -1,
            persistent: true,
    ...
    updateSettings: () => {
      ...
      for (let id of MainParser.Inactives.ignore) {
        t.push(
          `<span class="inactivesIgnoreToggle" data-id="${id}" title="${i18n('Boxes.InactivesSettings.NoAlert')}">🤐${MainParser.CityEntities[id].name}</span></br>`,
        );
      }
      ...
      for (let id of MainParser.Inactives.list) {
        t.push(
          `<span class="inactivesIgnoreToggle" data-id="${id}" title="${i18n('Boxes.InactivesSettings.AlertActive')}">⚠️${MainParser.CityEntities[id].name}</span></br>`,
        );
      }
      ...
      $('.inactivesIgnoreToggle').on('click', (e) => {
        ...
        localStorage.setItem(
          'LimitedBuildingsIgnoreList',
          JSON.stringify(MainParser.Inactives.ignore),
        );
        MainParser.Inactives.updateSettings();
      });
    },
...
if (
  window.foeHelperBgApiHandler !== undefined &&
  window.foeHelperBgApiHandler instanceof Function
) {
...
console.log(
  'FOE Helper version ' +
    extVersion +
    ' started' +
    (extVersion.indexOf('beta') > -1 ? ' in Beta Mode' : '') +
    '. ID: ' +
    extID,
);
