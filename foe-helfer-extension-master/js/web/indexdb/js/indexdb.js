...
let IndexDB = {
  ...
  _dbPromise: new Promise(
    (resolveCb) => (window._dbPromiseResolver = resolveCb),
  ),
  ...
  _applyIndexSchema(db) {
    db.version(1).stores({
      ...
      greatbuildings:
        '++id,playerId,name,&[playerId+name],level,currentFp,bestRateNettoFp,bestRateCosts,date',
    ...
    db.version(2).stores({
      ...
      greatbuildings:
        '++id,playerId,name,&[playerId+name],level,currentFp,bestRateNettoFp,bestRateCosts,date',
      investhistory:
        '++id,playerId,entity_id,&[playerId+entity_id],name,level,max_progress,current_progress,profit,currentFp,fphistory,date',
      forgeStats: '++id,type,amount,date', // FP Collector
      statsGBGPlayers: 'date', // battleground
      statsGBGPlayerCache: 'id, date', // Cache of players for using in gbgPlayers
      statsRewards: 'date', // Collected rewards by Himeji, etc
  ...
  mergeDatabases: async (playerId) => {
    ...
    if (await IndexDB.db.players.count()) {
      return;
    }
    clearLog();
    ...
    async function cloneTables(source, tables) {
      ...
      for (let destTable of destTables) {
        ...
        await IndexDB.db[destTable].bulkAdd(
          await source[sourceTable].toArray(),
        );
      }
    }
  ...
  cloneDB: async (srcName, dstName, version) => {
    ...
    if (await Dexie.exists(dstName)) {
      console.error(
        `Aborted. Please firstly delete "${dstName}" before continue.`,
      );
      return false;
    }
  ...
  calculateSpace: async () => {
    ...
    for (let table of db.tables.map((it) => it.name)) {
      ...
      const sizePerItemBytes =
        new Blob([JSON.stringify(data)]).size / data.length;
