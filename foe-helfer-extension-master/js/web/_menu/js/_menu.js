...
let _menu = {
  ...
  OverflowCheck: (selMenu = 'Box', flag) => {
    if (
      ...
      !flag &&
      selMenu != MainParser.SelectedMenu
    ) {
      $('#menu_box').remove();
      $('.tooltip').remove();
  ...
  unit_Btn: () => {
    let btn_UnitBG = _menu.MakeButton(
      ...
      '<em id="unit-Btn-closed" class="tooltip-error">' +
        i18n('Menu.Unit.Warning') +
        '<br></em>' +
        i18n('Menu.Unit.Desc'),
      true,
    );
  ...
  calculator_Btn: () => {
    let btn_CalcBG = _menu.MakeButton(
      ...
      '<em id="calculator-Btn-closed" class="tooltip-error">' +
        i18n('Menu.Calculator.Warning') +
        '<br></em>' +
        i18n('Menu.Calculator.Desc'),
      true,
    );
  ...
  partCalc_Btn: () => {
    let btn_OwnBG = _menu.MakeButton(
      ...
      '<em id="partCalc-Btn-closed" class="tooltip-error">' +
        i18n('Menu.OwnpartCalculator.Warning') +
        '<br></em>' +
        i18n('Menu.OwnpartCalculator.Desc'),
      true,
    );
  ...
  shopAssist_Btn: () => {
    let desc =
        '<i id="shopAssist-Btn-closed" class="tooltip-error">' +
        i18n('Menu.ShopAssist.DescWarning') +
        '</i> ' +
        i18n('Menu.ShopAssist.Desc'),
      red = true;
   ...
   */
  productions_Btn: () => {
    let pB = _menu.MakeButton(
      'productions',
      i18n('Menu.Productions.Title'),
      i18n('Menu.Productions.Desc'),
    );
   ...
   */
  minigame_aztecs_Btn: () => {
    let btn_Aztek = _menu.MakeButton(
      'minigame_aztecs',
      i18n('Menu.AztecMiniGame.Title'),
      i18n('Menu.AztecMiniGame.Desc'),
      true,
    );
  ...
  productionsRating_Btn: () => {
    let btn_prodratBG = _menu.MakeButton(
      'productionsRating',
      i18n('Menu.ProductionsRating.Title'),
      i18n('Menu.ProductionsRating.Desc'),
    );
  ...
  playerProfile_Btn: () => {
    let btn_playerProfileBG = _menu.MakeButton(
      ...
      '<em id="PlayerProfile-Btn-closed" class="tooltip-error">' +
        i18n('Menu.PlayerProfile.Warning') +
        '<br></em>' +
        i18n('Menu.PlayerProfile.Desc'),
      true,
    );
    ...
    btn_playerProfile.append(
      '<img src="' + srcLinks.GetPortrait(ExtPlayerAvatar) + '" />',
    );
   ...
   */
  infobox_Btn: () => {
    let btn_Info = _menu.MakeButton(
      'infobox',
      i18n('Menu.Info.Title'),
      i18n('Menu.Info.Desc'),
    );
  ...
  findGB_Btn: () => {
    let btn_ = _menu.MakeButton(
      'findGB',
      i18n('Boxes.findGB.Title'),
      i18n('Menu.findGB.Desc'),
    );
  ...
  campagneMap_Btn: () => {
    let btn_MapBG = _menu.MakeButton(
      ...
      '<em id="campagneMap-Btn-closed" class="tooltip-error">' +
        i18n('Menu.Campagne.Warning') +
        '<br></em>' +
        i18n('Menu.Campagne.Desc'),
      true,
    );
   ...
   */
  cityMap_Btn: () => {
    let btn_CityBG = _menu.MakeButton(
      'cityMap',
      i18n('Menu.Citymap.Title'),
      i18n('Menu.Citymap.Desc'),
    );
   ...
   */
  qiMap_Btn: () => {
    let btn_QIMapBG = _menu.MakeButton(
      'qiMap',
      i18n('Menu.QIMap.Title'),
      i18n('Menu.QIMap.Desc'),
      true,
    );

    let btn_QIMap = $('<span />').on('click', function () {
      if (Object.keys(QIMap.CurrentMapData).length > 0) QIMap.showBox();
    });
   ...
   */
  hiddenRewards_Btn: () => {
    let btn_RewardsBG = _menu.MakeButton(
      'hiddenRewards',
      i18n('Menu.HiddenRewards.Title'),
      i18n('Menu.HiddenRewards.Desc'),
    );
  ...
  recurringQuests_Btn: () => {
    let btn_RewardsBG = _menu.MakeButton(
      'recurringQuests',
      i18n('Menu.recurringQuests.Title'),
      i18n('Menu.recurringQuests.Desc'),
    );
    ...
    return btn_RewardsBG.append(
      btn_Rewards,
      $(
        `<span id="recurring-count" class="hud-counter" style="${!Recurring.data.count || !Recurring.data.showCounter ? 'display:none;' : ''}">${Recurring.data.count || 0}</span>`,
      ),
    );
  },
   ...
   */
  notice_Btn: () => {
    let btn_NoticeBG = _menu.MakeButton(
      'notice',
      i18n('Menu.Notice.Title'),
      i18n('Menu.Notice.Desc'),
    );
  ...
  settings_Btn: () => {
    let btn = _menu.MakeButton(
      'settings',
      i18n('Menu.Settings.Title'),
      i18n('Menu.Settings.Desc'),
    );
   ...
   */
  stats_Btn: () => {
    let btn_StatsBG = _menu.MakeButton(
      'stats',
      i18n('Menu.Stats.Title'),
      i18n('Menu.Stats.Desc'),
    );
  ...
  greatBuildings_Btn: () => {
    let btn = _menu.MakeButton(
      'greatBuildings',
      i18n('Menu.greatbuildings.Title'),
      i18n('Menu.greatbuildings.Desc'),
    );
  ...
  market_Btn: () => {
    let btn = _menu.MakeButton(
      ...
      '<em id="market-Btn-closed" class="tooltip-error">' +
        i18n('Menu.Market.Warning') +
        '<br></em>' +
        i18n('Menu.Market.Desc'),
      true,
    );
   ...
   */
  blueGalaxy_Btn: () => {
    let OwnGalaxy = Object.values(MainParser.CityMapData).find(
      (obj) => obj['cityentity_id'] === 'X_OceanicFuture_Landmark3',
    );
    ...
    let btn = _menu.MakeButton(
      'blueGalaxy',
      i18n('Menu.Bluegalaxy.Title'),
      i18n('Menu.Bluegalaxy.Desc'),
    );
    ...
    return btn.append(
      btn_sp,
      $('<span id="hidden-blue-galaxy-count" class="hud-counter">0</span>'),
    );
  },
  ...
  moppelHelper_Btn: () => {
    ...
    let btn = _menu.MakeButton(
      'moppelHelper',
      i18n('Menu.Moppelhelper.Title'),
      i18n('Menu.Moppelhelper.Desc'),
    );
  ...
  fpCollector_Btn: () => {
    let btn = _menu.MakeButton(
      'fpCollector',
      i18n('Menu.fpCollector.Title'),
      i18n('Menu.fpCollector.Desc'),
    );
  ...
  alerts_Btn: () => {
    let btn = _menu.MakeButton(
      'alerts',
      i18n('Menu.Alerts.Title'),
      i18n('Menu.Alerts.Desc'),
    );
  ...
  investment_Btn: () => {
    let btn = _menu.MakeButton(
      'investment',
      i18n('Menu.Investment.Title'),
      i18n('Menu.Investment.Desc'),
    );
  ...
  guildMemberstat_Btn: () => {
    let btn = _menu.MakeButton(
      ...
      '<em id="guildmemberstat-Btn-closed" class="tooltip-error">' +
        i18n('Menu.GuildMemberStat.Warning') +
        '<br></em>' +
        i18n('Menu.GuildMemberStat.Desc'),
      true,
    );
  ...
  gexStat_Btn: () => {
    let btn = _menu.MakeButton(
      'gexStat',
      i18n('Menu.GexStat.Title'),
      i18n('Menu.GexStat.Desc'),
    );
    ...
    return btn
      .append(
        btn_sp,
        $(
          `<span id="gex-attempt-count" class="hud-counter">${GExAttempts.count || 0}</span>`,
        ),
      )
      .ready(GExAttempts.refreshGUI);
  },
  ...
  castle_Btn: () => {
    let btn = _menu.MakeButton(
      'castle',
      i18n('Menu.Castle.Title'),
      i18n('Menu.Castle.Desc'),
    );
  ...
  compare_friends_threads_Btn: () => {
    let btn = _menu.MakeButton(
      'compare_friends_threads',
      i18n('Menu.CompareFriendsThreads.Title'),
      i18n('Menu.CompareFriendsThreads.Desc'),
    );
  ...
  discord_Btn: () => {
    let btn = _menu.MakeButton(
      'discord',
      i18n('Menu.Discord.Title'),
      i18n('Menu.Discord.Desc'),
    );
  ...
  music_Btn: () => {
    let btn = _menu.MakeButton(
      'music',
      i18n('Menu.Music.Title'),
      i18n('Menu.Music.Desc'),
    );
  ...
  musicControl_Btn: () => {
    let btn = _menu.MakeButton(
      'musicControl',
      i18n('Menu.MusicControl.Title'),
      i18n('Menu.MusicControl.Desc'),
    );
