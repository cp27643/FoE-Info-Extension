...
FoEproxy.addHandler('GuildRaidsService', 'getState', (data, postData) => {
  QiProgress.GlobalRankingTimeout = setTimeout(() => {
    ...
    if (
      QiProgress.curDateFilter === null ||
      QiProgress.curDateEndFilter === null
    ) {
      QiProgress.curDateFilter = moment
        .unix(QiProgress.CurrentQISeason)
        .subtract(11, 'd')
        .format('YYYYMMDD');
      QiProgress.curDateEndFilter = moment
        .unix(QiProgress.CurrentQISeason)
        .format('YYYYMMDD');
    }
  }, 500);
...
let QiProgress = {
  ...
  checkForDB: async (playerID) => {
    ...
    QiProgress.db.version(1).stores({
      snapshots:
        '&[player_id+qiround+time],[qiround+player_id], [date+player_id], qiround',
      history: '&qiround',
    });
  ...
  BuildProgressList: async (qiRound) => {
    ...
    for (let i in QiProgress.qiRound) {
      ...
      if (QiProgress.PrevAction !== null && histView === false) {
        let playerOld = QiProgress.PrevAction.find(
      ...
      b.push(
        '<td class="text-center" data-number="' +
          playerNew.actions +
          '">' +
          HTML.Format(playerNew.actions) +
          newActions +
          '</td>',
      );
      b.push(
        '<td class="text-center" data-number="' +
          playerNew.progress +
          '">' +
          HTML.Format(playerNew.progress) +
          newProgress +
          '</td>',
      );
      b.push('</tr>');
    ...
    t.push(
      '<table id="QiProgressTable" class="foe-table' +
        (histView === false ? ' chevron-right exportable' : '') +
        '">',
    );
    ...
    t.push(
      '<th class="text-center" data-export="Actions">' +
        i18n('Boxes.QiProgress.Actions') +
        '<br> <strong class="text-warning">(' +
        HTML.Format(tA) +
        ')</strong></th>',
    );
    t.push(
      '<th class="text-center" data-export="Progress">' +
    ...
    $('#qiContentWrapper')
      ...
      .promise()
      .done(function () {
        $('#QiProgressListBody tr.showdetailview')
          .off('click')
          .on('click', function () {
            let player_id = $(this).data('player');
            let qiRound = $(this).data('qiround');

            QiProgress.curDetailViewFilter = {
              content: 'player',
              player_id: player_id,
              qiround: qiRound,
            };
        ...
        let newPlayerProgress = $('#QiProgressTable tbody').find(
          'tr.new',
        ).length;
        if (newPlayerProgress > 0) {
          $('button#qi_filterProgressList').html(
            '&#8593; ' + newPlayerProgress,
          );
    ...
    if (QiProgress.PrevActionTimestamp !== null) {
      ...
      $('.time-diff').text(
        HTML.i18nReplacer(i18n('Boxes.QiProgress.LastSnapshot'), {
          time: time,
        }),
      );
    }
  ...
  ProgressListSettingsSaveValues: () => {
    QiProgress.ProgressSettings.showRoundSelector =
    ...
    localStorage.setItem(
      'QiProgressProgressSettings',
      JSON.stringify(QiProgress.ProgressSettings),
    );
  ...
  ShowPlayerDetailsBox: (d) => {
    if ($('#QiProgressPlayerDetails').length === 0) {
      ...
      if (localStorage.getItem('QiProgressPlayerDetailsCords') === null) {
        ptop =
          $('#QiProgress').length !== 0 ? $('#QiProgress').position().top : 0;
        pright =
          $('#QiProgress').length !== 0 ?
            $('#QiProgress').position().left + $('#QiProgress').width() + 10
          : 0;
        $('#QiProgressPlayerDetails')
          .css('top', ptop + 'px')
  ...
  BuildPlayerDetailContent: async (d) => {
    ...
    if (content === 'player') {
      detaildata = await QiProgress.db.snapshots
        .where({ qiround: qiround, player_id: player_id })
        .toArray();

      playerName = detaildata[0].name;
      dailyProgress = detaildata
        .reduce(
          function (res, obj) {
      ...
      h.push(
        '<div class="pname dark-bg text-center">' +
          playerName +
          ': ' +
          moment.unix(qiround).subtract(11, 'd').format(i18n('DateShort')) +
          ` - ` +
          moment.unix(qiround).format(i18n('Date')) +
          '</div>',
      );
      h.push(
      ...
      h.push(
        '<th class="is-number" data-type="qi-playerlog-group">' +
          i18n('General.Date') +
          '</th>',
      );
      h.push(
        '<th class="is-number text-center" data-type="qi-playerlog-group">' +
          HTML.i18nTooltip(i18n('Boxes.QiProgress.Actions')) +
          '</th>',
      );
      h.push(
        '<th class="is-number text-center" data-type="qi-playerlog-group">' +
          HTML.i18nTooltip(i18n('Boxes.QiProgress.Progress')) +
          '</th>',
      );
      h.push('</tr>');
      ...
      dailyProgress.forEach((day) => {
        ...
        h.push(
          `<td class="is-number text-center" data-number="${day.actions}">${HTML.Format(day.actions)}</td>`,
        );
        h.push(
          `<td class="is-number text-center" data-number="${day.progress}">${HTML.Format(day.progress)}</td>`,
        );
        h.push('</tr>');
      });
      ...
      detaildata = await QiProgress.db.snapshots
        .where({ qiround: qiround })
        .and(function (item) {
          return (
            item.date >= QiProgress.curDateFilter &&
            item.date <= QiProgress.curDateEndFilter
          );
        })
        .toArray();
      ...
      h.push(
        '<div class="datetimepicker"><button id="qiLogDatepicker" class="btn">' +
          QiProgress.formatRange() +
          '</button></div>',
      );
      h.push('<table id="QiProgressLogTable" class="foe-table qilog"><thead>');
      h.push('<tr class="sorter-header">');
      h.push(
        '<th class="is-number" data-type="qi-log-group">' +
          i18n('Boxes.QiProgress.Date') +
          '</th>',
      );
      h.push(
        '<th class="case-sensitive" data-type="qi-log-group">' +
          i18n('Boxes.QiProgress.Player') +
          '</th>',
      );
      h.push(
        '<th class="is-number text-center" data-type="qi-log-group">' +
          HTML.i18nTooltip(i18n('Boxes.QiProgress.Actions')) +
          '</th>',
      );
      h.push(
      ...
      detaildata.forEach((e) => {
        ...
        h.push(
          `<td class="is-number text-center" data-number="${e.actions}">${HTML.Format(e.actions)}</td>`,
        );
        h.push(
          `<td class="is-number text-center" data-number="${e.progress}">${HTML.Format(e.progress)}</td>`,
        );
        h.push('</tr>');
      });
    ...
    $('#QiProgressPlayerDetailsBody')
      ...
      .done(function () {
        ...
        $('#QiProgressPlayerDetailsBody tr.sorter-header').on(
          'click',
          function () {
        ...
        $('#QiProgressPlayerDetailsBody > .foe-table tr').on(
          'click',
          function () {
  ...
  SetBoxNavigation: async (qiRound) => {
    ...
    let ProgressSettings =
      storageSettings != null && storageSettings != 'undefined' ?
        JSON.parse(localStorage.getItem('QiProgressProgressSettings'))
      : '{}';

    QiProgress.ProgressSettings.showRoundSelector =
      ProgressSettings.showRoundSelector !== null ?
        ProgressSettings.showRoundSelector
      : QiProgress.ProgressSettings.showRoundSelector;
    QiProgress.ProgressSettings.showProgressFilter =
      ProgressSettings.showProgressFilter !== null ?
        ProgressSettings.showProgressFilter
      : QiProgress.ProgressSettings.showProgressFilter;

    if (QiProgress.AllRounds === undefined || QiProgress.AllRounds === null) {
      // get all available entires
      const qiRounds = await QiProgress.db.history
        .where('qiround')
        .above(0)
        .keys();
      qiRounds.sort(function (a, b) {
        return b - a;
      });
    ...
    if (qiRound && QiProgress.AllRounds && QiProgress.AllRounds.length) {
      ...
      if (QiProgress.ProgressSettings.showRoundSelector) {
        h.push(
          `${i18n('Boxes.QiProgress.QiRound')} <button class="btn btn-set-week" data-week="${previousweek}"${previousweek === null ? ' disabled' : ''}>&lt;</button> `,
        );
        h.push(`<select id="qi-select-qiRound">`);

        QiProgress.AllRounds.forEach((week) => {
          h.push(
            `<option value="${week}"${qiRound === week ? ' selected="selected"' : ''}>` +
      ...
      if (qiRound === QiProgress.CurrentQISeason) {
        ...
        if (QiProgress.ProgressSettings.showProgressFilter === 1) {
          h.push(
            `<button id="qi_filterProgressList" title="${HTML.i18nTooltip(i18n('Boxes.QiProgress.ProgressFilterDesc'))}" class="btn" disabled>&#8593;</button>`,
          );
        }
        h.push(`</div>`);
      }
      ...
      .done(function () {
        $('.btn-set-week')
          ...
          .on('click', function () {
            ...
            if (!QiProgress.AllRounds.includes(week)) {
              return;
            }
        ...
        $('#qi-select-qiRound')
          ...
          .on('change', function () {
            ...
            if (
              !QiProgress.AllRounds.includes(week) ||
              week === QiProgress.CurrentQISeason
            ) {
              return;
            }
  ...
  ShowSettings: () => {
    ...
    c.push(
      `<input id="gf_showRoundSelector" name="showroundswitcher" value="1" type="checkbox" ${Settings.showRoundSelector === 1 ? ' checked="checked"' : ''} /> <label for="gf_showRoundSelector">${i18n('Boxes.QiProgress.ShowRoundSelector')}</label></p>`,
    );
    c.push(
      `<p class="text-left"><input id="gf_showProgressFilter" name="showprogressfilter" value="1" type="checkbox" ${Settings.showProgressFilter === 1 ? ' checked="checked"' : ''} /> <label for="gf_showProgressFilter">${i18n('Boxes.QiProgress.ShowProgressFilter')}</label></p>`,
    );
    c.push(
      `<p><button id="save-QiProgressPlayerBox-settings" class="btn" style="width:100%" onclick="QiProgress.ProgressListSettingsSaveValues()">${i18n('Boxes.General.Save')}</button></p>`,
    );
    c.push(
      `<hr><p>${i18n('Boxes.General.Export')}: <span class="btn-group"><button class="btn" onclick="HTML.ExportTable($('#QiProgressTable'),'csv','QI')" title="${HTML.i18nTooltip(i18n('Boxes.General.ExportCSV'))}">CSV</button>`,
    );
    c.push(
      `<button class="btn" onclick="HTML.ExportTable($('#QiProgressTable'),'json','QI')" title="${HTML.i18nTooltip(i18n('Boxes.General.ExportJSON'))}">JSON</button></span></p>`,
    );
  ...
  HandleQiProgress: async (d) => {
    ...
    if (localStorage.getItem('QiProgress.NewAction') !== null) {
      QiProgress.PrevAction = JSON.parse(
        localStorage.getItem('QiProgress.NewAction'),
      );
      QiProgress.PrevActionTimestamp = parseInt(
        localStorage.getItem('QiProgress.NewActionTimestamp'),
      );
    ...
    await QiProgress.UpdateDB('history', {
    ...
    localStorage.setItem(
      'QiProgress.NewAction',
      JSON.stringify(QiProgress.NewAction),
    );

    QiProgress.NewActionTimestamp = moment().unix();
    localStorage.setItem(
      'QiProgress.NewActionTimestamp',
      QiProgress.NewActionTimestamp,
    );
  ...
  ToggleProgressList: (id) => {
    ...
    if (act === 'hide') {
      if (nelem.length !== 0) {
        ...
        localStorage.setItem(
          'QiProgressProgressSettings',
          JSON.stringify(QiProgress.PlayerBoxSettings),
        );
        $('#QiProgressTable > thead .text-warning').hide();
        oelem.hide();
        $('#' + id).addClass('filtered btn-green');
  ...
  DeleteOldSnapshots: async (qiround) => {
    let deleteCount = await QiProgress.db.snapshots
      .where('qiround')
      .notEqual(qiround)
      .delete();
  },
