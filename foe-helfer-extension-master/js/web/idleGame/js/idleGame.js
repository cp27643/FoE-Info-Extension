...
FoEproxy.addHandler('IdleGameService', 'getState', (data, postData) => {
  ...
  if (
    !Settings.GetSetting('ShowEventChest') ||
    !(Settings.GetSetting('EventHelperIdle') === undefined ?
      true
    : Settings.GetSetting('EventHelperIdle'))
  ) {
    return;
  }
  ...
  if (
    !idleGame.settings.currentEvent ||
  ...
  for (let t in data.responseData.taskHandler.completedTasks) {
    if (
      !Object.hasOwnProperty.call(
        data.responseData.taskHandler.completedTasks,
        t,
      )
    )
      continue;
  ...
  idleGame.ProgressDegree =
    Number(data.responseData.idleCurrencyAmount.degree) || 0;

  if (!!data?.responseData?.taskHandler?.inProgressTasks) {
    for (let t of data.responseData.taskHandler.inProgressTasks) {
      idleGame.Taskprogress[t.id] = {
        value: t.currentProgress.value || 0,
        degree: t.currentProgress.degree || 0,
      };
    }
  }
  ...
  if (data.responseData.stage) {
    ...
    if (!idleGame.settings.Strategy['CurrentVariant'])
      idleGame.settings.Strategy['CurrentVariant'] = 0;
    if (idleGame.Variant != idleGame.settings.Strategy.CurrentVariant) {
      idleGame.settings.Strategy.CurrentVariant = idleGame.Variant;
      if (!idleGame.settings.Strategy[idleGame.event][idleGame.Variant])
        idleGame.settings.Strategy[idleGame.event][idleGame.Variant] = [];
      for (let x in idleGame.settings.Strategy[idleGame.event][
        idleGame.Variant
      ]) {
        idleGame.settings.Strategy[idleGame.event][idleGame.Variant][x].check =
          false;
      }
      idleGame.settings.targets = {
        workshop_1: 0,
        workshop_2: 0,
        workshop_3: 0,
        workshop_4: 0,
...
let idleGame = {
  ...
  images: {
    st_patricks_event: {
      idleCurrency:
        '/shared/seasonalevents/stpatricks/event/stpatrick_task_idle_currency_thumb.png',
      workshop_1:
        '/shared/seasonalevents/stpatricks/event/stpatrick_task_goods_hats_thumb.png',
      workshop_2:
        '/shared/seasonalevents/stpatricks/event/stpatrick_task_goods_flowers_thumb.png',
      workshop_3:
        '/shared/seasonalevents/stpatricks/event/stpatrick_task_goods_cake_thumb.png',
      workshop_4:
        '/shared/seasonalevents/stpatricks/event/stpatrick_task_goods_drinks_thumb.png',
      workshop_5:
        '/shared/seasonalevents/stpatricks/event/stpatrick_task_goods_fireworks_thumb.png',
      transport_1:
        '/shared/seasonalevents/stpatricks/event/stpatrick_task_shipyard_thumb.png',
      market_1:
        '/shared/seasonalevents/stpatricks/event/stpatrick_task_parade_thumb.png',
    },
    fellowship_event: {
      idleCurrency:
        '/shared/seasonalevents/fellowship/event/fellowship_task_idle_currency_thumb.png',
      workshop_1:
        '/shared/seasonalevents/fellowship/event/fellowship_task_goods_spices_thumb.png',
      workshop_2:
        '/shared/seasonalevents/fellowship/event/fellowship_task_goods_drinks_thumb.png',
      workshop_3:
        '/shared/seasonalevents/fellowship/event/fellowship_task_goods_farm_thumb.png',
      workshop_4:
        '/shared/seasonalevents/fellowship/event/fellowship_task_goods_bakery_thumb.png',
      workshop_5:
        '/shared/seasonalevents/fellowship/event/fellowship_task_goods_butchery_thumb.png',
      transport_1:
        '/shared/seasonalevents/fellowship/event/fellowship_task_carriage_thumb.png',
      market_1:
        '/shared/seasonalevents/fellowship/event/fellowship_task_banquette_thumb.png',
    },
  },
  texts: {
    st_patricks_event: {
  ...
  settings: JSON.parse(
    localStorage.getItem('idleGameSettings') ||
      '{"hiddenTables":[],"minimized":false,"Strategy":{},"targets":{"workshop_1": 0, "workshop_2": 0, "workshop_3": 0, "workshop_4": 0, "workshop_5": 0, "transport_1": 0, "market_1": 0}}',
  ),
  ...
  selectEventData: () => {
    ...
    idleGame.finishTownDiscount =
      1 - data.nextStageCostReductionPercentage / 100;
  },
  ...
  updateTarget: (event) => {
    ...
    if (event.key === 'Enter') {
      idleGame.settings.targets[event.srcElement.dataset.station] = Math.max(
        Math.floor(Math.min(Number(event.srcElement.value), 999) || 0, 0),
      );
      idleGame.saveSettings();
    }
  ...
  idleGameUpdateDialog: () => {
    ...
    for (let b in idleGame.data) {
      ...
      if (
        idleGame.data[b].degree > degree &&
        idleGame.data[b].type === 'work'
      ) {
    ...
    for (let b in idleGame.data) {
      ...
      if (idleGame.data[b].type === 'work') {
        sum +=
          Math.pow(1000, idleGame.data[b].degree - degree) *
          idleGame.data[b].production;
    ...
    for (let x in idleGame.data) {
      ...
      $('#idleGame_' + x + 'Level').text(
        `${idleGame.data[x].level} → ${idleGame.data[x].next}`,
      );
      $('#idleGame_' + x).text(
        `${idleGame.bigNum(idleGame.data[x].need)}${idleGame.iGNums[idleGame.data[x].ndegree]}`,
      );
      $('#idleGame_' + x + 'Time').html(
        `${idleGame.time(idleGame.data[x].need, idleGame.data[x].ndegree, sum, degree, 0, 0, fest, festd)}`,
      );
      $('#idleGame_' + x).attr(
        'data-original-title',
        `${idleGame.bigNum(idleGame.data[x].need)} ${idleGame.iGNumTitles[idleGame.data[x].ndegree]}`,
      );
    }

    $('#idleGame_Work').text(
      `${idleGame.bigNum(work)} ${idleGame.iGNums[workd]}`,
    );
    $('#idleGame_Work').attr(
      'data-original-title',
      `${idleGame.bigNum(work)} ${idleGame.iGNumTitles[workd]}<br>${worktitle}`,
    );
    $('#idleGame_Ship').text(
      `${idleGame.bigNum(ship)} ${idleGame.iGNums[shipd]}`,
    );
    $('#idleGame_Ship').attr(
      'data-original-title',
      `${idleGame.bigNum(ship)} ${idleGame.iGNumTitles[shipd]}`,
    );
    $('#idleGame_Fest').text(
      `${idleGame.bigNum(fest)} ${idleGame.iGNums[festd]}`,
    );
    $('#idleGame_Fest').attr(
      'data-original-title',
      `${idleGame.bigNum(fest)} ${idleGame.iGNumTitles[festd]}`,
    );
    ...
    for (let t = 0; t < 3; t++) {
      ...
      if (
        target === 'transport_1' &&
        targetProduction * Math.pow(1000, targetDegree - workd) > work
      ) {
    ...
    let discounted_time = idleGame.time(
      discounted,
      Td,
      sum,
      degree,
    ...
    $('.idleGame_Town').html(
      `<span data-original-title="${text_currentrun}">${text_currentrun_short}</span> &middot; ${text_nexttown}`,
    );
  ...
  Production: (building) => {
    ...
    let t =
      building.baseData.productionDuration + building.baseData.rechargeDuration;
    let pbonus = 0;
    let tbonus = 0;
    ...
    for (let rank in building.baseData.rankProductionLevels) {
      if (
        !Object.hasOwnProperty.call(
          building.baseData.rankProductionLevels,
          rank,
        )
      )
        continue;
  ...
  time: (amount, da, hourly, dh, stock, ds, fest, df) => {
    ...
    time +=
      t0.h < 24 ?
        ` <img class="clickable" data-original-title="${i18n('Boxes.idleGame.SetTimer')}" src="${srcLinks.get('/shared/gui/plus_offer/plus_offer_time.png', true)}" alt="" onclick="idleGame.addAlert(${t0.h},${t0.m})">`
      : ``;
    return time;
  },
  ...
  bigNum: (number) => {
    bigNum =
      Number(number.toPrecision(3)) >= 1000 ?
        `${Math.floor(number)}`
  ...
  StratUndo: () => {
    ...
    for (
      strat = 0;
      strat <
      idleGame.settings.Strategy[idleGame.event][idleGame.Variant].length;
      strat++
    ) {
      if (
        idleGame.settings.Strategy[idleGame.event][idleGame.Variant][strat]
          .check == false
      )
        break;
    }
    ...
    if (
      ...
      $('#idleGame_StratCheck').html('check');
    strat--;
    idleGame.settings.Strategy[idleGame.event][idleGame.Variant][strat].check =
      false;
          ...
          if (type == 'M' || type == 'L') {
            ...
            switch (building) {
              case 'F':
                ...
                break;
              case 'T':
                building = 'transport_1';
                break;
              default:
                building = 'workshop_' + building;
            }
  ...
  DisplayStrat: (strat) => {
    if (strat - 1 >= 0) {
      $('#idleGame_StratPrev').html(
        idleGame.settings.Strategy[idleGame.event][idleGame.Variant][strat - 1]
          .text,
      );
  ...
  modifyStrategy: () => {
    let list = idleGame.settings.Strategy[idleGame.event][idleGame.Variant]
      .map(
        (x) =>
          x.text +
  ...
  saveStrategy: () => {
    ...
    idleGame.settings.Strategy[idleGame.event][idleGame.Variant] = lines.map(
      (x) => {
  ...
  addAlert: (hours, minutes) => {
    ...
    MainParser.sendExtMessage({
      ...
      HTML.ShowToastMsg({
        head: 'Idle Game',
        text: HTML.i18nReplacer(i18n('Boxes.idleGame.AlertSetText'), {
          minutes: minutes,
          hours: hours,
        }),
        type: 'success',
