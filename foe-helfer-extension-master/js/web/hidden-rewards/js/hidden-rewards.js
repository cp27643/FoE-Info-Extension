...
FoEproxy.addHandler('HiddenRewardService', 'getOverview', (data, postData) => {
  ...
  HiddenRewards.Cache = HiddenRewards.prepareData(
    data.responseData.hiddenRewards,
  );

  HiddenRewards.GEprogress = JSON.parse(
    localStorage.getItem('HiddenRewards.GEprogress') || '0',
  );

  HiddenRewards.RefreshGui(fromHandler);
  if (HiddenRewards.FirstCycle) {
    //Timer setzen
    ...
    data.responseData.hiddenRewards.forEach((x) => {
      if (x.startTime && x.startTime > GameTime.get())
        setTimeout(
          ...
          (x.startTime + 5 - GameTime.get()) * 1000,
        );
    });
  }
});
...
FoEproxy.addHandler(
  ...
  'getOverview',
  (data, postData) => {
    HiddenRewards.GEprogress =
      data?.responseData?.progress?.currentEntityId || 0;
    localStorage.setItem(
      'HiddenRewards.GEprogress',
      JSON.stringify(HiddenRewards.GEprogress),
    );
    HiddenRewards.RefreshGui();
  },
);
...
let HiddenRewards = {
  ...
  RefreshGui: (fromHandler = false) => {
    ...
    for (let i = 0; i < HiddenRewards.Cache.length; i++) {
      ...
      if (
        StartTime > MainParser.getCurrentDateTime() ||
        EndTime < MainParser.getCurrentDateTime()
      )
        continue;
      if (
        HiddenRewards.Cache[i].isGE &&
        !(
          HiddenRewards.GElookup[HiddenRewards.Cache[i].positionGE] <=
          Math.floor((HiddenRewards.GEprogress % 32) / 8)
        )
      ) {
    ...
    if ($('#HiddenRewardBox').length >= 1) {
      if (
        fromHandler &&
        HiddenRewards.FilteredCache.length === 0 &&
        $('#HiddenRewardBox').length
      ) {
      ...
      [
        ...new Set(
          Object.values(MainParser.CityMapData)
            .filter((x) => x.type == 'street')
            .map((x) => x.cityentity_id),
        ),
      ].filter(
        (x) =>
          MainParser.CityEntities[x].requirements.street_connection_level == 2,
      ).length;
    let warning =
      HiddenRewards.FilteredCache.filter((x) => x.twolane).length > 0 &&
      !twolane;
    if (warning) {
      h.push(
        `<div class="dark-bg"><div class="warning">${i18n('Boxes.HiddenRewards.twolaneWarning')}</div></div>`,
      );
    }
    ...
    if (HiddenRewards.FilteredCache.length > 0) {
      for (let idx in HiddenRewards.FilteredCache) {
        ...
        if (hiddenReward.type.indexOf('outpost') > -1) {
          img =
            'Shard_' +
            hiddenReward.type.substr(hiddenReward.type.length - 2, 2);
        }
        h.push(
          '<td class="incident" title="' +
            HTML.i18nTooltip(hiddenReward.type) +
            '"><img src="' +
            extUrl +
            'js/web/hidden-rewards/images/' +
            img +
            '.png" alt=""></td>',
        );
        ...
        h.push(
          '<td class="">' +
            i18n('Boxes.HiddenRewards.Disappears') +
            ' ' +
            moment.unix(hiddenReward.expires).fromNow() +
            '</td>',
        );
        h.push('</tr>');
      }
      ...
      h.push(
        '<td colspan="3">' + i18n('Boxes.HiddenRewards.NoEvents') + '</td>',
      );
    }
  ...
  ShowSettingsButton: () => {
    ...
    let h = [];
    h.push(
      `<p class="text-center"><label for="countrelics">${i18n('Settings.CountRelics')}<label><br>`,
    );
    h.push(
      `<select oninput="HiddenRewards.SaveSettings(this.value)"/><option value="0" ${CountRelics == 0 ? 'selected="selected"' : ''}>${i18n('Boxes.HiddenRewards.CountAll')} </option><option value="1" ${CountRelics == 1 ? 'selected="selected"' : ''}>${i18n('Boxes.HiddenRewards.onlyVis')} </option><option value="2" ${CountRelics == 2 ? 'selected="selected"' : ''}>${i18n('Boxes.HiddenRewards.none')} </option></p>`,
    );
