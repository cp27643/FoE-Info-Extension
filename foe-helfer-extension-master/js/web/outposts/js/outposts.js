...
let Outposts = {
  ...
  PlannedTiles: JSON.parse(
    localStorage.getItem('Outposts_PlannedTiles') || '{}',
  ),
  ...
  BuildInfoBox: () => {
    if ($('#outpostConsumables').length === 0) {
      ...
      const window = /** @type {HTMLElement} */ (
        document.getElementById('outpostConsumables')
      );
      window.addEventListener('change', (event) => {
        ...
        if (
          ...
          target.type === 'radio' &&
          target.checked &&
          target.name.startsWith(namePrefix) &&
          ...
          localStorage.setItem(
            'Outposts_PlannedTiles',
            JSON.stringify(Outposts.PlannedTiles),
          );
    ...
    {
      let oldPlannedFormat = Object.values(Outposts.PlannedTiles).find(
        (planned) => typeof planned === 'string',
      );
      if (oldPlannedFormat) {
        // @ts-ignore
        Outposts.PlannedTiles = {
          [Outposts.OutpostData.content]: Outposts.PlannedTiles,
        };
      }
    }
  ...
  BuildInfoBoxContent: () => {
    ...
    const goodProductionResourceId =
      primaryResourceId === 'deben' ? 'egyptians_loot' : primaryResourceId;
    const resourceIDs = [
      ...OutpostData.goodsResourceIds,
      'diplomacy',
      goodProductionResourceId,
    ];
    ...
    const currentRun = {
      ...
      productionBonusProbability:
        OutpostData.completedPlaythroughs === undefined ?
          OutpostData.playthroughs[0].productionBonusProbability
        : OutpostData.completedPlaythroughs < OutpostData.playthroughs.length ?
          OutpostData.playthroughs[OutpostData.completedPlaythroughs]
            .productionBonusProbability
        : OutpostData.playthroughs[OutpostData.playthroughs.length - 1]
            .productionBonusProbability,
    };

    // Diplomatische Gebäude raussuchen, die erforscht sind
    /** @type {{name: string, diplomacy: number}[]}} */
    const UnlockedDiplomacyBuildings = advancements
      .filter(
        (building) =>
          building.isUnlocked &&
          building.rewards[0].toLocaleLowerCase().indexOf('diplomacy') > -1,
      )
      .map((building) => {
      ...
      .reverse();
    /** @type {FoE_JSON_Goods} */
    const currStock = Object.fromEntries(
      resourceIDs.map((id) => [id, ResourceStock[id] || 0]),
    );
    ...
    let tileSelectablePrices =
      Outposts.CityMap ?
        ...
        (
          Outposts.CityMap.tilesets.filter(
            (t) => t.available && t.type === 'selectablePrice',
          )
        )
      : undefined;
    ...
    const nextTilesCosts =
      tileSelectablePrices ?
        tileSelectablePrices.map((tile) => [
          tile.id,
          tile.requirements.cost.resources,
        ])
      : null;
    /** @type {number} */
      ...
      (acc, building) => {
        ...
        if (production.__class__ === 'CityEntityProductionProduct') {
          ...
          if (amount != null) {
            return (
              acc +
              (amount / production.production_time) * (60 * 60 * 5) /* 5h */
            );
          }
        } else if (
          production.__class__ === 'CityEntityResourcesWithRequirementsProduct'
        ) {
    ...
    $('#outpostConsumablesHeader > .title').text(
      i18n('Boxes.Outpost.TitleShort') + OutpostData.contentName,
    );
    ...
    // summen checkbox
    t.push(
      '<p class="info-line"><span><label>' +
        i18n('Boxes.Outpost.ShowSums') +
        '<input type="checkbox" onclick="Outposts.asSum(this.checked)"' +
        (displaySums ? ' checked' : '') +
        '/></label></span><span>',
    );
    ...
    if (currentRun) {
      t.push(
        HTML.i18nReplacer(i18n('Boxes.Outpost.infoLine'), {
          ...
          chanceX4: MainParser.round(
            currentRun.productionBonusProbability * 100,
          ),
        }),
      );
    }
    ...
    t.push(
      '</span><span><strong>' +
        GoodsData[primaryResourceId].name +
        ': ' +
        HTML.Format(ResourceStock[primaryResourceId] || 0) +
        '</strong> (+ ' +
        (current5HProductionRate > 0 ?
          HTML.Format(MainParser.round(current5HProductionRate))
        : '???') +
        `/5h)` +
        '</span>',
    );
    ...
    if (nextTilesCosts) {
      ...
      for (let [tileID, tileCost] of nextTilesCosts) {
        ...
        if (i === 0) {
          t.push(
            '<td>' +
              '<input type="checkbox" onclick="Outposts.listAllTiles(this.checked)"' +
              (displayAllTiles ? ' checked' : '') +
              '/>' +
          ...
          if (resourceID === 'diplomacy' && displayAllTiles) {
            t.push(
              '<td class="text-center">' +
                '<label><input type="radio" value="#off" name="foe_helper_' +
                tileID +
                '" ' +
                (plannedTiles[tileID] == null ? ' checked' : '') +
                '/><span class="outpost_tile_off">' +
                i18n('Boxes.Outpost.tileNotPlanned') +
                '</span></label>' +
                '</td>',
            );
          } else {
    ...
    t.push(
      '<th class="text-center">' + i18n('Boxes.Outpost.TitleFree') + '</th>',
    );
    ...
    for (let resourceID of resourceIDs) {
      ...
      if (
        ...
        IconID = 'fine_' + IconID;
      t.push(
        `<th class="text-center"><span class="goods-sprite sprite-50 ${IconID} goods-name" title="${GoodsData[resourceID].name}"></span></th>`,
      );
    }
    ...
    for (let advancement of advancements) {
      ...
      for (let resourceID of resourceIDs) {
        let resourceCost = 0;
        if (resourceID !== goodProductionResourceId) {
          //Normale Resourcen
          resourceCost = cost[resourceID];
        } else {
          //Goldmünzen bzw. Beute => abhängig von anderen Güterkosten
          for (let CostResourceName in sums) {
            if (
              CostResourceName === 'diplomacy' ||
              CostResourceName === goodProductionResourceId
            )
              continue;

            resourceCost += Math.max(
              Math.ceil(
                (sums[CostResourceName] -
                  (ResourceStock[CostResourceName] | 0)) /
                  5,
              ) * (goodProductionResourceId === 'egyptians_loot' ? 50 : 1000),
              0,
            );
          }
        }
        ...
        if (unlocked) {
          // bereits erforscht
          t.push(
            '<span class="text-muted">' + HTML.Format(resourceCost) + '</span>',
          );
          t.push('</td>');
          continue;
        }
        ...
        const displayVal = HTML.Format(
          (
            displaySums &&
              resourceID !== 'diplomacy' &&
              resourceID !== goodProductionResourceId
          ) ?
            resourceSumAfter
          : resourceCost,
        );
        ...
        if (resourceInStock >= resourceSumAfter) {
          ...
          t.push(
            displayVal +
              ...
              HTML.Format(resourceInStock - resourceSumAfter) +
              '</small>',
          );
        }
        ...
        if (resourceID === 'diplomacy') {
          ...
          if (rest > 0) {
            ...
            t.push(
              '<span class="diplomacy-ask">?<span class="diplomacy-tip">' +
                content.join('<br>') +
                '</span></span>',
            );
          }
    ...
    if (nextTilesCosts) {
      ...
      if (found) {
        ...
        t.push(
          '<td><strong>' +
            i18n('Boxes.Outpost.ExpansionsSum') +
            '</strong></td><td></td>',
        );
        ...
        for (let resourceID of resourceIDs) {
          ...
          if (resourceCost > 0) {
            ...
            const resourceSumBefore = sums[resourceID];
            const resourceSumAfter =
              resourceID === 'diplomacy' ? resourceCost : (
                resourceSumBefore + resourceCost
              );
            sums[resourceID] = resourceSumAfter;

            const displayVal = HTML.Format(
              displaySums ? resourceSumAfter : resourceCost,
            );
    ...
    for (let resourceID of resourceIDs) {
      t.push(
        '<td class="text-center">' + HTML.Format(sums[resourceID]) + '</td>',
      );
    }
    ...
    for (let resourceID of resourceIDs) {
      t.push(
        '<td class="text-center">' +
          HTML.Format(currStock[resourceID]) +
          '</td>',
      );
    }
    ...
    t.push(
      '<td><strong>' +
        i18n('Boxes.Outpost.DescStillMissing') +
        '</strong></td><td colspan=""></td>',
    );
    ...
    for (let resourceID of resourceIDs) {
      ...
      if (currentRun) {
        difference2 =
          resourceID !== goodProductionResourceId ?
            Math.floor(
              difference / (1 + 3 * currentRun.productionBonusProbability),
            )
          : currStock[resourceID] -
            Math.floor(
              sums[resourceID] /
                (1 + 3 * currentRun.productionBonusProbability),
            );
      }
      t.push(
        '<td class="text-center text-' +
          (difference < 0 ? 'danger' : 'success') +
          (resourceID !== 'diplomacy' && difference < 0 && difference2 != null ?
            '" title="' +
            HTML.Format(difference2) +
            ' ' +
            i18n('Boxes.Outpost.including4x')
          : '') +
          '">' +
          HTML.Format(difference) +
          '</td>',
      );
    }
    ...
    t.push(
      `<button class="btn" onclick="Outposts.SubmitData()">${i18n('Boxes.CityMap.OutpostSubmit')}</button>`,
    );
    t.push('</td>');
    t.push('</tr>');
  ...
  UpdateOutpostData: () => {
    ...
    let currentOutpost = outposts.find(
      (outpost) =>
        outpost.startedAt !== undefined && outpost.startedAt > LastStartedTime,
    );
...
FoEproxy.addHandler(
  'OutpostService',
  'getAll',
  (/** @type {FoE_NETWORK_OutpostService_getAll} */ data, _postData) => {
...
FoEproxy.addHandler(
  'OutpostService',
  'start',
  (/** @type {FoE_NETWORK_OutpostService_start} */ data, _postData) => {
...
FoEproxy.addHandler(
  'AdvancementService',
  'getAll',
  (/** @type {FoE_NETWORK_AdvancementService_getAll} */ data, _postData) => {
...
FoEproxy.addHandler(
  'AdvancementService',
  'unlock',
  (/** @type {FoE_NETWORK_AdvancementService_unlock} */ data, postData) => {
    if (postData instanceof Array) {
      postData = postData.find(
        (request) =>
          request.requestClass === 'AdvancementService' &&
          request.requestMethod === 'unlock',
      );
    }
    if (
      postData &&
      data.responseData.__class__ === 'Success' &&
      Outposts.Advancements
    ) {
      let advancement = Outposts.Advancements.find(
        (advancement) => !advancement.isUnlocked,
      );
      if (advancement) {
...
FoEproxy.addHandler(
  ...
  'startProduction',
  (
    /** @type {FoE_NETWORK_CityProductionService_startProduction} */ data,
    _postData,
  ) => {
...
FoEproxy.addHandler(
  'CityMapService',
  'getCityMap',
  (/** @type {FoE_NETWORK_CityMapService_getCityMap} */ data, _postData) => {
...
FoEproxy.addHandler(
  'CityMapService',
  'placeExpansion',
  (/** @type {FoE_NETWORK_CityMapService_placeExpansion} */ data, postData) => {
    ...
    if (city) {
      ...
      if (postData instanceof Array) {
        ...
        postData = postData.find(
          (post) =>
            post.requestClass === 'CityMapService' &&
            post.requestMethod === 'placeExpansion',
        );
      }
      if (postData) {
...
FoEproxy.addHandler(
  'CityMapService',
  'removeBuilding',
  (/** @type {FoE_NETWORK_CityMapService_removeBuilding} */ data, postData) => {
    const city = Outposts.CityMap;
    if (city) {
      ...
      if (postData instanceof Array) {
        ...
        postData = postData.find(
          (post) =>
            post.requestClass === 'CityMapService' &&
            post.requestMethod === 'removeBuilding',
        );
      }
      if (postData) {
        postData.requestData.forEach((removedID) => {
          const idx = entities.findIndex(
            (tile) => (tile.id || 0) === (removedID || 0),
          );
          if (idx >= 0) {
