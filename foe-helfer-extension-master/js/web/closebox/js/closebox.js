...
let CloseBox = {
  Excludes: {
    ...
    LiveGildFighting: {
      name: i18n('Menu.Gildfight.Title'),
      inj: 'guildfights',
    },
  },
  ...
  InitSettings: () => {
    ...
    for (const k in Settings) {
      if (!Settings.hasOwnProperty(k) || !CloseBox.Settings.hasOwnProperty(k)) {
        continue;
      }
  ...
  Show: () => {
    ...
    let align =
      CloseBox.Settings.BoxAlignment === 1 ? ' vertical' : ' horizontal';

    if (CloseBox.Settings.HideAllButton) {
      h.push(
        `<button style="width: ${bs}px; height: ${bs}px; font-size: ${Math.round(bs / 2)}px;" id="cb-hide-all-windows" class="btn btn-green closebox${align}">&#128065</button>`,
      );
    }

    if (CloseBox.Settings.CloseAllButton) {
      h.push(
        `<button style="width: ${bs}px; height: ${bs}px; font-size: ${Math.round(bs / 2)}px;" id="cb-close-all-windows" class="btn closebox${align}">X</button>`,
      );
    }
  ...
  ShowAllBoxes: () => {
    $('#cb-hide-all-windows')
      .removeClass('invisible btn-delete')
      .addClass('btn-green');
  ...
  CloseAllBoxes: () => {
    ...
    $.each(openBoxes, function () {
      let box = $(this);
      if (
        box.attr('id') !== 'CloseBox' &&
        box.attr('id') !== 'TooltipContainer' &&
        !CloseBox.Settings.Excludes.includes(box.attr('id'))
      ) {
        box.remove();
      }
    });
  ...
  CloseBoxSettings: () => {
    ...
    c.push(
      `<p class="text-left"><span class="settingtitle">${i18n('Boxes.CloseBox.View')}</span>`,
    );
    c.push(
      `<input id="cb_close_all_button" name="closeallbutton" value="1" type="checkbox" ${Settings.CloseAllButton ? ' checked="checked"' : ''} /> <label for="cb_close_all_button"><i>${i18n('Boxes.CloseBox.CloseAllButton')}</i></label></p>`,
    );
    c.push(
      `<input id="cb_hide_all_button" name="hideallbutton" value="1" type="checkbox" ${Settings.HideAllButton ? ' checked="checked"' : ''} /> <label for="cb_hide_all_button"><i>${i18n('Boxes.CloseBox.HideAllButton')}</i></label></p>`,
    );
    c.push(
      `<p>${i18n('Boxes.CloseBox.ButtonSize')} <select id="cb_buttonsize" name="buttonsize">`,
    );

    for (let k in ButtonSizes) {
      c.push(
        `<option value="${ButtonSizes[k]}" ${Settings.ButtonSize === ButtonSizes[k] ? ' selected="selected"' : ''}>${k * 1 + 1}</option>`,
      );
    }

    c.push(`</select></p>`);
    c.push(
      `<p>${i18n('Boxes.CloseBox.BoxAlignment')} <select id="cb_boxalignment" name="boxalignment">`,
    );
    c.push(
      `<option value="0" ${Settings.BoxAlignment === 0 ? ' selected="selected"' : ''}>${i18n('Boxes.CloseBox.Horizontal')}</option>`,
    );
    c.push(
      `<option value="1" ${Settings.BoxAlignment === 1 ? ' selected="selected"' : ''}>${i18n('Boxes.CloseBox.Vertical')}</option>`,
    );

    c.push(`</select></p>`);
    c.push(
      `<p class="text-left"><span class="settingtitle">${i18n('Boxes.CloseBox.ExcludeFromClosing')}</span>`,
    );

    for (let k in Excludes) {
      if (!Excludes.hasOwnProperty(k)) {
        continue;
      }

      c.push(
        `<input id="cb_exc_${k}" class="cb_exludes" name="excludes[]" value="${k}" type="checkbox" ${Settings.Excludes.includes(k) ? ' checked="checked"' : ''} /> <label for="cb_exc_${k}"><i>${Excludes[k].name}</i></label><br />`,
      );
    }
    c.push(
      `<p class="text-left"><span class="settingtitle">${i18n('Boxes.CloseBox.Automation')}</span>`,
    );
    c.push(
      `<input id="cb_auto_hide_on_battle" name="autohideonbattle" value="1" type="checkbox" ${Settings.AutoHideOnBattle ? ' checked="checked"' : ''} /> <label for="cb_auto_hide_on_battle"><i>${i18n('Boxes.CloseBox.AutoHideOnBattle')}</i></label></p>`,
    );
    c.push(
      `<hr><p><button id="save-GexStat-settings" class="btn" style="width:100%" onclick="CloseBox.SaveSettings()">${i18n('Boxes.General.Save')}</button></p>`,
    );
  ...
  SaveSettings: () => {
    ...
    CloseBox.Settings.CloseAllButton = !!$('#cb_close_all_button').is(
      ':checked',
    );
    CloseBox.Settings.BoxAlignment = parseInt($('#cb_boxalignment').val());
    CloseBox.Settings.AutoHideOnBattle = !!$('#cb_auto_hide_on_battle').is(
      ':checked',
    );
