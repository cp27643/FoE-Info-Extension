...
let srcLinks = {
  ...
  get: (filename, full = false, noerror = false) => {
    ...
    if (!srcLinks.FileList) {
      ...
      if (!CS) {
        ...
        filenameP[1] = 'png';
        CS =
          srcLinks.FileList['/city/gui/citymap_icons/antiquedealer_flag.png'];
      }
    }
  ...
  getReward: (icon) => {
    ...
    if (icon.substring(1, 2) === '_') {
      url = srcLinks.get(
        `/city/buildings/${MainParser.CityEntities?.[icon]?.asset_id?.replace(/(\D*?)_(.*)/, '$1_SS_$2')}.png`,
        true,
      );
    } else if (url == '' || url.indexOf('antiquedealer_flag') > -1)
      url = srcLinks.get(
        `/shared/unit_portraits/armyuniticons_90x90/armyuniticons_90x90_${icon}.jpg`,
        true,
        true,
      ); // does not work :(
    ...
    if (url.indexOf('antiquedealer_flag') > -1)
      url = srcLinks.get(
        `/shared/icons/reward_icons/reward_icon_${icon}.png`,
        true,
        true,
      );
    if (url.indexOf('antiquedealer_flag') > -1)
      url = srcLinks.get(
        `/city/buildings/${icon?.replace(/(\D*?)_(.*)/, '$1_SS_$2')}.png`,
        true,
      );
  ...
  getQuest: (icon) => {
    let url1 = srcLinks.get(
      `/shared/icons/quest_icons/${icon}.png`,
      true,
      true,
    );
  ...
  icons: (x) => {
    ...
    if (link.includes('antiquedealer_flag'))
      link = srcLinks.get(`/shared/icons/goods/icon_fine_${x}.png`, true, true);
    if (link.includes('antiquedealer_flag'))
      link = srcLinks.get(
    ...
    if (link.includes('antiquedealer_flag'))
      link = srcLinks.get(
        `/city/buildings/${MainParser.CityEntities?.[x]?.asset_id?.replace(/(\D*?)_(.*)/, '$1_SS_$2')}.png`,
        true,
      );
