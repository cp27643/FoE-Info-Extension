...
let Alerts = (function () {
  ...
  tmp.data = {
    ...
    next: () => {
      ...
      if (now < tmp.data.options.timestamp.next) {
        return null;
      }

      if (!tmp.data.options.timestamp.next) {
        tmp.data.options.timestamp.next = now;
      }
      let n = tmp.data.options.timestamp.next;
      tmp.data.options.timestamp.next =
        now + tmp.data.options.timestamp.increment;
    ...
    refresh: async () => {
      ...
      for (let alert of alerts) {
        ...
        const newExpire = tmp.repeat.nextExpiration(
          alert.data.expires,
          alert.data.repeat,
          timestamp,
        );
  ...
  (tmp.model = {
    ...
    (tmp.repeat = {
      ...
      update: (alert, timestamp) => {
        ...
        if (repeat > -1) {
          let changes = {
            expires: tmp.repeat.nextExpiration(
              alert.expires,
              repeat,
              timestamp,
            ),
      ...
      body: {
        build: () => {
          let labels = {
            ...
            preferences:
              '<span>' + i18n('Boxes.Alerts.Tabs.Preferences') + '</span>',
          };
          ...
          tmp.web.body.tabs.addHead(
            'alerts-tab-preferences',
            labels.preferences,
          );

          tmp.web.body.tabs.addContent(
            'alerts-tab-list',
            tmp.web.body.tabs.tabListContent(),
          );
          tmp.web.body.tabs.addContent(
            'alerts-tab-preferences',
            tmp.web.body.tabs.tabPreferencesContent(),
          );
          ...
          $('#AlertsBody')
            ...
            .done(function () {
              ...
              $('#AlertsBody')
                .find('span.button-alert-popup-new')
                .on('click', function () {
                  tmp.web.popup.type.create.show();
                });

              $('#AlertsBody')
                .find('span.button-alert-create-all-sectors')
                .on('click', function () {
                  let reset = confirm(
                    i18n('Boxes.Alerts.Form.ConfirmSectorAlerts'),
                  );
                  if (reset)
          ...
          tabPreferencesContent: () => {
            ...
            for (const [key, entry] of entries) {
              ...
              if (entry.value === true || entry.value === false) {
                let checked_active =
                  entry.value === true ? 'checked="checked"' : '';
                let checked_inactive =
                  entry.value === false ? 'checked="checked"' : '';
      ...
      forms: {
        aux: {
          ...
          textareaUpdateCounter: (textarea, counter) => {
            ...
            tmp.web.forms.aux.textareaCounter.text(
        ...
        actions: {
          createSectors: () => {
            ...
            if (tmp.model.battlegrounds.provinces) {
              tmp.model.battlegrounds.provinces.forEach(
                function (province, id) {
                  let expires =
                    (province['lockedUntil'] -
                      tmp.preferences.data.early.value) *
                    1000;
                  if (!isNaN(expires)) {
                    let alert = {
                      id: null,
        ...
        data: () => {
          return {
            ...
            persistent:
              $('input[name=alert-persistent]:checked', '#alert-form').val() ===
              'on',
          };
        },
        ...
        render: (data) => {
          ...
          if (!timestamp) {
            ...
            labels.presets.now = data.alert.title;
          }
          ...
          if (tmp.model.battlegrounds.provinces) {
            tmp.model.battlegrounds.provinces.forEach(function (province, id) {
              let value =
                (province['lockedUntil'] - tmp.preferences.data.early.value) *
                1000;
            ...
            setActions: (parent) => {
              ...
              $(parent)
                ...
                .on('propertychange input', function () {
                  tmp.web.forms.aux.textareaUpdateCounter(
                    '#alert-body',
                    '#alert-body-counter',
                  );
                });
              ...
              $(parent)
                .find('span.button-discard-alert')
                .on('click', function () {
                  tmp.web.popup.type.edit.close();
                });

              $(parent)
                .find('span.button-save-alert')
                .on('click', function () {
              ...
              $(parent)
                .find('span.button-preview-alert')
                .on('click', function () {
                  tmp.web.forms.actions.previewNew();
                });

              $(parent)
                .find('span.datetime-preset')
                .on('click', function () {
        ...
        battlegrounds: (responseData) => {
          if (responseData && responseData.map && responseData.map.provinces) {
            ...
            if (responseData.battlegroundParticipants) {
              responseData.battlegroundParticipants.forEach(
            ...
            provinces.forEach(function (province, id) {
              ...
              if (
                participants[province.ownerId] &&
                participants[province.ownerId].clan
              ) {
              ...
              if (item && item.type) {
                switch (item.type) {
                  case 'antiquesExchange': {
                    tmp.model.antique.exchange =
                      (item.time - tmp.preferences.data.early.value) * 1000;
                    break;
                  }
                  case 'antiquesAuction': {
                    tmp.model.antique.auction =
                      (item.time - tmp.preferences.data.early.value) * 1000;
                    break;
                  }
                  case 'antiquesAuctionCooldown': {
                    tmp.model.antique.cooldown =
                      (item.time - tmp.preferences.data.early.value) * 1000;
                    break;
                  }
...
let TimeManager = (function () {
  ...
  let tmp = {
     ...
     */
    isSubscribed: (o) => {
      return tmp.observers.filter((observer) => observer === o).length === 1;
    },
    notify: (data) => {
