...
FoEproxy.addHandler(
  ...
  'getEncounter',
  (data, postData) => {
    if (postData[0]['requestClass'] !== 'GuildExpeditionService') return;
...
FoEproxy.addHandler(
  ...
  (data, postData) => {
    if (
      ...
      !postData[0]['requestData'][0]['type'] ||
      postData[0]['requestData'][0]['type'] !== 'guild_expedition'
    )
      return;
    ...
    UnitGex.DB_Data['Units'][round] =
      data['responseData']['state']['unitsOrder'];
...
let UnitGex = {
  ...
  insertIntoDB: async () => {
    ...
    await UnitGex.db['Units'].put(UnitGex.DB_Data).catch((err) => {
  ...
  buildBody: async () => {
    ...
    for (let i = 1; i <= 64; i++) {
      ...
      if (entries.length === 0) {
        ...
        const OT1 = E['Units'][0]
          .filter((e) => e['teamFlag'] === 1)
          .sort(
            (a, b) => a['initialUnitOrderIndex'] - b['initialUnitOrderIndex'],
          );
        const ET1 = E['Units'][0]
          .filter((e) => e['teamFlag'] === 2)
          .sort(
            (a, b) => a['initialUnitOrderIndex'] - b['initialUnitOrderIndex'],
          );
  ...
  PrepareUnit: (entry) => {
    let type = Unit.Types.find(
        (obj) => obj['unitTypeId'] === entry['unitTypeId'],
      ),
