...
let _menu_right = {
  ...
  BuildOverlayMenu: () => {
    let hud = $('<div />')
        .attr({ id: 'foe-helper-hud', class: 'hud-right' })
  ...
  Prepare: () => {
    ...
    _menu.HudCount = Math.floor(
      ($(window).outerHeight() - 20 - $('#foe-helper-hud').offset().top) / 48,
    );
  ...
  CheckButtons: async () => {
    ...
    if (
      jQuery
        ...
        .click.filter((elem) => elem.selector == '.hud-btn-down-active')
        .length == 0
    ) {
    ...
    if (
      jQuery
        ._data($('body').get(0), 'events')
        .click.filter((elem) => elem.selector == '.hud-btn-up-active').length ==
      0
    ) {
    ...
    $('#foe-helper-hud-slider').sortable({
      ...
      sort: function () {
        ...
        $('.is--sorting .hud-btn-down-active')
          .mouseenter(function (e) {
  ...
  ClickButtonDown: () => {
    ...
    if (
      _menu.ActiveSlide * _menu.HudHeight >
      $('#foe-helper-hud-slider').height()
    )
      _menu.MenuScrollTop =
        -($('#foe-helper-hud-slider').height() / _menu.HudHeight - 1) *
        _menu.HudHeight;
  ...
  ClickButtonUp: () => {
    ...
    if (_menu.ActiveSlide == 1) _menu.MenuScrollTop = 0;
    else _menu.MenuScrollTop += _menu.HudHeight;
