...
let Kits = {
      ...
      $('#kitsBodyTopbar')
        .append(
          $('<label />')
            ...
            .text(i18n('Boxes.Kits.FilterSets') + ':\xA0')
            .append(
              $('<input />')
                .attr({
                  class: 'game-cursor',
        ...
        .append(
          $('<label />')
            .attr({ class: 'game-cursor' })
            .text(i18n('Boxes.General.FilterItems') + ':\xA0')
            .append(
              $('<input />')
                .attr({
                  class: 'game-cursor',
  ...
  ReadSets: () => {
    ...
    if (!Kits.fragmentURL)
      Kits.fragmentURL = srcLinks.get(
        '/shared/icons/icon_tooltip_fragment.png',
        true,
      );
    ...
    for (let k in MainParser.SelectionKits) {
      ...
      const options =
        MainParser.SelectionKits[k].options ||
        MainParser.SelectionKits[k].eraOptions.BronzeAge.options;
      for (let o of options) {
        if (
          !['BuildingItemPayload', 'UpgradeKitPayload'].includes(
            o.item.__class__,
          )
        )
          continue;
        let id =
          o.item.upgradeItemId || o.item.selectionKitId || o.item.cityEntityId;
    ...
    let addItems = (set, idx) => {
      for (let r of set) {
        if (r.type === 'set') {
          ...
          const options =
            MainParser.SelectionKits[r.id].options ||
            MainParser.SelectionKits[r.id].eraOptions.BronzeAge.options;
          for (let o of options) {
            if (
              !['BuildingItemPayload', 'UpgradeKitPayload'].includes(
                o.item.__class__,
              ...
              continue;
            let id =
              o.item.upgradeItemId ||
              o.item.selectionKitId ||
              o.item.cityEntityId;
            if (!selectionKits[id]) selectionKits[id] = [];
    ...
    for (let k in MainParser.Inventory) {
      ...
      if (MainParser.Inventory[k]?.item?.reward?.type === 'set') {
        addItems(
          MainParser.Inventory[k].item.reward.rewards,
          MainParser.Inventory[k].item.reward.id,
        );
      }
    }
    ...
    for (let u of Object.values(MainParser.BuildingUpgrades)) {
      ...
      let upgradeType =
        us.includes('gold') ? 'golden'
        : us.includes('silver') ? 'silver'
        : us.includes('ascended') ? 'ascended'
        : us[0];
      let upgradeCount = JSON.parse(
        `{"${upgradeType}":${u.upgradeSteps.length - 1}}`,
      );
      for (let i = 1; i < u.upgradeSteps.length; i++) {
        for (b of u.upgradeSteps[i].buildingIds) {
          ...
          if (Kits.upgradeKits[b]) {
            buildingList = [
              ...buildingList,
              ...Kits.upgradeKits[b].buildingList,
            ];
            upgradeList = [...upgradeList, ...Kits.upgradeKits[b].upgradeList];
            upgradeCount = {
              ...upgradeCount,
              ...Kits.upgradeKits[b].upgradeCount,
            };
      ...
      for (let b of u.upgradeSteps[0].buildingIds) {
        if (sK.length > 0)
          selectionKits[b] = Array.from(
            new Set([...sK, ...(selectionKits[b] || [])]),
          );
        let i = Object.keys(Kits.upgradeKits)[
          Object.values(Kits.upgradeKits).findIndex((x) =>
            x.buildingList.includes(b),
          )
        ];
        if (i) {
          Kits.upgradeKits[i].buildingList = [
            ...Kits.upgradeKits[i].buildingList,
            ...buildingList,
          ];
          Kits.upgradeKits[i].upgradeList = [
            ...Kits.upgradeKits[i].upgradeList,
            ...upgradeList,
          ];
          Kits.upgradeKits[i].upgradeCount = {
            ...Kits.upgradeKits[i].upgradeCount,
            ...upgradeCount,
          };
        } else {
    ...
    for (let k in kits) {
      ...
      if (kits[k].buildings) {
        for (let b in kits[k].buildings) {
          ...
          if (
            kits[k].buildings[b].first &&
            Kits.upgradeKits?.[kits[k].buildings[b].first]?.upgradeList
          ) {
            Kits.upgradeKits[kits[k].buildings[b].first]?.upgradeList.forEach(
              (x) => (kits[k].buildings[b][x] = x),
            );
          }
    ...
    let create = (type, id, showMissing = true) => {
      return {
        ...
        item:
          inv[id] ||
          (type === 'first' ? entities[id]
          : type === 'asset' ? entities[id]
          : id),
        fragments: inv['fragment#' + id]?.inStock,
        reqFragments: inv['fragment#' + id]?.required,
        missing:
          (inv[id]?.inStock || 0) < 1 &&
          (inv['fragment#' + id]?.inStock || 0) < 1,
    ...
    for (let set in kits) {
      ...
      if (Name) {
        ...
        if (Name === 'Kits') {
          ...
          ChainSetIco =
            '<img src="' +
            srcLinks.get('/shared/icons/' + sName + '.png', true) +
            '" class="chain-set-ico">';
          ...
          let itemName =
            MainParser.CityEntities[kits[set].buildings[0].first].name;
        ...
        if (i18nKey === i18nTranslation)
          i18nTranslation = GroupName.replace(/_/g, ' '); //No translation => Fallback to GroupName

        KitText = i18nTranslation;
        show = true;
        if (GroupName !== 'Events') t += '</div>';
        t +=
          `<div class="group"><h1 class="grouphead" onclick="Kits.toggleGroup(event)">` +
          KitText +
          '</h1>';
      } else {
        // No name and group set => Show udate
      ...
      let eff = '';
      if (
        kits[set].buildings?.[0]?.first &&
        MainParser.CityEntities[kits[set].buildings[0].first]
      ) {
        let f = Kits.upgradeKits?.[kits[set].buildings[0].first];
        let upgradeCount = f?.upgradeCount;
        if (upgradeCount) {
          upgrades =
            '<span class="upgrades" data-original-title="' +
            i18n('Boxes.Kits.Upgrades') +
            '" data-toggle="tooltip">';
          let first = true;
          ...
          for (let i in upgradeCount) {
            ...
            upgrades +=
              (first ? '<span class="base">1</span>' : '') +
              `<span class="${i}">${upgradeCount[i]}</span>`; //title="'+i18n('Boxes.Kits.Base')+'"
            first = false;
          }
          upgrades += '</span>';
        }
        if (f?.buildingList) {
          let rating = Productions.rateBuildings(
            [kits[set].buildings[0].first, ...f?.buildingList]?.slice(-3),
            true,
          );
          let title = '';
          if (!rating) break;
          for (r of rating) {
            if (title === '') {
              ...
              title =
                `${r.building?.name || r.name}: ${Math.round(100 * r.rating.totalScore)}<br>` +
                title;
            }
          }
          let top = rating.pop();
      ...
      // Asset listing
      if (assetRow.length) {
        t += `<h3 class="assets-header ${
          !show ? 'all-missing'
          : !showA ? 'row-missing'
          : ''
        }">${i18n('Boxes.Kits.Extensions')}</h3>`;
        let rowTd = '';
        assetRow.forEach((e) => {
          rowTd += Kits.ItemDiv(e);
        });
  ...
  ItemDiv: (el) => {
    ...
    try {
      if (el.type === 'first' || el.type === 'asset') {
        url =
          '/city/buildings/' +
          [aName.slice(0, 1), '_SS', aName.slice(1)].join('') +
          '.png';
      }
    } catch (error) {
      console.error('Error processing element in ItemDiv:', error.message, {
        element: el,
        assetName: aName,
      });
    }
    ...
    if (!title) {
      if (el.type === 'update') {
        title =
          MainParser.BuildingUpgrades[item] ?
            MainParser.BuildingUpgrades[item].upgradeItem.name
          : i18n('Boxes.Kits.UpgradeKit');
      } else if (el.type === 'kit') {
        title =
          MainParser.SelectionKits[item] ?
            MainParser.SelectionKits[item].name
          : i18n('Boxes.Kits.SelectionKit');
      }
    }
  ...
  GetInventoryArray: () => {
    ...
    for (let i in MainParser.Inventory) {
      ...
      let required = null;
      let id =
        x.item.upgradeItemId ||
        x.item.selectionKitId ||
        x.item.cityEntityId ||
        x.itemAssetName;
      ...
      if (x.item.__class__ === 'FragmentItemPayload') {
        id =
          'fragment#' +
          (x.item.reward.assembledReward.type === 'building' ?
            x.item.reward.assembledReward.subType
          : x.item.reward.assembledReward.id ||
            x.item.reward.assembledReward.iconAssetName);
  ...
  ToggleView: () => {
    ...
    $('#kits-triplestate-button').text(
      i18n('Boxes.Kits.TripleStateButton' + Kits.ShowMissing),
    );
  },
  ...
  _filter: () => {
    ...
    Kits._filterItems();
    Kits._filterMissing();
    if ($('#kits-showFavourites')[0].classList.contains('btn-active'))
      $('.notFavourite').hide();
  },
  ...
  _filterSets: () => {
    const filterRegExps = $(
      '#kitsBodyTopbar input[data-type="filter-sets-text"]',
    )
      .val()
      .split('||')
  ...
  _filterItems: () => {
    const filterRegExps = $(
      '#kitsBodyTopbar input[data-type="filter-items-text"]',
    )
      .val()
      .split('||')
      .filter((it) => it.trim().length > 0)
    ...
    if (filterRegExps && filterRegExps.length >= 1) {
      ...
      allRows.each((i, e) => {
        ...
        if (itemDivs.length > 0) {
          itemDivs.each((i, e) => {
            const item = $(e);
            const show = filterRegExps.some(
              (it) => it.test(item.text()) || it.test(item.html()),
            );
            if (show) {
              visibleItemsCount++;
            } else {
  ...
  CreateUpgradeSchemes: () => {
    ...
    for (let upgrade of Object.values(MainParser.BuildingUpgrades)) {
      if (['silver_upgrade_kit_BOWL22A'].includes(upgrade.upgradeItem.id))
        continue; // faulty game data
      ...
      buildingList = buildingList
        .flat()
        .map((x) => ({ buildingId: x, upgradeId: upgradeId }));
      ...
      for (let endBuilding of finalBuildings) {
        if (startBuildings[endBuilding]) {
          ...
          startBuildings[buildingList[0].buildingId] =
            startBuildings[endBuilding];
  ...
  BuildingsFromInventory: () => {
    ...
    upgradeBuildings.push(
      ...Object.values(Kits.UpgradeSchemes)
        .map((x) => x.upgradeSteps.map((y) => y.buildingId))
        .flat(),
    );
    //Flatten Inventory
    let Inventory = {};
    let InventoryAdd = (id, amount) => {
      ...
      if (id.substring(1, 2) == '_' && !upgradeBuildings.includes(id)) {
        if (output[id]) {
          ...
          output[id] = {
            building: 'inInventory',
            amount: amount,
            chains: [
              {
                chain: [
                  { type: 'building', id: id, from: 'inventory', count: 1 },
                ],
                count: amount,
              },
            ],
    ...
    for (let i of Object.values(MainParser.Inventory)) {
      if (i.itemAssetName === 'icon_fragment') {
        if (i.item.reward.assembledReward.subType === 'selection_kit') {
          InventoryAdd(
            i.item.reward.assembledReward.id,
            Math.floor(i.inStock / i.item.reward.requiredAmount),
          );
          if (
            i.item.reward.assembledReward.iconAssetName !=
            i.item.reward.assembledReward.id
          )
            Kits.specialCases[i.item.reward.assembledReward.id] =
              i.item.reward.assembledReward.iconAssetName;
        }
        ...
        if (i.item.reward.assembledReward.type == 'building') {
          InventoryAdd(
            i.item.reward.assembledReward.subType,
            Math.floor(i.inStock / i.item.reward.requiredAmount),
          );
        }
      ...
      } else if (i.item?.reward?.type === 'set') {
        //check if this works when there is a league reward with nested sets
        InventoryAddSet(i.item.reward.rewards, i.inStock);
      }
    }
    //flatten CityBuildings
    cityBuildings = {};
    Object.values(MainParser.CityMapData).forEach(
      (x) =>
        (cityBuildings[x.cityentity_id] =
          (cityBuildings[x.cityentity_id] || 0) + 1),
    );
      ...
      if (id.substring(1, 2) == '_' && !upgradeBuildings.includes(id)) {
        for (let kit of kits) {
          ...
          if (output[id]) {
            ...
            output[id].chains.push({
              chain: [
                { type: 'selectionKit', id: kit, from: 'inventory', count: 1 },
              ],
              count: Inventory[kit],
            });
          } else
            output[id] = {
              kitsUsed: Inventory[kit],
              amount: Inventory[kit],
              chains: [
                {
                  chain: [
                    {
    ...
    for (let [buildingId, scheme] of Object.entries(Kits.UpgradeSchemes)) {
      ...
      do {
        //repeat for non-ascended version if ascended version is found
        ...
        if (ignoreAscended)
          buildingId = upgradeSteps[upgradeSteps.length - 1].buildingId;
        ...
        let SKs = Array.from(
          new Set(items.map((x) => Kits.selectionOptions[x] || []).flat()),
        );
        ...
        for (let sk of SKs) {
          for (let o of MainParser.SelectionKits[sk].options) {
            let i = upgradesIndexed.indexOf(
              o.item.cityEntityId || o.item.upgradeItemId || 'test',
            );
            if (i > -1) sKvalues[sk] += Math.pow(2, i);
            else sKvalues[sk] += 0.01;
          }
        }
        ...
        for (let i of items) {
          if (Kits.selectionOptions[i])
            SO[i] = Kits.selectionOptions[i].sort(
              (a, b) => sKvalues[a] - sKvalues[b],
            );
          if (cityBuildings[i]) {
        ...
        if (Inv[buildingId]) {
          ...
          for (let i = 0; i < Inv[buildingId]; i++) {
            chains.push([
              { type: 'building', from: 'inventory', id: buildingId },
            ]);
          }
          maxLevel = upgradeSteps.length - (ignoreAscended ? 1 : 0);
          if (
            Object.keys(upgrades).join('').includes('ascended') &&
            !ignoreAscended
          )
            ascended = true;
        }
          ...
          for (let k of Kits.selectionOptions[buildingId] || []) {
            if (Inv[k]) {
              ...
              for (let i = 0; i < Inv[k]; i++) {
                chains.push([
                  { type: 'building', from: 'selectionKit', id: k },
                ]);
              }
              maxLevel = upgradeSteps.length - (ignoreAscended ? 1 : 0);
              if (
        ...
        while (true) {
          ...
          if (level >= 0) {
            ...
            if (
              level <= upgradeSteps.length &&
              maxLevel === 0 &&
              kitCount + buildingsFromInventory > 0
            ) {
        ...
        if (amount > 0 && (buildingsFromInventory > 0 || kitCount > 0)) {
          ...
          for (let [u, a] of Object.entries(upgrades)) {
            ...
            upgradeCount[upgradeType].is =
              (upgradeCount[upgradeType].is || 0) + Math.min(a, maxLevel);
            upgradeCount[upgradeType].max =
              (upgradeCount[upgradeType].max || 0) + a;
