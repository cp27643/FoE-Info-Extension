...
FoEproxy.addHandler(
  ...
  (data, postData) => {
    ...
    if (
      data &&
      data.responseData &&
      +MainParser.getCurrentDate() - GexStat.ResponseBlockTime > 2000
    ) {
...
let GexStat = {
  ...
  UpdateData: async (source, data) => {
    ...
    switch (source) {
      case 'championship':
        ...
        let currentGuildRank =
          rankdata[ExtGuildID] && rankdata[ExtGuildID].rank ?
            rankdata[ExtGuildID].rank
          : 0;
        ...
        if ($('#GexStatBody').length) {
          GexStat.ShowTabContent(
            GexStat.CurrentStatGroup,
            GexStat.CurrentGexWeek,
          );
        }
        break;
      ...
      case 'participation':
        ...
        for (let k in data) {
          if (data.hasOwnProperty(k)) {
            ...
            partdata[k].expeditionPoints =
              data[k].expeditionPoints ? data[k].expeditionPoints : 0;
  ...
  Show: async (gexweek) => {
    ...
    if (gexweek === undefined || gexweek === null) {
      ...
      gexweek =
        GexRanking && GexRanking.gexweek ? GexRanking.gexweek : undefined;
    } else {
      GexRanking = await GexStat.db.ranking
        .where('gexweek')
        .equals(gexweek)
        .first();
    }
    ...
    if (gexweek === undefined || !GexRanking || !GexRanking.participants) {
      ...
      h.push(
        `<div class="no-data"><p>${i18n('Boxes.GexStat.ResultsNoData')}</p></div>`,
      );
    ...
    h.push(`<table id="gexsRankingTable" class="foe-table">` + `<tbody><tr>`);
    ...
    for (let x = 0; x < GexPaticipants.length; x++) {
      ...
      h.push(`<tr>`);
      h.push(
        `<td class="td-rank"><span class="winner-rank rank-${rankClass}"><span>${participant.rank}</span></span></td>`,
      );
      h.push(
        `<td>` +
      ...
      h.push(
        `<td class="progress"><div class="progbar rank-${rankClass}${stripedClass}" style="width: ${progressWidth}%"></div> ${participant.points}%</td>`,
      );
      h.push(
        `<td><div class="flex justify-content-between mbottom2"><div>${i18n('Boxes.GexStat.Rank')}: ${participant.worldrank} </div><div>${i18n('Boxes.GexStat.Member')}: ${participant.memberCount}</div></div>` +
  ...
  ShowParticipation: async (gexweek) => {
    ...
    if (gexweek === undefined || gexweek === null) {
      ...
      gexweek =
        GexParticipation && GexParticipation.gexweek ?
          GexParticipation.gexweek
        : undefined;
    } else {
      GexParticipation = await GexStat.db.participation
        .where('gexweek')
        .equals(gexweek)
        .first();
    }
    ...
    if (!GexParticipation || !GexParticipation.participation) {
      GexStat.hidePreloader('#GexStat');
      h.push(
        `<div class="no-data"><p>${i18n('Boxes.GexStat.ParticipationNoData')}</p></div>`,
      );
      $('#gexsContentWrapper').html(h.join(''));
      return;
    }
    ...
    h.push(
      `<div class="participation_overview justify-content-between"><div>${i18n('Boxes.GexStat.Points')}<br />${HTML.Format(GexParticipation.expeditionPoints)}</div>` +
    ...
    h.push(
      `<table id="gexsParticipationTable" class="foe-table">` +
    ...
    for (let x = 0; x < participation.length; x++) {
      ...
      h.push(`<tr>`);
      h.push(
        `<td class="text-center is-number" data-number="${member.rank}">${member.rank}</td>`,
      );
      h.push(
        `<td class="text-center is-number" data-number="${level}"><span class="level${encounterClass}" title="${HTML.i18nTooltip(i18n('Boxes.GexStat.Level') + ' ' + level)}"></span></td>`,
      );
      h.push(
        `<td class="case-sensitive" data-text="${helper.str.cleanup(member.name)}">` +
      ...
      h.push(
        `<td class="is-number" data-number="${member.expeditionPoints}">${HTML.Format(member.expeditionPoints)}</td>`,
      );
      h.push(`<td class="is-number" data-number="${level}">${level}</td>`);
      h.push(
        `<td class="is-number" data-number="${member.solvedEncounters || 0}">${member.solvedEncounters || 0}/${base}</td>`,
      );
      h.push(
        `<td class="is-number" data-number="${member.trial || 0}">${member.trial || 0}</td>`,
      );
  ...
  ShowCourse: async () => {
    ...
    let GexParticipation = await GexStat.db.participation
      .reverse()
      .limit(GexStat.Settings.showRoundLimit)
      .toArray();
    let GexRanking = await GexStat.db.ranking
      .reverse()
      .limit(GexStat.Settings.showRoundLimit)
      .toArray();

    if (
      (!GexParticipation || !GexParticipation.length) &&
      (!GexRanking || !GexRanking.length)
    ) {
      GexStat.hidePreloader('#GexStat');
      $('#gexsContentWrapper').html(
        `<div class="no-data"><p>${i18n('Boxes.GexStat.ParticipationNoData')}</p></div>`,
      );
      return;
    }
    ...
    let data = [
      ...[GexRanking, GexParticipation]
        .reduce(
          (m, a) => (
            a.forEach(
              (o) =>
                (m.has(o.gexweek) && Object.assign(m.get(o.gexweek), o)) ||
                m.set(o.gexweek, o),
            ),
            m
          ),
    ...
    for (let x = 0; x < data.length; x++) {
      ...
      CourseData.weeks.push(
        moment(gexweek.gexweek * 1000).format(i18n('Date')),
      );
      CourseData.pointSum +=
        gexweek.expeditionPoints !== undefined ? gexweek.expeditionPoints : 0;
      CourseData.pointsData.push(
        gexweek.expeditionPoints !== undefined ?
          gexweek.expeditionPoints
        : null,
      );
      CourseData.encounterData.push(
        gexweek.solvedEncounters !== undefined ?
          gexweek.solvedEncounters
        : null,
      );
      CourseData.allMemberData.push(
        gexweek.countMember !== undefined ? gexweek.countMember : null,
      );

      if (!gexweek.participation && !gexweek.activeMembers) {
        CourseData.activeMemberData.push(null);
        hasParticipation = false;
      }
      if (!gexweek.participants && !gexweek.currentGuildRank) {
        CourseData.rankData.push(null);
      ...
      if (!gexweek.activeMembers && hasParticipation) {
        ...
        // Update DB
        await GexStat.db.participation.update(gexweek.gexweek, {
          activeMembers: count,
        });
      ...
      if (!gexweek.currentGuildRank && hasParticipants) {
        ...
        //Update DB
        await GexStat.db.ranking.update(gexweek.gexweek, {
          currentGuildRank: rankdata[0].rank,
        });
    ...
    GexStat.Chart = new Highcharts.chart(
      'gexsContentWrapper',
      {
        title: {
        ...
        subtitle: {
          text:
            CourseData.weeks[0] +
            ' - ' +
            CourseData.weeks[CourseData.weeks.length - 1] +
            ' (' +
            CourseData.weeks.length +
            ' ' +
            i18n('Boxes.GexStat.Rounds') +
            ')',
        },
        yAxis: [
          {
            ...
            labels: {
              enabled:
                GexStat.Settings.showAxisLabel && series.yaxis.includes(0),
            },
            title: {
              enabled:
                GexStat.Settings.showAxisLabel && series.yaxis.includes(0),
          ...
          },
          {
            ...
            title: {
              enabled:
                GexStat.Settings.showAxisLabel && series.yaxis.includes(1),
              text:
                i18n('Boxes.GexStat.Member') +
                ' / ' +
                i18n('Boxes.GexStat.Participant'),
            },
            labels: {
              enabled:
                GexStat.Settings.showAxisLabel && series.yaxis.includes(1),
          ...
          },
          {
            ...
            labels: {
              enabled:
                GexStat.Settings.showAxisLabel && series.yaxis.includes(2),
            },
            title: {
              enabled:
                GexStat.Settings.showAxisLabel && series.yaxis.includes(2),
      ...
      function (chart) {
        ...
        $('#GexStat').on('resize', function () {
          GexStat.showPreloader('#GexStat');
          chart.setSize(
            $(this).find('#gexsContentWrapper').width(),
            $(this).find('#gexsContentWrapper').height(),
            GexStat.hidePreloader(),
          );
        });
      },
  ...
  ShowTabContent: (StatGroup, week) => {
    if ($('#GexStatSettingsBox').length) {
      $('#GexStatSettingsBox').remove();
    }
  ...
  SetBoxNavigation: async (gexweek) => {
    ...
    if (GexStat.GexWeeks === undefined || GexStat.GexWeeks === null) {
      ...
      let gexWeeksRanking = await GexStat.db.ranking
        .where('gexweek')
        .above(0)
        .keys();
      let gexWeeksParticipation = await GexStat.db.participation
        .where('gexweek')
        .above(0)
        .keys();

      if (
        (gexWeeksRanking && gexWeeksRanking.length) ||
        (gexWeeksParticipation && gexWeeksParticipation.length)
      ) {
        //make unique Set of Gex weeks
        GexStat.GexWeeks = [
          ...new Set([...gexWeeksRanking, ...gexWeeksParticipation]),
        ];
        GexStat.GexWeeks.sort(function (a, b) {
          return b - a;
        });
      }
    }
    ...
    h.push(
      `<li${GexStat.CurrentStatGroup === 'Ranking' ? ' class="active"' : ''}><a class="toggle-statistic" data-value="Ranking"><span>${i18n('Boxes.GexStat.Ranking')}</span></a></li>`,
    );
    h.push(
      `<li${GexStat.CurrentStatGroup === 'Participation' ? ' class="active"' : ''}><a class="toggle-statistic" data-value="Participation"><span>${i18n('Boxes.GexStat.MemberParticipation')}</span></a></li>`,
    );
    h.push(
      `<li${GexStat.CurrentStatGroup === 'Course' ? ' class="active"' : ''}><a class="toggle-statistic" data-value="Course"><span>${i18n('Boxes.GexStat.Course')}</span></a></li>`,
    );
    h.push(`</ul>`);
    h.push(`</div>`);
    ...
    if (gexweek && GexStat.GexWeeks && GexStat.GexWeeks.length) {
      ...
      h.push(
        `<div id="gexs_weekswitch" class="weekswitch dark-bg" data-group="${GexStat.CurrentStatGroup}">${i18n('Boxes.GexStat.Gex')} ${i18n('Boxes.GexStat.Week')} <button class="btn btn-set-week" data-week="${previousweek}"${previousweek === null ? ' disabled' : ''}>&lt;</button> `,
      );
      h.push(`<select id="gexs-select-gexweek">`);

      GexStat.GexWeeks.forEach((week) => {
        h.push(
          `<option value="${week}"${gexweek === week ? ' selected="selected"' : ''}>` +
            moment.unix(week - 518400).format(i18n('Date')) +
            ` - ` +
      ...
      h.push(`</select>`);
      h.push(
        `<button class="btn btn-set-week" data-week="${nextweek}"${nextweek === null ? ' disabled' : ''}>&gt;</button>`,
      );
      h.push(`</div>`);
    }
      ...
      .done(function () {
        ...
        $('#gexs-select-gexweek')
          ...
          .on('change', function () {
            ...
            if (
              !GexStat.GexWeeks.includes(week) ||
              week === GexStat.CurrentGexWeek
            ) {
              return;
            }
  ...
  GexStatSettings: () => {
    ...
    c.push(
      `<p class="text-left"><span class="settingtitle">${i18n('Boxes.GexStat.General')}</span>${i18n('Boxes.GexStat.DeleteDataOlderThan')} <select id="gexsDeleteOlderThan" name="deleteolderthan">`,
    );
    deleteAfterWeeks.forEach((weeks) => {
      let option = '';
      if (weeks === 0) {
        option = i18n('Boxes.GexStat.Never');
      } else {
        option = weeks + ' ' + i18n('Boxes.GexStat.Weeks');
      }

      c.push(
        `<option value="${weeks}" ${Settings.deleteOlderThan === weeks ? ' selected="selected"' : ''}>${option}</option>`,
      );
    });

    c.push(`</select>`);
    c.push(
      `<hr><p class="text-left"><span class="settingtitle">${i18n('Boxes.GexStat.Course')}</span>` +
    ...
    c.push(
      `<p class="text-left">${i18n('Boxes.GexStat.CompareLast')} <select id="gexsShowRoundLimit" name="showroundlimit">`,
    );
    showRounds.forEach((round) => {
      let option = round + ' ' + i18n('Boxes.GexStat.Rounds');
      c.push(
        `<option value="${round}" ${Settings.showRoundLimit === round ? ' selected="selected"' : ''}>${option}</option>`,
      );
    });

    c.push(`</select></p>`);
    c.push(
      `<hr><p class="text-left"><span class="settingtitle">${i18n('Boxes.General.Export')}</span>` +
        `${i18n('Boxes.GexStat.ExportLast')} <select id="gexsExportLimit" name="exportlimit">`,
    );
    exportLimits.forEach((round) => {
      let option = round + ' ' + i18n('Boxes.GexStat.Rounds');
      c.push(
        `<option value="${round}" ${Settings.exportLimit === round ? ' selected="selected"' : ''}>${option}</option>`,
      );
    });
    c.push(`</select></p>`);

    let disabledExport = '';
    if (GexStat.CurrentStatGroup === 'Course') {
      disabledExport = ' disabled';
    }

    c.push(
      `<p class="text-left"><button class="btn" onclick="GexStat.ExportContent('${GexStat.CurrentStatGroup}','csv')" title="${HTML.i18nTooltip(i18n('Boxes.General.ExportCSV'))}"${disabledExport}>CSV</button>` +
    ...
    c.push(
      `<hr><p><button id="save-GexStat-settings" class="btn" style="width:100%" onclick="GexStat.SettingsSaveValues()">${i18n('Boxes.GexStat.Save')}</button></p>`,
    );
  ...
  SettingsSaveValues: async () => {
    ...
    GexStat.Settings.showAxisLabel =
      $('#gmsShowAxisLabel').is(':checked') ? true : false;

    let chartSeries = (GexStat.Settings.chartSeries = []);

    if ($('#gmsShowChartPoints').is(':checked')) {
      chartSeries.push('points');
    }
    if ($('#gmsShowChartEncounters').is(':checked')) {
      chartSeries.push('encounters');
    }
    if ($('#gmsShowChartMember').is(':checked')) {
      chartSeries.push('member');
    }
    if ($('#gmsShowChartParticipants').is(':checked')) {
      chartSeries.push('participants');
    }
    if ($('#gmsShowChartRank').is(':checked')) {
      chartSeries.push('rank');
    }
  ...
  InitSettings: () => {
    ...
    for (const k in Settings) {
      if (!Settings.hasOwnProperty(k) || !GexStat.Settings.hasOwnProperty(k)) {
        continue;
      }
  ...
  GetChartSeries: async (data) => {
    ...
    const chartSeries = {
      points: {
        ...
        zones: buildZones(data.pointsData),
        zoneAxis: 'x',
        connectNulls: true,
      ...
      encounters: {
        gridLineWidth: 0,
        zones: buildZones(data.encounterData),
        zoneAxis: 'x',
        connectNulls: true,
      ...
      rank: {
        name: i18n('Boxes.GexStat.Rank'),
        zones: buildZones(data.rankData),
        zoneAxis: 'x',
        connectNulls: true,
        data: data.rankData,
        color: '#d6dae0',
  ...
  ExportContent: async (content, type) => {
    let exportLimit =
      $('#gexsExportLimit').length ?
        parseInt($('#gexsExportLimit').val())
      : GexStat.Settings.exportLimit;
    let exportData = [];
    let FileContent = '';
    ...
    switch (content) {
      case 'Ranking':
        let Ranking = await GexStat.db.ranking
          .reverse()
          .limit(exportLimit)
          .toArray();
        if (!Ranking) {
        ...
        Ranking.sort((a, b) => a.gexweek - b.gexweek).forEach((gexweek) => {
          if (!gexweek.gexweek || !gexweek.participants) {
            return;
          }
      ...
      case 'Participation':
        let Participation = await GexStat.db.participation
          .reverse()
          .limit(exportLimit)
          .toArray();
        if (!Participation) {
        ...
        Participation.sort((a, b) => a.gexweek - b.gexweek).forEach(
          (gexweek) => {
            ...
            participation
              ...
              .forEach((participant) => {
                exportData.push([
                  ...
                  participant.name,
                  participant.expeditionPoints,
                  participant.solvedEncounters,
                  participant.rank,
                  participant.trial || 0,
                ]);
              });
  ...
  showPreloader: (id) => {
    if (!$('#gexs-loading-data').length) {
      $(id).append(
        '<div id="gexs-loading-data"><div class="loadericon"></div></div>',
      );
    }
  },
...
FoEproxy.addHandler(
  ...
  'GuildExpeditionStateNotification',
  (data, postData) => {
    if (data.responseData) GExAttempts.updateState(data.responseData);
  },
);
...
let GExAttempts = {
  ...
  refreshGUI: () => {
    ...
    if (
      GExAttempts.state.GEprogress === 159 ||
      GExAttempts.count === 0 ||
      !GExAttempts.state.active
    ) {
    ...
    if (
      !GExAttempts.deactivationTimer &&
      GExAttempts.state.deactivationTime &&
      GExAttempts.state.active
    ) {
      GExAttempts.deactivationTimer = setTimeout(
        () => {
          GExAttempts.state.active = false;
          GExAttempts.state.activationTime =
            GExAttempts.state.deactivationTime + 86400;
    ...
    if (
      ...
      GExAttempts.activationTimer = setTimeout(
        () => {
          ...
          GExAttempts.state.deactivationTime =
            GExAttempts.state.activationTime + 604800;
  ...
  setNext: (time) => {
    ...
    if (time) {
      ...
      let amount = Math.floor(
        (moment().unix() - GExAttempts.last + 100) / 3600,
      );
...
FoEproxy.addHandler(
  ...
  'getUnlockCosts',
  (data, postData) => {
    GexStockWarning.check(
      data.responseData.treasuryResources.resources,
      data.responseData.unlockCosts.resources,
    );
  },
);
