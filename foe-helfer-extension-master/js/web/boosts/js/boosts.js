...
FoEproxy.addHandler('StartupService', 'getData', (data, postData) => {
  Boosts.InitLB(
    data.responseData.city_map.entities.filter(
      (x) => x.type == 'greatbuilding',
    ),
  );
  Boosts.TimeIn.add(
    data.responseData.city_map.entities.filter(
      (x) => x.state.decaysAt || x.state.__class__ == 'ConstructionState',
    ),
  );
}),
...
let Boosts = {
  ...
  InitLB: async (LBs) => {
    let boosts = LBs.filter(
      (x) => x.bonus?.type && x.player_id == ExtPlayerID,
    ).map((x) => ({
  ...
  InitQIAP: async () => {
    ...
    QIActions.capacity =
      (GoodsData.guild_raids_action_points?.abilities?.autoRefill?.maxAmount ||
        200000) - Boosts.Sums['guild_raids_action_points_capacity'];
  },
  ...
  Add: (AllBoosts) => {
    ...
    for (let b of AllBoosts || []) {
      ...
      if (b.origin === 'inventory_item') {
        BoostPotions.activate(b.type, {
          expire: b.expireTime,
          target: b.targetedFeature || 'all',
          value: b.value,
        });
        if (b.expireTime) {
    ...
    add: async (buildings) => {
      ...
      let addToList = (boost) => {
        ...
        if (
          !Boosts.Timer.id ||
          !Boosts.Timer.next ||
          boost.startTime < Boosts.Timer.next
        ) {
          clearTimeout(Boosts.Timer.id);
          Boosts.Timer.id = setTimeout(
            Boosts.Timer.execute,
            (boost.startTime - GameTime.get() + 2) * 1000,
          );
      ...
      for (let building of buildings || []) {
        ...
        let metaData = structuredClone(
          MainParser.CityEntities[building.cityentity_id],
        );
        let era = Technologies.getEraName(
          building.cityentity_id,
          building.level,
        );
        ...
        for (let boost of NCE.boosts || []) {
          ...
          if (metaData?.components?.AllAge?.limited) {
            boost.expireTime =
              building.state.decaysAt ||
              building.state.next_state_transition_at +
                metaData.components.AllAge.limited.config.expireTime;
            Boosts.TimeOut.add(boost);
          }
          if (
            building.state.__class__ == 'ConstructionState' &&
            (building.state.pausedAt || boost.type == '')
          ) {
            addToList(boost);
        ...
        if (metaData?.components?.AllAge?.limited) {
          let target =
            metaData.components.AllAge.limited.config.targetCityEntityId;
  ...
  TimeOut: {
    ...
    add: (boost) => {
      if (
        !Boosts.TimeOut.id ||
        !Boosts.Timer.next ||
        boost.expireTime < Boosts.TimeOut.next
      ) {
        clearTimeout(Boosts.Timer.id);
        Boosts.Timer.id = setTimeout(
          Boosts.Timer.execute,
          (boost.expireTime - GameTime.get() + 2) * 1000,
        );
  ...
  Timer: {
    ...
    execute: () => {
      ...
      Boosts.TimeOut.list = Boosts.TimeOut.list.filter(
        (x) => x.expireTime > refTime,
      );
      Boosts.Remove(toRemove);

      let toAdd = Boosts.TimeIn.list.filter((x) => x.startTime <= refTime);
      Boosts.TimeIn.list = Boosts.TimeIn.list.filter(
        (x) => x.startTime > refTime,
      );
      Boosts.Add(toAdd);
      let list = [...Boosts.TimeOut.list, ...Boosts.TimeIn.list].map(
        (x) => x.startTime || x.expireTime,
      );
  ...
  Remove: (boosts) => {
    ...
    for (b of boosts || []) {
      for (let type of Object.keys(Boosts.ListByType)) {
        if (b.boostId) {
          Boosts.ListByType[type] = Boosts.ListByType[type].filter(
            (x) => x.id != b.boostId,
          );
        }
        if (b.entityId) {
          Boosts.ListByType[type] = Boosts.ListByType[type].filter(
            (x) => x.entityId != b.entityId,
          );
        }
      }
      if (b.entityId) {
