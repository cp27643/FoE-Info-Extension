  ...
  for (let q in data.responseData) {
    ...
    if (quest.id >= 900000 && quest.id < 1000000) {
      if (quest.genericRewards?.length > 0) {
        if (
          !Recurring.data.Questlist[quest.id] &&
          quest.genericRewards[0].flags.includes('random')
        ) {
          Recurring.data.Questlist[quest.id] = {
            title: quest.title,
            diamonds: false,
        ...
        if (
          quest.genericRewards[0].subType == 'medals' ||
          quest.genericRewards[0].subType == 'premium'
        ) {
          Recurring.data.Questlist[quest.id].diamonds = true;
        }
        if (!Recurring.data.Questlist[quest.id].era)
          Recurring.data.Questlist[quest.id].era = CurrentEraID;
        if (!Recurring.data.Questlist[quest.id].conditions) {
          Recurring.data.Questlist[quest.id].conditions =
            quest.successConditions;
          Recurring.data.Questlist[quest.id].groups =
            quest.successConditionGroups;
        }
      }
    }
  }
...
let Recurring = {
  data: JSON.parse(localStorage.getItem('Recurring')) || {
    Questlist: {},
    count: 0,
    showCounter: false,
    hideTasks: true,
  ...
  BuildBox: () => {
    ...
    h.push(
      `<table id="recurringTable" class="foe-table${!!Recurring.data.hideTasks ? ' hideTasks' : ''}">`,
    );

    h.push('<thead class="sticky">');
    h.push('<tr>');
    h.push(
      `<th onclick="Recurring.hideTasks()">${i18n('Boxes.RecurringQuests.Table.Quest')} ⇋</th>`,
    );
    h.push(
      `<th onclick="Recurring.hideTasks()">${i18n('Boxes.RecurringQuests.Table.Tasks')} ⇋</th>`,
    );
    h.push(
      '<th><img src="' +
        srcLinks.get('/shared/icons/premium.png', true) +
        '" alt="" width="20px" height="20px">?</th>',
    );
    h.push('</tr>');
    h.push('</thead>');
    ...
    for (let q of Recurring.data.filter) {
      ...
      h.push(`<tr>`);
      h.push(
        `<td title="${Recurring.getTasksTitle(quest.groups, quest.conditions)}"><span>${quest.title}</span></td>`,
      );
      h.push(
        `<td title="${Recurring.getTasksTitle(quest.groups, quest.conditions)}">${Recurring.getTasks(quest.groups, quest.conditions)}</td>`,
      );
      h.push(
        `<td><span class="switchState" data-id="${q}">${quest.diamonds ? '✓' : '?'}</span></td>`,
      );
      h.push('</tr>');
    }
    for (let q of Recurring.data.filter2) {
      ...
      h.push(
        `<td class="check"><span class="switchState" data-id="${q}">${quest.diamonds ? '✓' : '?'}</span></td>`,
      );
      h.push('</tr>');
    }
    h.push('</tbody>');
    ...
    $('#RecurringQuestsBoxBody .switchState').on('mousedown', function () {
      ...
      Recurring.loadingTimer = setTimeout(() => {
        ...
        id = $(this).data('id');
        Recurring.data.Questlist[id].diamonds =
          !Recurring.data.Questlist[id].diamonds;
        Recurring.SaveSettings();
        Recurring.BuildBox();
  ...
  SetCounter: () => {
    ...
    if (Recurring.data.count === 0 || !Recurring.data.showCounter)
      $buttonNumber.hide();
  },
  ...
  ShowSettingsButton: () => {
    let h = [];
    h.push(
      `<label><input type="checkbox" oninput="Recurring.SaveSettings(this.checked)" ${Recurring.data.showCounter ? 'checked' : ''}/>${i18n('Boxes.RecurringQuests.showCounter')}<label>`,
    );
  ...
  getTasks: (groups, conditions) => {
    ...
    for (let x in groups) {
      ...
      for (let c of groups[x].conditionIds) {
        ...
        let img = srcLinks.getQuest(
          conditions.find((item) => item.id == c).iconType,
        );
