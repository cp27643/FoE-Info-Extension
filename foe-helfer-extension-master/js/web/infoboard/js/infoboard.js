...
let Infoboard = {
  ...
  History: [],
  MaxEntries: 0,
  GbgProvShortNameFl: JSON.parse(
    localStorage.getItem('InfoBox.Settings.GbgProvShortNameFl') || 'false',
  ),
  ...
  Box: () => {
    ...
    if ($('#BackgroundInfo').length === 0) {
      ...
      if (localStorage.getItem('infoboxSavedFilter') === null)
        localStorage.setItem(
          'infoboxSavedFilter',
          JSON.stringify(Infoboard.SavedFilter),
        );
      else
        Infoboard.SavedFilter = JSON.parse(
          localStorage.getItem('infoboxSavedFilter'),
        );
    ...
    h.push(
      '<input type="checkbox" class="dropdown-checkbox" id="infobox-checkbox-toggle"><label class="dropdown-label game-cursor" for="infobox-checkbox-toggle">' +
    ...
    h.push('<ul>');
    h.push(
      '<li><label class="game-cursor"><input type="checkbox" data-type="auction" class="filter-msg game-cursor" ' +
        (Infoboard.SavedFilter.includes('auction') ? 'checked' : '') +
        '> ' +
        i18n('Boxes.Infobox.FilterAuction') +
        '</label></li>',
    );
    h.push(
      '<li><label class="game-cursor"><input type="checkbox" data-type="gex" class="filter-msg game-cursor" ' +
        (Infoboard.SavedFilter.includes('gex') ? 'checked' : '') +
        '> ' +
        i18n('Boxes.General.Guild_Expedition.short') +
        '</label></li>',
    );
    h.push(
      '<li><label class="game-cursor"><input type="checkbox" data-type="gbg" class="filter-msg game-cursor" ' +
        (Infoboard.SavedFilter.includes('gbg') ? 'checked' : '') +
        '> ' +
        i18n('Boxes.General.Guild_Battlegrounds.short') +
        '</label></li>',
    );
    h.push(
      '<li><label class="game-cursor"><input type="checkbox" data-type="trade" class="filter-msg game-cursor" ' +
        (Infoboard.SavedFilter.includes('trade') ? 'checked' : '') +
        '> ' +
        i18n('Boxes.Infobox.FilterTrade') +
        '</label></li>',
    );
    h.push(
      '<li><label class="game-cursor"><input type="checkbox" data-type="level" class="filter-msg game-cursor" ' +
        (Infoboard.SavedFilter.includes('level') ? 'checked' : '') +
        '> ' +
        i18n('Boxes.Infobox.FilterLevel') +
        '</label></li>',
    );
    h.push(
      '<li><label class="game-cursor"><input type="checkbox" data-type="msg" class="filter-msg game-cursor" ' +
        (Infoboard.SavedFilter.includes('msg') ? 'checked' : '') +
        '> ' +
        i18n('Boxes.Infobox.FilterMessage') +
        '</label></li>',
    );
    h.push(
      '<li><label class="game-cursor"><input type="text" data-type="text" placeholder="1.9|A1: M" class="textfilter filter-msg game-cursor" value=' +
        (Infoboard.SavedFilter.includes('text') ?
    ...
    h.push(
      '<button class="btn btn-reset-box">' +
        i18n('Boxes.Infobox.ResetBox') +
        '</button>',
    );
  ...
  PostMessage: (bd, add = true) => {
    ...
    if ($('#BackgroundInfo').length > 0) {
      if (bd['class'] !== 'welcome' && add) {
        if (
          Infoboard.MaxEntries > 0 &&
          Infoboard.History.length >= Infoboard.MaxEntries
        ) {
      ...
      let status = $('input[data-type="' + bd['class'] + '"]').prop('checked'),
        ...
        type = bd['type'],
        tr = $('<tr />'),
        filterStatus = textfilter.some((e) =>
      ...
      if ((!status || !filterStatus) && bd.class !== 'welcome') {
        ...
        if (
          Infoboard.MaxEntries > 0 &&
          $('#BackgroundInfoTable tbody tr').length >= Infoboard.MaxEntries
        ) {
          while (
            Infoboard.MaxEntries > 0 &&
            $('#BackgroundInfoTable tbody tr').length >= Infoboard.MaxEntries
      ...
      tr.append(
        '<td></td>' +
          ...
          type +
          '<br><small><em>' +
          moment(bd['date']).format('HH:mm:ss') +
          '</em></small></td>' +
          '<td>' +
          msg +
          '</td>',
    ...
    $('#BackgroundInfo').on('change', '.filter-msg', function () {
      ...
      $('.filter-msg').each(function () {
        if (
          ...
          if (Infoboard.SavedFilter.includes($(this).data('type')))
            Infoboard.SavedFilter.splice(
              Infoboard.SavedFilter.indexOf($(this).data('type')),
              1,
            );
        }
      });
      ...
      $('#BackgroundInfoTable tbody tr').each(function () {
        ...
        if (
          (active.some((e) => type.startsWith(e)) &&
            textfilter.some((e) =>
              $(tr.children()[2])
                .html()
                .toLowerCase()
  ...
  StartTitleBlinking: (txt) => {
    ...
    Infoboard.TitleBlinkEvent = setInterval(() => {
      document.title =
        document.title === Infoboard.OriginalDocumentTitle ?
          txt
        : Infoboard.OriginalDocumentTitle;
    }, 750);
  },
  ...
  ShowSettings: () => {
    ...
    let h = [];
    h.push(
      `<p><input id="autoStartInfoboard" name="autoStartInfoboard" value="1" type="checkbox" ${autoOpen === true ? ' checked="checked"' : ''} />` +
        ` <label for="autoStartInfoboard">${i18n('Boxes.Settings.Autostart')}</label>`,
    );
    h.push(
      `<p><input id="gbgProvShortNameFl" name="gbgProvShortNameFl" value="1" type="checkbox" ${Infoboard.GbgProvShortNameFl === true ? ' checked="checked"' : ''} />` +
        ` <label for="gbgProvShortNameFl">${i18n('Boxes.Infobox.Settings.GbgProvShortName')}</label>`,
    );
    h.push(
      `<p><label for="infoboxentry-length">${i18n('Settings.InfoboxEntryCount.Desc')}</label><input class="setting-input" type="number" id="infoboxentry-length" step="1" min="1" max="2000" value="${messagesAmount}"></p>`,
    );
    h.push(
      `<p><button onclick="Infoboard.SaveSettings()" id="saveInfoboardSettings" class="btn" style="width:100%">${i18n('Boxes.Settings.Save')}</button></p>`,
    );
  ...
  SaveSettings: () => {
    ...
    localStorage.setItem(
      'AutoOpenInfoBox',
      $('#autoStartInfoboard').is(':checked'),
    );
    localStorage.setItem(
      'InfoBox.Settings.GbgProvShortNameFl',
      Infoboard.GbgProvShortNameFl,
    );
...
let Info = {
  ...
  ItemAuctionService_updateBid: (d) => {
    let PlayerLink = MainParser.GetPlayerLink(
      d['player']['player_id'],
      d['player']['name'],
    );
    ...
    return {
      class: 'auction',
      type: 'Auktion',
      msg: HTML.i18nReplacer(i18n('Boxes.Infobox.Messages.Auction'), {
        player: PlayerLink,
  ...
  ConversationService_getNewMessage: (d) => {
    let chat = MainParser.Conversations.find(
        (obj) => obj.id === d['conversationId'],
      ),
      header,
      message,
      image;
    ...
    if (d['text'] !== '') {
      ...
      if (d['attachment']['type'] === 'great_building') {
        ...
        message = HTML.i18nReplacer(
          ...
          {
            building:
              MainParser.CityEntities[d['attachment']['cityEntityId']]['name'],
  ...
  ConversationService_getConversationUpdate: (d) => {
    ...
    let message =
      '<div><strong class="bright">' +
      i18n('Boxes.Infobox.UnknownConversation') +
      '</strong></div>';
    return {
      class: 'message',
      type: i18n('Boxes.Infobox.FilterMessage'),
  ...
  GuildBattlegroundService_getProvinces: async (d) => {
    ...
    if (data['lockedUntil'] !== undefined) {
      ...
      if (
        data['lockedUntil'] <
        Math.floor(MainParser.getCurrentDateTime() / 1000) + 14390
      )
        return undefined;

      let p = bP.find((o) => o['participantId'] === data['ownerId']),
        colors = GuildFights.SortedColors.find(
          (c) => c['id'] === data['ownerId'],
        );
    ...
    for (let i in data['conquestProgress']) {
      ...
      let provLabel =
        Infoboard.GbgProvShortNameFl ? prov['short'] : prov['name'];
  ...
  OtherPlayerService_newEventgreat_building_contribution: (d) => {
    ...
    if (d['rank'] >= 6) {
      ...
      let Entity = Object.values(MainParser.CityEntities).find(
        (obj) => obj['name'] === d['great_building_name'],
      );
      (EntityID = Entity['id']),
    ...
    return {
      class: 'level',
      type: 'Level-Up',
      msg: HTML.i18nReplacer(i18n('Boxes.Infobox.Messages.LevelUp'), {
        player: PlayerLink,
  ...
  OtherPlayerService_newEventtrade_accepted: (d) => {
    ...
    return {
      ...
      msg: HTML.i18nReplacer(i18n('Boxes.Infobox.Messages.Trade'), {
        player: PlayerLink,
  ...
  GuildExpeditionService_receiveContributionNotification: (d) => {
    ...
    return {
      class: 'gex',
      type: 'GEX',
      msg: HTML.i18nReplacer(i18n('Boxes.Infobox.Messages.GEX'), {
        player: PlayerLink,
