...
let BlueGalaxy = {
  ...
  GalaxyFactor: 0,
  sort: JSON.parse(
    localStorage.getItem('BlueGalaxySorting') || '{"col":null,"order":null}',
  ),
    ...
    if ($('#bluegalaxy').length === 0) {
      ...
      $('#bluegalaxy').on('blur', '#OlderGoodsValue', function () {
        ...
        localStorage.setItem(
          'BlueGalaxyOlderGoodsValue',
          BlueGalaxy.OlderGoodsValue,
        );
  ...
  CalcBody: (data) => {
    ...
    let Buildings = [],
      FPB =
        Productions.Boosts['fp'] === undefined ?
          (Boosts.Sums['forge_points_production'] + 100) / 100
        : Productions.Boosts['fp'];
    (FPBoost = (FP) => {
      return Math.round(FP * FPB);
    }),
      (showBGFragments = JSON.parse(
        localStorage.getItem('showBGFragments') || 'true',
      ));

    for (let CityEntity of Object.values(MainParser.CityBuildingsData)) {
      if (
        ['main_building', 'greatbuilding', 'off_grid'].includes(CityEntity.type)
      ) {
        continue;
      }
      ...
      if (CityEntity.state.production) {
        ...
        CityEntity.state.production.forEach((product) => {
          if (product.resources?.strategy_points)
            ...
            product.resources?.subType == 'strategy_points'
          )
            FP += FPBoost(product.resources.amount);
          else if (
            product.type == 'genericReward' &&
            product.resources?.type == 'good'
          )
            GoodsSum += product.resources.amount;
          else if (
            product.type == 'genericReward' &&
            product.resources?.type == 'forgepoint_package'
          )
            FP += parseInt(product.resources.subType);
          else if (
            product.type == 'genericReward' &&
            product.resources?.id == 'large_forgepoint_package_40'
          )
          ...
          if (product.type == 'resources' || product.type == 'guildResources')
            for (let j = 0; j < GoodsList.length; j++) {
              ...
              if (product.resources[GoodID]) {
                ...
                else GuildGoodsSum += product.resources[GoodID];
              }
            }

          if (
            product.type == 'genericReward' &&
            product.resources?.subType == 'fragment'
          ) {
        ...
        if (GoodsSum > 0 || FP > 0 || FragmentAmount > 0 || OlderGoodsSum > 0) {
          Buildings.push({
            ...
            CombinedValue:
              FP +
              BlueGalaxy.GoodsValue * GoodsSum +
              BlueGalaxy.OlderGoodsValue * OlderGoodsSum,
          });
        }
      }
    }
    ...
    Buildings = Buildings.sort(function (a, b) {
      if (BlueGalaxy.sort.col) {
        return (
          (BlueGalaxy.sort.order == 'ascending' ? -1 : 1) *
          (b[BlueGalaxy.sort.col] - a[BlueGalaxy.sort.col])
        );
      } else {
    ...
    if (BlueGalaxy.DoubleCollections > 0)
      h.push(
        i18n('Boxes.BlueGalaxy.AvailableCollections') +
          ' ' +
          BlueGalaxy.DoubleCollections +
          '<br>',
      );

    h.push(i18n('Boxes.BlueGalaxy.GoodsValue') + ' ');
    h.push(
      '<input type="number" id="goodsValue" step="0.01" min="0" max="1000" value="' +
        BlueGalaxy.GoodsValue +
        '" title="' +
        HTML.i18nTooltip(i18n('Boxes.BlueGalaxy.TTGoodsValue')) +
        '">',
    );
    if (BlueGalaxy.GoodsValue > 0) {
      h.push(
        '<small> (' +
          HTML.i18nReplacer(i18n('Boxes.BlueGalaxy.GoodsPerFP'), {
            goods: Math.round((1 / BlueGalaxy.GoodsValue) * 100) / 100,
          }) +
          ')</small>',
      );
    }
    ...
    h.push(
      '<input type="number" id="OlderGoodsValue" step="0.01" min="0" max="1000" value="' +
        BlueGalaxy.OlderGoodsValue +
        '" title="' +
        HTML.i18nTooltip(i18n('Boxes.BlueGalaxy.TTGoodsValue')) +
        '">',
    ...
    if (BlueGalaxy.OlderGoodsValue > 0) {
      h.push(
        '<small> (' +
          HTML.i18nReplacer(i18n('Boxes.BlueGalaxy.GoodsPerFP'), {
            goods: Math.round((1 / BlueGalaxy.OlderGoodsValue) * 100) / 100,
          }) +
          ')</small>',
      );
    }
    ...
    table.push(
      '<thead class="sticky">' +
        ...
        '<th class="no-sort"></th>' +
        '<th class="no-sort" data-type="bg-group">' +
        i18n('Boxes.BlueGalaxy.Building') +
        '</th>' +
        (showBGFragments ?
          '<th class="is-number icon fragments ' +
          (BlueGalaxy.sort.col == 'FragmentAmount' ?
            BlueGalaxy.sort.order
          : '') +
          '" title="' +
          i18n('Boxes.BlueGalaxy.Fragments') +
          '" data-type="bg-group" data-colname="FragmentAmount"><span></span></th>'
        : '') +
        '<th class="is-number icon fp ' +
        (BlueGalaxy.sort.col == 'FP' ? BlueGalaxy.sort.order : '') +
        '" title="' +
        i18n('Boxes.BlueGalaxy.FP') +
        '" data-type="bg-group" data-colname="FP"><span></span></th>' +
        '<th class="is-number icon old_goods ' +
        (BlueGalaxy.sort.col == 'OlderGoods' ? BlueGalaxy.sort.order : '') +
        '" title="' +
        i18n('Boxes.BlueGalaxy.OlderGoods') +
        '" data-type="bg-group" data-colname="OlderGoods"><span></span></th>' +
        '<th class="is-number icon goods ' +
        (BlueGalaxy.sort.col == 'Goods' ? BlueGalaxy.sort.order : '') +
        '" title="' +
        i18n('Boxes.BlueGalaxy.Goods') +
        '" data-type="bg-group" data-colname="Goods"><span></span></th>' +
        '<th class="is-number icon guildgoods ' +
        (BlueGalaxy.sort.col == 'GuildGoods' ? BlueGalaxy.sort.order : '') +
        '" title="' +
        i18n('Boxes.GuildMemberStat.GuildGoods') +
        '" data-type="bg-group" data-colname="GuildGoods"><span></span></th>' +
        ...
        '<th colspan="2" class="case-sensitive no-sort" data-type="bg-group">' +
        i18n('Boxes.BlueGalaxy.DoneIn') +
        '</th>' +
        '</tr>' +
        '</thead>',
    );
    ...
    for (let i = 0; i < 50 && i < Buildings.length; i++) {
      // limits the list to max 50 items

      let isPolivated =
        MainParser.CityBuildingsData[Buildings[i]['ID']].state.isPolivated;
      table.push('<tr>');
      table.push(
        '<td>' +
          (isPolivated != undefined ?
            isPolivated ? '<span class="text-bright">★</span>'
            : '☆'
          : '') +
          '</td>',
      );
      table.push(
        '<td data-text="' +
          Buildings[i]['name'].replace(/[. -]/g, '') +
          '">' +
          Buildings[i]['name'] +
          '</td>',
      );
      if (showBGFragments) {
        let items = Productions.showBuildingItems(
          true,
          Buildings[i].building,
        )[0];
        table.push(
          '<td data-number="' +
            Buildings[i].FragmentAmount +
            '">' +
            (items != false ? items : '') +
            '</td>',
        );
      ...
      table.push(
        '<td class="text-center" data-number="' +
          Buildings[i].FP +
          '">' +
          HTML.Format(Buildings[i]['FP']) +
          '</td>',
      );
      table.push(
        '<td class="text-center" data-number="' +
          Buildings[i].OlderGoods +
          '">' +
          HTML.Format(Buildings[i]['OlderGoods']) +
          '</td>',
      );
      table.push(
        '<td class="text-center" data-number="' +
          Buildings[i].Goods +
          '">' +
          HTML.Format(Buildings[i]['Goods']) +
          '</td>',
      );
      table.push(
        '<td class="text-center" data-number="' +
          Buildings[i].GuildGoods +
          '">' +
          HTML.Format(Buildings[i]['GuildGoods']) +
          '</td>',
      );
      ...
      if (
        Buildings[i].In == 0 ||
        Buildings[i].At * 1000 <= MainParser.getCurrentDateTime()
      ) {
        table.push(
          '<td style="white-space:nowrap"><strong class="success">' +
            i18n('Boxes.BlueGalaxy.Done') +
            '</strong></td>',
        );
      } else {
        table.push(
          '<td style="white-space:nowrap"><strong class="error">' +
            moment.unix(Buildings[i].At).fromNow() +
            '</strong></td>',
        );
      }

      table.push(
        '<td class="text-right"><span class="show-entity" data-id="' +
          Buildings[i]['ID'] +
          '"><img class="game-cursor" src="' +
          extUrl +
          'css/images/hud/open-eye.png"></span></td>',
      );
      table.push('</tr>');
    }
    ...
    $('#BGTable th').on('click', (e) => {
      ...
      localStorage.setItem(
        'BlueGalaxySorting',
        JSON.stringify(BlueGalaxy.sort),
      );
      BlueGalaxy.CalcBody();
    });
  },
  ...
  ShowSettings: () => {
    ...
    let showBGFragments = JSON.parse(
      localStorage.getItem('showBGFragments') || 'true',
    );

    let h = [];
    h.push(
      `<p><input id="autoStartBGHelper" name="autoStartBGHelper" value="1" type="checkbox" ${autoOpen === true ? ' checked="checked"' : ''} /> <label for="autoStartBGHelper">${i18n('Boxes.Settings.Autostart')}</label></p>`,
    );
    h.push(
      `<p><input id="showBGFragments" name="showBGFragments" value="1" type="checkbox" ${showBGFragments === true ? ' checked="checked"' : ''} /> <label for="showBGFragments">${i18n('Boxes.Settings.showBGFragments')}</label></p>`,
    );
    h.push(
      `<p><button onclick="BlueGalaxy.SaveSettings()" id="save-bghelper-settings" class="btn" style="width:100%">${i18n('Boxes.Settings.Save')}</button></p>`,
    );
  ...
  SaveSettings: () => {
    let value = false;
    if ($('#autoStartBGHelper').is(':checked')) value = true;
    localStorage.setItem('ShowBlueGalaxyHelper', value);
    let showBGFragments = false;
    if ($('#showBGFragments').is(':checked')) showBGFragments = true;
