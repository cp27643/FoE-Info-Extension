...
FoEproxy.addHandler('BattlefieldService', 'all', (data, postData) => {
  ...
  const state =
    data.responseData.__class__ === 'BattleRealm' ?
  ...
  let noTournament =
    !data?.responseData?.state?.ranking_data?.tournament_running ||
    data?.responseData?.battleType?.type == 'guild_expedition';
...
FoEproxy.addHandler(
  ...
  (data, postData) => {
    ...
    if (data.responseData[1]) {
      ...
      fightType +=
        data.responseData[1].fightType.value == fightType ?
          ''
        : data.responseData[1].fightType.value;
    }
...
FoEproxy.addHandler(
  ...
  (data, postData) => {
    ...
    if (bt == 'pvp_arena') {
      opponent =
        BattleAssist.ArenaOpponents[data.responseData?.battleType?.difficulty];
      if (!opponent) return;
      let wave1 = data.responseData.state.unitsOrder
        .filter((x) => x.ownerId != ExtPlayerID)
        .map((x) => x.unitTypeId);
    ...
    if (
      data.responseData.state.winnerBit != 2 &&
      unitsLost < BattleAssist.AASettings.lostUnits &&
      HPstart - HPcurrent < BattleAssist.AASettings.lostHP
    )
      return;
    if (
      data.responseData.state.winnerBit == 2 &&
      !BattleAssist.AASettings.battleLost
    )
      return;
    if (
      bt != 'pvp_arena' &&
      BattleAssist.armyAdvice[opId] &&
      BattleAssist.armyAdvice[opId].bonus < BattleAssist.armyRecent[0]?.bonus
    )
      return;
...
FoEproxy.addHandler('BattlefieldService', 'surrender', (data, postData) => {
  ...
  if (
    BattleAssist.armyRecent[0] &&
    BattleAssist.armyAdvice[BattleAssist.armyRecent[0]?.id] &&
    BattleAssist.armyAdvice[BattleAssist.armyRecent[0]?.id].bonus <
      BattleAssist.armyRecent[0]?.bonus
  )
    return;
  BattleAssist.ShowAddAdvice();
});
...
FoEproxy.addHandler('PVPArenaService', 'getOverview', (data, postData) => {
  BattleAssist.ArenaOpponents = Object.assign(
    {},
    ...data.responseData?.opponents.map((x) => ({
      [x.difficulty]: {
        id:
          x?.opposingPlayer?.player?.name +
          '#' +
          x?.opposingPlayer?.player?.player_id,
        bonus: x.defenseArmyBoosts?.defenseBoost || 0,
      },
    })),
  );
  ...
  (postData) => {
    ...
    if (postData.requestData?.[0]?.battleType == 'pvp_arena') {
      setTimeout(() => {
        ...
        if (
          !BattleAssist.armyAdvice[
            'pvp_arena%' + BattleAssist.SelectedOpponent?.id
          ]
        )
          return;
        BattleAssist.ShowArmyAdvice(
          BattleAssist.armyAdvice[
            'pvp_arena%' + BattleAssist.SelectedOpponent.id
          ].advice,
        );
 ...
 */
let BattleAssist = {
  armyAdvice: JSON.parse(
    localStorage.getItem('BattleAssistArmyAdvice') || '{}',
  ),
  armyRecent: [],
  UnitOrder: null,
  AASettings: JSON.parse(
    localStorage.getItem('BattleAssistAASettings') ||
      '{"lostUnits":2,"lostHP":40,"battleLost":true,"battleSurrendered":true}',
  ),
  GEArmies: {},
  ArenaOpponents: {},
  ...
  ShowRogueDialog: () => {
    ...
    $('#battleAssistRogueDialogBody').html(
      `${i18n('Boxes.BattleAssist.Text.Rogue')}`,
    );
  },
  ...
  ShowAddAdvice: () => {
    ...
    $('#battleAssistAddAdviceBody').html(
      `<div onclick="BattleAssist.ShowArmyAdviceConfig()">${i18n('Boxes.BattleAssistAddAdvice')}</div>`,
    );
  },
  ...
  ShowArmyAdviceConfig: () => {
    ...
    for ([key, value] of Object.entries(BattleAssist.armyAdvice)) {
      if (/^GE5/.test(key)) {
        BattleAssist.armyAdvice[key.replace(/^GE5/, 'defense%')] =
          structuredClone(value);
        delete BattleAssist.armyAdvice[key];
        localStorage.setItem(
          'BattleAssistArmyAdvice',
          JSON.stringify(BattleAssist.armyAdvice),
        );
      }
    }
    ...
    if (BattleAssist.armyRecent.length > 0) {
      ...
      for (let recent of BattleAssist.armyRecent) {
        ...
        if (PlayerName)
          html +=
            `<span>` +
            srcLinks.icons('feature_pvp_arena') +
            `</span>` +
            PlayerName;
        ...
        let id = recent.id;
        if (!advice) {
          //process neutral/old advice data
    ...
    $(`.battleAssistOverrideType`).on('click', (event) => {
      ...
      $(`.AASetAdvice[data-id="${id}"] .battleAssistOverrideType`).removeClass(
        'active',
      );
      $(event.target).parent().addClass('active');
      let type = $(event.target).parent().data('type');
      let overrideId =
        type + (type != '' ? '%' : '') + id.replace(/^(attack|defense)+%/, '');
  ...
  AddAdvice: (id, bonus, advice) => {
    ...
    BattleAssist.armyAdvice[id] = {
      bonus: bonus,
      advice: advice,
      wave1: BattleAssist.armyRecent[i].wave1,
      wave2: BattleAssist.armyRecent[i].wave2,
    };
  },
  AAExp: () => {
  ...
  ShowAASettingsButton: () => {
    ...
    h.push(
      `<input type="Number" id="AAlostUnits" oninput="BattleAssist.SaveAASettings()" value="${BattleAssist.AASettings.lostUnits}"></td></tr><tr><td>`,
    );
    h.push(`${i18n('Boxes.BattleAssistAAConfig.lostHP')}</td><td>`);
    h.push(
      `<input type="Number" id="AAlostHP" oninput="BattleAssist.SaveAASettings()" value="${BattleAssist.AASettings.lostHP}"></td></tr><tr><td>`,
    );
    h.push(`${i18n('Boxes.BattleAssistAAConfig.battleLost')}</td><td>`);
    h.push(
      `<input type="checkbox" id="AAbattleLost" oninput="BattleAssist.SaveAASettings()"${BattleAssist.AASettings.battleLost ? ' checked' : ''}></td></tr><tr><td>`,
    );
    h.push(`${i18n('Boxes.BattleAssistAAConfig.battleSurrendered')}</td><td>`);
    h.push(
      `<input type="checkbox" id="AAbattleSurrendered" oninput="BattleAssist.SaveAASettings()"${BattleAssist.AASettings.battleSurrendered ? ' checked' : ''}></td></tr></table>`,
    );
  ...
  SaveAASettings: () => {
    ...
    BattleAssist.AASettings.battleSurrendered = $(
      '#AAbattleSurrendered',
    )[0].checked;
    localStorage.setItem(
      'BattleAssistAASettings',
      JSON.stringify(BattleAssist.AASettings),
    );
  },
  processArmies: (wave1, wave2, bonus, type) => {
    ...
    if (wave2) {
      wave2.sort(
        (a, b) => BattleAssist.UnitOrder[b] - BattleAssist.UnitOrder[a],
      );
    ...
    if (!advice) {
      //process old advice data
      advice =
        BattleAssist.armyAdvice[id.replace(/^(attack|defense)+%/, '')]?.advice;
      aBonus =
        BattleAssist.armyAdvice[id.replace(/^(attack|defense)+%/, '')]?.bonus;
    }
