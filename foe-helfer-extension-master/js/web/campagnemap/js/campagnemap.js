...
FoEproxy.addHandler('CampaignService', 'start', (data, postData) => {
  localStorage.setItem(
    'AllProvinces',
    JSON.stringify(data.responseData.provinces),
  );
});
...
let KampagneMap = {
  ...
  CalcBody: () => {
    ...
    if (KampagneMap.Provinces === null) return;
    ...
    if (OffeneProvinzen.length === 0) {
      ...
      h.push(
        '<div><strong>' +
          i18n('Boxes.Campagne.AlreadyDone') +
          '</strong></div>',
      );
      h.push('</div>');
      ...
      OffeneProvinzen.forEach((prov) => {
        for (let ResourceName in prov['resourcePrice']['resources']) {
          if (
            !prov['resourcePrice']['resources'].hasOwnProperty(ResourceName)
          ) {
            break;
          }
          ...
          RequiredResources[ResourceName] +=
            prov['resourcePrice']['resources'][ResourceName];
        }
      ...
      h.push(
        '<thead>' +
          '<tr>' +
          '<th>' +
          i18n('Boxes.Campagne.Resource') +
          '</th>' +
          '<th>' +
          i18n('Boxes.Campagne.DescRequired') +
          '</th>' +
          '<th>' +
          i18n('Boxes.Campagne.DescInStock') +
          '</th>' +
          '<th class="text-right">' +
          i18n('Boxes.Campagne.DescStillMissing') +
          '</th>' +
          '</tr>' +
          '</thead>',
      );
