...
FoEproxy.addHandler(
  ...
  (data, postData) => {
    ...
    if (
      ...
      postData &&
      postData[0] &&
      postData[0]['requestData'] &&
      postData[0]['requestData'][0] &&
      postData[0]['requestData'][0][0] === Unit.alca.id
    ) {
      ...
      if (
        ...
        data.responseData['updatedEntities'][0] &&
        data.responseData['updatedEntities'][0]['state'] &&
        data.responseData['updatedEntities'][0]['state'][
        ...
        Unit.NextAmount =
          data.responseData['updatedEntities'][0]['state']['current_product'][
            'amount'
          ];
      }
      ...
      if (data.responseData.militaryProducts.length > 0) {
        localStorage.setItem(
          'LastAlcatrazUnits',
          JSON.stringify(data.responseData.militaryProducts),
        );
      }
    }
  },
...
let Unit = {
  ...
  PrepareCoords: () => {
    ...
    for (let i in Unit.CoordsRaw.frames) {
      ...
      let n = Unit.CoordsRaw.frames[i][0].replace(r, ''), // cut of "armyuniticons_50x50_"
        l =
          Unit.CoordsRaw.frames[i][1] === 1 ?
            '0'
          : parseInt(Unit.CoordsRaw.frames[i][1]) * -1,
        t =
          Unit.CoordsRaw.frames[i][2] === 1 ?
            '0'
          : parseInt(Unit.CoordsRaw.frames[i][2]) * -1;

      s.push(
        `.unit_icon.${n}{background-position: calc(${l}px*var(--unit_scale)) calc(${t}px*var(--unit_scale))}`,
      );
    }
  ...
  BuildBox: () => {
    ...
    document
      .querySelector('#UnitOverview')
      .style.setProperty('--unit_scale', unitScale);
    ...
    attack.push('<th></th>');
    attack.push(
      '<th class="text-center" style="width:29%">' +
        i18n('Boxes.Units.Unit') +
        '</th>',
    );
    attack.push(
      '<th class="text-center">' + i18n('Boxes.Units.Status') + '</th>',
    );
    attack.push(
      '<th class="text-center">' + i18n('Boxes.Units.Attack') + '</th>',
    );
    attack.push(
      '<th class="text-center">' + i18n('Boxes.Units.Defend') + '</th>',
    );
    attack.push('</tr>');
    attack.push('</thead>');
    ...
    for (let i in Unit.Attack) {
      ...
      let type = Unit.Types.find(
          (obj) => obj['unitTypeId'] === Unit.Attack[i]['unitTypeId'],
        ),
        cache = Unit.Cache['units'].find(
          (obj) => obj['unitId'] === Unit.Attack[i]['unitId'],
        ),
      ...
      attack.push(
        `<td><span class="unit_icon ${Unit.Attack[i]['unitTypeId']} unit_skill ${type['unitClass']}"></span></td>`,
      );
      attack.push('<td>' + type['name'] + '</td>');

      let status = cache['currentHitpoints'] * 10;
      attack.push(
        '<td class="text-center"><span class="health"><span class="bar" style="width:' +
          status +
      ...
      let Attack =
          MainParser.round(type['baseDamage'] * (AttackBoost / 100)) +
          type['baseDamage'],
        Defense =
          MainParser.round(type['baseArmor'] * (DefenseBoost / 100)) +
          type['baseArmor'];

      attack.push(
        '<td class="text-center"><em><small>+' +
          AttackBoost +
          '%</small></em> <strong class="text-success">= ' +
          Attack +
          '</strong></td>',
      );
      attack.push(
    ...
    for (let i = Unit.Attack.length; i < 8; i++) {
      attack.push('<tr>');
      attack.push(
        '<td colspan="5" class="text-center"><strong class="text-danger"><em>' +
          i18n('Boxes.Units.NotFilled') +
          '</em></strong></td>',
      );
      attack.push('</tr>');
    }
    ...
    for (let i in Unit.Defense) {
      ...
      let type = Unit.Types.find(
          (obj) => obj['unitTypeId'] === Unit.Defense[i]['unitTypeId'],
        ),
        cache = Unit.Cache['units'].find(
          (obj) => obj['unitId'] === Unit.Defense[i]['unitId'],
        ),
        era = Technologies.Eras[type['minEra']];

      defense.push(
        `<td><span class="unit_icon ${Unit.Defense[i]['unitTypeId']} unit_skill ${type['unitClass']}"></span></td>`,
      );
    ...
    for (let i in Unit.ArenaDefense) {
      ...
      let type = Unit.Types.find(
          (obj) => obj['unitTypeId'] === Unit.ArenaDefense[i]['unitTypeId'],
        ),
        cache = Unit.Cache['units'].find(
          (obj) => obj['unitId'] === Unit.ArenaDefense[i]['unitId'],
        ),
        era = Technologies.Eras[type['minEra']];

      arenaDefense.push(
        `<td><span class="unit_icon ${Unit.ArenaDefense[i]['unitTypeId']} unit_skill ${type['unitClass']}"></span></td>`,
      );
    ...
    for (let i in c) {
      ...
      let d = Unit.Types.find(
        (obj) => obj['unitTypeId'] === c[i]['unitTypeId'],
      );
    ...
    pool.push(
      '<th class="text-center">' + i18n('Boxes.Units.Unbind') + '</th>',
    );
    pool.push('</tr>');
    pool.push('</thead>');
    ...
    for (let era = eras.length; era >= 0; era--) {
      ...
      for (let i in eras[era]) {
        ...
        pool.push('<tr>');
        pool.push(
          `<td><span class="unit_icon ${eras[era][i]['id']} unit_skill ${eras[era][i]['unitClass']}"></span></td>`,
        );
        pool.push('<td>' + eras[era][i]['name'] + '</td>');
        pool.push(
          '<td class="text-center">' + eras[era][i]['attached'] + '</td>',
        );
        pool.push(
          '<td class="text-center">' + eras[era][i]['unattached'] + '</td>',
        );
        pool.push('</tr>');
      }
    }
    ...
    if (Unit.alca) {
      $('#UnitOverview')
        .find('.unit-tabs')
        .prop('style', 'height:calc(100% - 80px)');
    }
  },
  ...
  BuildTimer: () => {
    ...
    // there was a harvest...
      ...
      text = HTML.i18nReplacer(i18n('Boxes.Units.NextUnitsIn'), {
        count: Unit.NextAmount,
        harvest: moment
          .unix(Unit.alca['state']['next_state_transition_at'])
          .format('HH:mm:ss'),
      });
    } else {
      let countDownDate = moment.unix(
        Unit.alca['state']['next_state_transition_at'],
      );
  ...
  RefreshAlca: (data) => {
    if (!Unit.alca) {
      Unit.alca = Object.values(MainParser.CityMapData).find(
        (obj) => obj['cityentity_id'] === 'X_ProgressiveEra_Landmark1',
      );
    }
    ...
    if (
      ...
      data['updatedEntities'][0] &&
      data['updatedEntities'][0]['cityentity_id'] ===
        'X_ProgressiveEra_Landmark1'
    ) {
      Unit.NextHarvest =
        data['updatedEntities'][0]['state']['next_state_transition_at'];
      Unit.BuildTimer();
    }
  },
      ...
      if (diff <= 0) {
        ...
        $('.alca-info').html(
          '<span class="text-danger"><strong>' +
            i18n('Boxes.Units.ReadyToLoot') +
            '</strong></span>',
        );
      } else $('.alca-countdown').text(moment.utc(diff).format('HH:mm:ss'));
    } else {
      clearInterval(intervalID);
      $('.alca-info').html(
        '<span class="text-danger"><strong>' +
  ...
  GetBoostSums: (Boosts) => {
    ...
    CurrentBoost =
      Boosts['attacker_defense_boost'] || Boosts['def_boost_attacker'];
    if (CurrentBoost !== undefined) {
      Ret['AttackDefenseBoost'] += CurrentBoost;
    }
    ...
    CurrentBoost =
      Boosts['defender_attack_boost'] || Boosts['att_boost_defender'];
    if (CurrentBoost !== undefined) {
      Ret['DefenseAttackBoost'] += CurrentBoost;
    }
  ...
  GetLastAlcaUnits: () => {
    ...
    for (let i in AlcaUnits) {
      ...
      let type = Unit.Types.find(
          (obj) => obj['unitTypeId'] === AlcaUnits[i]['unitTypeId'],
        ),
        era = Technologies.Eras[type['minEra']];
    ...
    last.push(
      '<th class="text-center">' + i18n('Boxes.Units.Quantity') + '</th>',
    );
    last.push(
      '<th class="text-center">' + i18n('Boxes.Units.Proportionally') + '</th>',
    );
    last.push('</tr>');
    last.push('</thead>');
    ...
    for (let i in LastAlca) {
      ...
      last.push(
        `<td><span class="unit_icon ${LastAlca[i]['id']} unit_skill ${LastAlca[i]['unitClass']}"></span></td>`,
      );
      ...
      last.push(
        '<td class="text-center">' +
          MainParser.round((LastAlca[i]['count'] * 100) / LastTotal) +
          '%</td>',
      );
  ...
  LoadSettings: () => {
    cachedSettings =
      JSON.parse(localStorage.getItem('UnitOverviewSettings')) || Unit.Settings;
    Unit.Settings.pictogramScaling =
      cachedSettings && cachedSettings.pictogramScaling !== undefined ?
        cachedSettings.pictogramScaling
      : Unit.Settings.pictogramScaling;
  },
  ...
  ShowSettings: () => {
    ...
    h.push(
      `<p><button onclick="Unit.SaveSettings()" id="unit-save-settings" class="btn" style="width:100%">${i18n('Boxes.Settings.Save')}</button></p>`,
    );
    ...
    $('#UnitOverviewSettingsBox').on('click', '.btn-set-value', function () {
      let value =
        parseFloat($('#pictogramScaling').val()) +
  ...
  SaveSettings: () => {
    Unit.Settings.pictogramScaling =
      1 <= $('#pictogramScaling').val() && $('#pictogramScaling').val() <= 4 ?
        $('#pictogramScaling').val()
      : Unit.Settings.pictogramScaling;
