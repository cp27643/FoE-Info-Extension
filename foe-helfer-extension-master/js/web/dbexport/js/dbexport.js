function progressCallback({ totalRows, completedRows }) {
  ...
  $('#dbex-loading-data .message .progressbar .state').css(
    'width',
    `${percent}%`,
  );
}
...
let DBExport = {
  ...
  Show: async () => {
    ...
    h.push(
      `<li${DBExport.CurrentTab === 'Export' ? ' class="active"' : ''}><a class="toggle-group" data-value="Export"><span>${i18n('Boxes.DBExport.Export')}</span></a></li>`,
    );
    h.push(
      `<li${DBExport.CurrentTab === 'Import' ? ' class="active"' : ''}><a class="toggle-group" data-value="Import"><span>${i18n('Boxes.DBExport.Import')}</span></a></li>`,
    );
    h.push(`</ul></div>`);
  ...
  ShowExportGroup: async () => {
    ...
    h.push(
      `<table id="dbex_ExportTable" class="foe-table"><thead><th><input id="dbex_toggleCheckbox" type="checkbox" value="1" checked="checked" /></th><th>${i18n('Boxes.DBExport.Description')}</th><th>${i18n('Boxes.DBExport.Type')}</th><th>${i18n('Boxes.DBExport.Tables')}</th><th>${i18n('Boxes.DBExport.Records')}</th></thead><tbody>`,
    );

    // Local Storage
    h.push(
      `<tr><td><input class="inpExport" type="checkbox" value="1"  data-type="localsettings" data-value="localsettings" checked="checked" /></td><td>${i18n('Boxes.DBExport.LocalSettings')}</td><td>local storage</td><td>0</td><td>${localStorage.length}</td></tr>`,
    );
    ...
    databases.forEach((item) => {
      ...
      h.push(
        `<tr><td><input class="inpExport" type="checkbox" value="1" data-type="indexdb" data-value="${item.dbname}" checked="checked" /></td><td>${item.dbname}</td><td>indexDB</td><td>${item.tableCount}</td><td>${item.rowCount}</td></tr>`,
      );
    });
    ...
    h.push(
      `<button class="btn" id="dbex_ExportSelection" href="#">${i18n('Boxes.DBExport.ExportData')}</button>`,
    );
    ...
    $('#dbexport_wrapper')
      ...
      .done(function () {
        ...
        $('#dbex_ExportSelection')
          .off('click')
          .on('click', async function () {
            let data = {};

            let indexdbnames = $(
              '#DBExport .inpExport[data-type=indexdb]:checkbox:checked',
            )
              .map(function () {
            ...
            let localSettings = $(
              '#DBExport .inpExport[data-type=localsettings]:checkbox:checked',
            )
              .map(function () {
                return $(this).data('value');
              })
  ...
  ShowImportGroup: async () => {
    ...
    h.push(
      `<label for="dbex_fileupload" tabindex="1" role="button"><div id="dbex_dropzone">${i18n('Boxes.DBExport.ImportDescription')}</div></label>`,
    );
  ...
  UploadFile: async (file) => {
    try {
      ...
      if (
        ...
        $('#debex_import_wrapper').html(
          `<p class="error">${i18n('Boxes.DBExport.ImportFileError')}</p>`,
        );
        DBExport.hidePreloader();
        return;
      }
      ...
      switch (file.type) {
        case 'application/json':
          $('#dbex-loading-data .message').html(
          ...
          if (
            file.name.search('localStorage') == 4 ||
            file.name.search('localStorage') == 5
          ) {
            await DBExport.ImportLocalStorage(file, file.name);
            $('#debex_import_wrapper').append(
              `<p class="success">${i18n('Boxes.DBExport.ImportSuccessful')}</p>`,
            );
            DBExport.hidePreloader();
            break;
          }
          ...
          let SuccessMessage =
            SuccessState === 1 ? i18n('Boxes.DBExport.ImportSuccessful')
            : SuccessState === 0 ? i18n('Boxes.DBExport.ImportPartlySuccessful')
            : i18n('Boxes.DBExport.ImportFileError');
          $('#debex_import_wrapper').append(
        ...
        case 'application/x-zip-compressed':
                  ...
                  promises.push(
                    zip
                      ...
                      .then(async function (blob) {
                        if (
                          zipEntry.name.search('localStorage') == 4 ||
                          zipEntry.name.search('localStorage') == 5
                        ) {
                          $('#dbex-loading-data .message').html(
                            '<span class="progress">' +
                              ++exportState +
                          ...
                          response = await DBExport.ImportLocalStorage(
                            blob,
                            zipEntry.name,
                          );
                        } else {
                          ...
                          let World = zipEntry.name.slice(
                            0,
                            zipEntry.name.indexOf('_'),
                          );
          ...
          Promise.all(promises).then(function () {
            let SuccessMessage =
              SuccessState === 1 ? i18n('Boxes.DBExport.ImportSuccessful')
              : SuccessState === 0 ?
                i18n('Boxes.DBExport.ImportPartlySuccessful')
              : i18n('Boxes.DBExport.ImportFileError');
            $('#debex_import_wrapper').append(
              `<p class="success">${SuccessMessage}</p>` +
                `<p><button onclick="location.reload();" class="btn">${i18n('Boxes.DBExport.ReloadPage')}</button></p>`,
  ...
  ImportLocalStorage: async (blob, filename) => {
    ...
    $('#debex_import_wrapper').append(
      `<p class="success">${filename} <span class="icon success"></span></p>`,
    );
  },
  ...
  ImportDexieDB: async (blob, World) => {
    ...
    if (!importMeta || !importMeta.data) {
      $('#debex_import_wrapper').append(
        `<p class="error"><span class="icon error">X</span><span class="errmsg">${i18n('Boxes.DBExport.ImportFileError')}</span></p>`,
      );
      return 0;
    }

    let dbName =
      importMeta.data.databaseName ? importMeta.data.databaseName : '';

    try {
      // check if DB has the right PlayerID and World
      let Filename = dbName.split(/_/);
      let ImportPlayerID =
        Array.isArray(Filename) ? parseInt(Filename.pop()) : undefined;

      if (ExtPlayerID !== ImportPlayerID) {
        $('#debex_import_wrapper').append(
          `<p class="error">${dbName} <span class="icon error">X</span><span class="errmsg">${i18n('Boxes.DBExport.WrongDBPlayerID')}</span></p>`,
        );
        return 0;
      }
      ...
      if (ExtWorld !== World) {
        $('#debex_import_wrapper').append(
          `<p class="error">${dbName} <span class="icon error">X</span><span class="errmsg">${i18n('Boxes.DBExport.WrongDBWorld')}</span></p>`,
        );
        return 0;
      }
      ...
      $('#debex_import_wrapper').append(
        `<p class="success">${dbName} <span class="icon success"></span></p>`,
      );
      return 1;
    } catch (error) {
      $('#debex_import_wrapper').append(
        `<p class="error">${dbName} <span class="icon error">X</span><span class="errmsg">${i18n('Boxes.DBExport.ImportFileError')}</span></p>`,
      );
      console.error('' + error);
      return 0;
    }
  ...
  ExportSelection: async (data) => {
    ...
    exportCounter =
      parseInt(saveIndexDB ? data.indexdb.length : 0) +
      parseInt(saveLocalStorage ? 1 : 0);
    ...
    if (data.localStorage === true) {
      ...
      let localStorageBlob = new Blob([localStorageJSON], {
        type: 'application/json',
      });
      $('#dbex-loading-data .message span.progress').html(
        ++exportState + ' / ' + exportCounter,
      );
      if (exportCounter > 1) {
        zip.file(
          ExtWorld +
    ...
    if (saveIndexDB) {
      for (let i in data.indexdb) {
        ...
        await DexieDB.open();
        try {
          const blob = await DexieDB.export({
            prettyJson: true,
            progressCallback,
          });
          if (exportCounter > 1) {
    ...
    $('#dbex-loading-data .message .progressbar').html(
      i18n('Boxes.DBExport.Compression'),
    );
    if (exportCounter > 1) {
      zip
  ...
  showPreloader: (id) => {
    if (!$('#dbex-loading-data').length) {
      $(id).append(
        '<div id="dbex-loading-data"><div class="message"></div><div class="loadericon"></div></div>',
      );
    }
  },
