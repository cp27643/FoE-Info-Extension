...
let Tooltips = {
  ...
  init: async () => {
    ...
    container.style =
      'z-index:1000; position: absolute; display: none; pointer-events: none;';
    ...
    $(
      `<div id="QIActions" class="helperTT" data-callback_tt="QIActions.TT">${srcLinks.icons('time')}</div>`,
    )
      .appendTo('body')
      .hide();
  ...
  checkposition: () => {
    try {
      if (Tooltips.containerActive) {
        if (
          Tooltips.Container.firstChild.clientHeight +
            7 +
            Number(Tooltips.Container.style.top.replace('px', '')) >
          Tooltips.Container.parentElement.clientHeight
        ) {
          newTop =
            Tooltips.Container.parentElement.clientHeight -
            Tooltips.Container.firstChild.clientHeight -
        ...
        if (
          Tooltips.Container.firstChild.clientWidth +
            7 +
            Number(Tooltips.Container.style.left.replace('px', '')) >
          Tooltips.Container.parentElement.clientWidth
        ) {
          let newLeft =
            Tooltips.Container.parentElement.clientWidth -
            Tooltips.Container.firstChild.clientWidth -
  ...
  buildingTT: async (e) => {
    ...
    let id =
      e?.currentTarget?.dataset?.meta_id ||
      MainParser?.CityMapData[buildingId]?.cityentity_id;
    if (!id) return;

    let era =
      e?.currentTarget?.dataset?.era ||
      Technologies.InnoEraNames[
        MainParser?.CityMapData[e?.currentTarget?.dataset?.id]?.level
      ];
    ...
    if (!eff && era)
      eff = Math.round(
        100 *
          Productions.rateBuildings([id], true, era)?.[0]?.rating.totalScore ||
          0,
      );
    ...
    setTimeout(() => {
      $('.handleOverflow').each((index, e) => {
        ...
        if (w < 0) e.style['animation-name'] = 'unset';
  ...
  genericEval: (rew) => {
    let [x1, amount, name] = rew.name.match(/^([+\-]?\d*)x? (.*)$/) || [
      '',
      1,
      rew.name,
    ];
    ...
    if (rew.iconAssetName == 'icon_fragment') {
      icon = srcLinks.icons(
        rew.assembledReward?.iconAssetName || rew.assembledReward?.subType,
      );
      name =
        name.replace(/Fragments? of/, '').replace(/.*?'(.*?)'.*/, '$1') +
        ' (' +
        rew.requiredAmount +
        ')';
  ...
  BuildingData: async (
    ...
    let span = (x, withHighlight = false) =>
      `<span ${withHighlight ? `class="${x > 0 ? 'positive' : 'negative'}"` : ''}>${numberWithCommas(x)}</span>`;
    let longSpan = (x) =>
      `<div class="overflowBox"><span class="handleOverflow">${x}</span></div>`;
    let range = (x, y, withHighlight = false) =>
      span(x, withHighlight) +
      (x != y ? `&nbsp;-&nbsp;` + span(y, withHighlight) : ``);
    let formatTime = (x) => {
    ...
    let capFirsts = (s) => {
      return s.replace('_', ' ').replace(/(\b[a-z](?!\s))/g, function (x) {
    ...
    if (meta.components) {
      ...
      if (levels?.AllAge?.socialInteraction?.interactionType) {
        if (levels?.AllAge?.socialInteraction?.interactionType === 'motivate') {
          ...
          levels?.AllAge?.socialInteraction?.interactionType === 'polish'
        ) {
      ...
      if (chain?.chainId) {
        let ChainMeta =
          MainParser.BuildingChains?.[chain?.chainId] ||
          MainParser.BuildingChains?.[chain?.chainId.toLowerCase()];
        set = srcLinks.icons(chain?.chainId) + ChainMeta.name;
        if (!ChainMeta.cityEntityIds.includes(meta.id))
          set +=
            '</td></tr><tr><td style="text-wrap-mode:wrap;">' +
            chain.description;
      }
      //Traits
      for (let a of meta.abilities || []) {
        ...
        if (
          a.__class__ === 'AffectsEnvironmentAbility' &&
          a.action?.type === 'add_unique_inhabitant'
        )
      ...
      for (let r of levels.AllAge?.ally?.rooms || []) {
        ...
        for (a of allies || []) {
          ...
          if (
            r.allyType === allydata.type &&
            (!r.rarity?.value || r.rarity?.value === allydata.rarity)
          )
            break;
          allydata = null;
        }
      ...
      if (levels.AllAge.eraRequirement?.era && era == '') {
        era =
          srcLinks.icons('era') +
          ' ' +
          i18n('Eras.' + Technologies.Eras[levels.AllAge.eraRequirement?.era]);
      }
      ...
      if (levels?.AllAge?.limited?.config?.expireTime) {
        if (efficiency) {
          let ratings = Productions.rateBuildings(
            [meta.id, levels.AllAge.limited.config.targetCityEntityId],
            true,
            era,
          )?.map((x) => Math.round(100 * x?.rating?.totalScore) || 0);
      ...
      if (
        levels?.[minEra]?.happiness?.provided &&
        levels[maxEra]?.happiness?.provided
      ) {
      ...
      for (let [i, b] of Object.entries(
        levels?.[minEra]?.boosts?.boosts || [],
      )) {
      ...
      let prods = '';
      let pCount =
        levels?.[minEra]?.production?.options?.length ||
        levels.AllAge?.production?.options?.length ||
        0;

      minLookup = Object.assign(
        structuredClone(levels?.[minEra]?.lookup?.rewards || {}),
        levels?.AllAge?.lookup?.rewards,
      );
      minProductions =
        levels?.[minEra]?.production?.options ||
        levels.AllAge.production?.options ||
        [];
      maxLookup = Object.assign(
        structuredClone(levels?.[maxEra]?.lookup?.rewards || {}),
        levels?.AllAge?.lookup?.rewards,
      );
      maxProductions =
        levels?.[maxEra]?.production?.options ||
        levels.AllAge.production?.options ||
        [];
      for (let [oIndex, option] of Object.entries(minProductions)) {
        ...
        for (let [pIndex, product] of Object.entries(option.products)) {
          ...
          if (product.type == 'unit') {
            if (product.amount != 0) {
              let iconId =
                product.unitTypeId == 'rogue' ? 'rogue'
                : product.unitTypeId.includes('champion') ? 'chivalry'
                : Unit.Types.filter(
                    (x) => x.unitTypeId == product.unitTypeId,
                  )[0].unitClass;
          ...
          if (product.type == 'genericReward') {
            ...
            let rewMax = Tooltips.genericEval(
              maxLookup[
                maxProductions?.[oIndex]?.products?.[pIndex]?.reward?.id
              ],
            );
          ...
          if (product.type == 'random') {
            ...
            for (let [rIndex, random] of Object.entries(product.products)) {
              ...
              if (random.product.type == 'resources') {
                for (let [res, amount] of Object.entries(
                  random.product.playerResources?.resources || {},
                )) {
                  if (amount != 0)
                    prods +=
                      srcLinks.icons(resMapper(res, 'goods')) +
                      range(
                        amount,
                        maxProductions?.[oIndex]?.products?.[pIndex]
                          ?.products?.[rIndex]?.product?.playerResources
                          ?.resources?.[res],
                      ) +
                      (res == 'each_special_goods_up_to_age' ?
                        '&nbsp;' + i18n('Boxes.Tooltip.Building.perEra')
                      : '');
                }
              }
              if (random.product.type == 'guildResources') {
                for (let [res, amount] of Object.entries(
                  random.product.guildResources?.resources || {},
                )) {
                  if (amount != 0)
                    prods +=
                      srcLinks.icons(resMapper(res, 'treasury_goods')) +
                      range(
                        amount,
                        maxProductions?.[oIndex]?.products?.[pIndex]
                          ?.products?.[rIndex]?.product?.guildResources
                          ?.resources?.[res],
                      );
                }
              }
              if (random.product.type == 'unit') {
                if (random.product.amount != 0) {
                  let iconId =
                    random.product.unitTypeId == 'rogue' ? 'rogue'
                    : random.product.unitTypeId.includes('champion') ?
                      'chivalry'
                    : Unit.Types.filter(
                        (x) => x.unitTypeId == random.product.unitTypeId,
                      )[0].unitClass;
                  prods +=
                    srcLinks.icons(iconId) +
                    range(
                      random.product.amount,
                      maxProductions?.[oIndex]?.products?.[
                        pIndex?.products?.[rIndex]?.product
                      ].amount,
                    );
                }
              }
              if (random.product.type == 'genericReward') {
                let rewBA = Tooltips.genericEval(
                  minLookup[random.product.reward.id],
                );
                let rewMax = Tooltips.genericEval(
                  maxLookup[
                    maxProductions?.[oIndex]?.products?.[pIndex]?.products?.[
                      rIndex
                    ]?.product?.reward?.id
                  ],
                );
                ...
                if (rewBA.icon + rewBA.name == rewMax.icon + rewBA.name) {
                  prods +=
                    rewBA.icon +
                    range(rewBA.amount, rewMax.amount) +
                    rewBA.fragment +
                    longSpan(rewBA.name);
                } else {
                  prods +=
      ...
      let costs = '';
      for ([resource, amount] of Object.entries(
        levels.AllAge?.buildResourcesRequirement?.cost?.resources || {},
      )) {
        if (amount > 0)
          costs += `<div>${srcLinks.icons(resource) + ' ' + span(amount)}</div>`;
      }
      //Chain Productions
      let first = true;
      for (let b of chain?.config?.bonuses || []) {
        if (Object.values(b.boosts).length > 0) {
          if (first) {
            provides +=
              '<tr><td style="text-wrap-mode:wrap;">' +
              chain.description +
              '</td></tr>';
            first = false;
          }
          ...
          provides +=
            srcLinks.icons(
              b.boosts[0].type + feature[b.boosts[0].targetedFeature],
            ) +
            ' ' +
            span(b.boosts[0].value) +
            Boosts.percent(b.boosts[0].type);
          provides += `</td></tr>`;
        }
      ...
      for (let [i, b] of Object.entries(chainMin?.config?.bonuses || [])) {
        for (j in Object.keys(b.boosts)) {
          ...
          provides +=
            srcLinks.icons(
              b.boosts[j].type + feature[b.boosts[j].targetedFeature],
            ) +
            ' ' +
            range(
              b.boosts[j].value,
              chainMax?.config?.bonuses[i].boosts[j].value,
            ) +
            Boosts.percent(b.boosts[0].type);
          provides += `</td></tr>`;
        }
      ...
      if (ally != '')
        out +=
          `<tr><th>${i18n('Boxes.Tooltip.Building.allyRooms')}</th></tr>` +
          ally;
      if (provides != '')
        out +=
          `<tr><th>${i18n('Boxes.Tooltip.Building.provides')}</th></tr>` +
          provides;
      if (prods != '')
        out +=
          `<tr><th>${i18n('Boxes.Tooltip.Building.produces')} ${pCount == 1 ? '(' + formatTime(levels.AllAge.production?.options?.[0].time || levels?.[minEra].production?.options?.[0].time) + ')' : ''}</th></tr>` +
          prods;
      if (costs != '')
        out +=
          `<tr><th>${i18n('Boxes.Tooltip.Building.costs')}</th></tr><tr><td class="multiCol">` +
          costs +
          `</td></tr>`;
      ...
      out +=
        levels.AllAge?.constructionTime?.time ?
          `<div>${srcLinks.icons('icon_time')}${formatTime(levels.AllAge.constructionTime.time)}</div>`
        : ``;
      ...
      if (traits != '')
        out +=
          `<tr><th>${i18n('Boxes.Tooltip.Building.traits')}</th></tr>` + traits;
    } else {
      let levels = Object.assign(
        {},
        ...(meta?.entity_levels?.map((x) => ({ [x.era]: x })) || []),
      );

      if (levels?.[minEra]) {
        maxEra =
          Technologies.EraNames[
            Math.max(...Object.keys(levels).map((x) => Technologies.Eras[x]))
          ];
      }
      let era = '',
        set = '',
      ...
      for (let a of meta.abilities || []) {
        ...
        if (a.__class__ == 'ChainStartAbility') {
          set =
            srcLinks.icons(a.chainId) +
            MainParser.BuildingChains[a.chainId].name +
            '</td></tr><tr><td style="text-wrap-mode:wrap;">' +
            a.description;
        }
        ...
        if (a.__class__ == 'NotPlunderableAbility') {
          traits +=
            `<tr><td>` +
            srcLinks.icons('eventwindow_plunder_repel') +
            i18n('Boxes.Tooltip.Building.noPlunder') +
            `</td></tr>`;
        }
        if (a.__class__ == 'AffectedByLifeSupportAbility') {
          traits +=
            `<tr><td>` +
            srcLinks.icons('life_support') +
            i18n('Boxes.Tooltip.Building.lifeSupport') +
            `</td></tr>`;
        }
      ...
      if (
        meta?.requirements?.min_era &&
        meta?.requirements?.min_era != 'MultiAge' &&
        era == ''
      ) {
      ...
      if (set != '') out += '<tr><td>' + set + '</td></tr>';
      if (info != '')
        out += '<tr><td style="text-wrap-mode:wrap;">' + info + '</td></tr>';
      ...
      if (meta.provided_population || meta.required_population) {
        ...
        (levels?.[minEra]?.provided_population &&
          levels?.[maxEra]?.provided_population) ||
        (levels?.[minEra]?.required_population &&
          levels?.[maxEra]?.required_population)
      ) {
      ...
      if (meta.provided_happiness) {
        ...
        levels?.[minEra]?.provided_happiness &&
        levels?.[maxEra]?.provided_happiness
      ) {
      ...
      if (
        levels?.[minEra]?.ranking_points &&
        levels?.[maxEra]?.ranking_points
      ) {
        provides += `<tr><td>${srcLinks.icons('rank') + ' ' + range(levels?.[minEra].ranking_points, levels?.[maxEra].ranking_points)}</td></tr>`;
      }

      for ([resource, amount] of Object.entries(
        meta?.static_resources?.resources || {},
      )) {
        if (amount > 0)
          provides += `<tr><td>${srcLinks.icons(resource) + ' ' + span(amount)}</td></tr>`;
      }

      let prods = '';
      if (meta.available_products) {
        if (
          levels?.[minEra]?.produced_money &&
          levels?.[maxEra]?.produced_money
        ) {
        ...
        for (let p of meta.available_products) {
          for (let [res, amount] of Object.entries(
            ...
            let t =
              meta?.available_products?.length != 1 ?
                '&nbsp;in ' + formatTime(p.production_time)
              : '';
        ...
        for (let a of abilityList.AddResourcesAbility || []) {
          for (let [res, amount] of Object.entries(
            a.additionalResources?.[minEra]?.resources || {},
          )) {
            if (amount != 0)
        ...
        for (let a of abilityList.RandomChestRewardAbility || []) {
          prods += `<tr><td><table class="randomProductions">`;
          for (let [id, rew] of Object.entries(
            a.rewards?.[minEra]?.possible_rewards,
          )) {
            prods += `<tr><td>`;
            let asset =
              rew?.reward?.type == 'resource' ?
                rew.reward.subType
              : rew.reward.iconAssetName;
            amountBA = rew.reward?.totalAmount || rew.reward?.amount;
            amountMax =
              a.rewards?.[maxEra]?.possible_rewards?.[id]?.reward
                ?.totalAmount ||
              a.rewards?.[maxEra]?.possible_rewards?.[id]?.reward.amount;
            if (
              rew.reward.type == 'chest' &&
              rew.reward?.possible_rewards?.[0]?.reward?.type == 'good'
            ) {
              asset = 'goods';
              amountBA =
                rew.reward?.possible_rewards?.[0]?.reward?.amount || amountBA;
              amountMax =
                a.rewards?.[maxEra]?.possible_rewards?.[id]?.reward
                  ?.possible_rewards?.[0]?.reward?.amount || amountMax;
            }
        ...
        for (let b of a.bonuses) {
          if (Object.values(b.boost).length > 0) {
            ...
            if (b.boost.AllAge) {
              boosts +=
                srcLinks.icons(
                  b.boost.AllAge.type + feature[b.boost.AllAge.targetedFeature],
                ) +
                ' ' +
                span(b.boost.AllAge.value) +
                Boosts.percent(b.boost.AllAge.type);
            }
            if (b.boost?.[minEra] && b.boost[maxEra]) {
              boosts +=
                srcLinks.icons(
                  b.boost?.[minEra].type +
                    feature[b.boost?.[minEra].targetedFeature],
                ) +
                ' ' +
                range(b.boost?.[minEra].value, b.boost[maxEra].value) +
                Boosts.percent(b.boost?.[minEra].type);
            }
            boosts += `</td></tr>`;
          } else {
            prods += `<tr><td>${b.level + 'x' + srcLinks.icons(a.setId)} ► `;
            if (b.revenue?.AllAge) {
              let [res, amount] = Object.entries(
                b.revenue?.AllAge?.resources,
              )[0];
              prods += srcLinks.icons(resMapper(res, 'goods')) + span(amount);
            }
            if (b.revenue?.[minEra] && b.revenue?.[maxEra]) {
              let [res, amount] = Object.entries(
                b.revenue?.[minEra]?.resources,
              )[0];
              prods +=
                srcLinks.icons(resMapper(res, 'goods')) +
                range(amount, b.revenue?.[maxEra].resources[res]);
            }
            prods += `</td></tr>`;
          }
        }
      ...
      let costs = '';
      for ([resource, amount] of Object.entries(
        meta?.requirements?.cost?.resources || {},
      )) {
        if (amount > 0)
          costs += `<div>${srcLinks.icons(resource) + ' ' + span(amount)}</div>`;
      }
      ...
      if (prods != '')
        out +=
          `<tr><th>${i18n('Boxes.Tooltip.Building.produces')} ${meta?.available_products?.length == 1 ? '(' + formatTime(meta.available_products[0].production_time) + ')' : ''}</th></tr>` +
          prods;
      if (costs != '')
        out +=
          `<tr><th>${i18n('Boxes.Tooltip.Building.costs')}</th></tr><tr><td class="multiCol">` +
      ...
      out +=
        meta.construction_time ?
          `<div>${srcLinks.icons('icon_time')}${formatTime(meta.construction_time)}</div>`
        : ``;
...
FoEproxy.addHandler(
  ...
  (data, postData) => {
    QIActions.hourlyBase = FHResourcesList.find(
      (x) => x.id == 'guild_raids_action_points',
    ).abilities.autoRefill.refillAmount;
  },
);
...
let QIActions = {
  ...
  setNext: (time) => {
    let timer = 3600000;
    let hourly =
      QIActions.hourlyBase +
      Boosts.Sums['guild_raids_action_points_collection'];
    ...
    if (time) {
      ...
      QIActions.count = Math.min(
        QIActions.count + amount * hourly,
        QIActions.capacity,
      );
  ...
  TT: () => {
    ...
    let fullAt =
      Math.ceil(
        (QIActions.capacity +
          (Boosts.Sums['guild_raids_action_points_capacity'] || 0) -
          QIActions.count) /
          hourly,
      ) *
        3600 +
...
FoEproxy.addHandler(
  ...
  async (data, postData) => {
    ...
    if (
      rewardIncidentSource !== 'battlegrounds_conquest' &&
      rewardIncidentSource !== 'guild_expedition_reward_notification'
    )
      return;
