...
FoEproxy.addHandler(
  ...
  async (data, postData) => {
    ...
    if (rewardIncidentSource == 'event_pass') {
      if (postData[0].requestData[0].indexOf('guild_raids') >= 0)
        rewardIncidentSource = 'guild_raids';
    }
    for (let reward of rewards) {
      ...
      await Stats.addReward(
        rewardIncidentSource,
        reward.amount || 0,
        reward.id,
      );
    }
...
FoEproxy.addHandler(
  ...
  async (data, postData) => {
    ...
    if (
      rewardIncidentSource != 'guild_raids' &&
      rewardIncidentSource.indexOf('guild_raids') >= 0
    )
      rewardIncidentSource = 'guild_raidsP'; //QI-Pass detection
    if (
      rewardIncidentSource.indexOf('event') < 0 &&
      !['guild_raids', 'guild_raidsP'].includes(rewardIncidentSource)
    )
      return; //exclude Main city collection "collect all", "aid_all"
    ...
    for (let reward of rewards) {
      ...
      if (rewardIncidentSource == 'guild_raids') {
        let ref = null;
        for (ref of Stats.QI.RewardLookUp?.[Stats.QI.currentNode]?.[
          reward.type + '#' + reward.subType
        ] || []) {
      ...
      for (let i = 0; i < n; i++) {
        let ris =
          rewardIncidentSource == 'guild_raidsP' ? 'guild_raids' : (
            rewardIncidentSource
          );
    ...
    for (let r of rewards) {
      if (r.reward.type == 'chest') {
        for (let c of r.reward.possible_rewards) {
          if (
            !Stats.QI.RewardLookUp[nodeId][
              c.reward.type + '#' + c.reward.subType
            ]
          )
            Stats.QI.RewardLookUp[nodeId][
              c.reward.type + '#' + c.reward.subType
        ...
        if (
          !Stats.QI.RewardLookUp[nodeId][r.reward.type + '#' + r.reward.subType]
        )
          Stats.QI.RewardLookUp[nodeId][
            r.reward.type + '#' + r.reward.subType
          ] = [];
        Stats.QI.RewardLookUp[nodeId][
          r.reward.type + '#' + r.reward.subType
        ].push(r.reward);
...
FoEproxy.addHandler(
  ...
  async (data, postData) => {
    ...
    const r =
      data.responseData?.resources?.resources || data.responseData?.resources;
    if (!r) return;
...
let Stats = {
  ...
  Init: () => {
    for (
      let Era = Technologies.Eras.BronzeAge;
      Era < Technologies.Eras.NextEra;
      Era++
    ) {
  ...
  isSelectedPlayerSources: () =>
    Stats.playerSources.includes(Stats.state.source),
  isSelectedTreasureSources: () =>
    Stats.treasureSources.includes(Stats.state.source),
  isSelectedUnitSources: () => Stats.unitSources.includes(Stats.state.source),
  isSelectedRewardSources: () =>
    Stats.rewardSources.includes(Stats.state.source),
  ...
  Show: (event) => {
    ...
    $('#statsBody').on('click', '[data-type]', function () {
      ...
      switch (type) {
        ...
        case 'selectSource':
          const isChangedToUnit =
            Stats.unitSources.includes(value) && !Stats.isSelectedUnitSources();
          const isChangedToPlayerSource =
            ['statsTreasurePlayerH', 'statsTreasurePlayerD'].includes(value) &&
            !Stats.isSelectedPlayerSources();
          const isChangedToClanTreasure =
            ['statsTreasureClanH', 'statsTreasureClanD'].includes(value) &&
            !Stats.isSelectedTreasureSources();
          const isChangedToReward =
            Stats.rewardSources.includes(value) &&
            !Stats.isSelectedRewardSources();
          const isChangedToGBG =
            Stats.gbgSources.includes(value) && !Stats.isSelectedGBGSources();

          if (isChangedToUnit) {
            // if Changed to units than select all eras by default
            Stats.state.eras = {};
            Object.keys(Stats.ResMap).map(
              (it) => (Stats.state.eras[it] = true),
            );
  ...
  Render: async () => {
    $('#statsBody').html(
      `<div class="options">${Stats.RenderOptions()}</div><div class="options-2"></div><div id="highcharts">Loading...</div>`,
    );
        ...
        if ($('#StatsDatePicker').length > 0) {
          ...
          Stats.DatePickerObj = new Litepicker({
            ...
            onSelect: async function (start, end) {
              ...
              if (
                end.getDate() === MainParser.getCurrentDate().getDate() &&
                end.getMonth() === MainParser.getCurrentDate().getMonth() &&
                end.getYear() === MainParser.getCurrentDate().getYear()
              )
  ...
  RenderOptions: () => {
    ...
    const btnSelectMyEra = Stats.RenderButton({
      ...
      isActive:
        ...
        selectedEras[0] === Technologies.EraNames[CurrentEraID],
      dataType: 'selectEras',
      disabled:
        !Stats.isSelectedPlayerSources() &&
        !Stats.isSelectedTreasureSources() &&
    ...
    const btnSelectNextEra = Stats.RenderButton({
      ...
      isActive:
        selectedEras.length === 1 &&
        selectedEras[0] === Technologies.EraNames[CurrentEraID + 1],
      dataType: 'selectEras',
      disabled:
        !Stats.isSelectedPlayerSources() &&
        !Stats.isSelectedTreasureSources() &&
    ...
    const btnSelectTwoLastEra = Stats.RenderButton({
      ...
      dataType: 'selectEras',
      value:
        Technologies.EraNames[CurrentEraID] +
        ',' +
        Technologies.EraNames[CurrentEraID - 1],
    });
    ...
    const btnGroupByEra = Stats.RenderBox({
      ...
      disabled:
        !Stats.isSelectedPlayerSources() && !Stats.isSelectedTreasureSources(),
    ...
    const chartTypes = ['line', 'streamgraph', 'delta'].map((it) =>
      Stats.RenderButton({
  ...
  formatRange: () => {
    let text = undefined;
    let dateStart = moment(MainParser.getCurrentDateTime()).subtract(
      10,
      'days',
    );
  ...
  RenderEraSwitchers: () => {
    ...
    const selectedErasI18n = Stats.getSelectedEras()
      .map((era) =>
        Technologies.Eras.hasOwnProperty(era) ?
          i18n('Eras.' + Technologies.Eras[era])
        : era,
      )
      .join(',');
  ...
  RenderBox: ({
    ...
    dataType,
    value,
  }) => `<label class="game-cursor${disabled ? ' hidden' : ''}">
			<input type="checkbox" data-type="${dataType}" data-value="${value}" class="filter-msg game-cursor" ${isActive ? 'checked' : ''}>${name}</label>`,
   ...
   * @returns {string}
   */
  RenderButton: ({ name, isActive, dataType, value, title, disabled }) =>
    `<button ${disabled ? 'disabled' : ''} class="btn btn-slim${!disabled && isActive ? ' btn-active' : ''} ${dataType}" data-type="${dataType}" data-value="${value}" title="${(title || '').replace(/"/g, '&quot;')}"><span>${name}</span></button>`,
   ...
   * @returns {string}
   */
  RenderTab: ({ name, isActive, dataType, value, title, disabled }) =>
    `<li ${disabled ? 'disabled' : ''} class="${value} ${!disabled && isActive ? 'active' : ''}" data-type="${dataType}" data-value="${value}" title="${(title || '').replace(/"/g, '&quot;')}"><a><span>&nbsp;</span></a></li>`,
  ...
  updateCharts: async (dates = null) => {
    ...
    if (Stats.isSelectedGBGSources()) {
      return await Stats.updateCommonChart(
        Stats.applyDeltaToSeriesIfNeed(await Stats.createGBGSeries()),
      );
    }

    if (Stats.isSelectedUnitSources()) {
      return await Stats.updateCommonChart(
        Stats.applyDeltaToSeriesIfNeed(await Stats.createUnitsSeries()),
      );
    }
    ...
    if (Stats.isSelectedTreasureSources()) {
      if (Stats.state.isGroupByEra) {
        return await Stats.updateCommonChart(
          Stats.applyDeltaToSeriesIfNeed(
            await Stats.createTreasureGroupByEraSeries(),
          ),
        );
      } else {
        return await Stats.updateCommonChart(
          Stats.applyDeltaToSeriesIfNeed(await Stats.createTreasureSeries()),
        );
      }
    }
    ...
    if (Stats.isSelectedPlayerSources()) {
      if (Stats.state.isGroupByEra) {
        return await Stats.updateCommonChart(
          Stats.applyDeltaToSeriesIfNeed(
            await Stats.createPlayerSourcesGroupByEraSeries(),
          ),
        );
      } else {
        return await Stats.updateCommonChart(
          Stats.applyDeltaToSeriesIfNeed(
            await Stats.createPlayerSourcesSeries(),
          ),
        );
      }
    }
  ...
  createGBGSeries: async () => {
    ...
    if (Stats.DatePickerFrom !== null && Stats.DatePickerTo !== null) {
      data = await IndexDB.db.statsGBGPlayers
        .where('date')
        .between(Stats.DatePickerFrom, Stats.DatePickerTo)
        .sortBy('date');
    } else {
  ...
  createUnitsSeries: async () => {
    ...
    if (Stats.DatePickerFrom !== null && Stats.DatePickerTo !== null) {
      let days =
        (Stats.DatePickerTo - Stats.DatePickerFrom) / 60 / 60 / 24 / 1000;

      let matchingDB = 'statsUnitsD';
      if (days <= 7) matchingDB = 'statsUnitsH';

      data = await IndexDB.db[matchingDB]
        .where('date')
        .between(Stats.DatePickerFrom, Stats.DatePickerTo)
        .sortBy('date');
    } else data = await IndexDB.db.statsUnitsD.orderBy('date').toArray();
    ...
    const series = filteredUnitIds.map((unitId) => {
      ...
      return {
        ...
        unitId,
        unitUrl: srcLinks.get(
          '/shared/unit_portraits/armyuniticons_50x50/armyuniticons_50x50_' +
            unitId +
            '.jpg',
          true,
        ),
  ...
  createTreasureGroupByEraSeries: async () => {
    ...
    if (Stats.DatePickerFrom !== null && Stats.DatePickerTo !== null) {
      ...
      if (days <= 7) matchingDB = 'statsTreasureClanH';

      data = await IndexDB.db[matchingDB]
        .where('date')
        .between(Stats.DatePickerFrom, Stats.DatePickerTo)
        .sortBy('date');
    } else data = await IndexDB.db.statsTreasureClanD.orderBy('date').toArray();
    ...
    const series = Stats.getSelectedEras().map((era) => {
      return {
        ...
        data: data.map(({ date, resources }) => [
          +date,
          Stats.ResMap[era].reduce(
            (acc, resName) => acc + (resources[resName] || 0),
            0,
          ),
        ]),
      };
  ...
  createTreasureSeries: async () => {
    ...
    if (selectedEras.length > 1) {
      ...
      selectedEras.forEach((era) => {
        ...
        Stats.ResMap[era].forEach((it, index) => {
          colors.push(
            Highcharts.color(hcColors[baseColor])
              .brighten(index * 0.05)
              .get(),
          );
        });
    ...
    const series = selectedResources.map((it) => {
      ...
      return {
        era:
          goodsData.era ? i18n('Eras.' + Technologies.Eras[goodsData.era]) : '',
        goodsId: it,
        name: goodsData.name,
        data: data.map(({ date, resources }) => {
  ...
  createPlayerSourcesGroupByEraSeries: async () => {
    ...
    if (Stats.DatePickerFrom !== null && Stats.DatePickerTo !== null) {
      ...
      if (days <= 7) matchingDB = 'statsTreasurePlayerH';
    ...
    if (chartType === 'delta') {
      ...
      series = series.map((s) => {
        if (isNegativeValuesAllowed) {
          s.data = s.data.map((it, index, array) => [
            it[0],
            index > 0 ? (it[1] || 0) - (array[index - 1][1] || 0) : 0,
          ]);
        } else {
          s.data = s.data.map((it, index, array) => [
            it[0],
            index > 0 ?
              Math.max(0, (it[1] || 0) - (array[index - 1][1] || 0))
            : 0,
          ]);
        }
  ...
  updateCommonChart: async ({
    ...
    pointFormat =
      pointFormat ||
      '<tr><td><span style="color:{point.color}">●</span> {series.name}:</td><td class="text-right"><b>{point.y}</b></td></tr>';
    ...
    Highcharts.chart('highcharts', {
      ...
      tooltip: {
        ...
        shared:
          series.length <= 8 ||
          series.filter(
            (x, index, array) => x.era == array[0].era && x.era != undefined,
  ...
  createRewardSeries: async () => {
    ...
    if (Stats.DatePickerFrom !== null && Stats.DatePickerTo !== null) {
      data = await IndexDB.db.statsRewards
        .where('date')
        .between(Stats.DatePickerFrom, Stats.DatePickerTo)
        .sortBy('date');
    } else data = await IndexDB.db.statsRewards.orderBy('date').toArray();
    ...
    data.forEach((it) => {
      ...
      groupedByRewardSource[type][it.reward] =
        groupedByRewardSource[type][it.reward] || 0;
      groupedByRewardSource[type][it.reward]++;
    });
    ...
    let serieData = Object.keys(seriesMapBySource).map((it) => {
      ...
      if (
        rewardInfo.type == 'resource' &&
        Stats.goodsSubTypes.includes(rewardInfo.subType)
      )
        rewardInfo.type = 'good';
      switch (rewardInfo.type) {
        case 'unit':
          if (rewardInfo.subType == 'rogue') {
            url = srcLinks.get(
              '/shared/unit_portraits/armyuniticons_50x50/armyuniticons_50x50_rogue.jpg',
              true,
            );
        ...
        default:
          url = '';
          if (
            rewardInfo.iconAssetName ||
            (rewardInfo.assembledReward &&
              rewardInfo.assembledReward.iconAssetName)
          ) {
            const icon =
              (
                rewardInfo.assembledReward &&
            ...
            if (
              rewardInfo.type == 'good' &&
              rewardInfo.iconAssetName == 'random_goods' &&
              rewardInfo.subType
            ) {
              url = srcLinks.get(
                `/shared/icons/reward_icons/reward_icon_random_goods.png`,
                true,
              );
            }
            if (rewardInfo.subType == 'fragment' && rewardInfo.subType) {
              if (
                rewardInfo.assembledReward.type == 'building' &&
                rewardInfo.subType
              ) {
                url = srcLinks.get(
                  `/city/buildings/${rewardInfo.assembledReward.subType.replace(/^(\w)_/, '$1_SS_')}.png`,
                  true,
                );
              }
            }
            ...
            url = srcLinks.get(
              `/city/buildings/${rewardInfo.subType.replace(/^(\w)_/, '$1_SS_')}.png`,
              true,
            );
          }
          if (url) {
    ...
    while (i < serieData.length) {
      let x = serieData.findIndex(
        (it, j) => j > i && it.name == serieData[i].name,
      );
      if (x >= 1) {
    ...
    if (Stats.state.filter != '') {
      serieData = serieData.filter((a) =>
        a.name.toLowerCase().includes(Stats.state.filter.toLowerCase()),
      );
    }
  ...
  updateRewardCharts: ({ series, title }) => {
    Highcharts.chart('highcharts', {
      ...
      tooltip: {
        ...
        headerFormat: '',
        pointFormat:
          '<span class="{point.iconClass}"></span>{point.pointImage} {point.name}: <b>{point.y} ({point.percentage:.1f}%)</b>',
      },
      accessibility: {
        point: {
          valueSuffix: '%',
   ...
   */
  getSelectedEras: () => {
    const selectedEras = Object.keys(Stats.state.eras).filter(
      (it) => Stats.state.eras[it],
    );
    // preserv order or era, filter again using ResMap keys
    return Object.keys(Stats.ResMap).filter((era) =>
  ...
  loadHighcharts: function () {
    ...
    async function load() {
      ...
      await Stats.promisedLoadCode(
        extUrl + 'vendor/highchart-8.0.4/foe/foe-theme.js',
      );
    ...
    IndexDB.db.statsRewards
      ...
      .catch((error) => {
        if (error.inner.name == 'ConstraintError') {
          setTimeout(() => {
            Stats.addReward(type, amount, reward);
          }, 1); //retry if two rewards came in "at the same time"
        } else {
          console.log(error);
        }
      });
...
let StockAlarm = {
  ...
  checkResources: async () => {
    ...
    if (alm.length == 0) return;
    await IndexDB.getDB();
    let x = IndexDB.db.statsTreasurePlayerH
      .orderBy('date')
      .reverse()
      .limit(2)
      .toArray();
  ...
  checkTreasury: async () => {
    ...
    if (alm.length == 0) return;
    await IndexDB.getDB();
    let x = IndexDB.db.statsTreasureClanH
      .orderBy('date')
      .reverse()
      .limit(2)
      .toArray();
  ...
  trigger: (alm) => {
    ...
    HTML.ShowToastMsg({
      ...
      text: replace(
        replace(i18n('Boxes.LowStock.LowStockMessage'), '%name%', alm.name),
        '%amount%',
        alm.value,
      ),
      type: 'warning',
  ...
  showDialogue: async () => {
    ...
    let xR = IndexDB.db.statsTreasurePlayerH
      .orderBy('date')
      .reverse()
      .limit(1)
      .toArray();
    ...
    let xT = IndexDB.db.statsTreasureClanH
      .orderBy('date')
      .reverse()
      .limit(1)
      .toArray();
    ...
    for (x of GoodsList) {
      ...
      if (R[x.id] > 0)
        OR.unshift(
          `<option value="${x.id}" data-name="${x.name}" class="${setClass ? 'LShighlight' : ''}">${x.name} (${R[x.id]})</option>`,
        );
      if (T[x.id] > 0)
        OT.unshift(
          `<option value="${x.id}" data-name="${x.name}" class="${setClass ? 'LShighlight' : ''}">${x.name} (${T[x.id]})</option>`,
        );
    }
    era = '';
    setClass = true;
    for (x of Unit.Types) {
      ...
      if (A[x.unitTypeId] > 0)
        OA.unshift(
          `<option value="${x.unitTypeId}" data-name="${x.name}" class="${setClass ? 'LShighlight' : ''}">${x.name} (${A[x.unitTypeId]})</option>`,
        );
    }
  ...
  addline: (type, id, name, value, repeat) => {
    ...
    switch (type) {
      ...
      case 'T':
        typeImg = srcLinks.get(
          '/shared/icons/reward_icons/reward_icon_treasury_goods.png',
          true,
        );
        break;
      case 'A':
        typeImg = srcLinks.get(
          '/shared/icons/reward_icons/reward_icon_all_units.png',
          true,
        );
        break;
    }
  ...
  remove: (type, id, name, value, repeat) => {
    let i = StockAlarm.Alarms.findIndex(
      (x) =>
        x.type == type &&
        x.id == id &&
        x.name == name &&
