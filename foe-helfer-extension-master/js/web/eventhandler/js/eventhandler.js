...
FoEproxy.addHandler(
  ...
  (data, postData) => {
    if (
      ...
      Settings.GetSetting('ShowPlayersMotivation')
    ) {
...
let EventHandler = {
  ...
  dbLoaded: new Promise((resolve) =>
    window.addEventListener('foe-helper#eventDBloaded', resolve, {
      capture: false,
      once: true,
      passive: true,
    }),
  ...
  Init: () => {
    ...
    let vShowHideColumns =
      JSON.parse(
        localStorage.getItem('MoppelHelper.Settings.ShowHideColumns'),
      ) || {};
  ...
  SaveSettings: () => {
    localStorage.setItem(
      'MoppelHelper.Settings.ShowHideColumns',
      JSON.stringify(EventHandler.ShowHideColumns),
    );
  },
  ...
  checkForDB: async (playerID) => {
    ...
    EventHandler.db.version(2).stores({
      Events:
        'eventid,date,eventtype,interactiontype,playerid,entityid,isneighbor,isguildmember,isfriend',
    });
    EventHandler.db.version(1).stores({
      Events:
        'eventid,date,eventtype,playerid,entityid,isneighbor,isguildmember,isfriend',
    });
  ...
  HandleEvents: (Events) => {
    ...
    for (let i = 0; i < Events.length; i++) {
      ...
      if (!Date) {
        //Datum nicht parsebar => überspringen
      ...
      if (Event['other_player']) {
        if (Event['other_player']['player_id'])
          PlayerID = Event['other_player']['player_id'];
        if (Event['other_player']['name'])
          PlayerName = Event['other_player']['name'];
    ...
    Promise.all(inserts).then((insertIsNewArr) => {
      ...
      if (InvalidDates.length > 0) {
        HTML.ShowToastMsg({
          ...
          text: HTML.i18nReplacer(i18n('Boxes.Investment.DateParseErrorDesc'), {
            InvalidDate: InvalidDates[0],
          }),
          type: 'error',
          hideAfter: 6000,
        });
        ...
        HTML.ShowToastMsg({
          ...
          text: HTML.i18nReplacer(
            count === 1 ?
              i18n('Boxes.Investment.PlayerFoundCount')
            : i18n('Boxes.Investment.PlayerFoundCounter'),
            { count: count },
          ),
          type: 'success',
          hideAfter: 2600,
      ...
      while ((match = matcher[day].exec(DateString)) !== null) {
        ...
        switch (day) {
          ...
          case 'date':
            ...
            refDate = moment([
              Number(match['groups']['year']) +
                (match['groups']['year'].length <= 2 ? 2000 : 0),
          ...
          default:
            ...
            if (refDate.isAfter(MainParser.getCurrentDate()))
              refDate = refDate.subtract(7 * 86400000); //Date is in the future => subtract 1 week
        }
  ...
  ShowMoppelHelper: () => {
    ...
    if ($('#moppelhelper').length === 0) {
      ...
      $('#moppelhelper').on('click', '.col-visibility-guildname', function () {
        EventHandler.ShowHideColumns.GuildName =
          !EventHandler.ShowHideColumns.GuildName;
        EventHandler.SaveSettings();
        EventHandler.CalcMoppelHelperTable();
      });
      ...
      $('#moppelhelper').on('click', '.col-visibility-points', function () {
        EventHandler.ShowHideColumns.Points =
          !EventHandler.ShowHideColumns.Points;
        EventHandler.SaveSettings();
        EventHandler.CalcMoppelHelperTable();
      });
  ...
  CalcMoppelHelperBody: async () => {
    ...
    /* Filters */
    h.push(
      '<div class="text-center dark-bg header"><strong class="title">' +
        i18n('Boxes.MoppelHelper.HeaderWarning') +
        '</strong><br></div>',
    );
    ...
    h.push(
      '<input type="checkbox" class="dropdown-checkbox" id="event-checkbox-toggle"><label class="dropdown-label game-cursor" for="event-checkbox-toggle">' +
        i18n('Boxes.Infobox.Filter') +
        '</label><span class="arrow"></span>',
    );
    h.push('<ul>');
    h.push(
      '<li><label class="game-cursor"><input type="checkbox" data-type="auction" class="filtermoppelevents game-cursor" ' +
        (EventHandler.FilterMoppelEvents ? 'checked' : '') +
        '> ' +
        i18n('Boxes.MoppelHelper.MoppelEvents') +
        '</label></li>',
    );
    h.push(
      '<li><label class="game-cursor"><input type="checkbox" data-type="gex" class="filtertavernvisits game-cursor" ' +
        (EventHandler.FilterTavernVisits ? 'checked' : '') +
        '> ' +
        i18n('Boxes.MoppelHelper.TavernVisits') +
        '</label></li>',
    );
    h.push(
      '<li><label class="game-cursor"><input type="checkbox" data-type="gbg" class="filterattacks game-cursor" ' +
        (EventHandler.FilterAttacks ? 'checked' : '') +
        '> ' +
        i18n('Boxes.MoppelHelper.Attacks') +
        '</label></li>',
    );
    h.push(
      '<li><label class="game-cursor"><input type="checkbox" data-type="trade" class="filterplunders game-cursor" ' +
        (EventHandler.FilterPlunders ? 'checked' : '') +
        '> ' +
        i18n('Boxes.MoppelHelper.Plunders') +
        '</label></li>',
    );
    h.push(
      '<li><label class="game-cursor"><input type="checkbox" data-type="level" class="filtertrades game-cursor" ' +
        (EventHandler.FilterTrades ? 'checked' : '') +
        '> ' +
        i18n('Boxes.MoppelHelper.Trades') +
        '</label></li>',
    );
    h.push(
      '<li><label class="game-cursor"><input type="checkbox" data-type="msg" class="filtergbs game-cursor" ' +
        (EventHandler.FilterGBs ? 'checked' : '') +
        '> ' +
        i18n('Boxes.MoppelHelper.GBs') +
        '</label></li>',
    );
    ...
    h.push('<div class="dropdown" style="float:right">');
    h.push(
      '<input type="checkbox" class="dropdown-checkbox" id="event-checkbox-col-sel"><label class="dropdown-label game-cursor" for="event-checkbox-col-sel">' +
        i18n('Boxes.MoppelHelper.Columns') +
        '</label><span class="arrow"></span>',
    );
    h.push('<ul>');
    for (var iColumn in EventHandler.ShowHideColumns) {
      ...
      switch (iColumn) {
        case 'GuildName':
          DropdownItemLabel = i18n('General.Guild');
          DropdownItemClass = 'col-visibility-guildname';
          break;
        case 'Era':
          DropdownItemLabel = i18n('Boxes.MoppelHelper.Era');
          DropdownItemClass = 'col-visibility-era';
          break;
        case 'Points':
          DropdownItemLabel = i18n('Boxes.MoppelHelper.Points');
          DropdownItemClass = 'col-visibility-points';
          break;
      }
      h.push(
        '<li><label class="game-cursor"><input type="checkbox" class="' +
          DropdownItemClass +
          ' game-cursor" ' +
          (EventHandler.ShowHideColumns[iColumn] ? 'checked' : '') +
          '> ' +
          DropdownItemLabel +
          '</label></li>',
      );
    ...
    h.push(
      '<li class="' +
        (!PlayerDictNeighborsUpdated ? 'disabled' : '') +
        ' ' +
        (EventHandler.CurrentPlayerGroup === 'Neighbors' ? 'active' : '') +
        '"><a class="toggle-players" data-value="Neighbors"><span>' +
        i18n('Boxes.MoppelHelper.Neighbors') +
        '</span></a></li>',
    );
    h.push(
      '<li class="' +
        (!PlayerDictGuildUpdated ? 'disabled' : '') +
        ' ' +
        (EventHandler.CurrentPlayerGroup === 'Guild' ? 'active' : '') +
        '"><a class="toggle-players" data-value="Guild"><span>' +
        i18n('Boxes.MoppelHelper.GuildMembers') +
        '</span></a></li>',
    );
    h.push(
      '<li class="' +
        (!PlayerDictFriendsUpdated ? 'disabled' : '') +
        ' ' +
        (EventHandler.CurrentPlayerGroup === 'Friends' ? 'active' : '') +
        '"><a class="toggle-players" data-value="Friends"><span>' +
        i18n('Boxes.MoppelHelper.Friends') +
        '</span></a></li>',
    );
    ...
    h.push(
      '<table id="moppelhelperTable" class="foe-table sortable-table exportable">',
    );
    h.push('</table>');
  ...
  CalcMoppelHelperTable: async () => {
    ...
    if (EventHandler.CurrentPlayerGroup === 'Friends') {
      if (!PlayerDictFriendsUpdated) {
        h.push(
          '<div class="text-center"><strong class="bigerror">' +
            i18n('Boxes.MoppelHelper.FriendsSocialTabTT') +
            '</strong></div>',
        );
        await $('#moppelhelperTable').html(h.join(''));
        return;
      }
      PlayerList = Object.values(PlayerDict).filter(
        (obj) => obj['IsFriend'] === true,
      );
    } else if (EventHandler.CurrentPlayerGroup === 'Guild') {
      if (!PlayerDictGuildUpdated) {
        h.push(
          '<div class="text-center"><strong class="bigerror">' +
            i18n('Boxes.MoppelHelper.GuildSocialTabTT') +
            '</strong></div>',
        );
        await $('#moppelhelperTable').html(h.join(''));
        return;
    ...
    h.push(
      '<th data-export="Rank" class="is-number ascending" data-type="moppelhelper">' +
        i18n('Boxes.MoppelHelper.Rank') +
        '</th>',
    );
    h.push('<th></th>');
    h.push(
      '<th data-export="Name" data-type="moppelhelper" class="name-col">' +
        i18n('Boxes.MoppelHelper.Name') +
        '</th>',
    );
    ...
    if (
      ...
      h.push(
        '<th data-export="GuildName" data-type="moppelhelper" class="name-col">' +
          i18n('General.Guild') +
          '</th>',
      );
    }
    if (EventHandler.ShowHideColumns.Era) {
      h.push(
        '<th class="is-number" data-export="Era" data-type="moppelhelper">' +
          i18n('Boxes.MoppelHelper.Era') +
          '</th>',
      );
    }
    if (EventHandler.ShowHideColumns.Points) {
      h.push(
        '<th data-export="Points" class="is-number" data-type="moppelhelper">' +
          i18n('Boxes.MoppelHelper.Points') +
          '</th>',
      );
    }
    ...
    for (let i = 0; i < EventHandler.MaxVisitCount; i++) {
      h.push(
        '<th data-export="Event' +
          (i + 1) +
          '" class="is-number" data-type="moppelhelper">' +
          i18n('Boxes.MoppelHelper.Event') +
          (i + 1) +
          '</th>',
      );
    }
    ...
    for (let i = 0; i < PlayerList.length; i++) {
      ...
      let Visits = await EventHandler.db['Events']
        .where('playerid')
      ...
      // Rank column
      h.push(
        '<td class="is-number" data-number="' +
          (i + 1) +
          '">#' +
          (i + 1) +
          '</td>',
      ...
      // Portrait column
      h.push(
        `<td><img style="max-width: 22px" src="${srcLinks.GetPortrait(Player['Avatar'])}" alt="${Player['PlayerName']}"></td>`,
      );

      // Player Name column
      h.push(
        '<td style="white-space:nowrap;text-align:left;" data-text="' +
          helper.str.cleanup(Player['PlayerName']) +
          '">',
      );

      h.push(`<span class="activity activity_${Player['Activity']}"></span> `);
      h.push(
        MainParser.GetPlayerLink(Player['PlayerID'], Player['PlayerName']),
      );

      // Player ID
      h.push(
        '<td style="display:none" data-text="' +
          Player['PlayerID'] +
          '">' +
          Player['PlayerID'] +
          '</td>',
      ...
      if (
        ...
        h.push(
          '<td style="white-space:nowrap;text-align:left;" data-text="' +
            helper.str.cleanup(Player['ClanName'] || '') +
            '">',
        );
        h.push(
          Player['ClanName'] ?
            MainParser.GetGuildLink(Player['ClanId'], Player['ClanName'])
          : '',
        );
      }
      ...
      if (EventHandler.ShowHideColumns.Era) {
        ...
        let pImg =
          (
            EventHandler.CurrentPlayerGroup === 'Neighbors' &&
            (pTime == -1 || pTime * 1000 > MainParser.getCurrentDateTime())
          ) ?
            pImage
          : '';
        h.push(
          `<td data-number="${Technologies.Eras[Player['Era']]}" exportvalue="${i18n('Eras.' + Technologies.Eras[Player['Era']])}">${pImg + i18n('Eras.' + Technologies.Eras[Player['Era']]) + pImg}</td>`,
        );
      }
      ...
      for (let j = 0; j < EventHandler.MaxVisitCount; j++) {
        if (j < Visits.length) {
          let Seconds =
            (MainParser.getCurrentDateTime() - Visits[j]['date'].getTime()) /
            1000;
          let Days = Seconds / 86400; //24*3600
          let StrongColor =
            Days < 3 * (j + 1) ?
              HTML.GetColorGradient(Days, 0, 3 * (j + 1), '00ff00', 'ffff00')
            : HTML.GetColorGradient(
                Days,
                3 * (j + 1),
                7 * (j + 1),
                'ffff00',
          ...
          let FormatedDays = HTML.i18nReplacer(
            i18n('Boxes.MoppelHelper.Days'),
            { days: Math.round(Days) },
          );
          let EventType = EventHandler.GetEventType(Visits[j]);

          h.push(
            '<td style="white-space:nowrap" class="events-image" data-number="' +
              Days +
              '"><span class="events-sprite-35 ' +
              EventType +
          ...
          h.push(
            '<td class="is-date" data-number="999999999"><strong style="color:#ff0000">' +
              i18n('Boxes.MoppelHelper.Never') +
              '</strong></td>',
          );
        }
      }
  ...
  GetEventType: (Event) => {
    if (
      Event['eventtype'] === 'social_interaction' &&
      (Event['interactiontype'] === 'motivate' ||
        Event['interactiontype'] === 'polish' ||
        Event['interactiontype'] === 'polivate_failed')
    )
      return 'MoppelEvent';
    ...
    if (
      Event['eventtype'] === 'social_interaction' &&
      Event['interactiontype'] === 'plunder'
    )
      return 'Plunder';
    if (Event['eventtype'] === 'trade_accepted') return 'Trade';
    if (
      Event['eventtype'] === 'great_building_built' ||
      Event['eventtype'] === 'great_building_contribution'
    )
      return 'GB';
    return 'Other';
  },
  ...
  ShowMoppelHelperSettingsButton: () => {
    let h = [];
    h.push(
      `<p class="text-center"><button class="btn" onclick="HTML.ExportTable($('#moppelhelperBody').find('.foe-table.exportable'), 'csv', 'MoppelHelper${EventHandler.CurrentPlayerGroup}')">${i18n('Boxes.General.ExportCSV')}</button></p>`,
    );
    h.push(
      `<p class="text-center"><button class="btn" onclick="HTML.ExportTable($('#moppelhelperBody').find('.foe-table.exportable'), 'json', 'MoppelHelper${EventHandler.CurrentPlayerGroup}')">${i18n('Boxes.General.ExportJSON')}</button></p>`,
    );
  ...
  DateShapes: (lng) => {
    const LngRegEx = {
      ...
      gr: {
        ...
        wednesday:
          /Τετάρτη στις (?<h>[012]?\d):(?<m>[0-5]?\d) (?<half>(a|p)m)/g,
