...
FoEproxy.addHandler('CardGameService', 'all', (data, postData) => {
  ...
  if (data.requestMethod == 'getHealthOffers') {
    cardGame.healthShop = {};
    let offers = [...data.responseData.healthOffers].sort(
      (a, b) => a.amount - b.amount,
    );
    for (h of offers) {
      let newhealth = cardGame.health + h.amount;
      if (newhealth > cardGame.maxHealth) continue;
      cardGame.healthShop[newhealth] =
        h.cost.resources[cardGame.data[cardGame.context].mainResource];
    }
  }
  if (data.requestMethod == 'buyCard') {
    cardGame.currencySpent.ability += cardGame.cardCost;
    cardGame.cardCost =
      data.responseData.cost.resources[
        cardGame.data[cardGame.context].mainResource
      ];
    cardGame.freshbuys = data.responseData.buyOptions
  ...
  if (data.requestMethod == 'redrawCard') {
    ...
    cardGame.redraw =
      data.responseData.redrawCost.resources[
        cardGame.data[cardGame.context].mainResource
      ];
    cardGame.card = {};
  }
  if (data.requestMethod == 'getOverview') {
    ...
    if (data.responseData.ongoingGame) {
      ...
      cardGame.cardsLeft =
        data.responseData.ongoingGame.playerState.drawPileCardIds;
      cardGame.cardOptions =
        data.responseData.ongoingGame.playerState.handCardIds;
      if (data.responseData.ongoingGame.playerState.handCardIds.length > 1) {
        cardGame.card = null;
      } else {
        cardGame.card = {
          ...cardGame.cards[
            data.responseData.ongoingGame.playerState.handCardIds[0]
          ],
        };
      }
      cardGame.level = Object.values(cardGame.nodes).filter(
        (x) => !!x.enemy,
      ).length;
      cardGame.isLastLevel =
        cardGame.nodes[data.responseData.ongoingGame.playerState.currentNodeId]
          .nextNodeIds.length == 0;
      cardGame.enemy =
        cardGame.nodes[
          data.responseData.ongoingGame.playerState.currentNodeId
        ].enemy;
      cardGame.redraw =
        data.responseData.ongoingGame.playerState.redrawCost.resources[
          cardGame.data[cardGame.context].mainResource
        ];
    } else {
      cardGame.cardsLeft = [];
  ...
  if (data.requestMethod == 'startLevel') {
    ...
    if (data.responseData.state) {
      ...
      cardGame.card = {
        ...cardGame.cards[data.responseData.state.playerState.handCardIds[0]],
      };
      cardGame.cardOptions = data.responseData.state.playerState.handCardIds;
      cardGame.level = 1;
      cardGame.isLastLevel =
        cardGame.nodes[data.responseData.state.playerState.currentNodeId]
          .nextNodeIds.length == 0;
      cardGame.enemy =
        cardGame.nodes[data.responseData.state.playerState.currentNodeId].enemy;
      cardGame.redraw =
        data.responseData.state.playerState.redrawCost.resources[
          cardGame.data[cardGame.context].mainResource
        ];
      ...
      try {
        ...
        cardGame.card = {
          ...cardGame.cards[data.responseData.playerState.handCardIds[0]],
        };
        cardGame.cardOptions = data.responseData.playerState.handCardIds;
        cardGame.level = 1;
        cardGame.isLastLevel =
          cardGame.nodes[data.responseData.playerState.currentNodeId]
            .nextNodeIds.length == 0;
        cardGame.enemy =
          cardGame.nodes[data.responseData.playerState.currentNodeId].enemy;
        cardGame.redraw =
          data.responseData.playerState.redrawCost.resources[
            cardGame.data[cardGame.context].mainResource
          ];
      } catch {
        cardGame.cardsLeft = [];
  ...
  if (['useCard'].includes(data.requestMethod)) {
    if (
      data.responseData?.nodeUpdates?.length == 2 ||
      (data.responseData?.nodeUpdates?.length == 1 &&
        data.responseData.playerState.state.value == 'card_buying')
    )
      cardGame.level += 1;
  }
  if (['useCard', 'finishCardBuying'].includes(data.requestMethod)) {
    cardGame.isLastLevel =
      cardGame.nodes[data.responseData.playerState.currentNodeId].nextNodeIds
        .length == 0;
    if (data.responseData?.nodeUpdates?.length > 0)
      cardGame.enemy =
        data.responseData.nodeUpdates[
          data.responseData.nodeUpdates.length - 1
        ].enemy;
    if (data.responseData?.playerState?.cardShop?.buyOptions)
  ...
  if (
    ['redrawCard', 'buyCard', 'getHealthOffers'].includes(data.requestMethod)
  ) {
    cardGame.showWarning(undefined);
  }
...
FoEproxy.addHandler('EventPassService', 'getPreview', (data, postData) => {
  if (['history_event'].includes(data.responseData.context))
...
FoEproxy.addHandler('RewardService', 'collectRewardSet', (data, postData) => {
  ...
  for (let r of data.responseData.reward.rewards) {
    if (Object.keys(cardGame.rewardcount).includes(r.iconAssetName))
      cardGame.rewardcount[r.iconAssetName] += r.amount;
  }
  cardGame.showCardsList();
});
...
let cardGame = {
  ...
  data: {
    halloween_event: {
      ...
      imgPath: {
        enemyDeck:
          '/shared/seasonalevents/halloween/event/halloween_card_enemy_deck_icon.png',
        playerHealth:
          '/shared/seasonalevents/halloween/event/halloween_card_player_health_icon.png',
        spentAbility:
          '/shared/seasonalevents/halloween/event/halloween_card_enemy_reward_card_icon.png',
        spentHealth:
          '/shared/seasonalevents/halloween/event/halloween_life_option_2.png',
        spentRedraw:
          '/shared/seasonalevents/halloween/event/halloween_card_enemy_reward_card_icon.png',
    ...
    history_event: {
      ...
      imgPath: {
        enemyDeck:
          '/shared/seasonalevents/history/event/history_battle_fight_button_icon.png',
        playerHealth:
          '/shared/seasonalevents/history/event/history_health_currency.png',
        spentAbility:
          '/shared/seasonalevents/history/event/history_battle_icon_special.png',
        spentHealth:
          '/shared/seasonalevents/history/event/history_life_option_2.png',
        spentRedraw:
          '/shared/seasonalevents/history/event/history_card_player_deck_icon.png',
  ...
  showWarning: (warning) => {
    ...
    if ($('#cardGameFightBlocker').length === 0) {
      ...
      blocker.src = srcLinks.get(
        '/city/gui/great_building_bonus_icons/great_building_bonus_plunder_repel.png',
        true,
      );
      blocker.title = warning;
      blocker.className = cardGame.context + ' helper-blocker';
      $('#game_body')[0].append(blocker);
      $('#cardGameFightBlocker').on('click', () => {
        $('#cardGameFightBlocker').remove();
      });
    }
  },
  ...
  checkHealth: () => {
    ...
    if (cardGame.card.cardType.value == 'ability') {
      for (a of cardGame.card.abilities) {
        if (
          a.__class__ == 'CardGameModifyHealthAbility' &&
          a.target.value == 'self'
        ) {
      ...
      minHealth +=
        (
          cardGame.card.cardFactionId ==
          cardGame.weakAgainst[cardGame.enemy.card.cardFactionId]
        ) ?
          cardGame.enemy.card.abilities[1].amount
        : 0;
    ...
    let warning =
      maxHealth <= 0 ? i18n('Boxes.cardGame.WarningCertainDeath')
      : minHealth <= 0 ? i18n('Boxes.cardGame.WarningPossibleDeath')
      : undefined;
    /*
  ...
  showCardsList: () => {
    ...
    for (let c of cards) {
      ...
      if (card?.cardType?.value == 'ability') {
        for (a of card.abilities) {
          if (a.__class__ == 'CardGameOpponentSelfAttackAbility') {
            dmg[c]['min'] -=
              cardGame.enemy.card.abilities[0].maxValue * a.factor;
            dmg[c]['max'] -=
              cardGame.enemy.card.abilities[0].minValue * a.factor;
          }
          if (
            a.__class__ == 'CardGameModifyHealthAbility' &&
            a.target.value == 'opponent'
          ) {
            dmg[c]['min'] -= a.maxValue;
            dmg[c]['max'] -= a.minValue;
          }
          if (
            a.__class__ == 'CardGameModifyHealthPercentageAbility' &&
            a.target.value == 'opponent'
          ) {
            dmg[c]['min'] -= Math.round(
              (cardGame.enemy.maxHealth * a.percentage) / 100,
            );
            dmg[c]['max'] -= Math.round(
              (cardGame.enemy.maxHealth * a.percentage) / 100,
            );
          }
        }
        ...
        dmg[c]['min'] -=
          card.abilities[0].maxValue +
          ((
            cardGame.enemy.card.cardFactionId ==
            cardGame.weakAgainst[cardGame.cards[c].cardFactionId]
          ) ?
            cardGame.cards[c].abilities[1].amount
          : 0);
        dmg[c]['max'] -=
          card.abilities[0].minValue +
          ((
            cardGame.enemy.card.cardFactionId ==
            cardGame.weakAgainst[cardGame.cards[c].cardFactionId]
          ) ?
    ...
    if (cardGame.context == 'halloween_event') {
      for (let r in cardGame.rewardcount) {
        ...
        h +=
          `<img style="height:40px" src="${srcLinks.get(`/shared/icons/reward_icons/reward_icon_${r}.png`, true)}">` +
          cardGame.rewardcount[r] +
          `&nbsp;&nbsp;`;
      }
    }
    ...
    for (let c of cards) {
      h += `<tr ${
        cardGame.cardOptions.includes(c) && cardGame.cardOptions.length > 1 ?
      ...
      if (cardGame.context == 'halloween_event') {
        ...
        highlight = '';
        if (
          cardGame.cards[c]?.cardType?.value != 'ability' &&
          cardGame.cards[c].abilities?.[1]
        ) {
        ...
        let img =
          cardGame.cards[c]?.cardType?.value == 'ability' ?
            ''
          : `style="background-image:url('${srcLinks.get(imgs.cards + cardGame.cards[c].cardFactionId + '.png', true)}')"`;
        ...
        h += `<td class="bonus ${highlight}"><div><span style=background-image:url('${
          cardGame.cards[c]?.cardType?.value == 'ability' ? ''
          : cardGame.weakAgainst[cardGame.cards[c].cardFactionId] == '' ? ''
          : srcLinks.get(
              imgs.cards +
        ...
        }')"></span>${cardGame.cards[c].abilities?.[1]?.amount ? -cardGame.cards[c].abilities[1].amount : ''}</div></td>`;
      }
      h += `</tr>`;
    }
    h += '</table>';
