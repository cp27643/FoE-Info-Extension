...
let Notice = {
  ...
  buildBox: (event = true) => {
    ...
    // global send is deactivated
      ...
      $('#noticesBody')
        .addClass('global-send-required-bg')
        .append(
          $('<span />')
            .addClass('global-send-required')
            .html(i18n('Boxes.Notice.GlobalSendRequired')),
        );
    }
  },
  ...
  prepareContent: () => {
    ...
    if (Notice.notes) {
      for (let i in Notice.notes) {
        if (!Notice.notes.hasOwnProperty(i)) {
          break;
        }
        ...
        if (n['player_group']) {
          switch (n['player_group']) {
            case 'guild':
              tabName = `🛡 ${i18n('Boxes.Notice.SelectPlayerGroupGuild')}`;
              break;
            case 'friend':
              tabName = `👩🏼🤝🧑 ${i18n('Boxes.Notice.SelectPlayerGroupFriend')}`;
              break;
            case 'neighbor':
              tabName = `⚔ ${i18n('Boxes.Notice.SelectPlayerGroupNeighbor')}`;
              break;
          }
        } else {
        ...
        tab.push(
          `<li><a href="#tab-${n['id']}" data-id="${n['id']}"><span>${tabName}</span></a></li>`,
        );

        for (let x in n['items']) {
          if (!n['items'].hasOwnProperty(x)) {
            break;
          }
          ...
          subtab.push(
            `<li><a href="#subtab-${itm['id']}" data-id="${itm['id']}" title="${itm['tab']}"><span>${itm['tab']}</span></a></li>`,
          );

          subdiv.push(
            `<div id='subtab-${itm['id']}' class="sub-tab" spellcheck="false" data-parent="${n['id']}" data-id="${itm['id']}">`,
          );
        ...
        div.push(
          `<div id='tab-${n['id']}' class="notice-wrapper">${subcontent}</div>`,
        );
      }
  ...
  Listener: () => {
    ...
    $('body').on(
      'click',
      '#notices-modalclose, #notices-modal-playersclose',
      function () {
  ...
  ShowModal: (type, id) => {
    let title =
        type === 'grp' ?
          i18n('Boxes.Notice.NewGroup')
        : i18n('Boxes.Notice.NewSide'),
      txt;
    ...
    inp.attr({
      ...
      placeholder:
        type === 'grp' ?
          i18n('Boxes.Notice.GroupName')
        : i18n('Boxes.Notice.SideName'),
      class: `inp-${type}-name`,
      'data-id': id,
    });
    ...
    btn
      .attr({
        ...
        onclick:
          type === 'itm' ?
            `Notice.SaveItemModal('${id === 'new' ? 'new' : id}')`
          : `Notice.SaveModal('${type}', '${id === 'new' ? 'new' : id}')`,
      })
    ...
    if (id !== 'new') {
      ...
      delBtn
        ...
        .text(
          type === 'grp' ?
            i18n('Boxes.Notice.DeleteGroup')
          : i18n('Boxes.Notice.DeleteItem'),
        );
  ...
  SaveItemModal: (id) => {
    let nN = $('.inp-itm-name').val(),
      ...
      sortVal =
        !$(`.inp-itm-sort`).val() ||
        $(`#tab-${grp}`).find('ul.vertical li').length + 1;
    ...
    MainParser.send2Server(
      ...
      (resp) => {
        if (resp['status'] === 'ERROR') {
          ...
          Notice.ActiveTab =
            Notice.notes.findIndex((idx) => idx.id === grp) + 1;
      ...
      (resp) => {
        if (resp['status'] === 'ERROR') {
          ...
          let grpIdx = Notice.notes.findIndex((g) => g.id === grpID),
            itmIdx = Notice.notes[grpIdx].items.findIndex(
              (i) => i.id === itmID,
            );
  ...
  ShowPlayerModal: (type, grp) => {
    ...
    const playerObj = Object.values(Notice.Players).filter(
      (v) => v[mapper[type]] === true,
    );
  ...
  SavePlayerToGroup: (id) => {
    ...
    const player = group.items.find(
      (e) => e.tab === PlayerDict[id]['PlayerName'],
    );
    ...
    Notice.ActiveTab =
      Notice.notes.findIndex((idx) => idx.id === Notice.ActualGrp) + 1;
  ...
  BuildSettingButtons: () => {
    ...
    $('ul.horizontal').sortable({
      ...
      stop: function () {
        ...
        MainParser.send2Server(
          { type: 'grp', grps: grps, apiToken: Notice.ApiToken },
          'Notice/sort',
          (resp) => {
            if (resp['status'] === 'ERROR') {
              HTML.ShowToastMsg({
    ...
    $('.notice-wrapper').each(function () {
      ...
      div.find('ul.vertical').sortable({
        ...
        stop: function (e, ui) {
          ...
          MainParser.send2Server(
            { type: 'itm', itms: itms, apiToken: Notice.ApiToken },
            'Notice/sort',
            (resp) => {
              if (resp['status'] === 'ERROR') {
                HTML.ShowToastMsg({
  ...
  DeleteElement: (type, id) => {
    MainParser.send2Server(
      { type: type, id: id, apiToken: Notice.ApiToken },
      'Notice/del',
      (resp) => {
        if (resp['status'] === 'ERROR') {
          HTML.ShowToastMsg({
