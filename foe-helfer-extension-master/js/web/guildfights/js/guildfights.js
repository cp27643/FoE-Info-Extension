...
FoEproxy.addWsHandler(
  ...
  (data) => {
    ...
    if (data.responseData.signal === 'focus') {
      ...
      let provinceName = GuildFights.MapData.map.provinces.find(
        (x) => x.id === provinceId,
      );
      $(
    ...
    GuildFights.GlobalRankingTimeout = setTimeout(() => {
      if (data.responseData['stateId'] !== 'participating') {
        GuildFights.CurrentGBGRound =
          parseInt(data.responseData['startsAt']) - 259200;

        if (
          GuildFights.curDateFilter === null ||
          GuildFights.curDateEndFilter === null
        ) {
          GuildFights.curDateFilter = moment
            .unix(GuildFights.CurrentGBGRound)
            .subtract(11, 'd')
            .format('YYYYMMDD');
          GuildFights.curDateEndFilter = moment
            .unix(GuildFights.CurrentGBGRound)
            .format('YYYYMMDD');
        }

        GuildFights.HandlePlayerLeaderboard(
...
FoEproxy.addHandler('TimerService', 'getTimers', (data, postData) => {
  ...
  data.responseData
    .filter((t) => t.type == 'battlegroundsAttrition')
    .forEach((t) => {
      if (!t.time) return;
      serverMidnight = moment.unix(t.time);

      GuildFights.serverOffset =
        serverMidnight.format('HH') * 60 + serverMidnight.format('mm') * 1;
...
let GuildFights = {
  ...
  showOwnSectors: 0,
  serverOffset: JSON.parse(
    localStorage.getItem('GuildFights.serverOffset') || 'null',
  ),
  ...
  checkForDB: async (playerID) => {
    ...
    GuildFights.db.version(1).stores({
      snapshots:
        '&[player_id+gbground+time],[gbground+player_id], [date+player_id], gbground',
      history: '&gbground',
    });
  ...
  HandlePlayerLeaderboard: async (d) => {
    ...
    if (localStorage.getItem('GuildFights.NewAction') !== null) {
      GuildFights.PrevAction = JSON.parse(
        localStorage.getItem('GuildFights.NewAction'),
      );
      GuildFights.PrevActionTimestamp = parseInt(
        localStorage.getItem('GuildFights.NewActionTimestamp'),
      );
    ...
    await GuildFights.UpdateDB('history', {
      participation: players,
      sumNegotiations: sumNegotiations,
      sumBattles: sumBattles,
    });

    GuildFights.GBGHistoryView = false;
    GuildFights.NewAction = players;
    localStorage.setItem(
      'GuildFights.NewAction',
      JSON.stringify(GuildFights.NewAction),
    );

    GuildFights.NewActionTimestamp = moment().unix();
    localStorage.setItem(
      'GuildFights.NewActionTimestamp',
      GuildFights.NewActionTimestamp,
    );
  ...
  SetBoxNavigation: async (gbground) => {
    let h = [];
    let i = 0;
    let PlayerBoxSettings =
      JSON.parse(localStorage.getItem('GuildFightsPlayerBoxSettings')) || '{}';

    GuildFights.PlayerBoxSettings.showRoundSelector =
      PlayerBoxSettings.showRoundSelector !== undefined ?
        PlayerBoxSettings.showRoundSelector
      : GuildFights.PlayerBoxSettings.showRoundSelector;
    GuildFights.PlayerBoxSettings.showLogButton =
      PlayerBoxSettings.showLogButton !== undefined ?
        PlayerBoxSettings.showLogButton
      : GuildFights.PlayerBoxSettings.showLogButton;
    GuildFights.PlayerBoxSettings.showProgressFilter =
      PlayerBoxSettings.showProgressFilter !== undefined ?
        PlayerBoxSettings.showProgressFilter
      : GuildFights.PlayerBoxSettings.showProgressFilter;

    if (
      GuildFights.GBGAllRounds === undefined ||
      GuildFights.GBGAllRounds === null
    ) {
      // get all available GBG entires
      const gbgRounds = await GuildFights.db.history
        .where('gbground')
        .above(0)
        .keys();
      gbgRounds.sort(function (a, b) {
        return b - a;
      });
    ...
    if (
      !gbground &&
      GuildFights.GBGAllRounds &&
      GuildFights.GBGAllRounds.length
    ) {
    ...
    if (
      ...
      if (GuildFights.PlayerBoxSettings.showRoundSelector) {
        h.push(
          `${i18n('Boxes.GuildMemberStat.GBFRound')} <button class="btn btn-set-week" data-week="${previousweek}"${previousweek === null ? ' disabled' : ''}>&lt;</button> `,
        );
        h.push(`<select id="gbg-select-gbground">`);

        GuildFights.GBGAllRounds.forEach((week) => {
          h.push(
            `<option value="${week}"${gbground === week ? ' selected="selected"' : ''}>` +
              moment.unix(week).subtract(11, 'd').format(i18n('Date')) +
              ` - ` +
              moment.unix(week).format(i18n('Date')) +
              `</option>`,
          );
        });
        ...
        h.push(`</select>`);
        h.push(
          `<button class="btn btn-set-week last" data-week="${nextweek}"${nextweek === null ? ' disabled' : ''}>&gt;</button>`,
        );
      }
      ...
      if (gbground === GuildFights.CurrentGBGRound) {
        ...
        if (GuildFights.PlayerBoxSettings.showProgressFilter === 1) {
          h.push(
            `<button id="gbg_filterProgressList" title="${HTML.i18nTooltip(i18n('Boxes.GuildFights.ProgressFilterDesc'))}" class="btn" disabled>&#8593;</button>`,
          );
        }

        if (GuildFights.PlayerBoxSettings.showLogButton === 1) {
          h.push(
            `<button id="gbg_showLog" class="btn">${i18n('Boxes.GuildFights.SnapshotLog')}</button>`,
          );
        }
        h.push(`</div>`);
      }
      ...
      .done(function () {
        ...
        $('#gbg-select-gbground')
          ...
          .on('change', function () {
            ...
            if (
              !GuildFights.GBGAllRounds.includes(week) ||
              week === GuildFights.CurrentGBGRound
            ) {
              return;
            }
        ...
        $('button#gbg_showLog')
          .off('click')
          .on('click', function () {
            GuildFights.curDetailViewFilter = {
              content: 'filter',
              gbground: GuildFights.CurrentGBGRound,
            };
  ...
  ToggleProgressList: (id) => {
    ...
    if (act === 'hide') {
      if (nelem.length !== 0) {
        ...
        localStorage.setItem(
          'GuildFightsPlayerBoxSettings',
          JSON.stringify(GuildFights.PlayerBoxSettings),
        );
   ...
   */
  SetTabs: (id) => {
    GuildFights.Tabs.push(
      '<li class="' +
        id +
        ' game-cursor"><a href="#' +
        id +
        '" class="game-cursor"><span>&nbsp;</span></a></li>',
    );
  },
  ...
  GetTabs: () => {
    return (
      '<ul class="horizontal dark-bg">' + GuildFights.Tabs.join('') + '</ul>'
    );
  },
  ...
  SetTabContent: (id, content) => {
    ...
    let style =
      GuildFights.TabsContent.length > 0 ? ' style="display:none"' : '';

    GuildFights.TabsContent.push(
  ...
  ShowDetailViewBox: (d) => {
    ...
    if ($('#GildPlayersDetailView').length === 0) {
      ...
      if (localStorage.getItem('GildPlayersDetailViewCords') === null) {
        ptop =
          $('#GildPlayers').length !== 0 ? $('#GildPlayers').position().top : 0;
        pright =
          $('#GildPlayers').length !== 0 ?
            $('#GildPlayers').position().left + $('#GildPlayers').width() + 10
          : 0;
        $('#GildPlayersDetailView')
          .css('top', ptop + 'px')
          .css('left', pright * 1 + 'px');
      }
    }
  ...
  BuildPlayerContent: async (gbground) => {
    ...
    if (
      ...
      let d = await GuildFights.db.history
        .where({ gbground: gbground })
        .toArray();
    ...
    for (let i in GuildFights.GBGRound) {
      ...
      if (GuildFights.PrevAction !== null && histView === false) {
        let playerOld = GuildFights.PrevAction.find(
          (p) => p['player_id'] === playerNew['player_id'],
        );
        ...
        if (playerOld !== undefined) {
          if (playerOld['negotiationsWon'] < playerNew['negotiationsWon']) {
            diffNegotiations =
              playerNew['negotiationsWon'] - playerOld['negotiationsWon'];
            negotaionAddOn =
          ...
          if (playerOld['attrition'] < playerNew['attrition']) {
            ...
            attritionAddOn =
              ' <small class="text-success">&#8593; ' + diffAttr + '</small>';
            change = true;
          }
        }
      }
      ...
      b.push(
        '<tr data-player="' +
          ...
          '" class="' +
          newProgressClass +
          (!histView ? 'showdetailview ' : '') +
          (playerNew['player_id'] === ExtPlayerID ? 'mark-player ' : '') +
          (change === true ? 'bg-green' : '') +
          '">',
      );
      ...
      b.push(
        '<td class="tdmin"><img src="' +
          srcLinks.GetPortrait(playerNew['avatar']) +
          '" alt=""></td>',
      );
    ...
    t.push(
      '<table id="GildPlayersTable" class="exportable foe-table' +
        (histView === false ? ' chevron-right' : '') +
        '">',
    );
    ...
    t.push(
      '<th colspan="3" data-export3="Player">' +
        i18n('Boxes.GuildFights.Player') +
        '</th>',
    );
    t.push(
      '<th class="text-center" data-export="Negotiations"><span class="negotiation" title="' +
        HTML.i18nTooltip(i18n('Boxes.GuildFights.Negotiations')) +
        '"></span> <strong class="text-warning">(' +
        HTML.Format(tN) +
        ')</strong></th>',
    );
    t.push(
      '<th class="text-center" data-export="Fights"><span class="fight" title="' +
        HTML.i18nTooltip(i18n('Boxes.GuildFights.Fights')) +
        '"></span> <strong class="text-warning">(' +
        HTML.Format(tF) +
        ')</strong></th>',
    );
    t.push(
      '<th class="text-center" data-export="Total">' +
        i18n('Boxes.GuildFights.Total') +
        ' <strong class="text-warning">(' +
        HTML.Format(tNF) +
        ')</strong></th>',
    );
    t.push(
      '<th class="text-center" data-export="Attrition">' +
        i18n('Boxes.GuildFights.Attrition') +
        ' <strong class="text-warning">(' +
        HTML.Format(tA) +
        ')</strong></th>',
    );
    ...
    $('#gbgContentWrapper')
      ...
      .promise()
      .done(function () {
        $('#GildPlayersBody tr.showdetailview')
          .off('click')
          .on('click', function () {
            ...
            if ($('#GildPlayersDetailView').length === 0) {
              ...
              GuildFights.BuildDetailViewContent(
                GuildFights.curDetailViewFilter,
              );
            }
          });
    ...
    if ($('#GildPlayersHeader .title').find('.time-diff').length === 0) {
      $('#GildPlayersHeader .title').append(
        $('<small />').addClass('time-diff'),
      );
    }
    ...
    if (GuildFights.PrevActionTimestamp !== null) {
      ...
      $('.time-diff').text(
        HTML.i18nReplacer(i18n('Boxes.GuildFights.LastSnapshot'), {
          time: time,
        }),
      );
    }
  ...
  BuildDetailViewContent: async (d) => {
    ...
    if (content === 'player') {
      detaildata = await GuildFights.db.snapshots
        .where({ gbground: gbground, player_id: player_id })
        .toArray();

      playerName = detaildata[0].name;
      dailyFights = detaildata
        .reduce(
          function (res, obj) {
      ...
      h.push(
        '<div class="pname dark-bg text-center">' +
          playerName +
          ': ' +
          moment.unix(gbground).subtract(11, 'd').format(i18n('DateShort')) +
          ` - ` +
          moment.unix(gbground).format(i18n('Date')) +
          '</div>',
      );
      h.push(
        '<p class="dark-bg" style="padding:5px;margin:0;">' +
          i18n('Boxes.GuildFights.SnapShotLogDisclaimer') +
          '</p>',
      );
      h.push(
        '<table id="gbgPlayerLogTable" class="foe-table gbglog"><thead class="sticky">',
      );
      h.push('<tr class="sorter-header">');
      h.push(
        '<th class="is-number" data-type="gbg-playerlog-group">' +
          i18n('Boxes.GuildFights.Date') +
          '</th>',
      );
      h.push(
        '<th class="is-number text-center" data-type="gbg-playerlog-group"><span class="negotiation" title="' +
          HTML.i18nTooltip(i18n('Boxes.GuildFights.Negotiations')) +
          '"></span></th>',
      );
      h.push(
        '<th class="is-number text-center" data-type="gbg-playerlog-group"><span class="fight" title="' +
          HTML.i18nTooltip(i18n('Boxes.GuildFights.Fights')) +
          '"></span></th>',
      );
      h.push(
        `<th class="is-number text-center" data-type="gbg-playerlog-group">${i18n('Boxes.GuildFights.Total')}</th>`,
      );
      h.push('</tr>');
      ...
      dailyFights.forEach((day) => {
        ...
        h.push(
          `<td class="is-number" data-number="${day.time}">${moment.unix(day.time).format(i18n('Date'))}</td>`,
        );
        h.push(
          `<td class="is-number text-center" data-number="${day.negotiations}">${HTML.Format(day.negotiations)}</td>`,
        );
        h.push(
          `<td class="is-number text-center" data-number="${day.battles}">${HTML.Format(day.battles)}</td>`,
        );
        h.push(
          `<td class="is-number text-center" data-number="${sum}">${HTML.Format(sum)}</td>`,
        );
        h.push('</tr>');
      });
      ...
      detaildata = await GuildFights.db.snapshots
        ...
        .and(function (item) {
          return (
            item.date >= GuildFights.curDateFilter &&
            item.date <= GuildFights.curDateEndFilter
          );
        })
        .toArray();
      ...
      h.push(
        '<div class="datetimepicker sticky"><button id="gbgLogDatepicker" class="btn">' +
          GuildFights.formatRange() +
          '</button></div>',
      );
      h.push(
        '<table id="GuildFightsLogTable" class="foe-table gbglog"><thead>',
      );
      h.push('<tr class="sorter-header">');
      h.push(
        '<th class="is-number" data-type="gbg-log-group">' +
          i18n('Boxes.GuildFights.Date') +
          '</th>',
      );
      h.push(
        '<th class="case-sensitive" data-type="gbg-log-group">' +
          i18n('Boxes.GuildFights.Player') +
          '</th>',
      );
      h.push(
        '<th class="is-number text-center" data-type="gbg-log-group"><span class="negotiation" title="' +
          HTML.i18nTooltip(i18n('Boxes.GuildFights.Negotiations')) +
          '"></span></th>',
      );
      h.push(
        '<th class="is-number text-center" data-type="gbg-log-group"><span class="fight" title="' +
          HTML.i18nTooltip(i18n('Boxes.GuildFights.Fights')) +
          '"></span></th>',
      );
      h.push(
        `<th class="is-number text-center" data-type="gbg-log-group">${i18n('Boxes.GuildFights.Total')}</th>`,
      );
      h.push('</tr>');
      ...
      detaildata.forEach((e) => {
        ...
        h.push(
          `<td class="is-number" data-number="${e.time}">${moment.unix(e.time).format(i18n('DateTime'))}</td>`,
        );
        h.push(
          `<td class="case-sensitive" data-text="${helper.str.cleanup(e.name)}">${e.name}</td>`,
        );
        h.push(
          `<td class="is-number text-center" data-number="${e.negotiations}">${HTML.Format(e.negotiations)}</td>`,
        );
        h.push(
          `<td class="is-number text-center" data-number="${e.battles}">${HTML.Format(e.battles)}</td>`,
        );
        h.push(
          `<td class="is-number text-center" data-number="${sum}">${HTML.Format(sum)}</td>`,
        );
    ...
    $('#GildPlayersDetailViewBody')
      ...
      .done(function () {
        ...
        $('#GildPlayersDetailViewBody tr.sorter-header').on(
          'click',
          function () {
        ...
        $('#GildPlayersDetailViewBody > .foe-table tr').on(
          'click',
          function () {
            if ($(this).next('tr.detailview').length) {
              ...
              GuildFights.BuildDetailViewLog({
                date: date,
                player: player,
                width: {
                  a: awidth,
  ...
  DeleteOldSnapshots: async (gbground) => {
    let deleteCount = await GuildFights.db.snapshots
      .where('gbground')
      .notEqual(gbground)
      .delete();
  },
  ...
  BuildDetailViewLog: async (data) => {
    let h = [];
    let d = await GuildFights.db.snapshots
      .where({ player_id: data.player, date: data.date })
      .reverse()
      .sortBy('date');
    ...
    h.push(
      `<tr class="detailview dark-bg"><td class="nopadding" colspan="${$('#GildPlayersDetailViewBody > .foe-table thead tr').find('th').length}"><table class="foe-table log"><body>`,
    );

    d.forEach((e) => {
      h.push(`<tr>`);
      h.push(
        `<td style="width: ${data.width.a}px">${moment.unix(e.time).format(i18n('DateTime'))}</td>`,
      );
      h.push(
        `<td style="width: ${data.width.b}px" class="text-center">${e.negotiations}</td>`,
      );
      h.push(
        `<td style="width: ${data.width.c}px" class="text-center">${e.battles}</td>`,
      );
      h.push(
        `<td style="width: ${data.width.d}px" class="text-center">${e.battles + e.negotiations * 2}</td>`,
      );
  ...
  BuildFightContent: () => {
    ...
    GuildFights.showGuildColumn =
      LiveFightSettings && LiveFightSettings.showGuildColumn !== undefined ?
        LiveFightSettings.showGuildColumn
      : 0;
    ...
    for (let i in mapdata) {
      ...
      let id = mapdata[i]['id'];
      mapdata[i]['neighbor'] = [];
      let linkIDs = ProvinceMap.ProvinceData().find((e) => e['id'] === id)[
        'connections'
      ];
      ...
      for (let x in linkIDs) {
        ...
        let neighborID = GuildFights.MapData['map']['provinces'].find(
          (e) => e['id'] === linkIDs[x],
        );
    ...
    h.push(
      '<button class="btn copybutton all" onclick="GuildFights.CopyToClipBoard(event)">' +
        i18n('Boxes.GuildFights.SelectAll') +
        '</button>',
    );
    h.push(
      '<button class="btn mapbutton" onclick="ProvinceMap.build()">' +
        i18n('Boxes.GuildFights.OpenMap') +
        '</button>',
    );
    h.push('</div>');
  ...
  BuildProgressTab: function () {
    ...
    progress.push(
      '<th class="prov-name" style="user-select:text">' +
        i18n('Boxes.GuildFights.Province') +
        '</th>',
    );
    ...
    progress.push(
      '<th>' + i18n('Boxes.GuildFights.RequiredProgress') + '</th>',
    );
    ...
    for (let i in mapdata) {
      ...
      for (let x in gbgGuilds) {
        ...
        if (
          mapdata[i]['ownerId'] !== undefined &&
          gbgGuilds[x]['participantId'] === mapdata[i]['ownerId']
        ) {
          // show current fights
          if (
            mapdata[i]['conquestProgress'].length > 0 &&
            mapdata[i]['lockedUntil'] === undefined
          ) {
            let pColor = ProvinceMap.getSectorColors(mapdata[i]['ownerId']);
            let provinceProgress = mapdata[i]['conquestProgress'];

            progress.push(
              `<tr id="province-${id}" data-id="${id}" data-tab="progress">`,
            );
            progress.push(
              `<td title="${i18n('Boxes.GuildFights.Owner')}: ${gbgGuilds[x]['clan']['name']}"><b><span class="province-color" style="background-color:${pColor['main']}"></span> ${mapdata[i]['title']}</b></td>`,
            );
            ...
            progress.push(
              `<td data-field="${id}-${mapdata[i]['ownerId']}" class="guild-progress">`,
            );
            ...
            for (let y in provinceProgress) {
              ...
              let color = ProvinceMap.getSectorColors(
                provinceProgress[y]['participantId'],
              );
              progress.push(
                `<span class="attack attacker-${provinceProgress[y]['participantId']} gbg-${color['cid']}">${provinceProgress[y]['progress']}</span>`,
              );
            }
            ...
            progress.push(
              '</td><td data-field="' +
                ...
                mapdata[i]['conquestProgress'][0].maxProgress +
                '</td>',
            );
          }
        }
      ...
      if (
        ...
        progress.push(
          `<td><b><span class="province-color" style="background-color:#555"></span> ${mapdata[i]['title']}</b></td>`,
        );

        if (GuildFights.showGuildColumn)
          progress.push(
            `<td><em>${i18n('Boxes.GuildFights.NoOwner')}</em></td>`,
          );
        ...
        for (let y in provinceProgress) {
          ...
          let color = GuildFights.SortedColors.find(
            (e) => e['id'] === provinceProgress[y]['participantId'],
          );
  ...
  BuildNextUpTab: function () {
    ...
    GuildFights.showAdjacentSectors =
      LiveFightSettings && LiveFightSettings.showAdjacentSectors !== undefined ?
        LiveFightSettings.showAdjacentSectors
      : 1;
    GuildFights.showOwnSectors =
      LiveFightSettings && LiveFightSettings.showOwnSectors !== undefined ?
        LiveFightSettings.showOwnSectors
      : 0;
    ...
    nextup.push(
      '<th class="time-static">' + i18n('Boxes.GuildFights.Time') + '</th>',
    );
    nextup.push(
      '<th class="time-dynamic">' + i18n('Boxes.GuildFights.Count') + '</th>',
    );
    ...
    for (let x in prov) {
      ...
      if (GuildFights.showAdjacentSectors)
        if (!prov[x].hasOwnProperty('neighbor')) {
          ...
          showCountdowns =
            prov[x]['neighbor'].includes(own['participantId']) ||
            (prov[x]['owner'] == own['clan']['name'] &&
              GuildFights.showOwnSectors);

      if (showCountdowns) {
        let countDownDate = moment.unix(prov[x]['lockedUntil'] - 2),
          color = GuildFights.SortedColors.find(
            (e) => e['id'] === prov[x]['ownerId'],
          ),
        ...
        nextup.push(
          `<tr id="timer-${prov[x]['id']}" class="timer" data-tab="nextup" data-id=${prov[x]['id']}>`,
        );
        nextup.push(
        ...
        nextup.push(
          `<td class="time-static" style="user-select:text">${moment(
            countDownDate,
          )
            .add(
              LiveFightSettings?.showServerTime ?
                -60 * (GuildFights.serverOffset ?? 0)
              : 0,
              'seconds',
            )
            .format('HH:mm:ss')}</td>`,
        ...
        nextup.push(
          `<td class="text-right" id="alert-${prov[x]['id']}">${GuildFights.GetAlertButton(prov[x]['id'])}</td>`,
        );
        nextup.push('</tr>');
      }
    }
  ...
  formatRange: () => {
    ...
    if (dateStart.isSame(dateEnd)) {
      ...
      text =
        `${dateStart.format(i18n('Date'))}` +
        ' - ' +
        `${dateEnd.format(i18n('Date'))}`;
    } else {
      text =
        `${dateStart.format(i18n('DateShort'))}` +
        ' - ' +
        `${dateEnd.format(i18n('Date'))}`;
    }
  ...
  CopyToClipBoard: (e) => {
    ...
    $('.timer.highlight-row').each(function () {
      copycache.push(
        GuildFights.MapData['map']['provinces'].find(
          (mapItem) => mapItem.id == $(this).data('id'),
        ),
      );
    });

    copycache.sort(function (a, b) {
      return a.lockedUntil - b.lockedUntil;
    });
    copycache.forEach((mapElem) => {
      let battleType = mapElem.isAttackBattleType ? '🔴' : '🔵';
      let LiveFightSettings = JSON.parse(
        localStorage.getItem('LiveFightSettings'),
      );
      let showTileColors =
        LiveFightSettings && LiveFightSettings.showTileColors !== undefined ?
          LiveFightSettings.showTileColors
        : 1;
    ...
    if (copy !== '') {
      if (
        GuildFights.serverOffset &&
        localStorage.getItem('Guildfights.TimeZoneWarningShown') === null
      ) {
  ...
  UpdateCounter: (countDownDate, intervalID, id) => {
    ...
    if (removeIt) {
      ...
      $(`#timer-${id}`)
        .find('.time-static')
        .html(`<strong class="text-success">offen</strong>`); // @ToDo: translate
  ...
  RefreshTable: (data) => {
    ...
    for (let i in data['conquestProgress']) {
      ...
      let d = data['conquestProgress'][i],
        ...
        pColor = ProvinceMap.getSectorColors(
          GuildFights.MapData.map?.provinces[data.id]?.ownerId,
        ),
        p = GuildFights.MapData['battlegroundParticipants'].find(
          (o) => o['participantId'] === d['participantId'],
        );
      ...
      if (cell.length === 0) {
        ...
        let mD = GuildFights.MapData['map']['provinces'].find(
          (d) => d.id === data['id'],
        );
        let provinceColor = pColor['main'] || 'rgba(0,0,0,0)';

        $('#progress')
          .find('table.foe-table')
          .prepend(
            newCell.append(
              $('<td />').append(
                $('<span />')
                  .css({
                    'background-color':
                      !provinceColor ? '#555' : provinceColor['main'],
                  })
                  .attr({ class: 'province-color' }),
                $('<b />').text(mD['title']),
              ),
              GuildFights.showGuildColumn ?
                $('<td />').text(p['clan']['name'])
  ...
  ShowPlayerBoxSettings: () => {
    ...
    c.push(
      `<p class="text-left"><span class="settingtitle">${i18n('Boxes.GuildFights.Title')}</span>` +
    ...
    c.push(
      `<p class="text-left"><input id="gf_showProgressFilter" name="showprogressfilter" value="1" type="checkbox" ${Settings.showProgressFilter === 1 ? ' checked="checked"' : ''} /> <label for="gf_showProgressFilter">${i18n('Boxes.GuildFights.ShowProgressFilter')}</label></p>`,
    );
    c.push(
      `<p class="text-left"><input id="gf_showLogButton" name="showlogbutton" value="1" type="checkbox" ${Settings.showLogButton === 1 ? ' checked="checked"' : ''} /> <label for="gf_showLogButton">${i18n('Boxes.GuildFights.ShowLogButton')}</label></p>`,
    );
    c.push(
      `<p><button id="save-GuildFightsPlayerBox-settings" class="btn" style="width:100%" onclick="GuildFights.PlayerBoxSettingsSaveValues()">${i18n('Boxes.General.Save')}</button></p>`,
    );
    c.push(
      `<hr><p>${i18n('Boxes.General.Export')}: <span class="btn-group"><button class="btn" onclick="HTML.ExportTable($('#GildPlayersTable'),'csv','GBG-PlayerList')" title="${HTML.i18nTooltip(i18n('Boxes.General.ExportCSV'))}">CSV</button>`,
    );
    c.push(
      `<button class="btn" onclick="HTML.ExportTable($('#GildPlayersTable'),'json','GBG-PlayerList')" title="${HTML.i18nTooltip(i18n('Boxes.General.ExportJSON'))}">JSON</button></span></p>`,
    );
  ...
  PlayerBoxSettingsSaveValues: () => {
    GuildFights.PlayerBoxSettings.showRoundSelector =
      $('#gf_showRoundSelector').is(':checked') ? 1 : 0;
    GuildFights.PlayerBoxSettings.showProgressFilter =
      $('#gf_showProgressFilter').is(':checked') ? 1 : 0;
    GuildFights.PlayerBoxSettings.showLogButton =
      $('#gf_showLogButton').is(':checked') ? 1 : 0;
  ...
  SetAlert: (id) => {
    ...
    const data = {
      title: prov.title,
      body: HTML.i18nReplacer(i18n('Boxes.GuildFights.SaveAlert'), {
        provinceName: prov.title,
      }),
    ...
    MainParser.sendExtMessage({
      ...
      HTML.ShowToastMsg({
        ...
        text: HTML.i18nReplacer(i18n('Boxes.GuildFights.SaveMessage.Desc'), {
          provinceName: prov.title,
        }),
        type: 'success',
        hideAfter: 5000,
  ...
  DeleteAlert: (provId) => {
    let prov = GuildFights.MapData['map']['provinces'].find(
      (e) => e.id === provId,
    );
    ...
    MainParser.sendExtMessage({
      ...
      HTML.ShowToastMsg({
        ...
        text: HTML.i18nReplacer(i18n('Boxes.GuildFights.DeleteMessage.Desc'), {
          provinceName: prov.title,
        }),
        type: 'success',
        hideAfter: 5000,
  ...
  ShowLiveFightSettings: () => {
    ...
    c.push(
      `<p><input id="showguildcolumn" name="showguildcolumn" value="1" type="checkbox" ${showGuildColumn === 1 ? ' checked="checked"' : ''} /> <label for="showguildcolumn">${i18n('Boxes.GuildFights.ShowOwner')}</label></p>`,
    );
    c.push(
      `<p><label for="showAdjacentSectors"><input id="showAdjacentSectors" name="showAdjacentSectors" value="0" type="checkbox" ${showAdjacentSectors === 1 ? ' checked="checked"' : ''} /> ${i18n('Boxes.GuildFights.ShowAdjacentSectors')}</label></p>`,
    );
    c.push(
      `<p><label for="showownsectors"><input id="showownsectors" name="showownsectors" value="0" type="checkbox" ${showOwnSectors === 1 ? ' checked="checked"' : ''} /> ${i18n('Boxes.GuildFights.ShowOwnSectors')}</label></p>`,
    );
    c.push(
      `<p><label for="showtilecolors"><input id="showtilecolors" name="showtilecolors" value="0" type="checkbox" ${showTileColors === 1 ? ' checked="checked"' : ''} /> ${i18n('Boxes.GuildFights.ShowTileColors')}</label></p>`,
    );
    c.push(
      `<p><label for="showservertime"><input id="showservertime" name="showservertime" value="0" type="checkbox" ${showServerTime === 1 ? ' checked="checked"' : ''} /> ${i18n('Boxes.GuildFights.ShowServerTime')}</label></p>`,
    );
    c.push(
      `<p><label for="serverOffset">${i18n('Boxes.GuildFights.serverOffset')}<input id="serverOffset" name="serverOffset" value="${GuildFights.serverOffset ?? ''}" type="text" maxlength="5" size = "5"/></label></p>`,
    );
    c.push(
      `<p><button onclick="GuildFights.SaveLiveFightSettings()" id="save-livefight-settings" class="btn" style="width:100%">${i18n('Boxes.GuildFights.SaveSettings')}</button></p>`,
    );
  ...
  SaveLiveFightSettings: () => {
    ...
    if ($('#showguildcolumn').is(':checked')) value.showGuildColumn = 1;

    if ($('#showAdjacentSectors').is(':checked')) value.showAdjacentSectors = 1;

    if ($('#showownsectors').is(':checked')) value.showOwnSectors = 1;

    if ($('#showtilecolors').is(':checked')) value.showTileColors = 1;

    if ($('#showservertime').is(':checked')) value.showServerTime = 1;
    ...
    if (GuildFights.serverOffset != null)
      localStorage.setItem(
        'GuildFights.serverOffset',
        JSON.stringify(GuildFights.serverOffset),
      );
    else localStorage.removeItem('GuildFights.serverOffset');
...
let ProvinceMap = {
  ...
  mapDrag: () => {
    ...
    const mouseDownHandler = function (e) {
      ...
      ProvinceMap.selectedProvince = ProvinceMap.findProvinceAt(
        canvasPos.x,
        canvasPos.y,
      );
      if (ProvinceMap.selectedProvince) ProvinceMap.showProvinceDetails();
    };
    ...
    ProvinceMap.Map.addEventListener(
      'mousedown',
      function (e) {
  ...
  prepare: () => {
    ...
    $('#ProvinceMapBody')
      .html(wrapper)
      .append(
        '<span id="zoomGBGMap" class="btn">' +
          i18n('Boxes.GvGMap.Action.Zoom') +
          '</span><div id="provDetails"></div>',
      );
    ...
    if (GuildFights.MapData.map['id'] === 'volcano_archipelago') {
      ProvinceMap.MapCTX.translate(
        ProvinceMap.Size.width / 2,
        ProvinceMap.Size.height / 2,
      );
    }
    ...
    Province.prototype.drawMapSector = function (
      mapType = 'waterfall_archipelago',
    ) {
      ...
      let mapStuff = {
        ...
        x:
          sector.flag.x *
            (ProvinceMap.Size.width /
              (hexwidthFactor * 2 + (hexwidthFactor * 2) / 3)) +
          ProvinceMap.Size.width /
            (hexwidthFactor * 2 + (hexwidthFactor * 2) / 3),
        y:
          sector.flag.y * (ProvinceMap.Size.height / (hexheightFactor * 2)) +
      ...
      if (mapType === 'volcano_archipelago') {
        ...
        mapStuff.x =
          xy.x *
          (sector.circlePosition.radius -
            sector.circlePosition.initRadius / 2) *
          Math.sin(
            sector.circlePosition.angle +
              sector.circlePosition.angleFragment / 2,
          );
        mapStuff.y =
          xy.y *
          (sector.circlePosition.radius -
        ...
        noRealignSectors = [
          1, 2, 29, 31, 30, 32, 33, 34, 35, 37, 38, 39, 41, 42, 43, 45, 46, 47,
          49, 50, 51, 53, 54, 55, 57, 58, 59,
        ];

        // realign some sectors on volcano map to have more space
        if (
          !noRealignSectors.includes(sector.id) &&
          sector.owner.flagImg == undefined
        )
      ...
      if (sector.owner.flagImg && sector.isSpawnSpot)
        ...
        if (
          ...
          sector.battleType == 'red' &&
          sector.owner.colors.cid !== 'own_guild_colour'
        )
          ProvinceMap.MapCTX.fillStyle = '#cf401e';
        else if (
          ProvinceMap.view == 'battleType' &&
          sector.battleType == 'blue' &&
          sector.owner.colors.cid !== 'own_guild_colour'
        )
        ...
        if (
          sector.lockedUntil === undefined &&
          sector.conquestProgress.length === 0
        )
          sector.drawTitleAndSlots(mapType, true, mapStuff.x, mapStuff.y);
        else {
          if (mapType === 'waterfall_archipelago') mapStuff.y = mapStuff.y - 10;
        ...
        mapStuff.y = mapStuff.y + 10;
        if (sector.lockedUntil !== undefined) mapStuff.y = mapStuff.y + 20;
    ...
    Province.prototype.drawUnlockTime = function (mapStuff) {
      ...
      let provinceUnlockTime =
        moment.unix(this.lockedUntil).format('HH:mm') != 'Invalid date' ?
          moment.unix(this.lockedUntil).format('HH:mm')
        : '';
      ProvinceMap.MapCTX.fillText(
        provinceUnlockTime,
        mapStuff.x,
        mapStuff.y + 5,
      );
    ...
    Province.prototype.drawTitleAndSlots = function (
      mapType,
      drawCentered = true,
      x,
      y,
      ...
      if (this.owner.colors.cid != 'own_guild_colour') {
        ...
        if (this.battleType == 'blue') ProvinceMap.MapCTX.fillStyle = '#00f';
      ...
      if (this.totalBuildingSlots != undefined) {
        let slots = '';
        if (this.totalBuildingSlots == 1) slots = '·';
        else if (this.totalBuildingSlots == 2) slots = '··';
        else if (this.totalBuildingSlots == 3) slots = '···';
    ...
    Province.prototype.drawStartSector = function (mapStuff, mapType) {
      ...
      flag_image.src = srcLinks.get(
        `/shared/clanflags/${this.owner.flagImg}.jpg`,
        true,
      );
      let sector = this;
    ...
    Province.prototype.drawSectorShape = function (
      ...
      if (mapType === 'volcano_archipelago') {
        ...
        path.moveTo(
          xy.x * (r - ir) * Math.sin(a),
          xy.y * (r - ir) * Math.cos(a),
        );
        path.lineTo(xy.x * r * Math.sin(a), xy.y * r * Math.cos(a));
        path.lineTo(
          xy.x * r * Math.sin(a + af / 2),
          xy.y * r * Math.cos(a + af / 2),
        );
        path.lineTo(xy.x * r * Math.sin(a + af), xy.y * r * Math.cos(a + af));
        path.lineTo(
          xy.x * (r - ir) * Math.sin(a + af),
          xy.y * (r - ir) * Math.cos(a + af),
        );
        path.closePath();
      } else {
        // WATERFALL / HEX MAP
    ...
    function init() {
      ...
      ProvinceMap.ProvinceData().forEach(function (i) {
        ...
        if (prov['ownerId']) {
          ...
          if (prov.isSpawnSpot) {
            let clan = GuildFights.MapData['battlegroundParticipants'].find(
              (c) => c['participantId'] === prov['ownerId'],
            );
            data['flagImg'] = clan['clan']['flag'].toLowerCase();
            data.isSpawnSpot = true;
          }
        } else if (prov.owner) data.ownerID = prov.owner.id;

        if (prov.lockedUntil) data.lockedUntil = prov.lockedUntil;
  ...
  showProvinceDetails: () => {
    let additionalData = GuildFights.MapData.map.provinces.find(
      (x) => x.id === ProvinceMap.selectedProvince.id,
    );
  ...
  RefreshSector: (socketData = []) => {
    ...
    if (socketData['id'] !== undefined)
      updatedProvince = ProvinceMap.Provinces.find(
        (p) => p.id === socketData['id'],
      );
    ...
    if (socketData.ownerId !== updatedProvince.owner.id) {
      ...
      updatedProvince.owner.colors = ProvinceMap.getSectorColors(
        socketData.ownerId,
      );
    }
   ...
   */
  BuildMap: () => {
    ProvinceMap.MapCTX.clearRect(
      0,
      0,
      ProvinceMap.Size.width,
      ProvinceMap.Size.height,
    );
