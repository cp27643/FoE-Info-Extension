...
let _menu_bottom = {
  ...
  BuildOverlayMenu: () => {
    let hud = $('<div />')
        .attr({ id: 'foe-helper-hud', class: 'hud-bottom' })
        .addClass('game-cursor'),
  ...
  SetMenuWidth: (reset = true) => {
    ...
    if (reset) {
      ...
      if (_menu.SlideParts > 1) {
        ...
        //Gesamtes Menü passt auf 1 Seite => Kein Scrollbutton nach unten
  ...
  Prepare: () => {
    ...
    _menu.HudCount = Math.floor(
      ($(window).outerWidth() - 50 - $('#foe-helper-hud').offset().left) /
        _menu_bottom.btnSize,
    );
    ...
    $('#foe-helper-hud-slider').width(
      $('#foe-helper-hud-slider').children().length * _menu_bottom.btnSize,
    );
  },
  ...
  CheckButtons: async () => {
    ...
    await ExistenceConfirmed(
      "jQuery._data($('body').get(0), 'events' ).click||$('.hud-btn')",
    );
    ...
    if (
      jQuery
        ._data($('body').get(0), 'events')
        .click.filter((elem) => elem.selector == '.hud-btn-right-active')
        .length == 0
    ) {
    ...
    if (
      jQuery
        ._data($('body').get(0), 'events')
        .click.filter((elem) => elem.selector == '.hud-btn-left-active')
        .length == 0
    ) {
    ...
    $('#foe-helper-hud-slider').sortable({
      ...
      sort: function () {
        $('.is--sorting .hud-btn-left-active')
        ...
        $('.is--sorting .hud-btn-right-active')
          .mouseenter(function (e) {
            $('.is--sorting .hud-btn-right-active').stop().addClass('hasFocus');

            setTimeout(() => {
              if (
                $('.is--sorting .hud-btn-right-active').hasClass('hasFocus')
              ) {
                _menu_bottom.ClickButtonRight();
              }
            }, 1000);
  ...
  ClickButtonRight: () => {
    ...
    if (
      _menu.ActiveSlide * _menu.HudWidth >
      $('#foe-helper-hud-slider').width()
    )
      _menu.MenuScrollLeft =
        -($('#foe-helper-hud-slider').width() / _menu.HudWidth - 1) *
        _menu.HudWidth;
  ...
  ClickButtonLeft: () => {
    ...
    if (_menu.ActiveSlide == 1) _menu.MenuScrollLeft = 0;
    else _menu.MenuScrollLeft += _menu.HudWidth;
