...
helper.preloader = {
  show: function (id) {
    ...
    $(id).append(
      '<div class="loading-data"><div class="loadericon"></div></div>',
    );
  },
...
let HTML = {
  ...
  Box: (args) => {
    ...
    if (args['onlyTitle'] !== true) {
      title = $('<span />')
        .addClass('title')
        .html(
          (extVersion.indexOf('beta') > -1 ? '(Beta) ' : '') +
            args['title'] +
            ' <small><em> - FoE Helper</em></small>',
        );
    }
    title = title.attr('title', title[0].textContent);
    let buttons = $('<div />')
        .attr('id', args['id'] + 'Buttons')
        .addClass('box-buttons'),
      head = $('<div />')
        .attr('id', args['id'] + 'Header')
        .attr('class', 'window-head')
        .append(title),
      body = $('<div />')
        .attr('id', args['id'] + 'Body')
        .attr('class', 'window-body'),
      div = $('<div />')
        .attr('id', args['id'])
        .attr('class', 'window-box open')
        .append(head)
        .append(body)
        .hide(),
    ...
    if (args['speaker']) {
      let spk = $('<span />')
        .addClass('window-speaker')
        .attr('id', args['speaker']);
      buttons.prepend(spk);
    ...
    if (cords) {
      ...
      if (cords.includes('|')) {
        cords = cords.split('|');
        cords = mouseActions.calcCoords(
          [Number(cords[1]), Number(cords[0])],
          'Center',
        );
      } else {
    ...
    $('body')
      ...
      .done(function () {
        ...
        $('#' + args['id'] + 'Header .box-buttons span').on(
          'pointerdown',
          (e) => {
            e.stopPropagation();
          },
            ...
            $(`#${args['id']}`).on(
              ...
              function () {
                ...
                if ($(`#${args['id']}SettingsBox`).length > 0) {
                  $(`#${args['id']}SettingsBox`).fadeToggle(
                    'fast',
                    function () {
                      $(this).remove();
                    },
        ...
        if (args['speaker']) {
          $('#' + args['speaker']).addClass(
            localStorage.getItem(args['speaker']),
          );
        }
  ...
  MinimizeBeforeBattle: () => {
    ...
    if (
      HideHelperDuringBattle === 'true' &&
      MenuSetting === 'Box' &&
      $('body').find('#menu_box').hasClass('open')
    ) {
      HTML.Minimize();
  ...
  DragBox: (el, save = true) => {
    document
      .getElementById(el.id + 'Header')
      .removeEventListener('pointerdown', dragMouseDown);
    ...
    function elementDrag(e) {
      ...
      let cords = mouseActions.calcCoords(
        [xStartEl - xStartM + e.clientX, yStartEl - yStartM + e.clientY],
        'Center',
      );
  ...
  GetColorGradient: (Value, MinValue, MaxValue, Color1, Color2) => {
    ...
    let Rgb1 = [
      Math.floor(Color1Int / 256 / 256),
      Math.floor(Color1Int / 256) % 256,
      Color1Int % 256,
    ];
    let Rgb2 = [
      Math.floor(Color2Int / 256 / 256),
      Math.floor(Color2Int / 256) % 256,
      Color2Int % 256,
    ];
  ...
  ExportTable: (Table, Format, FileName) => {
    ...
    $(Table).each(function () {
      ...
      if (Format === 'json') {
        ...
        let ValidColumnNames = ColumnNames.filter(function (a) {
          return a !== undefined;
        });
          ...
          for (let j = 0; j < ValidColumnNames.length; j++) {
            ...
            if (CurrentCell !== undefined) {
              if ($.isNumeric(CurrentCell)) {
                CurrentCells.push(
                  Number(CurrentCell).toLocaleString(i18n('Local'), {
                    useGrouping: false,
                  }),
                );
              } else {
      ...
      MainParser.ExportFile(
        BlobData,
        FileName + '-' + moment().format('YYYY-MM-DD') + '.' + Format,
      );
    });
  },
  ...
  ParseFloatLocalIfPossible: (NumberString) => {
    if (HTML.IsReversedFloatFormat === undefined) {
      //FloatFormat bestimmen, wenn noch unbekannt
