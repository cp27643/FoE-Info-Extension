...
let Treasury = {
  ...
  CalcBody: () => {
    ...
    h.push(
      i18n('Boxes.Treasury.RowNumber') +
        ': ' +
        HTML.Format(Treasury.Logs.length) +
        '<br>',
    );
    h.push(
      '<span class="btn button-reset">' +
        i18n('Boxes.Treasury.Reset') +
        '</span>',
    );
    h.push(
      '<span class="btn button-export">' +
        i18n('Boxes.Treasury.Export') +
        '</span>',
    );
  ...
  Export: () => {
    ...
    for (let i = 0; i < Treasury.Logs.length; i++) {
      ...
      CurrentLine.push(
        typeof CurrentLog['createdAt'] == 'object' ?
          CurrentLog['createdAt'].toLocaleString().replace(/,/g, '')
        : CurrentLog['createdAt'].replace(/;/g, ''),
      );
    ...
    let BOM = '\uFEFF';
    let Blob1 = new Blob([BOM + ExportString], {
      type: 'application/octet-binary;charset=ANSI',
    });
    MainParser.ExportFile(
      Blob1,
      'GuildTreasury-' + moment().format('YYYY-MM-DD') + '.csv',
    );
  },
  ...
  ShowSettings: () => {
    ...
    let h = [];
    h.push(
      `<p><input id="autoStartTreasuryExport" name="autoStartTreasuryExport" value="1" type="checkbox" ${autoOpen === true ? ' checked="checked"' : ''} /> <label for="autoStartMarket">${i18n('Boxes.Settings.Autostart')}</label></p>`,
    );
    h.push(
      `<p><button onclick="Treasury.SaveSettings()" id="save-treasury-settings" class="btn" style="width:100%">${i18n('Boxes.Settings.Save')}</button></p>`,
    );
