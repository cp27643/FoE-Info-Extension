...
FoEproxy.addHandler('ItemStoreService', 'getStore', (data, postData) => {
  ...
  shopAssist.unlockProgress = Object.assign(
    {},
    ...data.responseData.unlockConditionsProgress.map((x) => ({
      [x.type + '#' + (x.subtype || x.context)]: x.amount,
    })),
  );
  shopAssist.alertsTriggered = {};
  shopAssist.checkAlerts();
  shopAssist.alertsTriggered = {};
  shopAssist.showDiscount =
    data.responseData.refresh?.refreshAt - GameTime.get() < 24 * 3600; //only show discount if less than 24h to next refresh
  ...
  if ($('#shopAssist').length === 0 && !Settings.GetSetting('ShowShopAssist'))
    return;
  shopAssist.Show();
});

FoEproxy.addHandler('ItemStoreService', 'purchaseSlot', (data, postData) => {
  let i = shopAssist.slots.findIndex(
    (x) => x.slotId === data.responseData.slot.slotId,
  );
...
FoEproxy.addHandler('ItemStoreService', 'getConfigs', (data, postData) => {
  shopAssist.shopMeta = Object.assign(
    {},
  ...
  if (cleaned)
    localStorage.setItem(
      'shopAssist.favourites',
      JSON.stringify(shopAssist.favourites),
    );
  ...
  if (cleaned)
    localStorage.setItem(
      'shopAssist.alerts',
      JSON.stringify(shopAssist.alerts),
    );
  localStorage.setItem(
    'shopAssist.shopMeta',
    JSON.stringify(shopAssist.shopMeta),
  );
});
  ...
  (data, postData) => {
    for (let shop of data.responseData) {
      ...
      for (let cond of shop.unlockConditionsProgress) {
        shopAssist.unlockProgress[
          cond.type + '#' + (cond.subtype || cond.context)
        ] += cond.amount;
      }
    }
    if ($('#shopAssist').length === 0) return;
...
let shopAssist = {
  ...
  favouritesOnly: JSON.parse(
    localStorage.getItem('shopAssist.favouritesOnly') || 'false',
  ),
  currencyfilter: {},
  ...
  updateDialog: () => {
    ...
    let hasFavourites =
      Object.keys(shopAssist.favourites?.[shopAssist.storeId] || {}).length > 0;
    let addRow = (slot) => {
      let h = ``;
      ...
      let limitedBuys =
        slot.purchaseLimit ?
          slot.purchaseLimit.remainingPurchases || 0
        : Infinity;
      if (slot.reward.subType == 'fragment') {
        neededFragments = Math.max(
          slot.reward.requiredAmount -
            ((stock.fragments || 0) % slot.reward.requiredAmount),
          0,
        );
      ...
      let limitReached =
        slot.purchaseLimit?.maxPurchases &&
        !slot.purchaseLimit.remainingPurchases;
      let hasLock =
        'unlockConditions' in slot && slot.unlockConditions.length > 0;
      let unlocked = true;
      for (let u of slot.unlockConditions || []) {
        if (u.type == 'resource_spend')
          for (let [r, amount] of Object.entries(
            u?.resourcesVO?.resources || {},
          )) {
          ...
          if (
            (shopAssist.unlockProgress?.[u.type + '#' + u.context] || 0) <
            u.amount
          ) {
            unlocked = false;
            break;
          ...
          if (
            (shopAssist.unlockProgress?.[
              u.type + '#' + u.rarityPurchase.rarity.value
            ] || 0) < u.rarityPurchase.amount
          ) {
            unlocked = false;
            break;
          }
      ...
      let isdiscounted =
        slot.discount && slot.discount > 0 && shopAssist.showDiscount;
      currencyClasses = ' ';
      Object.entries(slot.baseCost?.resources || {}).forEach(
        ([res, amount]) => {
          ...
          if (u.type == 'resource_spend')
            for (let [r, amount] of Object.entries(
              ...
              costs +=
                ...
                HTML.Format(
                  shopAssist.unlockProgress?.[u.type + '#' + r] || 0,
                ) +
                '/' +
                amount +
                srcLinks.icons(r) +
            ...
            costs +=
              ...
              srcLinks.regEx(
                RegExp(
                  `store.*?${u.context.replace('_event', '')}.*?grand_prize`,
                ),
              ) +
              '</div>';
          } else if (u.type == 'rarity') {
            costs +=
              ...
              '/' +
              u.rarityPurchase.amount +
              '<img src="' +
              srcLinks.get(
                '/item_store/store_shared/item_store_rarity_icon_common.png',
                true,
              ) +
              '"></div>';
          }
      ...
      Object.entries(slot.baseCost?.resources || {}).forEach(
        ([res, amount]) => {
          ...
          if (ResourceStock[res] == undefined || ResourceStock[res] < cost)
            canBuy = false;
          costs +=
            `<div class="text-right${isdiscounted ? ' shopDiscount' : ''}">` +
            HTML.Format(cost) +
            srcLinks.icons(res) +
            '</div>';
        },
      ...
      if (slot.reward.subType == 'fragment') {
        ...
        Object.entries(slot.baseCost?.resources || {}).forEach(
          ([res, amount]) => {
            let cost =
              neededBuys * Math.ceil(amount * (1 - (slot.discount || 0)));
        ...
        if (neededFragments < slot.reward.requiredAmount) {
          let allTT =
            '<table class="foe-table shopAssistTable"><tr><th>' +
            i18n('Boxes.ShopAssist.Full') +
            `</th></tr><tr>`;
          costs = '';
          canBuy = true;
          fullBuys = Math.floor(
            slot.reward.requiredAmount / slot.reward.amount,
          );
          Object.entries(slot.baseCost?.resources || {}).forEach(
            ([res, amount]) => {
              let cost =
                fullBuys * Math.ceil(amount * (1 - (slot.discount || 0)));
              if ((ResourceStock[res] || 0) < cost) canBuy = false;
              costs +=
                `<div class="text-right">` +
                HTML.Format(cost) +
      ...
      costs = '';
      canBuy = false;
      let maxBuys = Math.min(
        slot.flag?.value == 'increasingCosts' ? 1 : Infinity,
        limitedBuys,
      );
      if (maxBuys > 0) {
        Object.entries(slot.baseCost?.resources || {}).forEach(
          ([res, amount]) => {
            maxBuys = Math.min(
              ...
              Math.floor(
                (ResourceStock[res] || 0) /
                  Math.ceil(amount * (1 - (slot.discount || 0))),
              ),
            );
          },
        );
      ...
      h += `<td class="costs ${(maxBuys > 0 && maxBuys == limitedBuys) || (maxBuys > 0 && limitedBuys == Infinity) ? 'canBuy' : ''} ${limitedBuys > 0 && limitedBuys < Infinity ? 'helperTT" data-callback_tt="shopAssist.allTT" data-slotid="' + slot.slotId + '"' : '"'}>
              ...
              : ``
						...
						<span>(<span class="${maxBuys > 0 ? 'buyable' : ''}">${maxBuys}</span>${
              (
                slot.flag?.value != 'increasingCosts' &&
                limitedBuys > 0 &&
                limitedBuys < Infinity
              ) ?
                '/' + limitedBuys
              : slot.flag?.value == 'increasingCosts' && limitedBuys > 0 ? '/?'
              : 'x'
            })
						</span>
					</div> 
      ...
      if (limitedBuys > 0 && limitedBuys < Infinity && limitedBuys > maxBuys) {
        ...
        Object.entries(slot.baseCost?.resources || {}).forEach(
          ([res, amount]) => {
            let cost =
              limitedBuys * Math.ceil(amount * (1 - (slot.discount || 0)));
            if ((ResourceStock[res] || 0) < cost) canBuy = false;
            costs +=
              `<div class="text-right">` +
              HTML.Format(cost) +
    ...
    for (let slot of shopAssist.slots) {
      if (shopAssist.alerts[shopAssist.storeId + '#' + slot.slotId])
        shopAssist.alerts[shopAssist.storeId + '#' + slot.slotId] =
          structuredClone(slot); //update alert data
      if (
        slot.purchaseLimit?.maxPurchases &&
        !slot.purchaseLimit.remainingPurchases
      ) {
    ...
    for (let slot of later) h += addRow(slot);
    ...
    $('#shopAssistFav').prop(
      'checked',
      shopAssist.favouritesOnly && hasFavourites,
    );
    ...
    $('#shopAssistFav').on('change', function (e) {
      ...
      localStorage.setItem(
        'shopAssist.favouritesOnly',
        JSON.stringify(shopAssist.favouritesOnly),
      );
    ...
    $('#shopAssistUnlock').on('change', function (e) {
      ...
      localStorage.setItem(
        'shopAssist.unlockedOnly',
        JSON.stringify(shopAssist.unlockedOnly),
      );
      if (shopAssist.unlockedOnly) {
        $('.shopAssistTable').addClass('unlockedOnly');
      } else {
    ...
    $('.shopFavourite').on('click', function (e) {
      ...
      if (!shopAssist.favourites?.[shopAssist.storeId])
        shopAssist.favourites[shopAssist.storeId] = {};
      shopAssist.favourites[shopAssist.storeId][id] =
        !shopAssist.favourites[shopAssist.storeId][id];
    ...
    $('.shopAlert').on('click', function (e) {
      ...
      if (!shopAssist.alerts?.[id]) {
        slotId = id.split('#')[1];
        shopAssist.alerts[id] = structuredClone(
          shopAssist.slots.find((x) => x.slotId === slotId),
        );
      } else {
  ...
  getStock: (reward) => {
    ...
    if (reward.type == 'building') {
      ...
      stock =
        Object.values(MainParser.Inventory).find(
          (x) => x.item.cityEntityId === buildingId,
        )?.inStock || 0;
    }
    ...
    if (reward.subType == 'selection_kit')
      stock =
        Object.values(MainParser.Inventory).find(
          (x) => x.item.selectionKitId === reward.id,
        )?.inStock || 0;
    if (reward.subType == 'upgrade_kit')
      stock =
        Object.values(MainParser.Inventory).find(
          (x) => x.item.upgradeItemId === reward.id,
        )?.inStock || 0;
    if (reward.type == 'unit')
      stock =
        Object.values(Unit?.Cache?.counts || {}).find(
          (x) => x.unitTypeId === reward.unit.unitTypeId,
        )?.unattached || '???';
  ...
  getBuildingIds: (reward) => {
    ...
    if (reward.type == 'building') {
      ...
      for (let option of MainParser.SelectionKits[reward.id].options) {
        if (option.item.__class__ == 'BuildingItemPayload')
          Ids.push(option.item.cityEntityId);
        if (option.item.__class__ == 'UpgradeKitPayload')
          Ids.push(getFromUpgrade(option.item.upgradeItemId));
      }
  ...
  TT: async (e) => {
    ...
    let eff = Object.assign(
      {},
      ...Productions.rateBuildings(buildingIds, true, CurrentEra)?.map((x) => ({
        [x.entityId]: Math.round(100 * x.rating?.totalScore || 0),
      })),
    );
    let meta = Object.assign(
      {},
      ...buildingIds.map((x) => ({ [x]: MainParser.CityEntities[x] })),
    );
    ...
    for (let [key, slot] of Object.entries(shopAssist.alerts)) {
      ...
      if (canBuy) {
        ...
        HTML.ShowToastMsg({
          show: 'force',
          head:
            i18n('Boxes.ShopAssist.Shop') +
            ' - ' +
            (shopAssist.shopMeta?.[shopId]?.name || ''),
  ...
  ShowSettings: () => {
    ...
    let h = [];
    h.push(
      `<p><label><input id="shopAssistAutoOpen" type="checkbox" ${autoOpen === true ? ' checked="checked"' : ''} />${i18n('Boxes.Settings.Autostart')}</label></p>`,
    );
    h.push(
      `<p><button onclick="shopAssist.SaveSettings()" id="save-bghelper-settings" class="btn" style="width:100%">${i18n('Boxes.Settings.Save')}</button></p>`,
    );
  ...
  SaveSettings: () => {
    let value = false;
    if ($('#shopAssistAutoOpen').is(':checked')) value = true;
