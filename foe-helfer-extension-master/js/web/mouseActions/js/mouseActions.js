...
let mouseActions = {
  ...
  simulate: (element, eventName, options = {}) => {
    if (
      !/^(?:click|dblclick|mouse(?:down|enter|leave|up|over|move|out))$/.test(
        eventName,
      )
    )
      return;
  ...
  randomClick: (coords, n = 1) => {
    let previousCoords = Object.assign({}, mouseActions.lastMouseCoords),
      r = () =>
        Math.floor(Math.random() * (2 * mouseActions.randomClickRadius + 1)) -
        mouseActions.randomClickRadius,
...
KeyboardEvents = (() => {
  ...
  async function pasteAsKeyEvents(text, sel = selector, delay = charDelay) {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      el.value =
        el.value.slice(0, pos) + ch + el.value.slice(el.selectionEnd || pos);
...
FoEproxy.addRequestHandler('CityMapService', 'placeBuilding', (data) => {
  if (
    MainParser.CityEntities[data.requestData[0].cityentity_id].type != 'street'
  )
    buildRepeat.click();
});
