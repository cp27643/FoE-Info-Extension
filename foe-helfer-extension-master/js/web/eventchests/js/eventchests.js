...
FoEproxy.addHandler('ChestEventService', 'getOverview', (data, postData) => {
  ...
  for (let i in Chests) {
    ...
    for (let ResourceName in Chests[i]['cost']['resources']) {
      if (!Chests[i]['cost']['resources'].hasOwnProperty(ResourceName))
        continue;
      CurrentChest['cost'] += Chests[i]['cost']['resources'][ResourceName];
    }

    if (!Chests[i]['grandPrizeContribution']) continue;
    CurrentChest['grandPrizeContribution'] =
      Chests[i]['grandPrizeContribution'];

    if (!Chests[i]['chest'] || !Chests[i]['chest']['possible_rewards'])
      continue;
    let PossibleRewards = Chests[i]['chest']['possible_rewards'];

    for (let j = 0; j < PossibleRewards.length; j++) {
      if (
        !PossibleRewards[j]['reward'] ||
        !PossibleRewards[j]['reward']['flags']
      )
        continue;
    ...
    CurrentChest['costpermainprizestep'] =
      CurrentChest['cost'] / CurrentChest['grandPrizeContribution'];
    CurrentChest['costperdailyprize'] =
      (CurrentChest['cost'] * 100) / CurrentChest['drop_chance'];
...
FoEproxy.addHandler('PresentGameService', 'getOverview', (data, postData) => {
  if (
    ...
    !(Settings.GetSetting('EventHelperPresent') === undefined ?
      true
    : Settings.GetSetting('EventHelperPresent'))
  )
    return;
      ...
      if (present.status.value !== 'used') {
        ...
        if (asset == 'icon_fragment') {
          if (present.reward.assembledReward.type == 'building')
            asset =
              MainParser.CityEntities[present.reward.assembledReward.subType]
                .asset_id;
          else asset = present.reward.assembledReward.iconAssetName;
          frag =
            '<span class="fragment">' +
            srcLinks.icons('icon_tooltip_fragment') +
            '</span>';
        }
        ...
        h.push(
          '<td class="icon">' +
            (icon.search('antiquedealer_flag') === -1 ? icon : '') +
            frag +
            '</td>',
        );
        h.push(
          '<td>' +
            present.reward.name +
            (present.status.value === 'visible' ?
              '<img class="visible" src="' +
              extUrl +
              'css/images/hud/open-eye.png" alt="">'
            : '') +
            '</td>',
        );
        h.push('</tr>');
  ...
  CalcBody: () => {
    ...
    h.push(
      '<thead>' +
        ...
        i18n('Boxes.EventChests.MainPrize') +
        '</th>' +
        '<th colspan="3" class="text-center">' +
        i18n('Boxes.EventChests.MainPrizeTitle') +
        '</th>' +
        '</tr>' +
        '<tr>' +
        '<th class="text-center">' +
        i18n('Boxes.EventChests.Cost') +
        '</th>' +
        '<th class="text-center">' +
        i18n('Boxes.EventChests.Steps') +
        '</th>' +
        '<th class="text-center">' +
        i18n('Boxes.EventChests.CostPerStep') +
        '</th>' +
        '<th class="text-center">' +
        i18n('Boxes.EventChests.Chance') +
        '</th>' +
        '<th class="text-center">' +
        i18n('Boxes.EventChests.CostPerPrize') +
        '</th>' +
        '<th class="text-center">' +
        i18n('Boxes.EventChests.Cost') +
        '</th>' +
    ...
    for (let i in EventChests.Chests) {
      ...
      BestMainPrizeCost = Math.min(
        BestMainPrizeCost,
        EventChests.Chests[i]['costpermainprizestep'],
      );
      BestDailyPrizeCost = Math.min(
        BestDailyPrizeCost,
        EventChests.Chests[i]['costperdailyprize'],
      );
    }
    ...
    for (let i in EventChests.Chests) {
      if (!EventChests.Chests.hasOwnProperty(i)) continue;
      let isBestDaily =
        EventChests.Chests[i]['costperdailyprize'] <= BestDailyPrizeCost;
      let isBestMain =
        EventChests.Chests[i]['costpermainprizestep'] <= BestMainPrizeCost;
      h.push('<tr>');
      h.push(
        '<td class="text-center text-bold ' +
          (isBestMain ? ' text-success' : 'text-warning') +
          '">' +
          EventChests.Chests[i]['cost'] +
          '</td>',
      );

      h.push(
        '<td class="text-center">' +
          EventChests.Chests[i]['grandPrizeContribution'] +
          '</td>',
      );
      h.push(
        '<td class="text-center border-right' +
          (isBestMain ? ' text-success text-bold' : '') +
          '">' +
          MainParser.round(EventChests.Chests[i]['costpermainprizestep'] * 10) /
            10 +
          '</td>',
      );
      ...
      h.push(
        '<td class="text-center border-left">' +
          EventChests.Chests[i]['drop_chance'] +
          '%</td>',
      );
      h.push(
        '<td class="text-center' +
          (isBestDaily ? ' text-success text-bold' : '') +
          '">' +
          MainParser.round(EventChests.Chests[i]['costperdailyprize']) +
          '</td>',
      );
      h.push(
        '<td class="text-center text-bold ' +
          (isBestDaily ? ' text-success' : 'text-warning') +
          '">' +
          EventChests.Chests[i]['cost'] +
          '</td>',
      );
