...
FoEproxy.addWsHandler('OtherPlayerService', 'newEvent', (data) => {
  ...
  if (
    data.responseData['other_player']['player_id'] !==
    Parts.CityMapEntity['player_id']
  )
    return; // Selber Spieler

  let Entity = Object.values(MainParser.CityEntities).find(
    (obj) => obj['name'] === data.responseData['great_building_name'],
  );
...
FoEproxy.addHandler(
  ...
  (data, postData) => {
    let open =
      postData[0].requestData[1] == ExtPlayerID ||
      localStorage.getItem('ShowOwnPartOnAllGBs') == 'true';
    if (
      $('#OwnPartBox').length === 0 &&
      localStorage.getItem('OwnPartAutoOpen') == 'true' &&
      open
    )
      Parts.Show();
  },
...
FoEproxy.addHandler('all', 'all', (data, postData) => {
  ...
  if (
    [
      'GreatBuildingsService.unlockLevel',
    ...
    setTimeout(() => {
      Parts.allowCopyPlaceSetting = true;
    }, 2000);
  } else if (
    ![
...
let Parts = {
    ...
    if ($('#OwnPartBox').length === 0) {
      ...
      $('#OwnPartBox').on('blur', '.arc-percent-input', function () {
        ...
        localStorage.setItem(
          Parts.GetStorageKey('ArcPercents', null),
          JSON.stringify(ArcPercents),
        );
      ...
      $('#OwnPartBox').on('click', '.btn-set-arc', function () {
        ...
        localStorage.setItem(
          Parts.GetStorageKey('ArcPercents', null),
          JSON.stringify(Parts.ArcPercents),
        );
      ...
      $('#OwnPartBox').on('blur', '#player-name', function () {
        ...
        localStorage.setItem(
          Parts.GetStorageKey('CopyOwnPlayerName', null),
          PlayerName,
        );
      ...
      $('#OwnPartBox').on('blur', '#build-name', function () {
        ...
        localStorage.setItem(
          Parts.GetStorageKey(
            'CopyGBName',
            Parts.CityMapEntity['cityentity_id'],
          ),
          BuildingName,
        );
      ...
      $('#OwnPartBox').on('click', '.form-check-input', function () {
        ...
        if (PlaceName) {
          if (PlaceName === 'all') {
            //all: auto deaktivieren, P1-5 aktivieren
            Parts.CopyModeAll = true;
            Parts.CopyModeAuto = false;
            Parts.CopyModeAutoUnsafe = false;
          } else if (PlaceName === 'auto') {
            //auto: all/auto-unsafe deaktivieren, P1-P5 ermitteln
            Parts.CopyModeAll = false;
            Parts.CopyModeAuto = true;
            Parts.CopyModeAutoUnsafe = false;
          } else if (PlaceName === 'auto-unsafe') {
            //auto-unsafe: all/auto deaktivieren, P1-5 ermitteln
            Parts.CopyModeAll = false;
            Parts.CopyModeAuto = false;
            Parts.CopyModeAutoUnsafe = true;
          } else {
            //P1-5: auto und all deaktivieren
            Parts.CopyModeAll = false;
            Parts.CopyModeAuto = false;
            Parts.CopyModeAutoUnsafe = false;
        ...
        if (OptionsName) {
          ...
          if (OptionsName === 'danger') {
            ...
            StorageKey = Parts.GetStorageKey(
              'CopyIncludeDanger',
              Parts.CopyFormatPerGB ?
                Parts.CityMapEntity['cityentity_id']
              : null,
            );
  ...
  CalcBody: async (NextLevel) => {
    ...
    if (NextLevel) {
      ...
      Total = parseInt(
        Parts.CityMapEntity['state']['forge_points_for_level_up'],
      );
    }

    // Restore Default settings
    if (Parts.FirstCycle) {
      let SavedArcPercents = localStorage.getItem(
        Parts.GetStorageKey('ArcPercents', null),
      );
      if (SavedArcPercents !== null)
        Parts.ArcPercents = JSON.parse(SavedArcPercents);

      let SavedCopyOwnPlayerName = localStorage.getItem(
        Parts.GetStorageKey('CopyOwnPlayerName', null),
      );
      ...
      Parts.CopyFormatPerGB =
        localStorage.getItem(Parts.GetStorageKey('CopyFormatPerGB', null)) ===
        'true';
      if (!Parts.CopyFormatPerGB) {
        let SavedCopyIncludeDanger = localStorage.getItem(
          Parts.GetStorageKey('CopyIncludeDanger', null),
        );
        if (SavedCopyIncludeDanger !== null)
          Parts.CopyIncludeDanger = SavedCopyIncludeDanger === 'true';

        let SavedCopyDangerPrefix = localStorage.getItem(
          Parts.GetStorageKey('CopyDangerPrefix', null),
        );
        if (SavedCopyDangerPrefix !== null)
          Parts.CopyDangerPrefix = SavedCopyDangerPrefix;

        let SavedCopyDangerSuffix = localStorage.getItem(
          Parts.GetStorageKey('CopyDangerSuffix', null),
        );
        if (SavedCopyDangerSuffix !== null)
          Parts.CopyDangerSuffix = SavedCopyDangerSuffix;

        let SavedCopyIncludePlayer = localStorage.getItem(
          Parts.GetStorageKey('CopyIncludePlayer', null),
        );
        if (SavedCopyIncludePlayer !== null)
          Parts.CopyIncludePlayer = SavedCopyIncludePlayer === 'true';

        let SavedCopyIncludeGB = localStorage.getItem(
          Parts.GetStorageKey('CopyIncludeGB', null),
        );
        if (SavedCopyIncludeGB !== null)
          Parts.CopyIncludeGB = SavedCopyIncludeGB === 'true';

        let SavedCopyIncludeLevel = localStorage.getItem(
          Parts.GetStorageKey('CopyIncludeLevel', null),
        );
        if (SavedCopyIncludeLevel !== null)
          Parts.CopyIncludeLevel = SavedCopyIncludeLevel === 'true';

        let SavedCopyIncludeFP = localStorage.getItem(
          Parts.GetStorageKey('CopyIncludeFP', null),
        );
        if (SavedCopyIncludeFP !== null)
          Parts.CopyIncludeFP = SavedCopyIncludeFP === 'true';

        let SavedCopyIncludeOwnPart = localStorage.getItem(
          Parts.GetStorageKey('CopyIncludeOwnPart', null),
        );
        if (SavedCopyIncludeOwnPart !== null)
          Parts.CopyIncludeOwnPart = SavedCopyIncludeOwnPart === 'true';

        let SavedCopyPreP = localStorage.getItem(
          Parts.GetStorageKey('CopyPreP', null),
        );
        if (SavedCopyPreP !== null) Parts.CopyPreP = SavedCopyPreP === 'true';

        let SavedCopyDescending = localStorage.getItem(
          Parts.GetStorageKey('CopyDescending', null),
        );
    ...
    if (
      PlayerID !== Parts.LastPlayerID ||
      EntityID !== Parts.LastEntityID ||
      Parts.Level !== Parts.LastLevel
    ) {
    ...
    if (!Parts.IsNextLevel) {
      for (let i = 0; i < Parts.Rankings.length; i++) {
        ...
        if (
          Parts.Rankings[i]['player'] &&
          Parts.Rankings[i]['player']['player_id'] ===
            Parts.CityMapEntity['player_id']
        ) {
        ...
        // Deleted player
          ...
          Parts.Rankings[i]['rank'] === undefined ||
          Parts.Rankings[i]['rank'] < 0
        ) {
          //undefined => Eigentümer oder gelöscher Spieler P1-5, -1 => gelöschter Spieler ab P6 abwärts
          Rest -= CurrentMaezen;
        ...
        if (Place < 5) {
          if (Parts.Rankings[i]['reward'] !== undefined) {
            let FPCount =
              (
                Parts.Rankings[i]['reward']['strategy_point_amount'] !==
                undefined
              ) ?
                parseInt(Parts.Rankings[i]['reward']['strategy_point_amount'])
              : 0;
            if (FPCount > 0) {
            ...
            // Medals
            MedalCount =
              Parts.Rankings[i]['reward']['resources'] !== undefined ?
                parseInt(Parts.Rankings[i]['reward']['resources']['medals'])
              : 0;
            ...
            // Blueprints
            let BlueprintCount =
              Parts.Rankings[i]['reward']['blueprints'] !== undefined ?
                parseInt(Parts.Rankings[i]['reward']['blueprints'])
              : 0;
    ...
    Parts.Maezens.sort(function (a, b) {
      return b - a;
    });
    ...
    for (let i = 0; i < 5; i++) {
      if (FPRewards[i] <= Parts.Maezens[i] || Rest <= Parts.Maezens[i]) {
        if (Parts.LockExistingPlaces) {
          //Bestehende Einzahlung absichern
          let NextMaezen =
            Parts.Maezens[i + 1] !== undefined ? Parts.Maezens[i + 1] : 0;
          Eigens[i] = Math.ceil(
            Rest +
              (Parts.TrustExistingPlaces ? 0 : NextMaezen) -
              Parts.Maezens[i],
          );
      ...
      Eigens[i] = Math.ceil(
        Rest +
          (Parts.TrustExistingPlaces ? 0 : Parts.Maezens[i]) -
          2 * FPRewards[i],
      );
      if (Eigens[i] < 0) {
        if (Parts.TrustExistingPlaces)
          Eigens[i] = Math.min(Eigens[i] + Parts.Maezens[i], 0);
    ...
    for (let i = FPRewards.length; i < Parts.Maezens; i++) FPRewards[i] = 0;

    for (let i = MedalRewards.length; i < Parts.Maezens; i++)
      MedalRewards[i] = 0;

    for (let i = BPRewards.length; i < Parts.Maezens; i++) BPRewards[i] = 0;

    let PlayerName = undefined;
    if (PlayerID === ExtPlayerID) {
      PlayerName = ExtPlayerName;
    } else {
      //LG eines anderen Spielers
    ...
    if (
      ...
      MainParser.CityMapData[Parts.CityMapEntity.id]?.level ===
        MainParser.CityMapData[Parts.CityMapEntity.id]?.max_level
    ) {
      h.push(
        '<div class="lg-not-possible" data-text="' +
          i18n('Boxes.Calculator.LGNotOpen') +
          '"></div>',
      );
    }
    // Info-Block
    ...
    if (PlayerName)
      h.push(
        `<span class="activity activity_${PlayerDict[PlayerID]['Activity']}"></span> <strong>${MainParser.GetPlayerLink(PlayerID, PlayerName)}</strong>`,
      );
    h.push('</div>');
    ...
    if (Parts.IsPreviousLevel) {
      ...
      if (Level)
        h.push(
          i18n('Boxes.OwnpartCalculator.Step') +
            ' ' +
            (Level - 1) +
            ' &rarr; ' +
            Level,
      ...
      if (Parts.IsNextLevel)
        h.push(
          '<button class="btn btn-slim btn-set-level" data-value="' +
            (Parts.Level - 1) +
            '">&lt;</button> ',
        );
      ...
      h.push(
        i18n('Boxes.OwnpartCalculator.Step') +
          ...
          Parts.Level +
          ' &rarr; ' +
          (parseInt(Parts.Level) + 1),
      );

      if (
        GreatBuildings.Rewards[Era] &&
        GreatBuildings.Rewards[Era][Parts.Level + 1]
      )
        h.push(
          ' <button class="btn btn-slim btn-set-level" data-value="' +
            (Parts.Level + 1) +
    ...
    if (customButtons) {
      ...
      bonuses.forEach((bonus) => {
        if (bonus === 'ark') investmentSteps.push(MainParser.ArkBonus);
        else investmentSteps.push(bonus);
      });
    }

    investmentSteps = investmentSteps.filter(
      (item, index) => investmentSteps.indexOf(item) === index,
    );
    investmentSteps.sort((a, b) => a - b);
    investmentSteps.forEach((bonus) => {
      h.push(
        `<button class="btn btn-mid btn-set-arc${Parts.ArcPercents[0] === bonus ? ' btn-active' : ''}" data-value="${bonus}">${bonus}%</button>`,
      );
    });
    ...
    h.push(
      '<table id="OwnPartTable" class="foe-table" style="margin-top:3px">',
    );
    h.push('<thead>');
    ...
    h.push(
      '<th class="text-center"><span class="forgepoints" title="' +
        HTML.i18nTooltip(i18n('Boxes.OwnpartCalculator.Deposit')) +
        '"></th>',
    );
    h.push(
      '<th class="text-center">' +
        i18n('Boxes.OwnpartCalculator.Done') +
        '</th>',
    );
    if (printsEnabled)
      h.push(
        '<th class="text-center"><span class="blueprint" title="' +
          HTML.i18nTooltip(i18n('Boxes.OwnpartCalculator.BPs')) +
          '"></span></th>',
      );
    if (medalsEnabled)
      h.push(
        '<th class="text-center"><span class="medal" title="' +
          HTML.i18nTooltip(i18n('Boxes.OwnpartCalculator.Meds')) +
          '"></span></th>',
      );
    if (!minView)
      h.push(
        '<th class="text-center">' +
          i18n('Boxes.OwnpartCalculator.Ext') +
          '</th>',
      );
    if (!minView)
      h.push(
        '<th class="text-center">' +
          i18n('Boxes.OwnpartCalculator.Arc') +
          '</th>',
      );
    h.push('</tr>');
    h.push('</thead>');
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (i === 0 && EigenStart > 0) {
        ...
        h.push('<tr>');
        let OwnPartStartText =
          Eigens[i] > 0 ? opt(Eigens[i], EigenCounter) : '-';
        h.push('<td>' + i18n('Boxes.OwnpartCalculator.OwnPart') + '</td>');
        h.push(
          '<td class="text-center"><strong class="' +
            (PlayerID === ExtPlayerID ? 'success' : '') +
            '">' +
            OwnPartStartText +
            '</strong></td>',
        );
        h.push(
          '<td class="text-center"><strong class="info">' +
            HTML.Format(EigenStart) +
            '</strong></td>',
        );
        if (printsEnabled && medalsEnabled) h.push('<td colspan="4"></td>');
        else if (printsEnabled || medalsEnabled)
      ...
      h.push(
        '<td>' +
          i18n('Boxes.OwnpartCalculator.Place') +
          ' ' +
          (i + 1) +
          '</td>',
      );
      ...
      if (Parts.PlaceAvailables[i]) {
        ...
        h.push(
          '<td class="text-center"><strong class="' +
            (PlayerID === ExtPlayerID ? '' : (
              'success' + (Parts.Maezens[i] > 0 ? ' copy-fp clickable' : '')
            )) +
            '" data-copy="' +
            (copyvalue > 0 ? copyvalue : '') +
            '">' +
        ...
        if (Parts.LeveltLG[i]) {
          h.push(
            `<td class="text-center"><strong class="error">${i18n('Boxes.OwnpartCalculator.levelt')}</strong></td>`,
          );
        } else if (Parts.DangerPlaces[i] > 5) {
          h.push(
            `<td class="text-center"><strong class="error">${i18n('Boxes.OwnpartCalculator.danger')} (${HTML.Format(Parts.DangerPlaces[i])}FP)</strong></td>`,
          );
        } else {
          h.push(
            '<td class="text-center"><strong class="info">-</strong></td>',
          );
        }
      } else {
        ...
        if (Parts.Maezens[i] > 0) {
          if (MaezenDiff > 0) {
            MaezenDiffString =
              ' <strong class="success"><small>(+' +
              HTML.Format(MaezenDiff) +
              ')</small></strong>';
          } else if (MaezenDiff < 0) {
            MaezenDiffString =
              ' <strong class="error"><small>(' +
              HTML.Format(MaezenDiff) +
              ')</small></strong>';
          }
        }
      ...
      if (printsEnabled)
        h.push(
          '<td class="text-center">' + HTML.Format(BPRewards[i]) + '</td>',
        );
      if (medalsEnabled)
        h.push(
          '<td class="text-center">' + HTML.Format(MedalRewards[i]) + '</td>',
        );
      if (!minView)
        h.push(
          '<td class="text-center"><input min="0" step="1" type="number" class="ext-part-input' +
            i +
            '" value="' +
            Parts.Exts[i] +
            '"></td>',
      ...
      if (!minView)
        h.push(
          '<td class="text-center"><input type="number" class="arc-percent-input" step="0.1" min="12" max="200" value="' +
            Parts.ArcPercents[i] +
            '"></td>',
        );
    ...
    if (MaezenRest > 0) {
      ...
      h.push(
        '<td>' +
          i18n('Boxes.OwnpartCalculator.Place') +
          ' 6' +
          (Parts.Maezens.length > 6 ? '-' + Parts.Maezens.length : '') +
          '</td>',
      );
      h.push('<td class="text-center">-</td>');
      h.push(
        '<td class="text-center"><strong class="info">' +
          HTML.Format(MaezenRest) +
          '</strong></td>',
      );
    ...
    h.push(
      ': <input id="lockexistingpayments" class="lockexistingpayments game-cursor" ' +
        (Parts.LockExistingPlaces ? 'checked' : '') +
        ' type="checkbox">' +
        i18n('Boxes.OwnpartCalculator.Lock'),
    );
    h.push(
      '<input id="trustexistingpayments" class="trustexistingpayments game-cursor" ' +
        (Parts.TrustExistingPlaces ? 'checked' : '') +
        ' type="checkbox">' +
        i18n('Boxes.OwnpartCalculator.Trust') +
        '</div>',
    );
    if (!minView) {
      ...
      h.push(
        '<td>' +
          i18n('Boxes.OwnpartCalculator.PatronPart') +
          ': <strong class="' +
          (PlayerID === ExtPlayerID ? '' : 'success') +
          '">' +
          HTML.Format(MaezenTotal + ExtTotal) +
          '</strong></td>',
      );
      h.push(
        '<td class="text-right">' +
          i18n('Boxes.OwnpartCalculator.OwnPart') +
          ': <strong class="' +
          (PlayerID === ExtPlayerID ? 'success' : '') +
          '">' +
          HTML.Format(EigenTotal) +
          '</strong></td>',
      );
      h.push('</tr>');
      h.push('<tr>');
      h.push(
        '<td>' +
          i18n('Boxes.OwnpartCalculator.LGTotalFP') +
          ': <strong>' +
          HTML.Format(Total) +
          '</strong></td>',
      );
      if (EigenStart > 0) {
        h.push(
          '<td class="text-right">' +
            i18n('Boxes.OwnpartCalculator.OwnPartRemaining') +
            ': <strong class="' +
            (PlayerID === ExtPlayerID ? 'success' : '') +
            '">' +
            HTML.Format(EigenTotal - EigenStart) +
            '</strong></td>',
        );
      } else {
        h.push('<td></td>');
    ...
    if (Parts.IsPreviousLevel === false) {
      ...
      if (Parts.IsNextLevel) {
        ...
        rest =
          Parts.CityMapEntity['state']['forge_points_for_level_up'] -
          Parts.Rankings.reduce(
            (acc, entry) => acc + (entry?.forge_points | 0),
            0,
          );
      }
      if (!minView) {
        ...
        h.push(
          '<em>' +
            i18n('Boxes.Calculator.Up2LevelUp') +
            ': <span id="up-to-level-up" class="copy-fp clickable" data-copy="' +
            rest +
            '">' +
            HTML.Format(rest) +
            '</span> ' +
            i18n('Boxes.Calculator.FP') +
            '</em>',
        );
        h.push('</div>');
      }
      ...
      if (Parts.SafePlaces.length > 0 || Parts.CopyModeAll) {
        //Copy bzw. Note Button nur einblenden wenn zumindest ein Platz safe ist
        h.push(
          '<span class="btn button-own">' +
            i18n('Boxes.OwnpartCalculator.CopyValues') +
            '</span>',
        );
        if (Parts.CityMapEntity['player_id'] === ExtPlayerID)
          h.push(
            '<span class="btn button-save-own">' +
              i18n('Boxes.OwnpartCalculator.Note') +
              '</span>',
          );
      } else {
      ...
      h.push('<div class="btn-group">');
      h.push(
        '<span class="btn button-powerleveling">' +
          i18n('Boxes.OwnpartCalculator.PowerLeveling') +
          '</span>',
      );
      h.push('</div>');
      h.push('</div>');
      ...
      if (SaveCopyLength > 0) {
        ...
        h.push(
          '<div class="text-center dark-bg d-flex" style="padding:5px 0;"><em style="max-width:350px"><strong>' +
            HTML.i18nReplacer(i18n('Boxes.OwnpartCalculator.GBsNoted'), {
              GBCount: SaveCopyLength,
            }) +
            ':</strong> ' +
            GBList +
            '</em></div>',
  ...
  CalcBackgroundBody: () => {
    ...
    let SavedBuildingName = localStorage.getItem(
      Parts.GetStorageKey('CopyGBName', Parts.CityMapEntity['cityentity_id']),
    );
    ...
    if (
      ...
      let SavedCopyPreP = localStorage.getItem(
        Parts.GetStorageKey('CopyPreP', Parts.CityMapEntity['cityentity_id']),
      );
    ...
    Parts.CopyPlayerName =
      PlayerID === ExtPlayerID ?
        Parts.CopyOwnPlayerName
      : PlayerDict[PlayerID]['PlayerName'];

    h.push('<section class="p2 bbd">');
    h.push(
      '<strong>' + i18n('Boxes.OwnpartCalculator.CopyValues') + '</strong>',
    );
    if (PlayerID === ExtPlayerID) {
      h.push(
        '<div class="flex between"><span>' +
          i18n('Boxes.OwnpartCalculator.PlayerName') +
          ':</span><input type="text" id="player-name" value="' +
          Parts.CopyPlayerName +
          '"></div>',
      );
    } else {
    ...
    h.push(
      '<div class="flex between"><span>' +
        i18n('Boxes.OwnpartCalculator.BuildingName') +
        ':</span><input type="text" id="build-name" value="' +
        Parts.CopyBuildingName +
        '"></div>',
    );
    h.push('</section><section class="p2 bbd">');
    h.push(
      '<strong>' + i18n('Boxes.OwnpartCalculator.IncludeData') + '</strong>',
    );
    let Options =
      '<div class="checkboxes">' +
      '<label class="form-check-label game-cursor" for="options-player"><input type="checkbox" class="form-check-input" id="options-player" data-options="player" ' +
      (Parts.CopyIncludePlayer ? 'checked' : '') +
      '> <span>' +
      i18n('Boxes.OwnpartCalculator.OptionsPlayer') +
      '</span></label>' +
      '<label class="form-check-label game-cursor" for="options-gb"><input type="checkbox" class="form-check-input" id="options-gb" data-options="gb" ' +
      (Parts.CopyIncludeGB ? 'checked' : '') +
      '> <span>' +
      i18n('Boxes.OwnpartCalculator.OptionsGB') +
      '</span></label>' +
      '<label class="form-check-label game-cursor" for="options-level"><input type="checkbox" class="form-check-input" id="options-level" data-options="level" ' +
      (Parts.CopyIncludeLevel ? 'checked' : '') +
      '> <span>' +
      i18n('Boxes.OwnpartCalculator.OptionsLevel') +
      '</span></label>' +
      '<label class="form-check-label game-cursor" for="options-fp"><input type="checkbox" class="form-check-input" id="options-fp" data-options="fp" ' +
      (Parts.CopyIncludeFP ? 'checked' : '') +
      '> <span>' +
      i18n('Boxes.OwnpartCalculator.OptionsFP') +
      '</span></label>' +
      '<label class="form-check-label game-cursor" for="options-descending"><input type="checkbox" class="form-check-input" id="options-descending" data-options="descending" ' +
      (Parts.CopyDescending ? 'checked' : '') +
      '> <span>' +
      i18n('Boxes.OwnpartCalculator.OptionsDescending') +
      '</span></label>' +
      '<label class="form-check-label game-cursor" for="options-levelup"><input type="checkbox" class="form-check-input" id="options-levelup" data-options="levelup" ' +
      (Parts.CopyIncludeLevelString ? 'checked' : '') +
      '> <span>' +
      i18n('Boxes.OwnpartCalculator.OptionsLevelUp') +
      '</span></label>' +
      '<label class="form-check-label game-cursor" for="options-ownpart"><input type="checkbox" class="form-check-input" id="options-ownpart" data-options="ownpart" ' +
      (Parts.CopyIncludeOwnPart ? 'checked' : '') +
      '> <span>' +
      i18n('Boxes.OwnpartCalculator.OptionsOwnPart') +
      '</span></label>' +
      '<label class="form-check-label game-cursor" for="options-prep"><input type="checkbox" class="form-check-input" id="options-prep" data-options="prep" ' +
      (Parts.CopyPreP ? 'checked' : '') +
      '> <span>P(xx)</span></label>' +
      '<label class="form-check-label game-cursor" for="options-danger"><input type="checkbox" class="form-check-input" id="options-danger" data-options="danger" ' +
      (Parts.CopyIncludeDanger ? 'checked' : '') +
      '> <span>' +
      i18n('Boxes.OwnpartCalculator.OptionsDanger') +
      '</span></label>' +
      '</div>';
    ...
    if (Parts.CopyIncludeDanger) {
      let DangerOptions =
        ...
        '</strong>' +
        '<div><span>' +
        i18n('Boxes.OwnpartCalculator.OptionsDangerPrefix') +
        ':</span><input type="text" class="form-text-input" id="options-danger-prefix" data-options="danger-prefix" value="' +
        Parts.CopyDangerPrefix +
        '"></div>' +
        '<div><span>' +
        i18n('Boxes.OwnpartCalculator.OptionsDangerSuffix') +
        ':</span><input type="text" class="form-text-input" id="options-danger-suffix" data-options="danger-suffix" value="' +
        Parts.CopyDangerSuffix +
        '"></div>';
    ...
    let cb =
      '<div class="checkboxes">' +
      '<label class="form-check-label game-cursor" for="chain-p1"><input type="checkbox" class="form-check-input" id="chain-p1" data-place="1" ' +
      (Parts.CopyPlaces[0] ? 'checked' : '') +
      '> <span>' +
      i18n('Boxes.OwnpartCalculator.Place') +
      ' 1</span></label>' +
      '<label class="form-check-label game-cursor" for="chain-p2"><input type="checkbox" class="form-check-input" id="chain-p2" data-place="2" ' +
      (Parts.CopyPlaces[1] ? 'checked' : '') +
      '> <span>' +
      i18n('Boxes.OwnpartCalculator.Place') +
      ' 2</span></label>' +
      '<label class="form-check-label game-cursor" for="chain-p3"><input type="checkbox" class="form-check-input" id="chain-p3" data-place="3" ' +
      (Parts.CopyPlaces[2] ? 'checked' : '') +
      '> <span>' +
      i18n('Boxes.OwnpartCalculator.Place') +
      ' 3</span></label>' +
      '<label class="form-check-label game-cursor" for="chain-p4"><input type="checkbox" class="form-check-input" id="chain-p4" data-place="4" ' +
      (Parts.CopyPlaces[3] ? 'checked' : '') +
      '> <span>' +
      i18n('Boxes.OwnpartCalculator.Place') +
      ' 4</span></label>' +
      '<label class="form-check-label game-cursor" for="chain-p5"><input type="checkbox" class="form-check-input" id="chain-p5" data-place="5" ' +
      (Parts.CopyPlaces[4] ? 'checked' : '') +
      '> <span>' +
      i18n('Boxes.OwnpartCalculator.Place') +
      ' 5</span></label>' +
      '<label class="form-check-label game-cursor" for="chain-all"><input type="checkbox" class="form-check-input" id="chain-all" data-place="all" ' +
      (Parts.CopyModeAll ? 'checked' : '') +
      '> <span>' +
      i18n('Boxes.OwnpartCalculator.All') +
      '</span></label>' +
      '<label class="form-check-label game-cursor" for="chain-auto"><input type="checkbox" class="form-check-input" id="chain-auto" data-place="auto" ' +
      (Parts.CopyModeAuto ? 'checked' : '') +
      '> <span>' +
      i18n('Boxes.OwnpartCalculator.Auto') +
      '</span></label>' +
      '<label class="form-check-label game-cursor" for="chain-auto-unsafe"><input type="checkbox" class="form-check-input" id="chain-auto-unsafe" data-place="auto-unsafe" ' +
      (Parts.CopyModeAutoUnsafe ? 'checked' : '') +
      '> <span>' +
      i18n('Boxes.OwnpartCalculator.AutoWithUnsafe') +
      '</span></label>' +
      '</div>';
    ...
    h.push('</section><section class="p2 bbd">');
    h.push(
      '<strong>' + i18n('Boxes.OwnpartCalculator.Preview') + '</strong><br>',
    );
    Parts.CopyString = Parts.GetCopyString();
    h.push(
      '<input type="text" id="copystring" value="' + Parts.CopyString + '">',
    );
    ...
    let div = $('<div />').addClass('OwnPartBoxBackground'),
      a = $('<div />')
        ...
        .append(
          $('<span />')
            .addClass('arrow game-cursor')
            .attr('id', 'OwnPartBoxArrow'),
        )
        .append(
          $('<div />')
            .addClass('OwnPartBoxBackgroundBody window-box')
            .append(h.join('')),
        );

    $OwnPartBox
      .append(div.append(a))
      .append($('<div />').addClass('black-bg').hide());
  ...
  BackGroundBoxAnimation: (show) => {
    ...
    if (show === true) {
      let e = /** @type {HTMLElement} */ (
        document.getElementsByClassName('OwnPartBoxBackgroundBody')[0]
      );
      ...
      $('.OwnPartBoxBackgroundBody').animate(
        { height: 0, opacity: 0 },
        250,
        function () {
  ...
  GetCopyString: () => {
    return Parts.GetCopyStringEx(
      (Places = Parts.CopyPlaces),
      (Maezens = Parts.Maezens),
      (Level = Parts.Level),
      (OwnPart = Parts.RemainingOwnPart),
      (PlaceAll = Parts.CopyModeAll),
      (PlaceAuto = Parts.CopyModeAuto),
      (PlaceAutoUnsafe = Parts.CopyModeAutoUnsafe),
      (DangerPlaces = Parts.DangerPlaces),
      (LeveltLG = Parts.LeveltLG),
    );
  },
  ...
  GetCopyStringEx: (
    ...
    if (
      Parts.CopyIncludeDanger &&
      Parts.CopyDangerPrefix !== '' &&
      (DangerPlaces.find((e) => e > 5) || LeveltLG.find((e) => e))
    )
      Ret.push(Parts.CopyDangerPrefix);

    if (Parts.CopyIncludePlayer) Ret.push(Parts.CopyPlayerName);

    if (Parts.CopyIncludeGB) Ret.push(Parts.CopyBuildingName);

    if (Parts.CopyIncludeLevelString)
      Ret.push(i18n('Boxes.OwnpartCalculator.OptionsLevelUp'));
    ...
    if (!NoPlacesSelected) {
      for (let i = 0; i < 5; i++) {
        ...
        if (
          Parts.CopyIncludeDanger &&
          Parts.CopyDangerSuffix !== '' &&
          (DangerPlaces[Place] > 5 || LeveltLG[Place])
        )
          Ret.push(Parts.CopyDangerSuffix);
      }
    } else if (PlaceAuto) {
    ...
    if (Parts.CopyIncludeOwnPart)
      Ret.push(
        i18n('Boxes.OwnpartCalculator.OwnPartShort') + '(' + OwnPart + ')',
      );
  ...
  BuildBoxPowerLeveling: (event) => {
    ...
    if ($('#PowerLevelingBox').length === 0) {
      ...
      box.on('click', '.button-powerlevel-copy', function () {
        ...
        let CopyParts = Parts.GetCopyStringEx(
          (Places = [true, true, true, true, true]),
          (Maezens = Parts.PowerLevelingData.Places[gb_level]),
          (Level = gb_level),
          (OwnPart = Parts.PowerLevelingData.EigenNettos[gb_level]),
          (PlaceAll = true),
          (PlaceAuto = false),
          (PlaceAutoUnsafe = false),
          (DangerPlaces = [0, 0, 0, 0, 0]),
  ...
  CalcBodyPowerLevelingData: () => {
    let EntityID = Parts.CityMapEntity['cityentity_id'],
      ...
      EndLevel =
        GreatBuildings.Rewards[Era] ?
          Math.min(
            Parts.PowerLevelingEndLevel,
            GreatBuildings.Rewards[Era].length,
          )
        : 0;
    ...
    for (let i = StartLevel; i < EndLevel; i++) {
      ...
      if (i < 10) {
        ...
        Totals[i] = Math.ceil(
          CityEntity['strategy_points_for_upgrade'][9] * Math.pow(1.025, i - 9),
        );
      }
      ...
      if (i != Parts.Level) {
        Places[i] = GreatBuildings.GetMaezen(
          GreatBuildings.Rewards[Era][i],
          Parts.ArcPercents,
        );
      ...
      let FPGreatBuilding = GreatBuildings.GreatBuildingsData.find(
        (obj) => obj.ID === EntityID && obj.FPProductions,
      );
      if (
        FPGreatBuilding &&
        !['X_FutureEra_Landmark1', 'X_AllAge_Expedition'].includes(EntityID)
      ) {
        //FP produzierende LGs ohne Arche
        HasDoubleCollection = true;
        if (i < FPGreatBuilding.FPProductions.length) {
          DoubleCollections[i] = FPGreatBuilding.FPProductions[i];
        } else {
          DoubleCollections[i] = MainParser.round(
            (FPGreatBuilding.FPProductions[9] * (i + 1)) / 10,
          );
        }
      } else {
        HasDoubleCollection = false;
  ...
  CalcTableBodyPowerLeveling: (h) => {
    ...
    for (let i = StartLevel; i < EndLevel; i++) {
      h.push('<tr>');
      h.push(
        '<td class="bright" style="white-space:nowrap">' +
          i +
          ' → ' +
          (i + 1) +
          '</td>',
      ...
      h.push(
        '<td><span class="hidden-text"> - #1 (</span>' +
          HTML.Format(Places[i][0]) +
          '<span class="hidden-text">)</span></td>',
      );
      h.push(
        '<td class="text-light"><span class="hidden-text"> - #2 (</span>' +
          HTML.Format(Places[i][1]) +
          '<span class="hidden-text">)</span></td>',
      );
      h.push(
        '<td><span class="hidden-text"> - #3 (</span>' +
          HTML.Format(Places[i][2]) +
          '<span class="hidden-text">)</span></td>',
      );
      h.push(
        '<td class="text-light"><span class="hidden-text"> - #4 (</span>' +
          HTML.Format(Places[i][3]) +
          '<span class="hidden-text">)</span></td>',
      );
      h.push(
        '<td><span class="hidden-text"> - #5 (</span>' +
          HTML.Format(Places[i][4]) +
          '<span class="hidden-text">)</span></td>',
      );
      if (HasDoubleCollection) {
        h.push(
          '<td class="success no-select"><strong>' +
            HTML.Format(EigenBruttos[i]) +
            '</strong></td>',
        );
        h.push(
          '<td class="no-select">' +
            HTML.Format(MainParser.round(DoubleCollections[i])) +
            '</td>',
        );
      }
      h.push(
        '<td><strong class="info no-select">' +
          HTML.Format(MainParser.round(EigenNettos[i])) +
          '</strong></td>',
      );
      h.push(
        '<td><span class="hidden-text">' +
          i +
          '</span><span class="btn button-powerlevel-copy">' +
          i18n('Boxes.PowerLeveling.CopyValues') +
          '</span></td>',
      );
      h.push('</tr>');
    }
  ...
  UpdateTableBodyPowerLeveling: () => {
    ...
    if (tableBody) {
      ...
      // Startlevel
      const startLevel = /** @type {HTMLInputElement} */ (
      ...
      // EndLevel
      const endLevel = /** @type {HTMLInputElement} */ (
      ...
      const ownPartSum = /** @type {HTMLElement} */ (
        document.getElementById('PowerLevelingBoxOwnPartSum')
      );
      ownPartSum.innerText = HTML.Format(
        MainParser.round(Parts.PowerLevelingData.OwnPartSum),
      );
    }
  },
  ...
  CalcBodyPowerLeveling: () => {
    ...
    h.push(
      '<div style="margin: 5px 10px 0 0;">' +
        i18n('Boxes.PowerLeveling.StartLevel') +
        ': <input type="number" id="startLevel" step="1" min=0" max="1000" value="' +
        StartLevel +
        '"></div>',
    );
    h.push(
      '<div style="margin: 5px 10px 0 0;">' +
        i18n('Boxes.PowerLeveling.EndLevel') +
        ': <input type="number" id="endLevel" step="1" min=10" max="1000" value="' +
        EndLevel +
        '"></div>',
    );
    h.push(
      '<div>' +
        i18n('Boxes.PowerLeveling.OwnPartSum') +
        ': <strong class="info" id="PowerLevelingBoxOwnPartSum">' +
        HTML.Format(MainParser.round(OwnPartSum)) +
        '</strong></div>',
    );
    h.push('</div>');
    h.push('</div>');
  ...
  ShowCalculatorSettings: () => {
    ...
    if (sB) {
      ...
      buttons = buttons.filter(
        (item, index) => buttons.indexOf(item) === index,
      ); // remove duplicates
    ...
    buttons.forEach((bonus) => {
      if (bonus === 'ark') {
        c.push(
          `<span class="btn-group"><input type="hidden" class="settings-values" value="ark"> <button class="btn btn-slim br">${MainParser.ArkBonus}%</button></span>`,
        );
      } else {
        c.push(
          `<span class="btn-group"><button class="btn btn-slim">${bonus}%</button> <input type="hidden" class="settings-values" value="${bonus}"> <span class="btn btn-delete btn-slim" onclick="Parts.SettingsRemoveRow(this)">x</span></span>`,
        );
      }
    });
    c.push('</section>');
    ...
    c.push(
      '<p class="bbd p5"><input id="copyformatpergb" class="copyformatpergb game-cursor" ' +
        (Parts.CopyFormatPerGB ? 'checked' : '') +
        ' type="checkbox"> <label for="copyformatpergb">' +
        i18n('Boxes.OwnpartCalculator.CopyFormatPerGB') +
        '</label>',
    );
    c.push(
      '<br><input type="checkbox" id="showmedals" class="showmedals game-cursor" ' +
        (showMedals == 'true' ? 'checked' : '') +
        '> <label for="showmedals">' +
        i18n('Settings.ShowOwnPartMedals.Desc') +
        '</label>',
    );
    c.push(
      '<br><input type="checkbox" id="showprints" class="showprints game-cursor" ' +
        (showPrints == 'true' ? 'checked' : '') +
        '> <label for="showprints">' +
        i18n('Settings.ShowOwnPartBP.Desc') +
        '</label>',
    );
    c.push(
      '<br><input type="checkbox" id="minview" class="minview game-cursor" ' +
        (minView == 'true' ? 'checked' : '') +
        '> <label for="minview">' +
        i18n('Settings.ShowOwnPartMinView.Desc') +
        '</label>',
    );
    c.push(
      '<br><input type="checkbox" id="openonaliengb" class="openonaliengb game-cursor" ' +
        (allGB == 'true' ? 'checked' : '') +
        '> <label for="openonaliengb">' +
        i18n('Settings.ShowOwnPartOnAllGBs.Desc') +
        '</label>',
    );
    c.push(
      '<br><input type="checkbox" id="autoOpen" class="autoOpen game-cursor" ' +
        (autoOpen == 'true' ? 'checked' : '') +
        '> <label for="autoOpen">' +
        i18n('Settings.ShowOwnPartAutoOpen.Desc') +
        '</label>',
    );
    c.push(
      '<br><input type="checkbox" id="includeStart" class="includeStart game-cursor" ' +
        (includeStart == 'true' ? 'checked' : '') +
        '> <label for="includeStart">' +
        i18n('Settings.ShowOwnPartIncludeStart.Desc') +
        '</label></p>',
    );

    // save button
    c.push(
      `<p class="text-center p2"><button id="save-calculator-settings" class="btn btn-green" onclick="Parts.SettingsSaveValues()">${i18n('Boxes.Calculator.Settings.Save')}</button></p>`,
    );
  ...
  SettingsSaveValues: () => {
    ...
    localStorage.setItem(
      Parts.GetStorageKey('CopyFormatPerGB', null),
      Parts.CopyFormatPerGB,
    );

    let openforeignGB = true;
    if ($('#openonaliengb').is(':not(:checked)')) openforeignGB = false;
    localStorage.setItem('ShowOwnPartOnAllGBs', openforeignGB);

    let showMedals = true;
    if ($('#showmedals').is(':not(:checked)')) showMedals = false;
    localStorage.setItem('OwnPartShowMedals', showMedals);

    let showPrints = true;
    if ($('#showprints').is(':not(:checked)')) showPrints = false;
    localStorage.setItem('OwnPartShowBP', showPrints);

    let minView = true;
    if ($('#minview').is(':not(:checked)')) minView = false;
    localStorage.setItem('OwnPartMinView', minView);

    let autoOpen = true;
    if ($('#autoOpen').is(':not(:checked)')) autoOpen = false;
    localStorage.setItem('OwnPartAutoOpen', autoOpen);
    let includeStart = true;
    if ($('#includeStart').is(':not(:checked)')) includeStart = false;
  ...
  setDonation: (value) => {
    if (!Parts.allowCopyPlace) helper.str.copyToClipboardLegacy(String(value));
    else {
