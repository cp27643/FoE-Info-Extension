...
FoEproxy.addHandler(
  ...
  (data, postData) => {
    ...
    reconstruction.draft = Object.assign(
      {},
      ...data.responseData.map((b) => ({ [b.entityId]: b })),
    );
    ...
    for (let b of data.responseData) {
      let id =
        MainParser.CityMapData[b.entityId].cityentity_id +
        '#' +
        (MainParser.CityMapData[b.entityId].level || 0);
      if (!reconstruction.count[id])
        reconstruction.count[id] = { placed: 0, stored: 0 };
      if (b.position) reconstruction.count[id].placed++;
      else {
...
FoEproxy.addRequestHandler('CityReconstructionService', 'saveDraft', (data) => {
  for (let x of data.requestData[0]) {
    let id =
      MainParser.CityMapData[x.entityId].cityentity_id +
      '#' +
      (MainParser.CityMapData[x.entityId].level || 0);
    ...
    if (x.position && !reconstruction.draft[x.entityId].position) {
      ...
      FoEproxy.triggerFoeHelperHandler('ReconstructionBuildingPlaced', {
        id: x.entityId,
        last: reconstruction.count[id].stored == 0,
      });

      $('#ReconstructionMapBody .map-grid').append(
        reconstruction.placeBuildingOnMap(x),
      );
    } else if (!x.position) {
      reconstruction.count[id].placed--;
      reconstruction.count[id].stored++;
    ...
    $('.reconstructionLine[data-page_id="' + id + '"] td:nth-child(2)').html(
      'x' + reconstruction.count[id].stored,
    );
    if (reconstruction.count[id].stored > 0)
      $('.reconstructionLine[data-page_id="' + id + '"]').show();
    else $('.reconstructionLine[data-page_id="' + id + '"]').hide();
...
let reconstruction = {
  ...
  pageUpdate: (id) => {
    ...
    if (
      ...
      return;
    let page =
      id[0] == 'W' ? 'prod' : reconstruction.pageMapper[meta.type] || meta.type;
    if (reconstruction.count[id].stored == 0) {
      //remove from pages
      reconstruction.pages[page].splice(
        reconstruction.pages[page].indexOf(id),
        1,
      );
    } else {
      //add to pages
  ...
  updateTable: () => {
    for (let [page, list] of Object.entries(reconstruction.pages)) {
      for (let i = 0; i < list.length; i++) {
        $(
          '.reconstructionLine[data-page_id="' + list[i] + '"] td:nth-child(3)',
        ).html(
          `<img src="${reconstruction.rcIcons[page]}">` +
            (Math.floor(i / 4) + 1),
        );
      }
    }
  ...
  showTable: () => {
    if (!reconstruction.rcIcons) {
      reconstruction.rcIcons = {
        happ: srcLinks.get(
          '/shared/gui/reconstructionmenu/rc_icon_happynessbuildings.png',
          true,
        ),
        prod: srcLinks.get(
          '/shared/gui/reconstructionmenu/rc_icon_productionbuildings.png',
          true,
        ),
        greatbuilding: srcLinks.get(
          '/shared/gui/constructionmenu/icon_greatbuilding.png',
          true,
        ),
        street: srcLinks.get(
          '/shared/gui/constructionmenu/icon_street.png',
          true,
        ),
      };
  ...
  showMap: () => {
    ...
    let storedUnit = parseInt(
      localStorage.getItem('ReconstructionMapScale') || 80,
    );
  ...
  placeBuildingOnMap: (data) => {
    let meta =
      MainParser.CityEntities[
        MainParser.CityMapData[data.entityId].cityentity_id
      ];
    if (
      meta.type.includes('hub') ||
      meta.type === 'off_grid' ||
      meta.type === 'outpost_ship' ||
      meta.type === 'friends_tavern'
    )
      return '';
  ...
  mapSettings: () => {
    ...
    $('#ReconstructionMapSettingsBox')
      .html(c)
      .promise()
      .done(function () {
        $('#ReconstructionMapSettingsBox .scale-view').on(
          'change',
          function () {
            let unit = parseFloat(
              $('.scale-view option:selected').data('scale'),
            );
        ...
        $('#ReconstructionMapSettingsBox .opacity').on('change', function () {
          let val = parseFloat(
            $('#ReconstructionMapSettingsBox .opacity').val(),
          );
