...
CABlocker = {
  ...
  addBlocker: (type = 'FP') => {
    $('body').append(`
            <div id="CollectAllOverlay" class="MapActivityHide ActiveOnmain clickable">
                ${[...new Set(MainParser.Quests.map((x) => x.category))]
                  .filter((x) => !['story', 'outpost', 'allies'].includes(x))
                  .map((x) => `<div class="QuestDummy"></div>`)
                  .join('')}
  ...
  checkBuildings: () => {
    ...
    setTimeout(async () => {
      ...
      await ExistenceConfirmed(
        'MainParser.CityMapData||MainParser.CityEntities',
      );
      CABlocker.setTimer();
      let now = GameTime.get();
      let finishedProductions = Object.values(MainParser.CityMapData).filter(
        (x) =>
          x.state &&
          x.state.productionOption &&
          (!x.state.next_state_transition_at ||
            x.state.next_state_transition_at < now) &&
          !x.state.pausedAt,
      );
      $('#CollectAllOverlay').remove();
      ...
      if (!FPcheck) {
        let notMotivated = finishedProductions.filter(
          (x) => x?.state?.socialInteractionId != 'motivate',
        );
        for (let building of notMotivated) {
          let meta = MainParser.CityEntities[building.cityentity_id];
          let motivateable =
            meta?.components?.AllAge?.socialInteraction?.interactionType ==
              'motivate' ||
            JSON.stringify(meta.abilities).includes('MotivatableAbility');
          if (motivateable) {
  ...
  setTimer: () => {
    ...
    let now = GameTime.get();
    let ongoingProductions = Object.values(MainParser.CityMapData).filter(
      (x) =>
        x.state &&
        x.state.productionOption &&
        x.state.next_state_transition_at > now &&
    ...
    let nextFinish = Math.min(
      ...ongoingProductions.map(
        (x) => new Date(x.state.next_state_transition_at),
      ),
    );
  ...
  checkFP: async () => {
    ...
    if (
      ...
      (GoodsData?.strategy_points?.abilities?.collectingRestricted?.maxAmount ||
        99)
    )
      return false;
