...
FoEproxy.addHandler(
  ...
  (data, postData) => {
    ...
    GBGBuildings.costs = {};
    data.responseData.availableBuildings.forEach(
      (x) => (GBGBuildings.costs[x.buildingId] = x.costs.resources),
    );
    GBGBuildings.buildings = data.responseData.placedBuildings
      .map((x) => x.id)
      .sort(
        (a, b) => (GBGBuildings.block[b] || 0) - (GBGBuildings.block[a] || 0),
      );
...
FoEproxy.addMetaHandler('battleground_buildings', (data, postData) => {
  GBGBuildings.BuildingData = Object.assign(
    {},
    ...JSON.parse(data.responseText).map((x) => ({ [x.id]: x })),
  );
});

FoEproxy.addHandler('ClanService', 'getTreasury', (data, postData) => {
  if (data.responseData.resources)
    GBGBuildings.treasury = data.responseData.resources;
...
FoEproxy.addHandler('ClanService', 'getTreasuryBag', (data, postData) => {
  ...
  if (data.responseData.resources)
    GBGBuildings.treasury = data.responseData.resources.resources;
  if (GBGBuildings.Timeout.B) {
    GBGBuildings.calc();
    return;
  }
...
let GBGBuildings = {
  treasury: {},
  block: {
    // get from GBG building meta-data: Object.assign({"free":0},...x.map(b=>({id:b.id,value:Number(b.description.replace(/.*? (\d+)% chance to not increase.*/gm,"$1"))})).filter(b=>b.value).sort((a,b)=>a.value-b.value).map(b=>({[b.id]:b.value})))
    free: 0,
    watchtower: 8,
  ...
  calc: () => {
    ...
    for (s of sets) {
      ...
      for (b of GBGBuildings.buildings) {
        ...
        if (i >= 0) {
          needed.splice(i, 1);
          keep.push(b);
        } else if (b != 'free') leftStanding.push(b);
      }
      if (leftStanding.length > 0) {
        for (b of needed) {
          let i = leftStanding.findIndex(
            (x) =>
              GBGBuildings.block[x] >= GBGBuildings.block[b] && b != 'free',
          );
          if (i >= 0) s['ignore'] = true;
        }
        if (leftStanding.length <= keep.filter((x) => x == 'free').length)
          s['ignore'] = true;
      }
      if (!s.ignore) {
        for (let n of needed) {
      ...
      s['title'] = title
        ...
        .map((x) =>
          HTML.i18nReplacer(i18n('Boxes.GBGBuildings.relativeCosts'), {
            good: GoodsData[x.good].name,
            amount: (x.rel * 100).toPrecision(2),
            era: i18n('Eras.' + Technologies.Eras[GoodsData[x.good].era]),
          }),
        )
        .join('\n');
    }
    ...
    for (let i = 0; i < sets.length; i++) {
      ...
      for (let j = i + 1; j < sets.length; j++) {
        ...
        if (
          sets[j][sortby] >= sets[i][sortby] &&
          sets[j].absCosts >= sets[i].absCosts
        ) {
          sets[j].ignore = true;
          continue;
        }
    ...
    let src = (b) => {
      let link = '';
      link = srcLinks.get(
        '/guild_battlegrounds/hud/guild_battlegrounds_sector_buildings_' +
          b +
          '_gbg2024.png',
        true,
        true,
      );
      if (link.includes('antiquedealer_flag'))
        link = srcLinks.get(
          '/guild_battlegrounds/hud/guild_battlegrounds_sector_buildings_' +
            b +
            '.png',
  ...
  createSets: () => {
    let sets = {};
    let current = GBGBuildings.buildings
      .map((x) => GBGBuildings.block[x] || 0)
      .reduce((a, b) => a + b, 0);
    if (current > GBGBuildings.settings.max)
      current = GBGBuildings.settings.max;
    let num = GBGBuildings.buildings.length;
    if (current >= GBGBuildings.settings.target) return [];
    let b = [
      ...Object.keys(GBGBuildings.costs).filter((value) =>
        Object.keys(GBGBuildings.block).includes(value),
      ),
      'free',
    ];
    let one = b.map((x) => [x]);
    ...
    all.forEach((x) => {
      ...
      if (t > current && ignore == false)
        sets[JSON.stringify(x)] = { all: x, block: t };
    });
  ...
  storeBuildingCosts: (data) => {
    ...
    if (stored.GBGRound != GuildFights.CurrentGBGRound) stored = {};
    ...
    localStorage.setItem(
      'GBGBuildingCosts',
      JSON.stringify(
        Object.assign(stored, { GBGRound: GuildFights.CurrentGBGRound }),
      ),
    );
  },
};
