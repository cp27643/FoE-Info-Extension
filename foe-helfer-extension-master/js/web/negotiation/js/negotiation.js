...
let Negotiation = {
  ...
  CalcBody: () => {
    ...
    if (Negotiation.CurrentTable !== null) {
      ...
      if (CurrentTry === 1) {
        h.push(
          '<th colspan="2"><label class="game-cursor" for="NegotiationSaveCurrentEraGoods">' +
            i18n('Boxes.Negotiation.SaveCurrentEraGoods') +
            '<input id="NegotiationSaveCurrentEraGoods" class="negotation-setting game-cursor" type="checkbox" data-id="NegotiationSaveCurrentEraGoods"' +
            (sceg === null || sceg === 'true' ? ' checked' : '') +
            '></label></th>',
        );
        h.push(
          '<th><label class="game-cursor" for="NegotiationSaveMedals">' +
            i18n('Boxes.Negotiation.SaveMedals') +
            '<input id="NegotiationSaveMedals" class="negotation-setting game-cursor" type="checkbox" data-id="NegotiationSaveMedals"' +
            (sm === null || sm === 'true' ? ' checked' : '') +
            '></label></th>',
        );
      }
      h.push(
        '<th class="text-right" colspan="' +
          ...
          (Negotiation.TryCountIsGreaterThan5 ?
            'data-title="' + i18n('Boxes.Negotiation.ChanceGreaterThan5')
          : '') +
          '">' +
          i18n('Boxes.Negotiation.Chance') +
          ': ' +
          HTML.Format(MainParser.round(Negotiation.CurrentTable['c'])) +
          (Negotiation.TryCountIsGreaterThan5 ? '% ⚠️ - ' : '% - '),
      );
      h.push('<b style="padding-right: 15px"> ');
      h.push(
        i18n('Boxes.Negotiation.Round') +
          ' ' +
          (Guesses.length + 1) +
          '/' +
          Negotiation.TryCount,
      );
      h.push('</b></strong></th>');
      h.push('</tr>');
      h.push('</thead>');
      ...
      h.push(
        '<td class="text-warning">' +
          i18n('Boxes.Negotiation.Average') +
          '</td>',
      );

      h.push(
        '<td colspan="4"><div id="good-sort" ' +
          (CurrentTry === 1 ? '  class="goods-dragable"' : '') +
          '>',
      );
      ...
      for (let i = 0; i < Negotiation.GoodCount; i++) {
        ...
        h.push(
          '<div class="good" data-slug="' +
            GoodName +
            '" title="' +
            HTML.i18nTooltip(i18n('Boxes.Negotiation.Stock')) +
            ' ' +
            HTML.Format(Stock) +
            '">' +
            '<span class="goods-sprite ' +
            GoodName +
            '"></span><br>' +
            '<span class="text-' +
            TextClass +
      ...
      if (Negotiation.CurrentTry === 1) {
        ...
        h.push(
          '<td colspan="5" class="text-center"><small>' +
            i18n('Boxes.Negotiation.DragDrop') +
            '</small></td>',
        );
        h.push('</tr>');
      }
    ...
    if (StockState === 1) {
      h.push(
        '<p class="text-center text-warning"><strong>' +
          i18n('Boxes.Negotiation.GoodsLow') +
          '</strong></p>',
      );
    } else if (StockState === 2) {
      h.push(
        '<p class="text-center text-danger"><strong>' +
          i18n('Boxes.Negotiation.GoodsCritical') +
          '</strong></p>',
      );
    }
        ...
        if (Negotiation.CurrentTable != null && Negotiation.CurrentTry === 1) {
          ...
          new Sortable(document.getElementById('good-sort'), {
            ...
            onEnd: function () {
              ...
              if (
                $('#negotiationBox')[0] !== undefined &&
                $('#negotiationBox')[0]['children'] !== undefined
              ) {
                for (
                  let i = 0;
                  i < $('#negotiationBox')[0]['children'].length;
                  i++
                ) {
                  if (
                    $('#negotiationBox')[0]['children'][i]['className'] ===
                    'tooltip fade top in'
                  ) {
              ...
              $('.good').each(function () {
                ...
                Negotiation.GoodsOrdered.push(
                  oldOrdered.find((info) => info.resourceId === resourceId),
                );
              });
              Negotiation.GoodsOrdered.forEach(
                (elem, i) => (elem.plannedPos = i),
              );
  ...
  createGuessLines: (h, includeLastLine) => {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      for (let place = 0; place < Negotiation.PlaceCount; place++) {
        ...
        const good_id =
          SlotGuess && SlotGuess.good ? SlotGuess.good.resourceId : 'empty';
        const matchStyleClass =
          SlotGuess.good !== null ?
            [' guess_match', ' guess_wrong_person', ' guess_fail'][
              SlotGuess.match
            ]
          : '';
        ...
        if (SlotSugestion) {
          const missmatch =
            !SlotGuess || SlotSugestion !== SlotGuess.good ? ' missmatch' : '';
          h.push(
            `<span class="goods-sprite cornered${missmatch} ${SlotSugestion.resourceId}"></span>`,
          );
        }
        h.push('</td>');
      }
    ...
    if (nextRoundSuggestion) {
      ...
      if (Negotiation.CurrentTable) {
        ...
        if (colorIdx + 1 < colors.length) {
          ...
          const colorR = Math.min(
            255,
            Math.max(0, MainParser.round(color1[0] * invMix + color2[0] * mix)),
          );
          const colorG = Math.min(
            255,
            Math.max(0, MainParser.round(color1[1] * invMix + color2[1] * mix)),
          );
          const colorB = Math.min(
            255,
            Math.max(0, MainParser.round(color1[2] * invMix + color2[2] * mix)),
          );
      ...
      for (let place = 0; place < Negotiation.PlaceCount; place++) {
        ...
        if (slotSugestion) {
          ...
          h.push(`<span class="goods-sprite ${good_id}"></span>`);
          h.push(
            `<span class="numberIcon" title="${HTML.i18nReplacer(i18n('Boxes.Negotiation.KeyboardTooltip'), { place: place + 1, slot: (slotSugestion.id + 1) % 10 })}">${place + 1} ${(slotSugestion.id + 1) % 10}</span>`,
          );
          h.push('</td>');
        } else {
          h.push('<td>&nbsp;</td>');
    ...
    for (let place = 0; place < Negotiation.PlaceCount; place++) {
      ...
      for (let good of GoodsOrdered) {
        if (good.canOccur.includes(place)) {
          ...
          h.push(
            `<span class="goods-sprite multiple ${good.resourceId}${hasToOccurClass}"></span>`,
          );
        }
      }
      h.push('</td>');
  ...
  StartNegotiation: (responseData, forcedTryCount) => {
    if (responseData.context === Negotiation.CONST_Context_GBG) {
      if (!$('#negotiation-Btn').hasClass('hud-btn-red')) {
        ...
        _menu.toolTipp(
          ...
          '<em id="negotiation-Btn-closed" class="tooltip-error">' +
            i18n('Menu.Negotiation.Warning') +
            '<br></em>' +
            i18n('Menu.Negotiation.Desc'),
        );
      }
    ...
    GoodsOrdered.sort((a, b) =>
      Negotiation.goodButtonCompare(a.resourceId, b.resourceId),
    );
    ...
    let tableName = Negotiation.GetTableName(
      Negotiation.TryCount,
      Negotiation.GoodCount,
    );
    Negotiation.GetTable(tableName).then((table) => {
      ...
      if (
        Settings.GetSetting('AutomaticNegotiation') &&
        $('#negotiationBox').length === 0
      ) {
        Negotiation.Show();
      }
    });
  ...
  SubmitTurn: (responseData) => {
    ...
    const CurrentGuess = [...new Array(Negotiation.PlaceCount).keys()].map(
      () => ({ good: null, match: 0 }),
    );
    ...
    for (let data of SlotData) {
      ...
      const goodInfo = GoodsOrdered.find(
  ...
  updateKnownPosition: (good) => {
    ...
    for (let otherGood of Negotiation.GoodsOrdered) {
      if (otherGood !== good) {
        ...
        otherGood.canOccur = otherGood.canOccur.filter(
          (place) => place !== fixedPlace,
        );
  ...
  updateNextGuess: () => {
    ...
    for (let i = 0; i < gu.length; i++) {
      GuessesSuggestion[PlaceMutation[i]] =
        gu[i] === 255 ? null : GoodsOrdered[gu[i]];
    }
    Negotiation.GuessesSuggestions[Negotiation.CurrentTry - 1] =
      GuessesSuggestion;
  },
  ...
  updateInitialPermutation: () => {
    if (Negotiation.CurrentTry !== 1) {
      console.error(
        'ERROR: Negotiation.updateInitialPermutation got called when not in first turn.',
      );
      return;
    }
    const PlaceCount = Negotiation.PlaceCount;
  ...
  findMatchingPermutation: () => {
    ...
    const MainTable =
      Negotiation.Tables[
        Negotiation.GetTableName(Negotiation.TryCount, Negotiation.GoodCount)
      ];
    ...
    for (let permutation of helper.permutations([
  ...
  GetGoodValue: (GoodName) => {
    ...
    if (GoodName === 'money') {
      ...
      let SaveMedalSetting = localStorage.getItem(
        'NegotiationSaveCurrentEraGoods',
      );
      if (SaveMedalSetting === 'false') {
        // default true
        Value = 100;
...
FoEproxy.addHandler('all', 'all', (data, postData) => {
  if (data.requestMethod === 'startNegotiation') {
    ...
    Negotiation.timeout = setTimeout(() => {
      ...
      Negotiation.StartNegotiation(
        /** @type {FoE_Class_NegotiationGame} **/ (Negotiation.tempStore),
      );
  ...
  if (
    data.requestClass === 'NegotiationGameService' &&
    data.requestMethod === 'submitTurn'
  ) {
    Negotiation.SubmitTurn(
      /** @type {FoE_Class_NegotiationGameResult} **/ (data.responseData),
    );
    return;
  }
  if (
...
let NegotiationDebugger = {
  ...
  BuildBox: () => {
    ...
    h.push(
      '<label>Num Goods: <select id="NegotiationDebuggerNumGoods" onchange="NegotiationDebugger.updateNumbers()">',
    );
    for (let i = 3; i <= 10; i++) {
      h.push(
        `<option value="${i}"${i === data.numGoods ? ' selected' : ''}>${i} Goods</option>`,
      );
    }
    h.push('</select></label>');

    h.push(
      '<label>Num Tries: <select id="NegotiationDebuggerNumTries" onchange="NegotiationDebugger.updateNumbers()">',
    );
    for (let i = 3; i <= 4; i++) {
      h.push(
        `<option value="${i}"${i === data.numTries ? ' selected' : ''}>${i} Tries</option>`,
      );
    }
    h.push('</select></label>');
    h.push('<button onclick="NegotiationDebugger.start()">Start</button>');
    h.push(
      `Always show Possible Matches: <input type="checkbox" onchange="NegotiationDebugger.changeAlwaysShowPossibleMatch(this)"${Negotiation.ContinueListing ? ' checked' : ''} />`,
    );

    h.push(
      '<div style="display:grid; grid-template-columns: repeat(auto-fill, 60px);">',
    );
    ...
    for (let i = 0; i < numGoods; i++) {
      ...
      h.push(
        `<div>Good ${i + 1}:<br/><span onclick="NegotiationDebugger.selectGoodFor(${i})" style="box-sizing: border-box;width: 40px${data.selected === i ? '; border: 1px solid red' : ''}" class="${goodSpriteClass(good)}"></span></div>`,
      );
    }
    h.push('</div>');
    ...
    if (data.selected >= 0) {
      h.push(
        '<div style="display: grid; grid-template-columns: repeat(auto-fill, 40px);background-color: rgba(200,200,200,0.4);border: 1px solid rgb(243, 214, 160);margin:5px;">',
      );
      for (let goodData of GoodsList) {
        let good = goodData.id || 'empty';
        h.push(
          `<span onclick="NegotiationDebugger.selectGood('${good}')" style="box-sizing: border-box;width: 40px${data.goods[data.selected] === good ? '; border: 1px solid red' : ''}" class="${goodSpriteClass(good)}"></span>`,
        );
      }
    ...
    for (let pos = 0; pos < 5; pos++) {
      ...
      h.push(
        `<div style="display: grid; grid-template-columns: repeat(auto-fill, 40px);">`,
      );
      for (let i = 0; i < numGoods; i++) {
        let good = data.goods[i] || 'empty';
        ...
        h.push(
          `<span onclick="NegotiationDebugger.selectGoodForSubmit(${pos}, ${i})" style="box-sizing: border-box;width: 40px${selected ? '; border: 1px solid red' : ''}" class="${goodSpriteClass(good)}"></span> `,
        );
      }
      h.push(`</div>`);
      ...
      if (selectedValue == null)
        selectedValue = Selectables[Selectables.length - 1].value;
      for (let selectable of Selectables) {
        const checked = selectable.value === selectedValue ? ` checked` : '';
        const checkedStyle =
          selectable.value === selectedValue ?
            ` style="background-color: ${selectable.selectedColor};"`
          : '';
        h.push(
          `<label${checkedStyle}>` +
            `${selectable.text}: ` +
  ...
  updateNumbers: () => {
    ...
    data.selected = -1;
    // @ts-ignore
    data.numGoods = Number.parseInt(
      document.getElementById('NegotiationDebuggerNumGoods').value,
    );
    // @ts-ignore
    data.numTries = Number.parseInt(
      document.getElementById('NegotiationDebuggerNumTries').value,
    );
    NegotiationDebugger.BuildBox();
  },
  ...
  submit: () => {
    ...
    for (let pos = 0; pos < 5; pos++) {
      ...
      const val = Number.parseInt(
        document.querySelector(
          'input[name=NegotiationDebugger_Submit' + pos + ']:checked',
        ).value,
      );
      if (val >= 0) {
        if (val !== 0) won = false;
    ...
    try {
      Negotiation.SubmitTurn(
        /** @type {FoE_Class_NegotiationGameResult}*/ ({
