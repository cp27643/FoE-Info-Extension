...
FoEproxy.addHandler('GrandPrizeService', 'getGrandPrizes', (data, postData) => {
  FPCollector.currentEvent = data.responseData[0]['context'].replace(
    /_tournament/g,
    '',
  );
});
...
FoEproxy.addHandler(
  ...
  (data, postData) => {
    if (!FPCollector.currentEvent) {
      FPCollector.currentEvent = data.responseData['context'].replace(
        /_tournament/g,
        '',
      );
    }
...
FoEproxy.addHandler('RewardService', 'collectReward', (data, postData) => {
  ...
  if (FPCollector.currentEvent !== null) {
    ...
    if (
      eventCheck.toLowerCase().includes('grandprize') ||
      eventCheck.includes('grand_prize') ||
      d['type'].includes('grand_prize') ||
      eventCheck.includes('event_pass')
    ) {
      event = FPCollector.currentEvent;
      notes = i18n('Boxes.FPCollector.grand_prize');
    }
  ...
  if (d['subType'] !== 'strategy_points') {
    if (data.responseData[1] === 'castle_system') {
      // Tägliche Belohnungskiste
      event = 'castle_system_daily_reward_chest';
      notes = d['name'];
      amount = d.rewards[0]['amount'];
    } else if (data.responseData[1] === 'pvp_arena') {
      // persönlicher Rang gewinn Truhe
      event = 'pvp_arena';
      notes = d['name'];
      amount = 0;
      rewards = d['rewards'];
    ...
    // default is hiddenreward or leaguereward or flying island incidents
    event = 'hiddenReward';
...
FoEproxy.addHandler(
  ...
  (data, postData) => {
    ...
    if (
      !d['rewardResources'] ||
      !d['rewardResources']['resources'] ||
      !d['rewardResources']['resources']['strategy_points'] ||
      !postData[0] ||
    ...
    StrategyPoints.insertIntoDB({
      ...
      notes:
        player ?
          `<img alt="${player['PlayerName']}" src="${srcLinks.GetPortrait(player['Avatar'])}"><span>${MainParser.GetPlayerLink(player['PlayerID'], player['PlayerName'])}</span>`
        : '',
...
FoEproxy.addHandler('OtherPlayerService', 'rewardPlunder', (data, postData) => {
  setTimeout(function () {
    for (let i = 0; i < data.responseData.length; i++) {
      ...
      if (
        PlunderReward['product'] &&
        PlunderReward['product']['resources'] &&
        PlunderReward['product']['resources']['strategy_points']
      ) {
        let PlunderedFP =
          PlunderReward['product']['resources']['strategy_points'];
        ...
        StrategyPoints.insertIntoDB({
          ...
          notes:
            player ?
              `<img src="${srcLinks.GetPortrait(MainParser.PlayerPortraits[player['Avatar']])}"><span>${MainParser.GetPlayerLink(player['PlayerID'], player['PlayerName'])}${entity ? ' - ' + entity['name'] : ''}</span>`
            : '',
          amount: PlunderedFP,
          date: moment(MainParser.getCurrentDate()).format('YYYY-MM-DD'),
        });
...
FoEproxy.addHandler('CityMapService', 'showAppliedBonus', (data, postData) => {
  let BonusId = BonusService.Bonuses.find(
    (object) => object.type === 'double_collection',
  )?.id;
  if (!BonusId) return;
  for (let j in data['responseData']['bonus']) {
    ...
    let CityMapID = data['responseData']['entityId'],
      ...
      era = Technologies.getEraName(
        buildingData.cityentity_id,
        buildingData.level,
      );

    let building = CityBuildings.createBuilding(metaData, era, buildingData);
    let FPproduction = parseFloat(
    ...
    let FP =
      FPproduction +
      Math.round(FPproduction * (Boosts.Sums.forge_points_production / 100));
...
let FPCollector = {
  ...
  currentDateEndFilter: moment(MainParser.getCurrentDate()).format(
    'YYYY-MM-DD',
  ),
    ...
    if ($('#fp-collector').length < 1) {
      ...
      await StrategyPoints.db['ForgePointsStats']
        ...
        .then((resp) => {
          ...
          FPCollector.minDateFilter = moment(resp.date)
            .subtract(1, 'minute')
            .toDate();
        })
        .catch(() => {
          FPCollector.minDateFilter = moment(
            MainParser.getCurrentDate(),
          ).toDate();
        });
      ...
      if (startMoment && endMoment) {
        while (startMoment.isBefore(endMoment, 'day')) {
          let checkDate = await StrategyPoints.db['ForgePointsStats']
            .where('date')
            .equals(moment(startMoment).format('YYYY-MM-DD'))
            .toArray();

          if (checkDate.length === 0) {
            FPCollector.lockDates.push(
              moment(startMoment).format('YYYY-MM-DD'),
            );
          }
        ...
        let checkPresent =
          await StrategyPoints.db['ForgePointsStats'].toArray();
  ...
  buildBody: async () => {
    let tr = [];
    FPCollector.TodayEntries = await StrategyPoints.db['ForgePointsStats']
      .where('date')
      .between(
        FPCollector.currentDateFilter,
        FPCollector.currentDateEndFilter,
        true,
        true,
      )
      .toArray();
    ...
    if (FPCollector.TodayEntries.length === 0) {
      tr.push(
        `<div class="text-center" style="padding:15px"><em>${i18n('Boxes.FPCollector.NoEntriesFound')}</em></div>`,
      );
    } else {
  ...
  HandleAdvanceQuest: (PostData) => {
    if (PostData['requestData'] && PostData['requestData'][0]) {
      ...
      for (let Quest of MainParser.Quests) {
        if (Quest['id'] !== QuestID || Quest['state'] !== 'collectReward')
          continue;
          ...
          for (let Reward of Quest['genericRewards']) {
            if (Reward['type'] === 'outpost_complete_item') {
              ...
              let outpostData = Outposts.OutpostData,
                playthrough =
                  (
                    outpostData.completedPlaythroughs <
                    outpostData.playthroughs.length
                  ) ?
                    outpostData.playthroughs[outpostData.completedPlaythroughs]
                  : outpostData.playthroughs[
                      outpostData.playthroughs.length - 1
                    ],
                amount =
                  (playthrough.rewards[0].subType === 'strategy_points' ?
                    playthrough.rewards[0].amount
                  : 0) +
                  (playthrough.additionalRewardFromBoost ?
                    playthrough.additionalRewardFromBoost.amount
                  : 0);
            ...
            } else if (Reward['type'] === 'forgepoint_package') {
