...
let CityMap = {
    ...
    let timer = () => {
      if (!MainParser.BuildingUpgrades) {
        ...
        resolve(
          Object.assign(
            {},
            ...Object.values(MainParser.BuildingUpgrades)
              .filter((x) => x.upgradeItem.id.includes('ascended'))
              .map((x) =>
                x.upgradeSteps[0].buildingIds.map((Id, i) => ({
                  [Id]: x.upgradeSteps[1].buildingIds[i],
                })),
              )
              .flat(),
          ),
  ...
  init: (event) => {
    ...
    if (null !== scale) CityMap.map.scale = parseInt(scale);
    if (null !== view) CityMap.map.view = view;
    if (null !== outpostScale)
      CityMap.map.outpostScale = parseInt(outpostScale);

    let Data = MainParser.CityMapData;
    if (ActiveMap === 'cultural_outpost') Data = CityMap.CulturalOutpost.data;
    else if (ActiveMap === 'era_outpost') Data = CityMap.EraOutpost.data;
    else if (ActiveMap === 'guild_raids') {
      Data = CityMap.QI.data;
      Title =
        i18n('Boxes.General.Quantum_Incursion.short') +
        ' ' +
        i18n('Boxes.General.Level') +
        ' ' +
        CityMap.QI.level;
  ...
  PrepareBox: (Title, elemId = 'citymap-main') => {
    ...
    wrapper
      .append(
        $('<div id="map-container" />').append(
          $(
            `<div id="grid-outer" data-unit="${scaleUnit}" data-view="${CityMap.map.view}" />`,
          )
            .append($('<div id="map-grid" />'))
            .append($('<div id="map-buildings" />')),
        ),
      )
      .append($('<div id="sidebar" />').append($('<div id="map-filters" />')));

    $('#' + elemId + 'Header > .title').attr(
      'id',
      'map' + CityMap.hashCode(Title),
    );
    ...
    let dropView = $('<select id="menu-view" class="game-cursor" />')
      .append(
        $('<option class="game-cursor" data-view="normal" />')
          .prop('selected', CityMap.map.view === 'normal')
          .text(i18n('Boxes.CityMap.NormalPerspecitve')),
      )
      .append(
        $('<option class="game-cursor" data-view="skew" />')
          .prop('selected', CityMap.map.view === 'skew')
          .text(i18n('Boxes.CityMap.CavalierPerspecitve')),
      );
    ...
    let scaleView = $('<select id="scale-view" class="game-cursor" />')
      .append(
        $('<option class="game-cursor" data-scale="60" />')
          .prop('selected', scaleUnit === 60)
          .text('60%'),
      )
      .append(
        $('<option class="game-cursor" data-scale="80" />')
          .prop('selected', scaleUnit === 80)
          .text('80%'),
      )
      .append(
        $('<option class="game-cursor" data-scale="100" />')
          .prop('selected', scaleUnit === 100)
          .text('100%'),
      )
      .append(
        $('<option class="game-cursor" data-scale="120" />')
          .prop('selected', scaleUnit === 120)
          .text('120%'),
      )
      .append(
        $('<option class="game-cursor" data-scale="140" />')
          .prop('selected', scaleUnit === 140)
          .text('140%'),
      )
      .append(
        $('<option class="game-cursor" data-scale="160" />')
          .prop('selected', scaleUnit === 160)
          .text('160%'),
      )
      .append(
        $('<option class="game-cursor" data-scale="180" />')
          .prop('selected', scaleUnit === 180)
          .text('180%'),
      );
    ...
    $('#' + elemId).on('change', '#scale-view', function () {
      ...
      $('#map-container').scrollTo($('.highlighted'), 800, {
        offset: { left: -280, top: -280 },
        easing: 'swing',
      });
    });
    ...
    if (ActiveMap === 'main') {
      menu.append(
        $(
          '<input type="text" id="BuildingsFilter" placeholder="' +
            i18n('Boxes.CityMap.FilterBuildings') +
            '" oninput="CityMap.filterBuildings(this.value)">',
        ),
      );
      menu.append(
        $('<div class="btn-group" />')
          .append(
            $('<button class="btn ml-auto" />')
              .attr({
                id: 'copy-meta-infos',
                onclick: 'CityMap.copyMetaInfos()',
              })
              .text(i18n('Boxes.CityMap.CopyMetaInfos')),
          )
          .append(
            $('<button class="btn ml-auto" />')
              .attr({
                id: 'show-submit-box',
                onclick: 'CityMap.showSubmitBox()',
              })
              .text(i18n('Boxes.CityMap.ShowSubmitBox')),
          ),
      );
    }
    oB.append(wrapper);
    ...
    if (ActiveMap === 'guild_raids')
      if (CityMap.QI.data) {
        menu.append(
          $(
            `<button class="btn ml-auto" id="copy-meta-infos" onclick="CityMap.copyMetaInfos()" style="margin-left:auto" />`,
          ).text(i18n('Boxes.CityMap.CopyMetaInfos')),
        );
    ...
    if (ActiveMap === 'cultural_outpost')
      $('#citymap-wrapper').append(
        '<img class="clickable openOverview" data-original-title="' +
          i18n('Menu.OutP.Title') +
          '" onClick="Outposts.BuildInfoBox()" src="' +
          extUrl +
          'css/images/menu/vikings_ship.png">',
      );

    if (ActiveMap === 'OtherPlayer') {
      let townhall = Object.values(CityMap.OtherPlayer.mapData).find(
        (x) => x.type === 'main_building',
      );
      CityMap.OtherPlayer.eraName =
        townhall.cityentity_id?.split('_')[1] ||
        townhall.entityId?.split('_')[1];

      $('#sidebar').append(
        $(
          '<a id="openEfficiencyRating" class="btn" onclick="Productions.ShowRating(true,\'' +
            CityMap.OtherPlayer.eraName +
            '\')">' +
            i18n('Menu.ProductionsRating.Title') +
            '</a>',
        ),
      );
    }
  ...
  BuildGrid: () => {
    ...
    if (ActiveMap === 'OtherPlayer') ua = CityMap.OtherPlayer.unlockedAreas;
    let xOffset = 0;
    let yOffset = 0;
    if (ActiveMap === 'cultural_outpost') {
  ...
  SetOutpostBuildings: () => {
    ...
    for (let b in buildings) {
      ...
      let hours = Math.round(
        (CityMapEntity.state.next_state_transition_in / 60 / 60) * 100,
      );
      if (
        ActiveMap === 'guild_raids' &&
        CityMapEntity.state.__class__ === 'ProducingState' &&
        CityMapEntity.state.next_state_transition_in < thresholdTime
      ) {
        collectSoon = ' collectSoon collect' + (hours < 100 ? '' : hours);
      }
      let collectionString = HTML.i18nReplacer(
        i18n('Boxes.CityMap.CollectSoon'),
        { hours: hours / 100 },
      );
      f = $('<span />')
        .addClass('entity ' + d['type'] + collectSoon)
        .css({
          width: xsize + 'em',
          height: ysize + 'em',
          left: xx + 'em',
  ...
  showQIStats: () => {
    ...
    out +=
      '<p class="text-center"><i>' + i18n('Boxes.CityMap.QIHint') + '</i></p>';
    out += '<div class="flex between" style="margin-bottom: 10px;">';
    out +=
      '<span><img src="' +
      srcLinks.get(`/shared/gui/constructionmenu/icon_expansion.png`, true) +
      '" />' +
      CityMap.QI.areas.length +
      '</span>';
    out +=
      '<div class="popStats"><span class="prod population">' +
      CityMap.QI.stats.availablePopulation +
      '/' +
      CityMap.QI.stats.totalPopulation +
      '</span> ';
    let euphoria = Math.round(CityMap.QI.stats.euphoriaBoost * 100);
    out +=
      '<span class="prod happiness euphoria' +
      euphoria +
      '" title="' +
      CityMap.QI.stats.euphoria +
      '">' +
      euphoria +
      '%</span></div>';
    out += '</div>';
  ...
  showQIBuildingList: () => {
    ...
    out +=
      ...
      '</th><th class="population textright"></th><th class="happiness textright"></th><th>' +
      i18n('Boxes.CityMap.Boosts') +
      '</th></tr></thead>';
    out += '<tbody>';

    let uniques = {};
    for (let b of buildings) {
      if (!uniques[b.cityentity_id]) uniques[b.cityentity_id] = 1;
      else uniques[b.cityentity_id] += 1;
    }
    let uniqueBuildings = [];
    for (let [id, count] of Object.entries(uniques)) {
      ...
      CityMap.QI.stats.totalPopulation +=
        building.population >= 0 ? building.population * count : 0;
    ...
    let euphoriaFactor =
      CityMap.QI.stats.euphoria / CityMap.QI.stats.totalPopulation;
    CityMap.QI.stats.euphoriaBoost = 1.5;
    if (euphoriaFactor <= 0.2) CityMap.QI.stats.euphoriaBoost = 0.2;
    else if (euphoriaFactor > 0.2 && euphoriaFactor <= 0.6)
      CityMap.QI.stats.euphoriaBoost = 0.6;
    else if (euphoriaFactor > 0.6 && euphoriaFactor <= 0.8)
      CityMap.QI.stats.euphoriaBoost = 0.8;
    else if (euphoriaFactor > 0.8 && euphoriaFactor <= 1.2)
      CityMap.QI.stats.euphoriaBoost = 1;
    else if (euphoriaFactor > 1.2 && euphoriaFactor <= 1.4)
      CityMap.QI.stats.euphoriaBoost = 1.1;
    else if (euphoriaFactor > 1.4 && euphoriaFactor < 2.0)
      CityMap.QI.stats.euphoriaBoost = 1.2;
    ...
    for (let building of uniqueBuildings) {
      ...
      out +=
        ...
        building.type +
        "'>" +
        "<td><div class='building' data-original-title='" +
        building.name +
        "'>" +
        "<img src='" +
        srcLinks.get(
          '/city/buildings/' +
            building.entityId.replace(/^(\D_)(.*?)/, '$1SS_$2') +
            '.png',
          true,
        ) +
        "'>" +
      ...
      if (building.production !== null) {
        if (building.type === 'goods' || building.type === 'military') {
          ...
          out +=
            building.production.guild_raids_supplies ?
              '<span class="prod guild_raids_supplies">' +
              HTML.Format(building.production.guild_raids_supplies * -1.0) +
              '</span> '
            : ' ';
          out +=
            building.production.guild_raids_money ?
              '<span class="prod guild_raids_money">' +
              HTML.Format(building.production.guild_raids_money * -1.0) +
              '</span> '
            : '';
        } else {
      ...
      if (building.boosts !== null) {
        for (let boost of building.boosts) {
          ...
          CityMap.QI.stats.boosts.hasOwnProperty(boost.type) ?
            (CityMap.QI.stats.boosts[boost.type] +=
              boost.value * building.count)
          : (CityMap.QI.stats.boosts[boost.type] =
              boost.value * building.count);
        }
      }
  ...
  setQIBuilding: (data) => {
    ...
    if (production !== undefined && production.length === 1)
      // goods and units have multiple production options, rest has one
      production =
        data.components?.AllAge?.production?.options[0]?.products[0]
          ?.playerResources?.resources;
    else if (production !== undefined && production.length > 1)
      production =
        data.components?.AllAge?.production?.options[3]?.products[0]
          ?.requirements?.resources;
    if (data.type === 'main_building')
      production = data.available_products[0].product.resources;

    let euphoria =
      data.components?.AllAge?.staticResources?.resources?.resources
        ?.guild_raids_happiness;
    let boosts = data.components?.AllAge?.boosts?.boosts;
    let population =
      data.components?.AllAge?.staticResources?.resources?.resources
        .guild_raids_population;
  ...
  setOutpostBuilding: (data) => {
    ...
    let population = Object.keys(data.staticResources?.resources).find((x) =>
      x.includes(populationName),
    );
    population = data.staticResources?.resources[population];
    // for buildings requiring pop
    if (population === undefined) {
      population = Object.keys(data.requirements?.cost?.resources).find((x) =>
        x.includes(populationName),
      );
  ...
  showOutpostBuildings: () => {
    ...
    out +=
      '<thead><tr><th colspan="2">' +
      i18n('Boxes.CityMap.Building') +
      '</th><th class="population textright"></th><th><span class="goods-sprite diplomacy"></span></th>' +
      '<th>' +
      i18n('Boxes.CityMap.Boosts') +
      '</th></tr></thead>';
    out += '<tbody>';
    ...
    for (let building of uniqueBuildings) {
      if (
        building.type !== 'impediment' &&
        building.type !== 'street' &&
        building.type !== 'off_grid'
      ) {
        out +=
          "<tr class='" +
          building.type +
        ...
        out +=
          '<td class="textright">' +
          (building.diplomacy > 0 ? building.diplomacy : '') +
          '</td>';
        out += '<td>';
        if (building.production !== null) {
          for (let [prod, value] of Object.entries(building.production)) {
  ...
  SetMapBuildings: async (Data = null) => {
    ...
    let ActiveId =
      $('#grid-outer').find('.highlighted').data('entityid') || null;
    ...
    if (ActiveMap === 'OtherPlayer')
      buildingData = CityBuildings.createBuildings(
        Object.values(CityMap.OtherPlayer.mapData),
      );
    else
      buildingData = CityBuildings.createBuildings(
    ...
    let buildingsWithoutStreets = Object.values(buildingData).filter(
      (x) => x.type !== 'street',
    );
    let buildingRatings = Object.values(buildingsWithoutStreets).map((x) =>
      parseInt(x.rating.totalScore * 100),
    );
    ...
    for (const building of Object.values(buildingData)) {
      if (
        building.coords.x < MinX ||
        building.coords.x > MaxX ||
        building.coords.y < MinY ||
        building.coords.y > MaxY
      )
        continue;

      let x =
          building.coords.x === undefined ?
            0
          : parseInt(building.coords.x * CityMap.map.gridSize),
        y =
          building.coords.y === undefined ?
            0
          : parseInt(building.coords.y * CityMap.map.gridSize),
      ...
      let canAscend =
        (await CityBuildings.canAscend(building.entityId)) ? ' ascendable' : '';
      ...
      CityMap.metrics.areaAvailable =
        CityMap.metrics.area - CityMap.metrics.areaOccupied;
      StreetsNeeded +=
        building.state.connected && building.type !== 'street' ?
          (parseFloat(Math.min(building.size.width, building.size.length)) *
  ...
  getAreas: () => {
    ...
    if (ActiveMap !== 'OtherPlayer') {
      $('.building-stats')
        .html(
          '<img src="' +
            srcLinks.get(
              `/shared/gui/constructionmenu/icon_expansion.png`,
              true,
            ) +
            '" />' +
            '<span data-original-title="' +
            i18n('Boxes.CityMap.FreeArea') +
            '">' +
            txtFree +
            '</span> / <span data-original-title="' +
            i18n('Boxes.CityMap.WholeArea') +
            '">' +
            total +
            '</span>',
        )
    ...
    let sortedBldTypes = [];
    for (let x in CityMap.metrics.buildingTypes)
      sortedBldTypes.push([x, CityMap.metrics.buildingTypes[x]]);
    ...
    areaStats.push(
      '<p class="text-center"><b>' +
        CityMap.metrics.buildings +
        ' ' +
        i18n('Boxes.CityMap.BuildingsAmount') +
        '</b></p>',
    );
    areaStats.push('<ul>');
    for (let x in sortedBldTypes) {
      ...
      const pct = parseFloat((100 * count) / CityMap.metrics.buildings).toFixed(
        1,
      );
    ...
    areaStats.push(
      '<ul class="highlight-map">' +
        '<li onClick="CityMap.highlightNoStreetBuildings()" class="clickable"><span data-original-title="' +
        i18n('Boxes.CityMap.roadless') +
        ', ' +
        parseFloat(
          (100 * CityMap.metrics.roadlessBuildings) / CityMap.metrics.buildings,
        ).toFixed(1) +
        '%"><img src="' +
        srcLinks.get(
        ...
        CityMap.metrics.roadlessBuildingsArea +
        '</span></li>' +
        '<li onClick="CityMap.highlightGBGBuildings()" class="clickable"><span data-original-title="' +
        i18n('Boxes.CityMap.buildingFromGBG') +
        ', ' +
        parseFloat(
          (100 * CityMap.metrics.gbgBuildings) / CityMap.metrics.buildings,
        ).toFixed(1) +
        '%"><img src="' +
        srcLinks.get(
          `/cash_shop/gui/cash_shop_icon_navi_gbg_selected.png`,
          true,
        ) +
        '" />' +
        CityMap.metrics.gbgBuildings +
        ...
        srcLinks.get(`/shared/gui/constructionmenu/icon_expansion.png`, true) +
        '" />' +
        CityMap.metrics.gbgArea +
        '</span></li>' +
        '<li onClick="CityMap.highlightQIBuildings()" class="clickable"><span data-original-title="' +
        i18n('Boxes.CityMap.buildingFromQI') +
        ', ' +
        parseFloat(
          (100 * CityMap.metrics.qiBuildings) / CityMap.metrics.buildings,
        ).toFixed(1) +
        '%"><img src="' +
        srcLinks.get(
          `/guild_raids/windows/guild_raids_guild_raid_emblem.png`,
          true,
        ) +
        '" />' +
        CityMap.metrics.qiBuildings +
        ...
        srcLinks.get(`/shared/gui/constructionmenu/icon_expansion.png`, true) +
        '" />' +
        CityMap.metrics.qiArea +
        '</span></li>' +
        '<li onClick="CityMap.highlightLimitedBuildings()" class="clickable"><span data-original-title="' +
        i18n('Boxes.CityMap.limited') +
        ', ' +
        parseFloat(
          (100 * CityMap.metrics.limitedBuildings) / CityMap.metrics.buildings,
        ).toFixed(1) +
        '%"><img src="' +
        srcLinks.get(
        ...
        CityMap.metrics.limitedBuildingsArea +
        '</span></li>' +
        '<li onClick="CityMap.highlightAscendableBuildings()" class="clickable"><span data-original-title="' +
        i18n('Boxes.CityMap.ShowAscendableBuildings') +
        ', ' +
        parseFloat(
          (100 * CityMap.metrics.ascendableBuildings) /
            CityMap.metrics.buildings,
        ).toFixed(1) +
        '%"><img src="' +
        srcLinks.get(`/shared/icons/limited_building_upgrade.png`, true) +
        '" />' +
        CityMap.metrics.ascendableBuildings +
        '</span> <span><img src="' +
        srcLinks.get(`/shared/gui/constructionmenu/icon_expansion.png`, true) +
        ...
        CityMap.metrics.ascendableBuildingsArea +
        '</span></li>' +
        '<li onClick="CityMap.highlightDecayedBuildings()" class="clickable"><span data-original-title="' +
        i18n('Boxes.CityMap.ShowDecayedBuildings') +
        ', ' +
        parseFloat(
          (100 * CityMap.metrics.decayedBuildings) / CityMap.metrics.buildings,
        ).toFixed(1) +
        '%"><img style="filter:saturate(0.5)" src="' +
        srcLinks.get(`/shared/icons/limited_building_downgrade.png`, true) +
        '" />' +
        CityMap.metrics.decayedBuildings +
        '</span> <span><img src="' +
        srcLinks.get(`/shared/gui/constructionmenu/icon_expansion.png`, true) +
    ...
    areaStats.push(
      `<label for="show-worst-buildings"><input type="checkbox" id="show-worst-buildings" onclick="CityMap.highlightWorstBuildings()" /> ${i18n('Boxes.CityMap.ShowWorstBuildings')}</label>`,
    );
    if (ActiveMap !== 'OtherPlayer')
      areaStats.push(
        '<span onclick="Productions.ShowRating()" class="clickable"></span>',
      );
    areaStats.push('</li>');

    if (ActiveMap !== 'OtherPlayer')
      areaStats.push(
        `<li class="clickable"><label for="highlight-old-buildings"><input type="checkbox" id="highlight-old-buildings" onclick="CityMap.highlightOldBuildings()"> ${i18n('Boxes.CityMap.HighlightOldBuildings')}</label></li>`,
      );
    areaStats.push('</ul>');
    ...
    legends.push(
      `<span class="older-1 diagonal"></span> ${$('#map-container .older-1').length} ${i18n('Boxes.CityMap.OlderThan1Era')}<br>`,
    );
    legends.push(
      `<span class="older-2 diagonal"></span> ${$('#map-container .older-2').length} ${i18n('Boxes.CityMap.OlderThan2Era')}<br>`,
    );
    legends.push(
      `<span class="older-3 diagonal"></span> ${$('#map-container .older-3').length} ${i18n('Boxes.CityMap.OlderThan3Era')}<br>`,
    );
    legends.push(
      `<span class="too-old diagonal"></span> ${$('#map-container .too-old').length} ${i18n('Boxes.CityMap.OlderThan4Era')}<br>`,
    );
  ...
  showSubmitBox: () => {
    ...
    if ($CityMapSubmit.length < 1) {
      ...
      let desc =
        '<p class="text-center">' + i18n('Boxes.CityMap.Desc1') + '</p>';
      desc +=
        '<p class="text-center" id="msg-line"><button class="btn" onclick="CityMap.SubmitData()">' +
        i18n('Boxes.CityMap.Submit') +
        '</button></p>';
  ...
  SubmitData: () => {
    ...
    let currentDate = new Date(),
      d = {
        ...
        blockedAreas: CityMap.removeDoubleUnderscoreKeys(
          CityMap.Main.blockedAreas,
        ),
        goods: GoodsData,
        metaIDs: {
  ...
  copyMetaInfos: () => {
    ...
    switch (ActiveMap) {
      case 'guild_raids':
        ...
        data.UnlockedAreas = CityMap.removeDoubleUnderscoreKeys(
          CityMap.QI.areas,
        );
        break;
      default:
        data.CityMapData = CityMap.removeDoubleUnderscoreKeys(
          MainParser.CityMapData,
        );
        data.UnlockedAreas = CityMap.removeDoubleUnderscoreKeys(
          CityMap.Main.unlockedAreas,
        );
        data.CityEntities = CityMap.removeDoubleUnderscoreKeys(
          MainParser.CityEntities,
        );
        break;
    }
    data.CityEntities = CityMap.removeDoubleUnderscoreKeys(
  ...
  GetBuildingSize: (CityMapEntity) => {
    ...
    Ret['is_connected'] =
      CityMapEntity['state']['__class__'] !== 'UnconnectedState' &&
      CityMapEntity['state']['pausedAt'] === undefined &&
      CityMapEntity['state']['pausedState'] === undefined;
    ...
    if (CityEntity['requirements']) {
      ...
      if (!['street', 'main_building'].includes(CityEntity['type'])) {
        Ret['streets_required'] =
          CityEntity['requirements']['street_connection_level'] | 0;
      } else {
      ...
      Ret['ysize'] = Size['y'];
      Ret['streets_required'] =
        CityEntity?.components?.AllAge?.streetConnectionRequirement
          ?.requiredLevel | 0;
    }
    ...
    Ret['street_area'] =
      Ret['is_connected'] ?
        (parseFloat(Math.min(Ret['xsize'], Ret['ysize'])) *
          Ret['streets_required']) /
        2
      : 0;
  ...
  filterBuildings: (string) => {
    ...
    for (sp of spans) {
      ...
      if (
        string !== '' &&
        title.substr(0, title.toLowerCase().indexOf(string.toLowerCase()) > -1)
      ) {
...
let CityBuildings = {
  ...
  setPopulation: (metaData, data, era) => {
    ...
    if (metaData.__class__ !== 'GenericCityEntity') {
      // not a generic building
      if (metaData.entity_levels.length > 0) {
        // special building
        ...
        if (metaData.requirements.cost) {
          if (metaData.type === 'decoration') return 0;
            ...
            if (data.bonus)
              if (data.bonus.type === 'population') return data.bonus.value;
      ...
      if (metaData.components[era]) {
        // (time) limited buildings lose their era data after expiring
    ...
    if (metaData.__class__ !== 'GenericCityEntity') {
      if (metaData.entity_levels.length > 0) {
        ...
        if (metaData.entity_levels[eraId].provided_happiness)
          return data?.state?.__class__ === 'PolishedState' ?
              metaData.entity_levels[eraId].provided_happiness * 2
            : metaData.entity_levels[eraId].provided_happiness;
        return happiness;
      } else if (bgHappiness) {
        // great building, e.g. Alcatraz
        if (bgHappiness.type === 'happiness') return bgHappiness.value;
        return 0;
      } else if (metaData.provided_happiness) {
        // decorations etc.
        return isPolivated ?
            metaData.provided_happiness * 2
          : metaData.provided_happiness;
      ...
      if (metaData.components[era]) {
        ...
        if (bHappiness) {
          ...
          else return bHappiness.provided ? bHappiness.provided : happiness;
        }
        return happiness;
      }
    }
  ...
  setPolivation(data, metaData) {
    ...
    metaData.abilities.forEach((ability) => {
      if (ability.__class__ === 'MotivatableAbility') isPolivationable = true;
    ...
    if (metaData.__class__ === 'GenericCityEntity')
      isPolivationable =
        metaData.components.AllAge.socialInteraction !== undefined;

    if (isPolivationable) {
      if (metaData.__class__ !== 'GenericCityEntity') {
        if (data?.state?.boosted) return data?.state?.boosted;
        else if (data?.state?.is_motivated) return true;
        else if (isPolishable) {
          // decorations etc.
          if (data?.state?.next_state_transition_in) return true;
        }
        return false;
      } else {
        // generic buildings
        if (
          data?.state?.socialInteractionStartedAt > 0 &&
          data?.state?.socialInteractionId === 'polish'
        ) {
          if (
            data?.state?.socialInteractionStartedAt +
              43200 -
          ...
          data?.state?.socialInteractionStartedAt > 0 &&
          data?.state?.socialInteractionId === 'motivate'
        )
          return true;
        else return false;
      }
  ...
  setChainBuilding(metaData) {
    ...
    if (metaData.__class__ === 'GenericCityEntity') {
      ...
      type =
        (
          metaData.components?.AllAge?.chain?.config?.__class__ ===
          'ChainStartConfig'
        ) ?
          'start'
        : 'link';
      ...
      x =
        metaData.components?.AllAge?.chain?.linkPositions[0].topLeftPoint.x ||
        0;
      y =
        metaData.components?.AllAge?.chain?.linkPositions[0].topLeftPoint.y ||
        0;
    } else {
      metaData.abilities.forEach((ability) => {
        if (ability.chainId) {
  ...
  createChainedBuilding(allLinkedBuildings = []) {
    ...
    for (link of allLinkedBuildings) {
      ...
      chainedBuilding.size.width =
        chainedBuilding.size.width +
        (link.coords.x !== chainedBuilding.coords.x ? link.size.width : 0);
      chainedBuilding.size.length =
        chainedBuilding.size.length +
        (link.coords.y !== chainedBuilding.coords.y ? link.size.length : 0);
      chainedBuilding.happiness += link.happiness;

      if (link.boosts !== undefined)
        chainedBuilding.boosts = [
          ...(chainedBuilding.boosts || []),
          ...link.boosts,
        ];
      if (link.production !== undefined && link.production !== false) {
        chainedBuilding.production = [
          ...chainedBuilding.production,
          ...link.production,
        ];
      }
  ...
  setSetBuilding(metaData) {
    ...
    if (setId !== undefined) return { name: setId };
  },
  ...
  setBuildingBoosts(metaData, data, era, withAlly = true) {
    ...
    if (metaData.__class__ !== 'GenericCityEntity') {
      metaData.abilities.forEach((ability) => {
        ...
        if (
          (isSet !== undefined &&
            ability.__class__ === 'BonusOnSetAdjacencyAbility') ||
          (isChain !== undefined && ability.__class__ === 'ChainLinkAbility')
        ) {
            ...
            if (bonus.boost.length === 0) return;
              ...
              if (bonus.boost[eraName]) {
                let boost = {
                  ...
                  type: Boosts.Mapper[bonus.boost[eraName].type] || [
                    bonus.boost[eraName].type,
                  ],
                ...
                let boost = {
                  ...
                  type: Boosts.Mapper[bonus.boost.AllAge.type] || [
                    bonus.boost.AllAge.type,
                  ],
      ...
      if (metaData.type === 'greatbuilding') {
        if (data.bonus?.type) {
          ...
          if (
            data.bonus.type !== 'happiness_amount' &&
            data.bonus.type !== 'population'
          )
            boosts.push(boost);
        }
    ...
    if (withAlly) {
      ...
      if (allyStats?.currentLevel?.boosts || allyStats?.boosts) {
        (allyStats?.currentLevel?.boosts || allyStats?.boosts || []).forEach(
          (abilityBoost) => {
            let boost = {
              feature: abilityBoost.targetedFeature,
              type: Boosts.Mapper[abilityBoost.type] || [abilityBoost.type],
  ...
  setState(data) {
    if (
      (data?.state?.__class__ === 'IdleState' &&
        !data?.cityentity_id?.includes('CastleSystem')) ||
      data?.state?.__class__ === undefined
    )
      return 'idle';
    else if (data.state.__class__ === 'ProductionFinishedState')
      return 'collectable';
    else if (data.state.__class__ === 'PlunderedState') return 'plundered';
    else if (data.cityentity_id.includes('CastleSystem')) return 'producing';
    return 'producing';
  },

  // building is not in construction menu
  isSpecialBuilding(metaData) {
    if (metaData.__class__ === 'GenericCityEntity') return true; // generic buildings are always special
  ...
  needsStreet(metaData) {
    ...
    if (needsStreet === undefined) {
      ...
      if (
        metaData.components?.AllAge?.streetConnectionRequirement !== undefined
      )
        needsStreet =
          metaData.components.AllAge.streetConnectionRequirement.requiredLevel;
    }
    ...
    if (state === 'producing') {
      if (data.cityentity_id.includes('CastleSystem')) {
        return {
          ...
          in:
            MainParser.CastleSystemChest.dailyRewardCollectionAvailableAt -
            parseInt(Date.now() / 1000),
        };
      }
      return {
        at: data?.state?.next_state_transition_at,
        in: data?.state?.next_state_transition_in,
      };
    } else if (state === 'collectable') return { at: moment().unix(), in: 0 };
    return { at: undefined, in: undefined };
  },

  isExpiredBuilding(data) {
    if (data.type === 'generic_building')
      if (data.decayedFromCityEntityId !== undefined) return true;
    return false;
  },
  ...
  isLimitedBuilding(metaData) {
    if (metaData.components?.AllAge?.limited !== undefined) {
      ...
      if (
        metaData.components?.AllAge?.limited.config.collectionAmount !==
        undefined
      )
  ...
  isBoostableBuilding(metaData) {
    if (
      metaData.type === 'greatbuilding' ||
      metaData.type === 'main_building'
    ) {
      return false;
    } else if (metaData.id.includes('CastleSystem')) {
      return false;
    ...
    // wishingwell types
      ...
      metaData.id.includes('L_AllAge_EasterBonus1') ||
      metaData.id.includes('L_AllAge_Expedition16') ||
      metaData.id.includes('L_AllAge_ShahBonus17')
    ) {
      return false;
    }
    return true;
  ...
  setConnection(metaData, data) {
    ...
    if (!connected) connected = data?.connected === 1;
    return connected;
  },
  ...
  hasLinks(building, connectedBuildings = [], dirX = 0, dirY = 0) {
    ...
    if (building.chainBuilding?.type === 'start') {
      dirX =
        building?.chainBuilding?.chainPosX !== 0 ?
          parseInt(
            building?.chainBuilding?.chainPosX /
              Math.abs(building?.chainBuilding?.chainPosX),
          )
        : 0; // e.g. (-3/3) || 0
      dirY =
        building?.chainBuilding?.chainPosY !== 0 ?
          parseInt(
            building?.chainBuilding?.chainPosY /
              Math.abs(building?.chainBuilding?.chainPosY),
          )
        : 0;
    }
    ...
    if (x < 0 || x < 0 || y > 72 || y > 72)
      return connectedBuildings; // min and max of current map
    else {
      if (
        nextBuilding === undefined ||
        nextBuilding.chainBuilding === undefined ||
        nextBuilding.chainBuilding?.name !== building.chainBuilding.name ||
        nextBuilding.chainBuilding?.type === 'start'
      )
  ...
  isLinked(building, x = 0, y = 0) {
    if (typeof building?.chainBuilding?.chainPosX === 'number') {
      ...
      let prevBuilding = CityBuildings.getBuildingByCoords(
        building?.coords?.x - x1,
        building?.coords?.y - y1,
      );

      if (y1 < 0 || x1 < 0 || x1 > 72 || y1 > 72)
        return false; // min and max of current map
      else {
        if (prevBuilding !== undefined) {
          if (
            prevBuilding.chainBuilding === undefined ||
            prevBuilding.chainBuilding?.name !== building.chainBuilding.name
          )
            return false;
          if (
            prevBuilding.chainBuilding?.name === building.chainBuilding.name &&
            prevBuilding.chainBuilding?.type === ('linked' || 'start')
          ) {
            return true;
          }
        }
  ...
  setAllProductions(metaData, data, era) {
    let productions = [];
    if (
      metaData.__class__ !== 'GenericCityEntity' &&
      metaData.type !== 'greatbuilding'
    ) {
      if (metaData.is_special) {
        ...
        if (metaData.entity_levels[Technologies.InnoEras[era]] !== undefined) {
          // base money is here
          let money =
            metaData.entity_levels[Technologies.InnoEras[era]].produced_money;
          if (money)
            productions.push({
              type: 'resources',
              needsMotivation: false,
              resources: { money: money },
              doubleWhenMotivated: this.setPolivationable(metaData),
            });
          let power =
            metaData.entity_levels[Technologies.InnoEras[era]].clan_power; // hall of fame lvl 1
          if (power)
            productions.push({
              type: 'guildResources',
              needsMotivation: false,
        ...
        if (
          metaData.__class__ === 'CityEntityRandomProductProductionBuilding'
        ) {
          // if weird old building, use current production
          let currentProduction = this.setCurrentProductions(
            data,
            metaData,
            era,
          );
      ...
      if (metaData.type === 'main_building') {
        // add emissary production to town hall
        MainParser.EmissaryService?.forEach((emissary) => {
          let resource = {
            type:
              emissary.bonus.type !== 'unit' ?
                'resources'
              : emissary.bonus.type,
            needsMotivation: false,
            resources:
              emissary.bonus.type === 'unit' ?
                this.setUnitReward(emissary.bonus, true)
              : { [emissary.bonus.subType]: emissary.bonus.amount },
          };
          productions.push(resource);
        });
        MainParser.BonusService?.forEach((bonus) => {
          // guild FP
      ...
      if (metaData.id.includes('CastleSystem')) {
        // add castle system stuff
        let currentLevel = Castle.curLevel;
        era = CurrentEra;
        if (MainParser.CastleSystemLevels[currentLevel - 1] !== undefined)
          MainParser.CastleSystemLevels[currentLevel - 1].dailyReward[
            era
          ].rewards.forEach((reward) => {
            let resources = { [reward.subType]: reward.amount };
            if (reward.id.search('#') !== -1) {
              // "goods#random#CurrentEra#30" "goods#random#PreviousEra#15"
              amount = reward.id.match(/\d+$/)[0];
              if (
                reward.id.search('goods') !== -1 &&
                reward.id.search('CurrentEra') !== -1
              )
                resources = { random_good_of_age: amount };
              else if (
                reward.id.search('goods') !== -1 &&
                reward.id.search('PreviousEra') !== -1
              )
              ...
              if (bonus.revenue.length === 0) return;
                ...
                if (bonus.revenue[Technologies.InnoEras[era]]) {
                  let resource = {
                    ...
                    needsMotivation: false,
                    resources:
                      bonus.revenue[Technologies.InnoEras[era]].resources,
                  };
                  productions.push(resource);
      ...
      let production =
        ...
        metaData.components.AllAge.production; // currently it is either allage or era, never both

      // attached chain buildings
      if (
        metaData.components.AllAge?.chain !== undefined &&
        metaData.components.AllAge?.chain?.config?.__class__ !==
          'ChainStartConfig'
      ) {
        // resources are located here
        let chainProductions =
          metaData.components.AllAge?.chain?.config?.bonuses[0]?.productions;
        if (chainProductions !== undefined)
          for (const prod of chainProductions) {
            ...
            // generic reward
              ...
              let lookUp =
                metaData.components.AllAge?.lookup?.rewards[prod.reward.id];
              resource.resources = {
                id: prod.reward.id,
                name: lookUp?.name,
      ...
      if (production) {
        if (metaData.type === 'production') {
          // production buildings do not have a default production
        ...
        production.options[0].products.forEach((product) => {
          ...
          if (product.type === 'resources') {
            ...
            let specialGood = Object.keys(resource.resources).find((x) =>
              x.includes('special_good'),
            );
            ...
            let objectKey =
              Object.keys(resource.resources).length === 1 ?
                Object.keys(resource.resources)[0]
              : null;
            ...
            if (product.products.length > 1) {
              product.products.forEach((reward) => {
                if (reward.product.type === 'genericReward') {
                  // currently: everything but forge points
                  let lookupData =
                    metaData.components[era]?.lookup.rewards[
                      reward.product.reward.id
                    ] ||
                    metaData.components.AllAge.lookup.rewards[
                      reward.product.reward.id
                    ];
                  let subType = lookupData.subType;
                  let amount = lookupData.totalAmount || lookupData.amount;
                  let type =
                    lookupData.type === 'set' ? 'consumable' : lookupData.type;

                  if (
                    reward.product.reward.id.search('good') !== -1 &&
                    reward.product.reward.id.search('fragment') === -1
                  ) {
                    subType = Object.keys(
                      this.setGoodsRewardFromGeneric(lookupData),
                    )[0];
                  ...
                  let name = this.setRewardNameFromLookupData(
                    lookupData,
                    metaData,
                  );
                  let newReward = {
                  ...
                  if (
                    reward.product.playerResources.resources.strategy_points !==
                    undefined
                  ) {
                    // FP
                    let newReward = {
                      ...
                      subType: Object.keys(
                        reward.product.playerResources.resources,
                      )[0],
                      amount:
                        reward.product.playerResources.resources
                          .strategy_points,
                    ...
                    // some goods, nextage are genericReward
            ...
            console.log(
              'setAllProductions() is missing an option for ',
              metaData.name,
            );
          }
          productions.push(resource);
    ...
    if (state !== 'idle') {
      if (metaData.__class__ !== 'GenericCityEntity') {
        if (data.state.current_product) {
          ...
          if (data.state.current_product.clan_power) {
            // HoF
            let production = {
          ...
          if (data.state.current_product.product) {
            if (data.state.current_product.product.resources) {
              ...
              if (data.state.current_product.name === 'special_goods') {
                // space carrier
          ...
          if (data.state.current_product.goods) {
            // great buildings
          ...
          if (data.state.current_product.name === 'penal_unit') {
            // alcatraz
            let production = {
              resources: {
                random: parseFloat(data.state.current_product.amount),
              },
              type: 'unit',
            };
            productions.push(production);
          ...
          if (data.state.is_motivated) {
            metaData.abilities.forEach((ability) => {
              // random units are not in the data, they are in the metaData for some reason
        ...
        if (data.state.productionOption) {
          data.state.productionOption.products.forEach((production) => {
            ...
            if (production.type === 'resources') {
              ...
              let specialGood = FHResourcesList.find(
                (x) =>
                  x.id ===
                    Object.keys(production.playerResources?.resources)[0] &&
                  x.abilities.specialResource?.type === 'specialResource',
              );
              if (specialGood) resource.type = 'special_goods';
              ...
              console.log(
                metaData.name,
                'setCurrentProductions() production is missing',
              );
  ...
  setGenericReward(product, metaData, era) {
    ...
    if (metaData.components[era]) {
      let lookupRewards = structuredClone(
        metaData.components[era].lookup?.rewards || {},
      );
      if (metaData.components?.AllAge?.chain) {
        MainParser.BuildingChains[
          metaData.components.AllAge.chain.chainId
        ]?.cityEntityIds.forEach((chainBuilding) => {
          Object.assign(
            lookupRewards,
            MainParser.CityEntities[chainBuilding]?.components?.[era]?.lookup
              ?.rewards || {},
          );
        });
      }
      if (product.reward.id.search('blueprint') !== -1) {
        if (lookupRewards[product.reward.id]) {
          ...
          // blueprint chest, e.g. vineyard
          for (const [key, reward] of Object.entries(lookupRewards)) {
            if (reward.id.search('blueprint') !== -1) lookupData = reward;
          }
        }
      } else if (product.reward.type === 'good') {
        // this can break if there is more than one generic goods reward for a building
        for (const [key, reward] of Object.entries(lookupRewards)) {
          if (reward.id.includes('good')) lookupData = reward;
        }
      } else if (
        product.reward.id.includes('goods') &&
        !/(fragment|rush)/.test(product.reward.id)
      ) {
        // for nextage goods, because they are in a chest (random ones)
        ...
        reward = {
          ...
          amount:
            ...
            parseInt(product.reward.id.match(/\d+$/)[0]),
        ...
        if (lookupData === undefined) {
          let chest = Object.keys(lookupRewards).find((x) =>
            x.includes('chest' || 'random_unit'),
          ); // currently only applies to wish fountain or things with "random unit chest"
    ...
    if (lookupData)
      ...
      console.log(
        'setGenericReward() data missing for',
        metaData.name,
        metaData,
        product,
      );
    ...
    // units
    if (
      (lookupData?.type === 'chest' &&
        lookupData.id.search('genb_random_') !== -1 &&
        lookupData.id.search('fragment') === -1) ||
      lookupData?.type === 'unit' ||
      lookupData?.icon === 'military'
    ) {
    ...
    if (
      metaData.components?.AllAge?.chain === undefined ||
      (metaData.components?.AllAge?.chain !== undefined &&
        product.reward.subType !== 'fragment')
    )
      reward = {
        id: product.reward.id,
        name: name,
    ...
    if (reward.type === 'good') return this.setGoodsRewardFromGeneric(reward);
  ...
  setGoodsRewardFromGeneric(reward) {
    ...
    if (reward.possible_rewards !== undefined)
      // random productions
    ...
    if (reward.id.includes('NextEra') && !reward.id.includes('special_goods')) {
      eraString = 'next_';
    } else if (reward.id.includes('PreviousEra')) {
      // currently unused
      eraString = 'previous_';
  ...
  setUnitReward(product, isEmissary = false) {
    ...
    if (isEmissary) {
      ...
      if (product.id.search('#') !== -1) {
        // era_unit#light_melee#NextEra#1
        let prefix = '';
        if (product.id.search('NextEra') !== -1) prefix = 'next#';
      ...
      if (product.reward.id.search('#') !== -1) {
        // era_unit#light_melee#NextEra#1
        let prefix = '';
        if (product.reward.id.search('NextEra') !== -1) prefix = 'next#';
  ...
  setRewardNameFromLookupData(lookupData, metaData) {
    ...
    else if (lookupData.__class__ === 'GenericRewardSet')
      // this is a dirty workaround for trees of patience, because i lack patience
      name = lookupData.rewards[0]?.name;
    else if (
      lookupData.subType === 'speedup_item' ||
      lookupData.subType === 'reward_item' ||
      lookupData.subType === 'boost_item' ||
      lookupData.type === 'forgepoint_package' ||
      lookupData.type === 'resource'
    )
      name = lookupData.name;
    else if (lookupData.type === 'unit') {
      // -> (next_)light_melee
      if (lookupData.id.search('#') !== -1) {
        // era_unit#light_melee#NextEra#1
        let prefix = '';
        if (lookupData.id.search('NextEra') !== -1) prefix = 'next_';

        name = prefix + lookupData.id.split('#')[1];
      } else name = lookupData.id.replace('unit_', '').replace(/\d+/, '');
    } else if (lookupData.type === 'blueprint')
      // id: "blueprint#random#3"
      name = lookupData.name.replace(/^\+?\d+\s*/, '');
    else if (
      lookupData.type === 'chest' &&
      lookupData.id.includes('blueprint')
    ) {
      // remove +20 from the name becuase the amount is in the amount
      name = lookupData.name.replace(/^\+?\d+\s*/, '');
    } else if (lookupData.type === 'chest') {
      // chest can be: BP - see above, units, goods (next age)
      name = lookupData.name;
    } else if (lookupData.type === 'consumable') name = lookupData.name;
    else if (lookupData.type === 'good') {
      name = lookupData.name.replace(/^\+?\d+\s]*/, '');
    } else {
      console.log(
        'setRewardNameFromLookupData(): undefined name from type',
        metaData.name,
        lookupData,
        lookupData.type,
        lookupData.subType,
  ...
  setOldProductionResourceFromAbility(ability, era) {
    ...
    if (ability.__class__ === 'AddResourcesToGuildTreasuryAbility')
      ...
      ability.rewards[era].possible_rewards.forEach((reward) => {
        ...
        let newReward = {
          ...
          name:
            type.includes('good') ?
              reward.reward.name.replace(/^\d+\s*/, '')
            : reward.reward.name,
          type: type,
          amount: amount,
    ...
    if (ability.additionalResources) {
      if (ability.additionalResources[era])
        // MultiAge
  ...
  getBuildingByCoords(x, y) {
    return Object.values(MainParser.CityBuildingsData).find(
  ...
  getBuildingGoodsByEra(current, building, boosted = false) {
    ...
    if (productions) {
      let goodsBoost = 0;
      if (boosted) goodsBoost = Boosts.Sums.goods_production || 1;
      ...
      for (let production of productions) {
        ...
        if (
          ...
          Object.keys(production.resources).forEach((resourceName) => {
            ...
            if (good !== undefined) {
              ...
              goodEra = Technologies.getPreviousEraIdByCurrentEraName(
                building.eraName,
              );
              isGood = true;
            } else if (resourceName.includes('next_age')) {
              goodEra = Technologies.getNextEraIdByCurrentEraName(
                building.eraName,
              );
              isGood = true;
            } else if (
              resourceName.includes('random_good_of_') ||
              resourceName.includes('all_goods_of_')
            ) {
              isGood = true;
            }

            if (isGood) {
              let boostedExtra = Math.round(
                (production.resources[resourceName] * goodsBoost) / 100,
              );
              if (resourceName.includes('all_goods_of_'))
                boostedExtra =
                  Math.round(
                    ((production.resources[resourceName] / 5) * goodsBoost) /
                      100,
                  ) * 5;

              if (goods.eras[goodEra] === undefined)
                goods.eras[goodEra] =
                  parseInt(production.resources[resourceName]) + boostedExtra;
              else
                goods.eras[goodEra] +=
                  parseInt(production.resources[resourceName]) + boostedExtra;
            }
        ...
        if (production.type === 'random') {
          // e.g. gentania windmill, eerie terror coaster
          let goodEra = Technologies.InnoEras[building.eraName];
          for (const resource of production.resources) {
            if (
              ...
              if (
                resource.type.includes('previous') ||
                resource.subType?.toLowerCase().includes('previous') ||
                resource.id?.toLowerCase().includes('previous')
              )
                goodEra = Technologies.getPreviousEraIdByCurrentEraName(
                  building.eraName,
                );
              else if (
                resource.type.includes('next') ||
                resource.subType?.toLowerCase().includes('next') ||
                resource.id?.toLowerCase().includes('next')
              )
                goodEra = Technologies.getNextEraIdByCurrentEraName(
                  building.eraName,
                );
              else
                goodEra = Technologies.getEraIdByCurrentEraName(
                  building.eraName,
                );

              let boostedExtra = Math.round(
                (resource.amount * goodsBoost) / 100,
              );

              if (goods.eras[goodEra] === undefined)
                goods.eras[goodEra] = parseFloat(
                  (resource.amount + boostedExtra) * resource.dropChance,
                );
              else
                goods.eras[goodEra] += parseFloat(
                  (resource.amount + boostedExtra) * resource.dropChance,
        ...
        if (
          production.type === 'genericReward' &&
          /good/.test(production.resources?.icon)
        ) {
          // e.g. eco hub
          let goodEra = Technologies.InnoEras[building.eraName];
          if (production.resources.id.includes('previous'))
            ...
            production.resources.icon === 'next_age_goods' ||
            production.resources.id.includes('next')
          )
            goodEra = Technologies.getNextEraIdByCurrentEraName(
              building.eraName,
            );
          ...
          else goods.eras[goodEra] += parseInt(production.resources.amount);
        }
      }
    }
  ...
  getBuildingGuildGoodsByEra(current, building, boosted = false) {
    ...
    if (productions) {
      let goodsBoost = 0;
      if (boosted) goodsBoost = Boosts.Sums.guild_goods_production || 1;
      for (let production of productions) {
        if (
          production.type !== 'guildResources' ||
          production.resources === undefined
        )
          continue;
        ...
        for (let resourceName of Object.keys(production?.resources)) {
          ...
          if (isGood) {
            ...
            if (resourceName.includes('all_goods_of_'))
              boostedExtra =
                Math.round(
                  ((production.resources[resourceName] / 5) * goodsBoost) / 100,
                ) * 5;
            // dont apply boost to GBs
            if (building.type == 'greatbuilding') boostedExtra = 0;
  ...
  createBuildings(
    ...
    for (building of data) {
      if (ActiveMap === 'OtherPlayer' && building.eraName !== undefined)
        continue;
      let metaData = Object.values(MainParser.CityEntities).find(
        (x) => x.id === building.cityentity_id,
      );
      let era = Technologies.getEraName(building.cityentity_id, building.level);
      let newCityEntity = CityBuildings.createBuilding(
        metaData,
        era,
        building,
        withAllies,
      ...
      else MainParser.CityBuildingsData[building.id] = newCityEntity;
    }

    return ActiveMap === 'OtherPlayer' ?
        CityMap.OtherPlayer.mapData
  ...
  createBuilding(metaData, era = CurrentEra, data = {}, withAlly = true) {
    ...
    let entity = {
      ...
      eraName:
        (data.cityentity_id || metaData.id).includes('CastleSystem') ?
          CurrentEra
        : era,
