    ...
    function xhrOnLoadHandlerExec() {
      ...
      if (url.indexOf('game/json?h=') > -1) {
        ...
        try {
          ...
          for (let entry of d) {
            if (
              entry['requestClass'] === 'StaticDataService' &&
              entry['requestMethod'] === 'getMetadata'
            ) {
              FoEproxy._addToHistory(
                entry.requestClass + '.' + entry.requestMethod,
              );
              FoEproxy._proxyAction(
                entry.requestClass,
                entry.requestMethod,
                entry,
                requestData,
  ...
  Object.assign(
    ...
    (function () {
      ...
      window.addEventListener(
        ...
        () => {
          while (
            FoEproxy._getXhrQueue() &&
            FoEproxy._getXhrQueue().length > 0
          ) {
