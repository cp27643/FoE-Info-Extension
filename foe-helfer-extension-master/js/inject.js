  ...
  function inject(
    ...
    function InjectCSS() {
      ...
      if (document.head !== null) {
        ...
        let cssFiles = ['variables', 'goods', cssname, 'boxes'];
    ...
    async function InjectCode(loadBeta, extUrl) {
      try {
        ...
        if (
            ...
            return new window.Promise(
              exportFunction(function (resolve, reject) {
                if (typeof data !== 'object' || typeof data.type !== 'string') {
                  reject(
                    'invalid request, data has to be of sceme: {type: string, ...}',
                  );
                  return;
                }
          ...
          exportFunction(callBgApi, window, {
            defineAs: 'foeHelperBgApiHandler',
          });
        }
        ...
        for (let i = 0; i < vendorScriptsToLoad.length; i++) {
          await promisedLoadCode(
            `${extUrl}vendor/${vendorScriptsToLoad[i]}.js?v=${v}`,
            'vendor',
          );
        }
        ...
        for (let i = 0; i < internalScriptsToLoad.length; i++) {
          await promisedLoadCode(
            `${extUrl}js/web/${internalScriptsToLoad[i]}/js/${internalScriptsToLoad[i]}.js?v=${v}`,
            'internal',
          );
        }
