...
(function (app) {
  ...
  function startPlacingStoredBuilding(metaId) {
    const stored = state.storedBuildings.find(
      (b) => String(b.meta.id) === String(metaId),
    );
    if (!stored) return;
  ...
  function continuePlacingStoredBuilding(metaId) {
    const nextStored = state.storedBuildings.find(
      (b) => String(b.meta.id) === String(metaId),
    );
  ...
  function updatePlacingBuildingPreview() {
    ...
    const snappedX = app.snapToGrid(
      state.lastMouseElem.x - state.placingBuilding.width / 2,
    );
    const snappedY = app.snapToGrid(
      state.lastMouseElem.y - state.placingBuilding.height / 2,
    );
  ...
  function handleCanvasMouseMove(e) {
    ...
    if (state.streetPlacement.active) {
      const currentTile = app.worldToTile(
        state.lastMouseElem.x,
        state.lastMouseElem.y,
      );
  ...
  function handleCanvasMouseDownPlace(e) {
    ...
    if (fromMeta) {
      ...
      const idx = state.storedBuildings.findIndex(
        (b) => String(b.meta.id) === String(placedMetaId),
      );
  ...
  function getStreetMeta() {
    return (
      Object.values(state.metaData).find((x) => x.type === 'street') || null
    );
  }
  ...
  function bindMapDrag() {
    ...
    const mouseDownHandler = (e) => {
      ...
      if (e.altKey) mode = 'pan';
        ...
        if (state.activeBuilding) {
          ...
          if (
            p.x >= state.activeBuilding.x &&
            p.x <= state.activeBuilding.x + state.activeBuilding.width &&
            p.y >= state.activeBuilding.y &&
            p.y <= state.activeBuilding.y + state.activeBuilding.height
          ) {
            mode = 'move';
          }
        }
    ...
    const mouseMoveHandler = (e) => {
      ...
      if (drag.mode === 'move') {
        ...
        const sidebarRect = dom.buildingsListEl
          .closest('.sidebar')
        ...
        dom.buildingsListEl
          .closest('.sidebar')
          .classList.toggle('drop-target', overSidebar);
  ...
  function bindEvents(init) {
    ...
    dom.removeStreetsBtn.addEventListener('click', () => {
      state.mapBuildings = state.mapBuildings.filter(
        (x) => x.meta.type !== 'street',
      );
      app.rebuildOccupiedTiles();
      app.redrawMap();
      app.updateStats();
    ...
    dom.resetBtn.addEventListener('click', () => {
      const reset = confirm(
        'Do you want to restart from scratch? Your changes will not be saved',
      );
      if (reset) {
