...
(function (app) {
  ...
  function slimMeta(meta) {
    ...
    if (meta.requirements?.street_connection_level !== undefined) {
      slim.requirements = {
        street_connection_level: meta.requirements.street_connection_level,
      };
    }
    ...
    if (allAge) {
      ...
      if (placementSize) {
        slimAllAge.placement = {
          size: { x: placementSize.x, y: placementSize.y },
        };
      }

      if (allAge.streetConnectionRequirement !== undefined) {
        slimAllAge.streetConnectionRequirement =
  ...
  function init(data) {
    ...
    state.metaById = new Map(
      Object.values(state.metaData).map((m) => [m.id, m]),
    );
    try {
  ...
  function buildingsFromEntries(entries) {
    return (entries || [])
      .map((entry) => {
        ...
        const effectiveMeta =
          meta.type === 'street' ? { ...meta, width: 1, length: 1 } : meta;
  ...
  function saveState() {
    try {
      ...
      alert(
        'Could not save to localStorage (storage may be full).\nUse Export to save a file instead.',
      );
    }
  }
    ...
    reader.onload = (e) => {
      try {
        ...
        try {
          ...
          console.warn(
            'Could not update localStorage after import:',
            storageErr,
          );
        }
        ...
        alert(
          'Could not read the save file. Make sure it is a valid FoE Planner export.',
        );
      }
    };
