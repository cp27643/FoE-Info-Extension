...
(function (app) {
  ...
  class MapBuilding {
    ...
    draw(context) {
      context.fillStyle =
        this.isSelected ? '#cfe5f0'
        : this.isActive ? '#66c440'
        : this.fill;
    ...
    drawName(context) {
      ...
      if (text.width < this.width) {
        context.fillText(
          this.name,
          this.x + this.width / 2,
          this.y + this.height / 2 - Math.ceil(FONT_SIZE * 0.3),
        );
        ...
        context.fillText(
          this.name.slice(textStart, textEnd),
          this.x + this.width / 2,
          this.y + this.height / 2 - Math.ceil(FONT_SIZE * 0.9),
        );
        textStart = textEnd;
        textEnd = Math.ceil(this.name.length / ratio) + textStart;
        const more = textEnd >= this.name.length ? '' : '…';
        context.fillText(
          this.name.slice(textStart, textEnd) + more,
          this.x + this.width / 2,
          this.y + this.height / 2 + Math.ceil(FONT_SIZE * 0.2),
        );
      }
      ...
      context.fillText(
        totalSize,
        this.x + this.width / 2,
        this.y + this.height / 2 + sizeOffset,
      );
