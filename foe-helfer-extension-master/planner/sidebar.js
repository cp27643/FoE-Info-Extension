...
(function (app) {
  ...
  function filterStoredBuildingGroups(groups) {
    return groups.filter((item) => {
      if (
        state.sidebarState.filterType !== 'all' &&
        item.type !== state.sidebarState.filterType
      ) {
        return false;
      }
      ...
      if (state.sidebarState.filterText) {
        ...
        const haystack =
          `${item.name} ${item.width}x${item.height} ${item.type}`.toLowerCase();
