...
// separate code from global scope
{
  ...
  const Alerts = (() => {
    ...
    function getValidateAlertData(data) {
      ...
      if (typeof data.expires === 'string')
        data.expires = Number.parseInt(data.expires);
      if (data.expires === undefined && typeof data.datetime === 'string')
        data.expires = new Date(data.datetime).getTime();
      if (typeof data.repeat === 'string')
        data.repeat = Number.parseInt(data.repeat);
      ...
      if (typeof data.title !== 'string')
        throw 'Alert: "data.title" needs to be a string';
      if (typeof data.body !== 'string')
        throw 'Alert: "data.body" needs to be a string';
      if (!Number.isInteger(data.expires))
        throw 'Alert: "data.expires" needs to be a integer';
      if (!Number.isInteger(data.repeat))
        throw 'Alert: "data.repeat" needs to be a integer';
      if (data.actions != null && !(data.actions instanceof Array))
        throw 'Alert: "data.actions" needs to be an array';
      if (typeof data.category !== 'string')
        throw 'Alert: "data.category" needs to be a string';
      if (typeof data.persistent !== 'boolean')
        throw 'Alert: "data.persistent" needs to be a boolean';
      if (typeof data.tag !== 'string')
        throw 'Alert: "data.tag" needs to be a string';
      if (typeof data.vibrate !== 'boolean')
        throw 'Alert: "data.vibrate" needs to be a boolean';
    ...
    async function createAlert(data, server, playerId) {
      ...
      return db.alerts.add(alert).then((/** @type {!number} */ id) => {
        browser.alarms.create(`foe-alert:${id}`, {
          when: data.expires,
        });
        return id;
      });
      ...
      return browser.notifications.create(
        ...
        Object.assign(
            ...
            },
          {
            ...
            contextMessage:
              'FoE-Helper − ' + trimPrefix(alert.server, 'https://'),
          },
        ),
      );
    }
    ...
    browser.alarms.onAlarm.addListener(async (alarm) => {
      ...
      if (
        ...
        alertId > Number.MAX_SAFE_INTEGER ||
        alertId < 0
      )
        return;
  ...
  async function handleWebpageRequests(request, sender) {
    ...
    if (typeof request !== 'object')
      return APIerror('expecting an object as message');
    if (typeof request.type !== 'string')
      return APIerror('expecting an "type": string');
      ...
      case 'alerts': {
        ...
        if (sender.id === browser.runtime.id) {
          if (typeof request.action !== 'string')
            return APIerror('expecting an "action": string');
          ...
          switch (action) {
            ...
            case 'setData': {
              ...
              if (!Number.isInteger(id))
                return APIerror('expecting an "id": integer');
          ...
          // limited alerts-API for external use
          if (!Number.isInteger(request.playerId))
            return APIerror('malformed request: expected "playerId": integer');
          if (typeof request.action !== 'string')
            return APIerror('malformed request: expected "action": string');
          ...
          switch (action) {
            ...
            case 'get': {
              ...
              const id = request.id;
              if (!Number.isInteger(id))
                return APIerror('malformed request: expected "id": integer');
